...
class Wallet {
  ...
  createUnsignedTransaction(publickey="", amount=0.0, fee=0.0, force_merge=0) {
    ...
    fee = BigInt((fee * 100000000));

    if (publickey === "") { publickey = this.returnPublicKey(); }

    var wallet_avail = this.calculateBalance();
    if (fee > wallet_avail) { fee = BigInt(0); }
...
****/
  ...
  onChainReorganization(block, lc) {
    ...
    if (lc) {
      ...
      for (let i = 0; i < block.transactions.length; i++) {
        ...
        let to_slips        = [];
        let from_slips      = [];
        for (let m = 0; m < slips.to.length; m++) { to_slips.push(slips.to[m].clone()); }
        for (let m = 0; m < slips.from.length; m++) { from_slips.push(slips.from[m].clone()); }
  ...
  signTransaction(tx) {
    ...
    //
    try {
console.log("SIGNING THE TRANSACTION THIS WAY");
