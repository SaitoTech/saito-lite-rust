...
class BurnFee {
  ...
  returnRoutingWorkNeededToProduceBlockInNolan(burnfee_previous_block, current_block_timestamp, previous_block_timestamp) {
    ...
    let burn_fee_previous_block_as_float = parseFloat(burn_fee_previous_block) / 100_000_000.0;
    let work_needed_float = burn_fee_previous_block_as_float / elapsed_time_float;

    // convert back to nolan for rounding / safety
    return Math.round(work_needed_float * 100_000_000.0);
  ...
  returnBurnFeeForBlockProducedAtCurrentTimestampInNolan(burn_fee_previous_block, current_block_timestamp, previous_block_timestamp) {
    ...
    let new_burnfee = Math.round(res1 * 100_000_000.0);
