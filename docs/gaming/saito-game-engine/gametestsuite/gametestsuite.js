...
class GameTestSuite extends GameTemplate {
	...
	addEventsToDom(app) {
		...
		if (game_self.game.crypto != '') {
			document.getElementById('saito_crypto').innerHTML =
		...
		//
		document.getElementById('add_player_boxes_button').onclick = async (
			e
		) => {
		...
		// secure dice roll
		//
		document.getElementById('secure_dice_roll_button').onclick = async (
			e
		) => {
		...
		// simultaneous pick
		//
		document.getElementById('simultaneous_pick_button').onclick = async (
			e
		) => {
		...
		// consecutive moves
		//
		document.getElementById('consecutive_moves_button').onclick = async (
			e
		) => {
		...
		// simultaneous moves
		//
		document.getElementById('simultaneous_moves_button').onclick = async (
			e
		) => {
		...
		//
		document.getElementById('deal_cards_to_player_button').onclick = async (
			e
		) => {
		...
		//
		document.getElementById('deal_cards_to_table_button').onclick = async (
			e
		) => {
		...
		//
		document.getElementById('receive_payment_button').onclick = async (
			e
		) => {
		...
		document.getElementById('toggle_cardbox_button').onclick = (e) => {
			if (!game_self.game.deck[0]) {
				alert(
					'Deal cards to players before toggling the Popup Cardbox'
				);
				return;
			}
	...
	async check_balance_test(app) {
		...
		game_self.addMove(
			'NOTIFY\tThe balance check is finished: balance adequate!'
		);
		game_self.updateLog(
			`The game engine is checking to see if the balance at address ${address} is at least ${amount} ${ticker}. The game will halt for all players until the balance is at this amount.`
		);
