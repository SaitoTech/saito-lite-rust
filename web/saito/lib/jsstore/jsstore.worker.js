    ...
    Join.prototype.getJoinTableInfo_ = function (joinOn) {
        ...
        if (true) {
            if (splittedjoinOn.length !== 2) {
                throw new LogHelper(ERROR_TYPE.InvalidJoinQuery, "The 'on' clause ('".concat(joinOn, "') is missing an '=' operator. Ensure the condition properly relates columns from both tables."));
            }
        }
    ...
    Join.prototype.checkJoinQuery_ = function (jointblInfo, qry) {
        ...
        if (true) {
            if (tableSchemaOf1stTable == null || tableSchemaOf2ndTable == null) {
                return new LogHelper(ERROR_TYPE.InvalidJoinQuery, "The 'on' condition references tables or columns ('".concat(table1.table, ".").concat(table1.column, "', '").concat(table2.table, ".").concat(table2.column, "') that do not exist or are not part of the join. Ensure that the tables and columns used in the 'on' condition match those specified in the 'from' and 'with' clauses."));
            }
        }
        var err;
        // check on info & with info 
        if (qry.with !== table2.table) {
            err = new LogHelper(ERROR_TYPE.InvalidJoinQuery, "The 'on' condition must reference a column from the table specified in 'with'. Found '".concat(qry.on, "', but '").concat(qry.with, "' was provided in 'with'."));
        }
    ...
    Update.prototype.execute = function (beforeExecute) {
        ...
        try {
            ...
            if (err)
                return promiseReject(err);
            return beforeExecute().then(function (_) {
                var txPromise = _this.initTransaction();
                var pResult;
                if (query.where != null) {
                    if (query.where.or || isArray(query.where)) {
                ...
                var promiseToUse = [pResult];
                if (query.returnImmediate === false && txPromise) {
                    promiseToUse.push(txPromise);
                }
                return promiseAll(promiseToUse).then(function () {
