...
const refreshOptionForm = function refreshOptionForm() {
	...
	const formatDate = (val) =>
		Datepicker.formatDate(val, config.format, config.lang);
		...
		switch (key) {
			...
			case 'daysOfWeekHighlighted':
				optsForm.querySelectorAll(`[name=${key}`).forEach((chkbox) => {
					chkbox.checked = val.includes(
						Number.parseInt(chkbox.value, 10)
					);
				});
				break;
...
const handleArrayOption = function handleArrayOption(name) {
	const checkedInputs = options.querySelectorAll(
		`input[name=${name}]:checked`
	);
	setOptions(
		name,
		Array.from(checkedInputs).map((el) => Number(el.value))
	);
};
...
function initialize() {
	...
	Object.keys(languages).forEach((lang) => {
		document.body.appendChild(
			parseHTML(`<script src="../dist/js/locales/${lang}.js"></script>`)
		);
