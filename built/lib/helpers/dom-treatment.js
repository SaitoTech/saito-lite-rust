var call = require("file-loader").call;
if (typeof window !== "undefined") {
    ...
    window.sconfirm = function (message) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            document.getElementById('alert-shim').onclick = function (event) {
                if (event.keyCode === 13) {
                    event.preventDefault();
    ...
    window.siteMessage = function (message, killtime) {
        if (killtime === void 0) { killtime = 9999999; }
    ...
    window.imgPop = function imgPop(img) {
        ...
        img.addEventListener('click', function (e) {
            ...
            wrapper.id = 'alert-wrapper';
            var html = '<div id="alert-shim">';
            html += "<img class=\"imgpop\" src=\"".concat(img.src, "\" />");
            html += "</div>";
            wrapper.innerHTML = html;
            document.body.appendChild(wrapper);
            // Open new tab with image
            document.querySelector('img.imgpop').addEventListener('click', function (e) {
                try {
                    window.open('').document.body.innerHTML = '<img src="' + img.src + '" />';
                }
                catch (err) {
                    console.error(err);
                }
...
//# sourceMappingURL=dom-treatment.js.map