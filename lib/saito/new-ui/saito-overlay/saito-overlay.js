...
class SaitoOverlay {
  ...
  remove(){
    try {
      ...
      let bs = `.saito-overlay-backdrop${this.ordinal}`;
      let overlay_backdrop_el = document.querySelector(bs);
      if (overlay_backdrop_el){
  ...
  setBackground(img_path) {
    ...
    let overlay_backdrop_el = document.querySelector(qs);
    if (overlay_backdrop_el) {
      overlay_backdrop_el.style.background = 'linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url("'+img_path+'")';
