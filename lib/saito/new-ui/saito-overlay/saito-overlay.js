...
class SaitoOverlay {
  ...
  constructor(app, withCloseBox = true) {
    this.random = app.crypto.hash(Math.random());          // each overlay gets random number to permit multiple overlays
    this.app = app;
  ...
  render(app, mod) {
    if (!document.querySelector(".saito-overlay-backdrop")) {
      this.app.browser.addElementToDom(SaitoOverlayTemplate(this.closebox));
    }
  }
    ...
    try {
      ...
      if (this.closebox) {
        overlay_el.innerHTML =
          `<div id="saito-overlay-closebox" class="saito-overlay-closebox saito-overlay-closebox-${this.random}"><i class="fas fa-times-circle saito-overlay-closebox-btn"></i></div>` +
          html;
        //Close by clicking on closebox
        let closebox_qs = ".saito-overlay-closebox-"+this.random;
