...
class GameMenu {
  ...
  render(app, mod) {
    ...
    if (this.initialized == 0) {
      //Return a list of modules that define a respondTo for game-menu
      app.modules.respondTo("game-menu").forEach((menMod) => {
        //Query the specify respondTo for this module
        let res = menMod.respondTo("game-menu");

        /* 
        for potential initialization functions, might be used to render a menu and  an array of submenu options
        */
        if (menMod.initialize) {
          menMod.initialize(app, mod);
        }
    ...
    app.browser.replaceElementById(GameMenuTemplate(html), "game-menu");
  ...
  attachEvents(app, game_mod) {
    console.log("Attach menu events");
    //
...
console.log(" A 3 " + app.modules.mods[i].slug);
...
console.log("ACTIVATE POPUP!");
            game_mod.request_no_interrupts = false;
            chatmod.activatePopup(chatmod.returnCommunityChat().id)
console.log("OPEN CHAT BOX");
            chatmod.openChatBox(chatmod.returnCommunityChat().id);
console.log("UNMUTE COMMUNITY CHAT!");
        ...
        if (game_mod.game.players.length > 2) {
          ...
          menu_self.addSubMenuOption("game-chat", {
            ...
            callback: function (app, game_mod) {
              ...
              newgroup.name = "All Players";
              if (newgroup) {
                chatmod.activatePopup(newgroup.id);
                chatmod.openChatBox(newgroup.id);
                chatmod.saveChat();
              } else {
                console.log("new group failed, resort to gid");
                alert("Error Creating Chat Group");
          ...
          if (game_mod.game.players[ii] != app.wallet.returnPublicKey()) {
            ...
            menu_self.addSubMenuOption("game-chat", {
              ...
              callback: function (app, game_mod) {
                ...
                // let newgroup = chatmod.createChatGroup(members, name);
                if (newgroup) {
                  newgroup.name = "Player: " + (ii + 1);
                  chatmod.activatePopup(newgroup.id);
                  chatmod.openChatBox(newgroup.id);
                  chatmod.saveChat();
                } else {
                  console.log("new group failed, resort to gid");
                  alert("Error Creating Player-to-Player Chat");
                  //chatmod.sendEvent("chat-render-request", {});
                  //chatmod.openChatBox(gid);
                }
              },
