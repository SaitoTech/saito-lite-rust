...
class GamePlayerBox {
    ...
    for (let player = 1; player <= this.game_mod.game.players.length; player++) {
      ...
      if (pb_self.game_mod.game.players[player - 1] !== pb_self.app.wallet.returnPublicKey()) {
        $(`#player-box-head-${boxId}`).on("dblclick", function () {
          app.connection.emit("open-chat-with", {key: pb_self.game_mod.game.players[player - 1]});
        });
      }
    }
  }
  ...
  refreshName(pnum, name = "", nameClass = "") {
    ...
    if (name == "") {
      ...
      if (!name){
        ...
        console.log(`Player ${pnum}: ${this.game_mod.game.players[pnum - 1]}`);
        console.log("Keys returns: "+this.app.keys.returnUsername(this.game_mod.game.players[pnum - 1]));
        console.log("Alt keys returns: "+ this.app.keys.returnIdentifierByPublicKey(this.game_mod.game.players[pnum - 1]));
      }
      if (name != "") {
        if (name.indexOf("@") > 0) {
  ...
  appendGraphic(html, pnum = -1){
    ...
    } catch (err) { console.log("could not update graphic div"); }
  }
