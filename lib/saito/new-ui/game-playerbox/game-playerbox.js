...
class GamePlayerBox {
  ...
  render(app, game_mod) {
    ...
    try {
      ...
      for (; i <= game_mod.game.players.length; i++) {
  ...
  refreshName(pnum, name = "", nameClass = "") {
    ...
    let identicon = this.app.keys.returnIdenticon(playerKey);

    if (name == "") {
      name = this.app.keys.returnUsername(playerKey);

      if (!name){
        console.log("Playerbox not finding player name --- ");
        console.log(`Player ${pnum}: ${playerKey}`);
        console.log("Keys returns: "+this.app.keys.returnUsername(playerKey));
        console.log("Alt keys returns: "+ this.app.keys.returnIdentifierByPublicKey(playerKey));
        name = playerKey;
      }
