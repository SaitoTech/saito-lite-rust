...
class SaitoCalendar {
  ...
  constructor(app, mod, selector = "") {
    ...
    this.app.connection.on('event-render-request', (tx) => {
      ...
      this.render(app, mod, this.selector);
  ...
  render(app, mod, selector = "") {
    ...
    if (selector != "" && selector != this.selector) { this.selector = selector; }
  ...
  renderCalendar() {
    ...
    for (let i = 1; i <= state.lastDay; i++) {
      ...
      if (
        ...
        days += `<div class="saito-calendar-day today" data-id="${i}"><span> <i class="fas fa-gamepad">  </i> </span>${i}</div>`;
      } else {
    ...
    console.log("Next days ", state.nextDays)
  ...
  attachEvents(app, mod) {
    ...
    document.querySelectorAll(".saito-calendar-day").forEach(el => {
      el.onclick = (e) => {
        ...
        calpop.render(app, mod, day, month, year, this.events);
