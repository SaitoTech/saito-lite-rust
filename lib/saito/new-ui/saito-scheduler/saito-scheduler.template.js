...
module.exports = (app, mod, options) => {
    ...
    <div class="saito-scheduler-event">
      ...
      <div class="saito-scheduler-details">
        ...
        <div><select id="schedule-type">
      `;
    for (let i = 0; i < options.mods.length; i++) {
      let is_selected = "";
      if (i == options.mods_index) { is_selected = " SELECTED"; }
      html += `<option value="${options.mods[i].returnSlug()}" ${is_selected}>${options.mods[i].name}</option>`;
    }
    html += `
        </select></div>
    ...
    html += `
      <div class="saito-scheduler-invite saito-user" id="saito-scheduler-invite">
        ...
        <div class="saito-address">${invite_text}</div>
        <div class="saito-userline">${invite_details}</div>
      </div>
    `;
