...
class SchedulerOverlay {
  ...
  attachEvents(app, mod, mycallback){
    ...
    var select_timezone = document.getElementById('schedule-datetime-timezone');
    for(i = 0; i < select_timezone.options.length; i++) {
      option = select_timezone.options[i];
      if (option.value == this.options.default_utc_offset) {
        select_timezone.options[i].selected = true;
      }
    }
