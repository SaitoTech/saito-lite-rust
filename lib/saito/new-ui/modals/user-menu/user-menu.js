...
class UserMenu {
	...
	let thisobj = this;
        ...
        if (!document.querySelector("#saito-user-menu")) {
            this.overlay.show(app, "", userMenuTemplate(app, this.publickey));
    ...
    addMenuItem(item, id) {
        document.querySelector(".saito-modal-content").innerHTML += `
          <div id="${id}" class="user_menu_item saito-modal-menu-option"><i class="${item.icon}"></i><div>${item.text}</div></div>
        `;
    }
}
