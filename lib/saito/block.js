...
class Block {
  ...
  async upgradeBlockToBlockType(blockType) {
    console.trace("upgrading block to block type : " + blockType);
    ...
    if (blockType === BlockType.Full) {
      let newBlock = await this.app.storage.loadBlockByFilename(this.app.storage.generageBlockFilename(this));
