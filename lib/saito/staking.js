...
class Staking {
  ...
  findWinningStaker(random_hash) {
    ...
    if (this.stakers.length == 0) { return null; }

    //
    // find winning staker
    //
    let x = BigInt('0x'+random_hash);
    let z = BigInt(this.stakers.length);
    let retrieve_from_pos = x % z;

    let winning_slip = this.stakers[parseInt(retrieve_from_pos.toString())].clone();
  ...
  onChainReorganization(block, lc) {
    ...
    for (let i = 0; i < block.transactions.length; i++) {
      ...
      if (tx.transaction.type === saito.transaction.TransactionType.StakerWithdrawal) {
        //
        ...
        //
        if (lc) {
          if (tx.transaction.from[0].type == saito.slip.SlipType.StakerWithdrawalPending) {
            this.removePending(tx.transaction.from[0]);
          }
          if (tx.transaction.from[0].type == saito.slip.SlipType.StakerWithdrawalStaking) {
      ...
      if (tx.transaction.type === saito.transaction.TransactionType.StakerDeposit) {
        for (let k = 0; k < tx.transaction.to.length; k++) {
          if (tx.transaction.to[k].type == saito.slip.SlipType.StakerDeposit) {
            //
            // roll forward
            //
            if (lc) {
    ...
    if (lc) {
      ...
      if (this.stakers.length == 0) {
        let return_value = this.resetStakerTable(block.returnStakingTreasury());
      ...
      if (this.pending.length == 0) {
        ...
        for (let k = 0; k < this.stakers.length; k++) {
          if (this.stakers[k].type == saito.slip.SlipType.StakerOutput) {
            this.pending.push(this.stakers[k]);
          }
          if (this.stakers[k].type == saito.slip.SlipType.StakerDeposit) {
    ...
    if (block.hasFeeTransaction() && block.hasGoldenTicket()) {
      ...
      let fee_transaction = block.returnFeeTransaction();
      let golden_ticket_transaction = block.returnGoldenTicketTransaction(); 

      //
      // grab random input from golden ticket
      //
      let golden_ticket_data = this.app.goldenticket.deserializeFromTransaction(golden_ticket_transaction);

      let target_hash = golden_ticket_data.target_hash;
      let next_random_number = golden_ticket_data.random_hash;
      let creator = golden_ticket_data.creator;
      ...
      for (let k = 0; k < fee_transaction.transaction.to.length; k++) {
        if (fee_transaction.transaction.to[k].type == saito.slip.SlipType.StakerOutput) {
          is_there_a_staker_output = true;
        }
      }
      if (is_there_a_staker_output == false || fee.transaction.transaction.from.length == 0) {
      ...
      if (lc) {
        ...
        if (this.stakers.length == 0) {
          ...
          //
          let slips_to_remove_from_staking = [];
          let slips_to_add_to_pending = [];
          let staker_slip_num = 0;

          for (let k = 0; k < fee_transaction.transaction.to.length; k++) {

            let staker_output = fee_transaction.transaction.to[k].clone();
            ...
            if (staker_output.type == saito.slip.SlipType.StakerOutput) {
              ...
              //
              let lucky_staker = this.findWinningStaker(next_random_number); // use first
      ...
      //
        ...
        for (let k = 0; k < fee_transaction.transaction.to.length; k++) {
          ...
          let staker_input = fee_transaction.transaction.from[staker_slip_num].clone();
          ...
          if (staker_output.type == saito.slip.SlipType.StakerOutput) {
            ...
            let slip_type = staker_input.type;
            if (slip_type == saito.slip.SlipType.StakerDeposit) {
              this.addDeposit(staker_input.clone());
            }
            if (slip_type == saito.slip.SlipType.StakerOutput) {
  ...
  resetStakerTable(staking_treasury) {
    ...
    //
    let staking_payout_per_block = staking_treasury / BigInt(this.app.blockchain.blockchain.genesis_period);
    ...
    let average_staked = total_staked / BigInt(this.stakers.length);

    //
    // calculate the payout for average stake
    //
    let m = staking_payout_per_block;
    let p = BigInt(this.stakers.length);

    let average_staker_payout = m / p;
    ...
    for (let i = 0; i < this.stakers.length; i++) {
      ...
      // get total staked
      //
      let my_staked_amount = this.stakers[i].returnAmount();
      ...
      // my stake PLUS (my stake / 1 * ( my_stake / average_staked ) * ( ( treasury / genesis_period )
      //
      let a = BigInt(my_staked_amount);
      let b = BigInt(average_staker_payout);
      let nominator = a * b;
      let denominator = average_staked;

      let staking_profit = nominator / denominator;
