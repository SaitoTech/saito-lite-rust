...
class Keychain {
  ...
  initialize() {
    ...
    for (let i = 0; i < this.app.options.keys.length; i++) {
      ...
      for (let m = 0; m < tk.identifiers.length; m++) {
        ...
        if (m == 0) { k.name = tk.identifiers[m]; }
      }
  ...
  fetchIdentifier(publickey = "", mycallback) {
    ...
    if (this.fetched_keys[publickey] == 1) { mycallback(identifier); }
  ...
  fetchPublicKey(identifier = null, mycallback) {
    ...
    if (this.app.crypto.isPublicKey(identifier)) { mycallback(identifier); }
  ...
  returnUsername(publickey) {
    ...
    if (name != "" && name != publickey) { return name; }
    if (name.length > 12) { return name.substr(0, 12) + "..."; }
    if (name[0]) { if (name[0].length > 12) { return name[0].substr(0, 12) + "..."; } }
    return name;
  }
