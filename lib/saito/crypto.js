...
class Crypto {
    ...
    verifyHash(buffer, sig, pubkey) {
        console.log("verifyHash", buffer, sig, pubkey);
        try {
    ...
    verifySignatureSource(msg, sig, pubkey) {
        try {
            return secp256k1.verify(Buffer.from(msg, 'hex'),
                                    Buffer.from(this.fromBase58(sig), 'hex'),
                                    Buffer.from(this.uncompressPublicKey(pubkey), 'hex')
            );
        } catch (err) {
            console.log(err);
            return false;
    ...
    verifyMessage(msg, sig, pubkey) {
        try {
            return secp256k1.verify(Buffer.from(this.hash(Buffer.from(msg, 'utf-8').toString('base64')), 'hex'),
                                    Buffer.from(this.fromBase58(sig), 'hex'),
                                    Buffer.from(this.uncompressPublicKey(pubkey), 'hex')
            );
        } catch (err) {
