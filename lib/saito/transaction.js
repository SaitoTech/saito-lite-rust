...
`;
    ...
    serialize(app) {
        ...
        ret.set(new Uint8Array([...inputs_len, ...outputs_len, ...message_len, ...path_len, ...signature, ...timestamp, transaction_type]), 0);
    ...
    serializeForSignature(app) {
        ...
        for (let i = 0; i < this.transaction.from.length; i++) {
            buffer = Buffer.concat([buffer, new Buffer(this.transaction.from[i].serializeInputForSignature(app))])
        }
        for (let i = 0; i < this.transaction.to.length; i++) {
...
//    buffer = Buffer.concat([buffer, Buffer.from(this.transaction.m, "hex")]);
    ...
    validate(app) {
        ...
        // transaction types.
        //
        if (this.transaction.type !== TransactionType.Fee && this.transaction.type !== TransactionType.ATR && this.transaction.type !== TransactionType.Vip && this.transaction.type !== TransactionType.Issuance) {
