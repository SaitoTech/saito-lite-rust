...
class SendBlockchainMessage {
    sync_type = undefined;
    starting_hash = [];
    blocks_data = [] < SendBlockchainBlockData > (0);
    app = {};
    ...
    static deserialize(bytes, app) {
        ...
        return new SendBlockchainMessage(sync_type, starting_hash, blocks_data);
    }

    serialize() {
        let bytes = Buffer.concat([this.app.binary.u8AsByte(this.sync_type), Buffer.from(this.starting_hash), this.app.binary.u32AsBytes(this.blocks_data.length)]);
        for (let block of this.blocks_data) {
            bytes = Buffer.concat([bytes,
                this.app.binary.u64AsBytes(block.block_id),
                Buffer.from(block.block_hash),
                this.app.binary.u64AsBytes(block.timestamp),
                Buffer.from(block.pre_hash),
                this.app.binary.u32AsBytes(block.number_of_transactions)
            ]);
        }
        return bytes;
    }
