...
class HandshakeChallengeMessage {
    ...
    static deserialize(bytes, app) {
        ...
        let handshake_challenge = new HandshakeChallengeMessage(challenger_octet, challenger_pubkey, opponent_octet, opponent_pubkey);
    ...
    serialize() {
        return Buffer.concat([this.challenger_node.ip_address, this.opponent_node.ip_address, this.challenger_node.public_key, this.opponent_node.public_key, this.app.binary.u64AsBytes(this.timestamp)]);
    }

    serializeWithSig(privateKey) {
        return this.app.crypto.signBuffer(Buffer.from(this.app.crypto.hash(this.serialize()), "hex"), privateKey);
    }
}
