...
class NetworkAPI {
    ...
    async initiateHandshake(ws) {
        ...
        console.log("handshake response length = " + handshake_init_response_data.length);
        ...
        let is_sig_valid = secp256k1.verify(
            Buffer.from(this.app.crypto.hash(handshake_init_response_data.slice(0, 82)), 'hex'),
            handshake_challenge.challenger_node.sig,
            handshake_challenge.challenger_node.ip_address
        );
        console.log("is valid sig ? ", is_sig_valid);

        let {signature, recovery} = secp256k1.sign(
            Buffer.from(this.app.crypto.hash(handshake_init_response_data), 'hex'),
