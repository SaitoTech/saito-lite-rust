...
class NetworkAPI {
    ...
    async connectAsWebClient(protocol, host, port) {
        ...
        socket.onopen = (event) => {
            ...
            socket.peer.sendRequestWithCallback("application message with callback", Buffer.from("application message data"), (error, response) => {
                if (error) {
                    console.error(error);
                    return;
                }
                console.log("response received for callback");
                console.log(Buffer.from(response, "hex").toString("utf-8"));
            });
        };
    ...
    deserializeAPIMessage(bytes) {
        return new APIMessage(Buffer.from(bytes.slice(0, 8), "hex").toString("utf-8"),
