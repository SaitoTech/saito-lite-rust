...
console.log("start block serializer");

        let transactions_length = this.u32AsBytes(0);
        if (block.transactions.length > 0) { transactions_length = this.u32AsBytes(block.transactions.length); }
        let id = this.u64AsBytes(new Big(block.block.id));
        let timestamp = this.u64AsBytes(new Big(block.block.timestamp));
        let previous_block_hash = Buffer.from(block.block.previous_block_hash, 'hex');
        let creator = Buffer.from(block.block.creator, 'hex');
        let merkle_root = Buffer.from(block.block.merkle, 'hex');
        let signature = Buffer.from(block.block.signature, 'hex');
        let treasury = this.u64AsBytes(new Big(block.block.treasury));
        let staking_treasury = this.u64AsBytes(new Big(block.block.staking_treasury));
        let burnfee = this.u64AsBytes(new Big(block.block.burnfee));
        let difficulty = this.u64AsBytes(new Big(block.block.difficulty));
console.log("A2");
...
console.log("done blok serializer");
        return ret;
    }
    /**
     * deserialize block
