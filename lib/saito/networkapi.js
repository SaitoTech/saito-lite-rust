...
class NetworkAPI {
    ...
    async initiateHandshake(ws) {
        ...
        console.log("handshake response", handshake_init_response_data);
        console.log("handshake response length = " + handshake_init_response_data.byteLength);
        console.log(handshake_challenge.challenger_node);
        console.log(handshake_challenge.opponent_node);
        ...
        console.log("public key : ", Buffer.from(handshake_challenge.challenger_node.public_key).toString("hex"));

        let is_sig_valid = secp256k1.verify(Buffer.from(this.app.crypto.hash(handshake_init_response_data.slice(0, saito.network.ChallengeSize)),
                                                        'hex'
                                            ),
                                            Buffer.from(handshake_challenge.challenger_node.sig),
                                            Buffer.from(handshake_challenge.challenger_node.public_key)
        );
        console.log("is valid sig ? ", is_sig_valid);

        let {
            signature, recovery
        } = secp256k1.sign(Buffer.from(this.app.crypto.hash(handshake_init_response_data), 'hex'),
                           Buffer.from(this.app.wallet.returnPrivateKey(), 'hex')
        );

        const bytes = Buffer.concat([Buffer.from(handshake_init_response_data), Buffer.from(signature)]);

        let handshake_complete_response_data = await this.sendAPICall(ws, "SHAKCOMP", bytes);

        console.log("handshake complete", handshake_complete_response_data);
    }
    ...
    async wsConnectAndInitialize(protocol, host, port) {
        ...
        ws.on('message', async (data) => {
            ...
            if (api_message.message_name === "RESULT__") {
                ...
                //     await peer.handlePeerCommand(peer, api_message);
                // }
                console.log("handling peer command - receiving peer id " + ws.peer.id);
                await ws.peer.handlePeerCommand(ws.peer, api_message);
            }
        });
        ws.on('error', (event) => {
            console.log(`[error] ${event.message}`);
    ...
    receiveAPIError(message) { // TODO : is this working or need fixing?
        ...
        let index = this.app.binary.u32FromBytes(message.message_data.slice(8, 12));
        if (this.api_callbacks[index]) {
            this.api_callbacks[index].reject(message.message_data.slice(12));
        } else {
    ...
    async wsConnectToRustPeer(protocol, host, port) {
        ...
        ws.on('message', async (data) => {
            console.log("message received on socket (peer)", data);
            let api_message = this.deserializeAPIMessage(data);
            if (api_message.message_name === "RESULT__") {
                this.receiveAPIResponse(api_message);
            } else if (api_message.message_name === "ERROR___") {
