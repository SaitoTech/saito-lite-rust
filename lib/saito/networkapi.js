...
class NetworkAPI {
    ...
    async initiateHandshake(ws) {
        ...
        let is_sig_valid = this.app.crypto.verifyHash(Buffer.from(this.app.crypto.hash(handshake_init_response_data.slice(0, saito.network.ChallengeSize)),
                                                        'hex'
                                            ),
                                            Buffer.from(handshake_challenge.challenger_node.sig).toString('hex'),
                                            this.app.crypto.toBase58(Buffer.from(handshake_challenge.challenger_node.public_key).toString('hex'))
        );
