...
class Deposit {
	...
	async renderCrypto() {
		try {
			...
			let balance = this.app.wallet.returnPreferredCryptoBalance();

			document.querySelector(`.saito-crypto-deposit-container .balance-amount`).innerHTML = this.app.crypto.formatBalance(balance);

			if (cryptomod?.confirmations) {
				document.querySelector('.network-confirmations-count').innerHTML = cryptomod.confirmations;
			} else {
				document.querySelector('.network-confirmations-count').style.display = 'none';
			}
