...
class MixinHistory {
	...
	async render() {
		...
		let withdraw = this.checkActiveWithdrawl();

		this.mod = await this.app.wallet.returnPreferredCrypto();
		this.overlay.show(HistoryTemplate(this.app, this.mod, this));
		let balance = (withdraw.amount > 0 && withdraw != false) ? 
					(Number(this.mod.balance) - Number(withdraw.amount)) 
					: await this.mod.formatBalance();		
