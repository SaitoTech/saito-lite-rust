...
class Withdraw {
    ...
    if (preferred_crypto.ticker == "SAITO") {
      ...
      fee = await preferred_crypto.returnWithdrawalFee(asset_id);
  ...
  async attachEvents() {
    ...
    if (document.querySelector("#withdrawal-form") != null) {
      document.querySelector("#withdrawal-form").onsubmit = (e) => {
        ...
        let amount = parseFloat(Number(document.querySelector("#withdraw-input-amount").value));
        let address = document.querySelector("#withdraw-input-address").value;
        let amount_avl = parseFloat(Number(document.querySelector("#withdraw-balance").value));
        let ticker = document.querySelector("#withdraw-ticker").value;
        amount_avl = Number(amount_avl);

        if (isNaN(amount)) {
          salert("Error: Amount should be a number");
          return false;
        }
      ...
      document.querySelector("#withdraw-confirm").onclick = async (e) => {
        ...
        try {
          ...
          /* What is this supposed to do - relevant function does not exist.
          If reactivated - please ensure to not have this run for SAITO payments
          if (fee == 0) {
            address = preferred_crypto.formatAddress(address);
          }
          */
