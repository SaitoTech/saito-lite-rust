...
class Withdraw {
  ...
  async loadCryptos(ticker = null) {
    ...
    if (preferred_crypto.ticker == "SAITO") {
      ...
      balance = this.app.wallet.balance;
    } else {
        ...
        if (document.querySelector("#withdrawal-form") != null) {
          ...
          document.querySelector("#withdraw-confirm").onclick = async (e) => {
            ...
            try {
              ...
              let hash = await this.app.wallet.sendPayment([sender], [address], [amount], (new Date().getTime()), btoa(sender+address+amount+Date.now()), function() {
                setTimeout(function(){
