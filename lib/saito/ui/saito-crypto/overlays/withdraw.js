...
class Withdraw {
    ...
    this.errors = {
        ...
        await this.fetchWithdrawFee(preferred_crypto);
			}
		);
	}
	...
	async loadCryptos(ticker = null) {
      ...
      id="crypto-option-${crypto_mod.name}" value="${crypto_mod.ticker}">${
		...
		if (preferred_crypto.ticker == 'SAITO') {
			...
			fee = await preferred_crypto.returnNetworkFee(asset_id);

			document.querySelector('.withdraw-info-value.fee').innerHTML =
				fee + ` ${preferred_crypto.ticker}`;
	   	...
	   	if (amount <= 0) {
	   		...
	   		document.querySelector('#withdraw-amount-error').innerHTML = 
				...
				`Error: Insufficent funds (` +
					...
					` available)`;
				this_withdraw.errors['amount'] = true;	
			} else if (amount_avl < amount + this_withdraw.fee) {
				console.log('error 3 //');
	      document.querySelector('#withdraw-amount-error').innerHTML = 
				'Error: Insufficent fee for withdraw';
				this_withdraw.errors['amount'] = true;
	    }
  ...
  async validateAdrressInput(preferred_crypto){ 
  	...
  	console.log('ticker:', preferred_crypto.ticker);

  	// check if third party crypto address is valid
		if (preferred_crypto.ticker != 'SAITO') {
			let valid = WAValidator.validate(address, preferred_crypto.ticker);
			...
			// check if address is satio publickey
			//let isPublicKey = this_withdraw.app.crypto.isPublicKey(address);
  ...
  checkSufficentFee(){
  	...
  	let amount_avl = Number(document.querySelector('#withdraw-balance').value);
		this_withdraw.fee = Number(this_withdraw.fee);
		document.querySelector("#withdraw-input-amount").disabled = false;
    ...
    this.errors['fee'] = false;
		...
		if (amount_avl < this_withdraw.fee) {
			document.querySelector("#withdraw-input-amount").disabled = true;
			document.querySelector('#withdraw-fee-error').innerHTML = 
