...
class Withdraw {
	...
	async render() {
		...
		if (document.getElementById('withdrawal-form')) {
			this.app.browser.replaceElementById(
				WithdrawTemplate(this.app, this.mod, this),
				'withdrawal-form'
			);
		} else {
	...
	async attachEvents() {
		...
		if (document.querySelector('#withdraw-input-address')) {
			document.querySelector('#withdraw-input-address').onblur = async (e) => {
				await this.validateAddressInput();
				await this.fetchWithdrawFee();
			};
		}

		if (document.querySelector('#withdraw-input-amount')) {
			document.querySelector('#withdraw-input-amount').onblur = async (e) => {
		...
		if (document.querySelector('#withdrawal-form') != null) {
			...
			document.querySelector('#withdraw-confirm').onclick = async (e) => {
				...
				try {
					...
					let sender = this.pc.returnAddress();
	...
	async fetchWithdrawFee() {
		...
		this.pc.returnWithdrawalFeeForAddress(address, (amt) => {
			this.fee = Number(amt);
