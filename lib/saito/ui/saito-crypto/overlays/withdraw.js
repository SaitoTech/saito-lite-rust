...
class Withdraw {
    ...
    this.errors = true;
        ...
        this.fetchWithdrawFee(preferred_crypto);
			}
		);
	}
  ...
  async addWithdrawActionItem(item, id) {
	...
	attachDropdownEvents() {
      ...
      document.querySelector(".withdraw-info-value.balance").innerHTML = `fetching...`;
        ...
        this_withdraw.ticker = preferred_crypto.ticker;
				...
				if (preferred_crypto.ticker != 'SAITO') {
	        preferred_crypto.getMixinAddress(this_withdraw.address, preferred_crypto.asset_id, function(res){

	        	if (Object.keys(res).length > 0 && res != null){
	        		document.querySelector('#withdraw-input-address').value = res.address;	
	        	}
	...
	async attachEvents() {
        ...
        let preferred_crypto = await this_withdraw.app.wallet.returnPreferredCrypto();
				this_withdraw.fetchWithdrawFee(preferred_crypto);
			};
		}
  ...
  fetchWithdrawFee(preferred_crypto){
    this_withdraw = this;
    let address = document.querySelector(
          '#withdraw-input-address'
        ).value;
