...
class Withdraw {
		...
		this.app.connection.on(
			...
			async (obj) => {
				...
				if (obj){
					...
					if (typeof obj.dynamicFee != 'undefined') {
						this.dynamicFee = obj.dynamicFee;
					}
    ...
    title="${item.text}"></i>`;
		...
		this.app.browser.addElementToSelector(html, `#withdraw-address-cont`);
	}
	...
	async loadCryptos(ticker = null) {
		...
		let sender = await preferred_crypto.returnAddress();
		let fee = 0;
		let balance = 0;
		let asset_id = '';
      ...
      id="crypto-option-${crypto_mod.name}" value="${crypto_mod.ticker}">${crypto_mod.ticker
				}</option>`;
			...
			if (crypto_mod.ticker == 'SAITO') {
				...
				img_html = `<img class="withdraw-img-${crypto_mod.ticker
		...
		balance = preferred_crypto.returnBalance();
	...
	attachDropdownEvents() {
		...
		document.querySelector('#withdraw-select-crypto').onchange = async (
			...
			setTimeout(async function () {
				...
				let preferred_crypto = this_withdraw.app.wallet.returnPreferredCrypto();
				...
				if (preferred_crypto.ticker != 'SAITO') {
					preferred_crypto.getMixinAddress(this_withdraw.address, preferred_crypto.ticker, async function (res) {
						...
						if (res != null) {
							...
							document.querySelector('#withdraw-address-error').innerHTML = `This user hasn't activated ${preferred_crypto.ticker} address`;
	...
	async attachEvents() {
		...
		if (document.querySelector("#withdraw-input-amount")) {
			document.querySelector("#withdraw-input-amount").onkeyup = async (e) => {
				await this_withdraw.validateAmountInput();
			}

			document.querySelector("#withdraw-input-amount").onchange = async (e) => {
		...
		if (document.querySelector('#withdrawal-form') != null) {
			document.querySelector('#withdrawal-form').onsubmit = (e) => {
				...
				let confirm_txt = `(fee: ${this_withdraw.fee} ${ticker})`;
				if (this_withdraw.dynamicFee == true) {
					let network_fee = document.querySelector("#network-fee").value;
					confirm_txt = `(fee: ${network_fee} ${ticker})`;
				}

				document.querySelector(
					'.withdraw-confirm-fee'
				).innerText = confirm_txt;
			...
			document.querySelector('#withdraw-confirm').onclick = async (e) => {
				...
				try {
					...
					let fee = null;
					if (this_withdraw.dynamicFee == true) {
						fee = document.querySelector('#network-fee').value;
					} else {
						fee = document.querySelector('#withdraw-fee').value;
					}
			...
			if (document.querySelector('#withdraw-max-btn') != null) {
				document.querySelector('#withdraw-max-btn').onclick = async (e) => {
					if (!document.querySelector('#withdraw-input-amount').disabled) {
						...
						document.querySelector('#withdraw-input-amount').value =
							(balance_as_float - Number(this_withdraw.fee));
						await this_withdraw.validateAmountInput();
					}
				};
			}
	...
	async fetchWithdrawFee(preferred_crypto) {
		...
		if (address.length > 30) {
			...
			await preferred_crypto.returnWithdrawalFeeForAddress(
				address,
				function (amt) {
          ...
          <span class="withdraw-fee-info-icon ${(preferred_crypto.ticker).toLowerCase()}">*</span>`;
			...
			// update internal fee info
			document.querySelector('#withdraw-internal-fee-info').innerHTML =
				'0 ' + preferred_crypto.ticker;

			// update external fee info
			if (this_withdraw.fee == 0) {
				// hide external network fee
				document.querySelector('#withdraw-external-fee-span').style.display = 'none';
			} else {
				// show external network fee
				document.querySelector('#withdraw-external-fee-info').innerHTML =
					this_withdraw.fee + ' ' + preferred_crypto.ticker;
				document.querySelector('#withdraw-external-fee-span').style.display = 'block';
			}
		...
		if (amount != '') {
			...
			if (amount <= 0) {
				...
				document.querySelector('#withdraw-amount-error').innerHTML =
					...
					` available)`;
				this_withdraw.errors['amount'] = true;
			} else if (amount + this_withdraw.fee > amount_avl) {
				document.querySelector('#withdraw-amount-error').innerHTML =
					`Error: Your withdrawal amount + transaction fee exceeds available balance. Please reduce the amount to cover withdrawal fee.`;
				this_withdraw.errors['amount'] = true;
			}
	...
	async validateAdrressInput(preferred_crypto) {
		...
		let valid = preferred_crypto.validateAddress(address);
