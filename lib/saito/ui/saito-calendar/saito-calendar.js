...
class SaitoCalendar {
	constructor(app, mod, container = '') {
		...
		this.app.connection.on('calendar-render-request', (event) => {
			console.log(event, "event")
			if(!event.link || !event.type || !event.datetime){
				return console.error("Event object is not valid")
			}
			this.events.push(event);
			this.render();
			let events_today = this.generateTodaysEvents()
			this.app.connection.emit('calendar-popup-render-request', events_today)
		})



		this.keys = this.app.keychain.returnKeys({ name: "scheduled_event" });
		for (let i = 0; i < this.keys.length; i++) {
			let key = this.keys[i];
			let event = {
				"datetime": new Date(key.startTime),
	...
	generateTodaysEvents() {
		let {day, month, month_as_num, year} = this.currentPeriod
		let events_today = [];
