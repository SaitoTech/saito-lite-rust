...
class SaitoCalendar {
  ...
  renderCalendar() {
    ...
    // and update HTML
    //
    document.querySelector(".saito-calendar-date h4").innerHTML =
    ...
    for (let i = 1; i <= this.state.lastDay; i++) {
      if (i === new Date().getDate() && this.date.getMonth() === new Date().getMonth()) {
        days += `<div class="saito-calendar-day saito-calendar-day-${i} saito-highlight" data-id="${i}">${i}</span></div>`;
      } else {
        days += `<div class="saito-calendar-day saito-calendar-day-${i} saito-mouseover" data-id="${i}">${i}</div>`;
      }
    }
