module.exports = SaitoCalendarPopupTemplate = (
	...
	events.sort((a, b) => {
		let dateA = new Date(a.invite ? a.invite.datetime : a.datetime);
		let dateB = new Date(b.invite ? b.invite.datetime : b.datetime);
		return dateA - dateB; 
	});
	...
	for (let i = 0; i < events.length; i++) {
		...
		} else if (invite.type && invite.link) {
			events_html += `
				<div class="saito-calendar-event">
					<div><strong>Event:</strong> ${invite.type}</div>
					<div><strong>Time:</strong> ${eh}:${em}</div>
					<div><strong>Duration:</strong> ${invite.duration}</div>
					<a class="" href="${invite.link}" target="_blank">Join Event</a>
				</div>`;
		}
	}
