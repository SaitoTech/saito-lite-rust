...
class SaitoScheduler {
  ...
  constructor(app, mod, options = { date: new Date() , tx : null , title : "New Event" , adds : []}) {
    ...
    for (let z = 0; z < options.adds.length; z++) {
      if (!this.options.adds.includes(options.adds[z])) { this.options.adds.push(options.adds[z]); }
    }
  ...
  attachEvents(mycallback = null) {
    ...
    document.querySelector(".saito-scheduler-button").onclick = (e) => {
      ...
      //
      invitemod.addPrivateSlotToInviteOptions(this.app.wallet.returnPublicKey());
