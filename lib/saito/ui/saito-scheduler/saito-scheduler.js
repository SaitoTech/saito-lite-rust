...
class SaitoScheduler {
	constructor(
		app,
		mod,
		options = { date: new Date(), tx: null, title: 'New Event', adds: [] }
	) {
		this.app = app;
		...
		this.options.default_utc_sign =
			Math.sign(this.options.default_utc_offset) == 1 ? '+' : '-';
	...
	attachEvents(mycallback = null) {
		...
		document.querySelector('.saito-scheduler-button').onclick = async (
			e
		) => {
			...
			//
			invitemod.addPrivateSlotToInviteOptions(
