...
class ModalSelectCrypto {
	...
	render() {
		...
		this.cryptomod.modal_overlay = new SaitoOverlay(
			this.app,
			this.mod,
			false
		);
		this.cryptomod.modal_overlay.show(
			SelectCryptoTemplate(this.app, this.mod, this.cryptomod)
		);
	}

	attachEvents() {
		this.cryptomod.activate();
		this.cryptomod.attachEventsModalSelectCrypto(
			this.app,
			this.mod,
			this.cryptomod
		);
