...
class MyUserMenu {
  ...
  async render() {
    ...
    if (!document.querySelector("#saito-user-menu")) {
      ...
      let mods = await this.app.modules.respondTo("user-menu");
      let index = 0;
      for (const mod of mods) {
        let item = await mod.respondTo("user-menu");

        if (item instanceof Array) {
          item.forEach((j) => {
            if (j.text === "Add Contact") {
              return;
            }
            if (j.text === "Challenge to Game") {
              return;
            }
            if (j.text === "View Profile") {
              j.text = "View My Profile";
            }
          ...
          if (item.text === "Challenge to Game") {
            continue;
          }
          if (item.text === "View Profile") {
            item.text = "View My Profile";
          }
