...
class GameHammerMobile {
  ...
  render(target = ".gameboard"){
    ...
    if (window.getComputedStyle(this.element).boxSizing == "content-box"){
      boardWidth += parseInt(window.getComputedStyle(this.element).paddingLeft) + parseInt(window.getComputedStyle(this.element).paddingRight);
      boardHeight += parseInt(window.getComputedStyle(this.element).paddingTop) + parseInt(window.getComputedStyle(this.element).paddingBottom);
    }
    if (this.debug) {console.log("With padding included:",boardWidth, boardHeight); }
    boardWidth += parseInt(window.getComputedStyle(this.element).marginLeft) + parseInt(window.getComputedStyle(this.element).marginRight);
    boardHeight += parseInt(window.getComputedStyle(this.element).marginTop) + parseInt(window.getComputedStyle(this.element).marginBottom);
    ...
    let scale = Math.floor(100 * Math.min(window.innerWidth / boardWidth, window.innerHeight / boardHeight)) / 100;

    console.log(this.originalSize);

    this.pinchZoomOrigin = this.getRelativePosition(this.element, {x: 0, y: 0}, this.originalSize, 1);
