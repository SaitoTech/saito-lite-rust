...
class GameHammerMobile {
	...
	render(target = '.gameboard', min_size = 0) {
		...
		let fit_width = Math.max(window.innerWidth, min_size);
		let fit_height = Math.max(window.innerHeight, min_size);
		...
		if (min_size){
			...
			if (min_size > window.innerWidth || min_size > window.innerHeight){
		...
		if (toggle){
			toggle.onclick = (e) => {
				...
				if (icon.classList.contains("fa-minimize")){
					...
					fit_width = Math.max(window.innerWidth, this.min_size);
					fit_height = Math.max(window.innerHeight, this.min_size);	
				}
	...
	makeDraggableBounded(target){
		...
		try {
			...
			//let threshold = 40; 
			// Don't let board go offscreen
			let threshold = element_width;

			let max_x = window.innerWidth - threshold;
			let min_x = threshold - element_width;
			...
			let max_y = window.innerHeight - threshold;
			let min_y = threshold - element_height;
