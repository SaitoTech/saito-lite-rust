    ...
    try{
      if (!document.getElementById("game_board_sizer")) {
        if (selector){
          ...
          this.app.browser.addElementToDom(GameBoardSizerTemplate(this.maxZoom));
        }
      }  
  ...
  attachEvents(target = "#gameboard") {
    ...
    let centerBoard = function(input){
      //Requery screen size
        ...
        if (this.debug) { console.log("Basic gameboard size:",boardWidth, boardHeight);}
        ...
        if (this.debug) {console.log("With padding included:",boardWidth, boardHeight); }
        boardWidth += parseInt(window.getComputedStyle(targetObject).marginLeft) + parseInt(window.getComputedStyle(targetObject).marginRight);
        boardHeight += parseInt(window.getComputedStyle(targetObject).marginTop) + parseInt(window.getComputedStyle(targetObject).marginBottom);
        topAdjustment += parseInt(window.getComputedStyle(targetObject).marginTop);

        if (this.debug) {console.log("and with margins:", boardWidth, boardHeight); }
        ...
        if (this.debug) {console.log("Scaled board:", targetObject.getBoundingClientRect().width, targetObject.getBoundingClientRect().height);}
        ...
        if (targetObject.getBoundingClientRect().width < window.innerWidth){
          let offset = Math.round((window.innerWidth - targetObject.getBoundingClientRect().width)/2) - 10;
    ...
    if (boardScaler){
      ...
      if (sizer_self.mod.loadGamePreference(sizer_self.mod.returnSlug() + "-board-scale")){
          if (this.debug) { console.log("Using saved board Preference"); }
          boardScaler.value =
            sizer_self.mod.loadGamePreference(sizer_self.mod.returnSlug() + "-board-scale");
          sizer_self.scaleBoard(targetObject);
      }else{
        if (this.debug) { console.log("Dynamically scaling board"); }
    ...
    $("#game_board_sizer i").on("click",function(){
      ...
      sizer_self.mod.saveGamePreference(sizer_self.mod.returnSlug() + "-board-scale",
               document.querySelector("#game_board_sizer input").value);  
    });
    
    // and adjust positioning
    let boardoffset = sizer_self.mod.loadGamePreference(sizer_self.mod.returnSlug() + "-board-offset");
    if (boardoffset) {
