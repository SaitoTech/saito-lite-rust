...
class GamePlayerBox {
  ...
  async render() {
    ...
    try {
      ...
      for (; i <= this.mod.game.players.length; i++) {
        let player = await this.playerBox(i);
  ...
  async attachEvents() {
    ...
    for (let player = 1; player <= this.mod.game.players.length; player++) {
      let boxId = await this.playerBox(player);
      $(`#player-box-head-${boxId}`).off();
      if (pb_self.mod.game.players[player - 1] !== pb_self.app.wallet.returnPublicKey()) {
        $(`#player-box-head-${boxId}`).on("dblclick", function () {
          pb_self.app.connection.emit("open-chat-with", {key: pb_self.mod.game.players[player - 1]});
        });
      }
    }
  }
  ...
  async refreshName(pnum, name = "", userline = "") {
    ...
    if (name == "") { name = this.app.keychain.returnUsername(publickey); }
    ...
    let html = `
      <div id="player-box-head-${await this.playerBox(pnum)}" class="player-box-head-${await this.playerBox(pnum)} player-box-head">
    ...
    this.app.browser.replaceElementBySelector(html, ("#player-box-head-" + (await this.playerBox(pnum))));

  }

  async refreshTitle(html, pnum = -1){
    let selector = "#player-box-head-" + (await this.playerBox(pnum));    
   ...
   */
  async addClass(className, player = -1) {
    let selector = "#player-box-" + await this.playerBox(player);
   ...
   */
  async addStatus() {
    let div = document.querySelector(`#player-box-log-${await this.playerBox(-1)}`);
    if (div) {
   ...
   * @param {int} pnum - the player number (according to game), -1 means this player
   */
  async refreshGraphic(html, pnum = -1) {
    this._updateDiv(`#player-box-graphic-${await this.playerBox(pnum)}`, html);
  }


  async appendGraphic(html, pnum = -1){
     try {
      let div = document.querySelector(`#player-box-graphic-${await this.playerBox(pnum)}`);
      if (div) {
        div.innerHTML += html;
      } 
   ...
   * @param {int} pnum - the player number (according to game), -1 means this player
   */
  async insertGraphic(elem_id, pnum = -1) {
    let div = document.getElementById(`player-box-graphic-${await this.playerBox(pnum)}`);
   ...
   * @param {int} pnum - the player number (according to game), -1 means this player
   */
  async refreshLog(html, pnum = -1) {
    this.app.browser.replaceElementContentBySelector(html, `#player-box-body-${await this.playerBox(pnum)}`);
    console.log("LOG INFO IS: " + JSON.stringify(document.querySelector(`#player-box-body-${await this.playerBox(pnum)}`).innerHTML));
  }

  async appendLog(html, pnum = -1){
    try{
      let div = document.getElementById(`player-box-body-${await this.playerBox(pnum)}`);
      if (div){
        div.innerHTML += html;
        console.log(html);
      }else{
        console.log(`player-box-body-${await this.playerBox(pnum)} not found`);
      }
   ...
   * @param {int} pnum - the player number (according to game), -1 means this player
   */
  async refreshInfo(html, pnum = -1) {
    this.app.browser.replaceElementContentBySelector(html, `#player-box-body-${this.playerBox(pnum)}`);
  }
   ...
   * @param {int} pnum - the player number (according to game), -1 means this player
   */
  async hideInfo(pnum = -1) {
    let selector = "#player-box-body-" + (await this.playerBox(pnum));
    try {
...
*/
  alertNextPlayer(pnum, alertType = "flash", stopTrigger = null, duration = 4000) {
    ...
    let selector = "#player-box-" + (await this.playerBox(pnum));
