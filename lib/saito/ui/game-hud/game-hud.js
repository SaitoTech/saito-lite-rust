...
class GameHud {
  ...
  resizeHudCards(cards, fixedSize = 0) {
    console.log("*** RESIZING HUD FROM CARDS***","fixedSize: " + fixedSize);
    try {
      let hud =  document.getElementById("hud");
      ...
      console.log("this.card_width: " + this.card_width);
      console.log("ideal width is: " + ideal_width);     
      ...
      if (ideal_width < 100 && colCt > 3){
        ...
        console.log("Resizing cards based on multiple rows, new ideal width:" + ideal_width);
      }     
      ...
      console.log(`Card Height: ${cardHeight}, Message Height:${msgHeight}`);
  
      switch (this.mode){
        case 0: this.resizeHudHeight(cardHeight+msgHeight+20+(fixedSize?20:0));
                break;
        case 1: this.resizeHudHeight(1.5*cardHeight + msgHeight + (fixedSize?20:0));
                break;
        case 2: this.resizeHudHeight((cardHeight+5)*(cards.length)+msgHeight+10+(fixedSize?20:0));
                break;
      }
  ...
  resizeHudHeight(definingHeight) {
    ...
    console.log("Hud height (orig):",window.getComputedStyle(hud).height,"New Hud Height:",newHeight);
