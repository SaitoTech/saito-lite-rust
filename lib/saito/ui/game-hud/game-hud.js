...
class GameHud {
  ...
  updateStatus(status_html) {
    ...
    if (status_obj) {
      status_obj.innerHTML = sanitize(status_html);
    }
  ...
  resizeHudCards(cards, fixedSize = 0) {
    ...
    try {
      ...
      console.log(
        `Width: ${ideal_width}x${colCt} = ${container_width} (${this.minWidth},${wwidth})`
      );
      ...
      switch (this.mode) {
        ...
        case 2:
          this.resizeHudHeight(
            (cardHeight + 5) * cards.length + msgHeight + 10 + (fixedSize ? 20 : 0)
          );
          break;
      }
    } catch (err) {
  ...
  resizeHudStatus(status) {
    ...
    Array.from(status.children).forEach((kid) => {
      ...
      console.log(
        `Analyzing: ` + kid,
        `Height: ${kidHeight}, Width: ${kid.getBoundingClientRect().width}`
      );
  ...
  resizeHudHeight(definingHeight) {
    ...
    console.log(
      "Hud height (orig):",
      window.getComputedStyle(hud).height,
      "New Hud Height:",
      newHeight
    );
    ...
    if (this.mode === 0 && !hud.style.top) {
      ...
      console.log(
        ...
        hud.style.top,
        "Computed:",
        window.getComputedStyle(hud).top,
        "Bound:",
        currentTop
      );
    }
