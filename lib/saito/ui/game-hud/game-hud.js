...
class GameHud {
	...
	render() {
		...
		if (!document.querySelector('#hud')) {
			this.app.browser.addElementToDom(
				GameHudTemplate(this.enable_mode_change)
			);
		}
	...
	attachEvents() {
		...
		// hud is minimizable
		try {
			let hud_toggle_button =
				document.getElementById('hud-toggle-button');
	...
	updateStatus(msg) {
		if (this.back_button == true && this.back_button_callback != null) {
			this.back_button_clicked = false;
			msg = `<div class="back-button">${this.mod.back_button_html}</div>${msg}`;
		}else{
			msg = `<div class="back-button" style="display:none;">${this.mod.back_button_html}</div>${msg}`;
		}
		...
		if (document.querySelector('.hud-body .status .back-button')) {
			document.querySelector('.hud-body .status .back-button').onclick = (
				e
			) => {
	...
	updateControls(msg) {
		if (document.querySelector('.hud-body .controls')) {
			document.querySelector('.hud-body .controls').innerHTML = msg;
		}
	}
	...
	attachControlCallback(mycallback = null) {
		if (this.game_mod.useCardbox) {
			console.log('attach changeable callback!');
		...
		document.querySelectorAll('.controls ul li.option').forEach((opt) => {
			opt.onclick = (e) => {
				document
					.querySelectorAll('.controls ul li.option')
					.forEach((opt) => {
						opt.onclick = (evt) => {};
						try {
							opt.remove();
				...
				console.log('triggering callback -- does it exist?');
				mycallback(id);
			};
		});
	}
