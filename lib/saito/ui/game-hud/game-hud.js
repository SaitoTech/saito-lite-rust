  ...
  renderStatus(app, game_mod) {
    try {
      ...
      if (status) {
        ...
        if (status.innerHTML !== this.status) {
 ...
 * 
 */ 
  updateStatusMessageAndShowCards(
    status_message_txt,
    hand,
    cards,
    callback = null
  ...
  returnCardItem(card, cards) {
    if (this.mode == 0) {
      return `<div id="${card.replace(
        / /g,
        ""
      )}" class="card showcard cardbox-hud cardbox-hud-status">${this.returnCardImage(
        card,
        cards,
        1
      )}</div>`;
