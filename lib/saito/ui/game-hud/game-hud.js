...
class GameHud {
    ...
    switch (deviceType) {
      case DESKTOP:
        if (hudToggle.classList.contains("fa-caret-up")) {
          ...
          hud.style.top = `${window.innerHeight - 30}px`;
  ...
  resizeHudCards(cards, fixedSize = 0) {
    ...
    try {
      ...
      let colCt = (this.mode === 2)? 1 : cards.length;
      ...
      let container_width = card_width * colCt + 20; //and account for the margin
      ...
      console.log(this.mode,cardHeight,colCt,msgHeight);
      this.resizeHudHeight((cardHeight+20)*(cards.length/colCt)+msgHeight+(fixedSize?20:0));  
   ...
   * If HUD is anchored to bottom to begin with, anchoring is preserved (hud-long)
   * If HUD grows beyond bottom of screen, HUD is repositioned inside the screen (hud-long)
   * If HUD height exceeds screen height, resized to screen height (hud-vertical)
   */
  resizeHudHeight(definingHeight) {
    ...
    let dockBottom =  (hud.getBoundingClientRect().bottom === window.innerHeight);

    let newHeight = Math.min(definingHeight + headerHeight + 20, window.innerHeight - 45);
