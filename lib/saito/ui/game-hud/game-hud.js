...
class GameHud {
  ...
  resizeHudCards(cards, fixedSize = 0) {
    ...
    try {
      ...
      let ideal_hud_width = 0;

console.log("fixedSize: " + fixedSize);
...
console.log("ideal width is: " + ideal_width);     
	...
	ideal_hud_width += ideal_width + 7;
      ...
      if (this.mode ===1){
        ...
        this.resizeHudHeight((cardHeight+10)*(cards.length/colCt)+msgHeight+(fixedSize?20:0));    
      }
      
      // resize hud width
      this.resizeHudWidth(ideal_hud_width);
  ...
  resizeHudHeight(definingHeight) {
    ...
    if (hudRect.bottom > window.innerHeight){
  ...
  resizeHudWidth(ideal_width) {
    let hud = document.getElementById("hud");
    let hud_header = document.querySelector("status-header");
    
    // if header is bigger than cards, exit
    if (hud_header) {
      if (parseInt(hud_header.style.width) > ideal_width) { return; }
    }

    // only for HUD-long
    if (this.mode === 0) {
      hud.style.width = ideal_width + "px";
    }
  }
