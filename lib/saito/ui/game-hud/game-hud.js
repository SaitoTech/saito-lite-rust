...
class GameHud {
  ...
  updateStatusHeaderOnly(status_msg){
    ...
    status_html = status_html.trim();
    
    if (!status_msg.match(/^<[\s\S]*>$/)){
    ...
    if (document.querySelectorAll("#status .status-header")) {
  ...
  updateStatus(status_html) {
    ...
    //if no wrapping html tags, wrap in a default div
    if (!status_html.match(/^<[\s\S]*>$/)){
  ...
  updateZIndex(new_zindex) {
    this.zIndex = new_zindex;
    if (document.querySelector(".hud")) {
      document.querySelector(".hud").style.zIndex = this.zIndex;
    }
  }
}
