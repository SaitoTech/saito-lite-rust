...
class GameObserver {
	...
	render() {
		...
		if (!document.getElementById('game-observer-hud')) {
			...
			document.getElementById('game-observer-hud').style.display =
				'block';
		}
	...
	attachEvents() {
		...
		document.getElementById('game-observer-next-btn').onclick = (e) => {
			document
				.getElementById('game-observer-next-btn')
		...
		console.log(
			'Paused/Halted: ' + this.is_paused + ' ' + this.game_mod.halted
		);
		...
		document.getElementById('game-observer-first-btn').onclick = (e) => {
			...
			this.game_mod.initializeObserverMode(
	...
	updateStep(step) {
		try {
			document.getElementById(
				'game-observer-status'
			).innerHTML = `Game step: ${step}`;
		} catch (err) {}
	}
	...
	async next() {
		...
		if (this.game_mod.processFutureMoves() == 0) {
			...
			this.arcade_mod.observerDownloadNextMoves(
				this.game_mod,
				function (mod) {
					...
					mod.halted = observer_self.is_paused ? 1 : 0;
