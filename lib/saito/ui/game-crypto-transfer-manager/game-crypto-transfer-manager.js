...
class GameCryptoTransferManager {
	...
	async confirmBalance(app, mod, crypto, amount, my_address = '') {
		let selected_crypto_ticker =
			app.wallet.returnCryptoModuleByTicker(crypto).ticker; //is this really necessary?
	...
	sendPayment(
		...
		try {
			...
			if (game_self.crypto_transfers_outbound_trusted) {
				setTimeout(function() {
			...
			// 	let ignore_checkbox =
			// 		document.querySelector('.ignore_checkbox');
	...
	async receivePayment(
		...
		try {
			...
			this.overlay.show(
				...
				() => {
					this.mod.updateLog('Not waiting for confirmation');
			...
			if (mycallback != null) {
				if (game_self.crypto_transfers_inbound_trusted == 1) {
					document.querySelector('.crypto-transfer-fast-mode').style.display = 'none';
					console.log('inside crypto_transfers_inbound_trusted = 1 ///////////////');
					setTimeout(function(){
						mycallback(document.querySelector('#auth_title'));
						this_self.hideOverlay();
					}, 5000);
				} else {
					//this_self.hideOverlay();
					console.log('inside crypto_transfers_inbound_trusted = 0 ///////////////');
			...
			if (confirmBtn) {
				confirmBtn.onclick = (e) => {					
					game_self.crypto_transfers_inbound_trusted = ignoreBtn.checked ? 1 : 0;
					console.log('game_self.crypto_transfers_inbound_trusted ////', game_self.crypto_transfers_inbound_trusted);
	...
	async approveGameStake(
		...
		try {
			...
			if (parseFloat(current_balance) >= parseFloat(stake)) {
				...
				this.overlay.show(
					GameCryptoTransferManagerStakeTemplate(app, sobj)
				);
			} else {
				this.overlay.closebox = true; //include a way to close in case the receive funds hangs
				this.overlay.show(
					GameCryptoTransferManagerDepositTemplate(app, sobj),
					() => {
			...
			document.querySelector('#approve-crypto-request-container #enable_staking_yes').onclick = (e) => {
				//
