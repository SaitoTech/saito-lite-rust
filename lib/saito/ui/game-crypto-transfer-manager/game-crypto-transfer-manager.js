...
class GameCryptoTransferManager {
	...
	async receivePayment(
		...
		try {
			...
			this.overlay.closebox = true; // we used to include a way to close in case the receive funds hangs but we should fix hangs instead
			this.overlay.show(
			...
			if (mycallback != null) {
				mycallback(document.querySelector('#auth_title'), this.overlay);
			}
			...
			if (ignoreBtn) {
				ignoreBtn.onchange = (e) => {
					mod.crypto_auto_settle = ignoreBtn.checked;
				};
			}
		} catch (err) {
			console.log(err);
