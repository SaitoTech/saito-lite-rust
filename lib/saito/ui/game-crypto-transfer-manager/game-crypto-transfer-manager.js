 ...
 */ 
    sendPayment(app, mod, sender, receiver, amount, ts, ticker, mycallback=null) {
      ...
      try{
        ...
        this.overlay.closebox = false; 
        this.overlay.show(GameCryptoTransferManagerSendTemplate(app, sobj));
        this.overlay.blockClose();
 ...
 */ 
    async receivePayment(app, mod, sender, receiver, amount, ts, ticker, mycallback=null) {
      ...
      try{
        ...
        this.overlay.show(GameCryptoTransferManagerReceiveTemplate(app, sobj));
        this.overlay.blockClose();
 ...
 */
	...
	if (current_balance >= stake) {
	  ...
	  //
	    ...
	    app.connection.emit('saito-crypto-deposit-render-request', { ticker : ticker , amount : stake });
	    //setTimeout(() => { window.location = `/redsquare?deposit=yes&ticker=${ticker}&stake=${stake}`;}, 1000);
	    return;
	  }
