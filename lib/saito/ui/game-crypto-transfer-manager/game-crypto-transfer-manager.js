...
class GameCryptoTransferManager {
	...
	sendPayment(
		...
		try {
			...
			if (app.options.gameprefs.crypto_transfers_outbound_trusted) {
				setTimeout(function() {
		...
		try {
			...
			if (mycallback != null) {
				if (app.options.gameprefs.crypto_transfers_inbound_trusted == 1) {
					if (document.querySelector('.crypto-transfer-fast-mode')) {
			...
			if (confirmBtn) {
				confirmBtn.onclick = async (e) => {					
					console.log('BTN CLICKED ////', app.options.gameprefs.crypto_transfers_inbound_trusted);
