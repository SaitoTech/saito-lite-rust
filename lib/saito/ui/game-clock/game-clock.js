...
class GameClock {
	...
	displayTime(clock) {
		...
		try {
			document.getElementById('game-clock-hours').innerHTML =
				sanitize(hours);
			document.getElementById('game-clock-minutes').innerHTML =
				sanitize(minutes);
			document.getElementById('game-clock-seconds').innerHTML =
				sanitize(seconds);
			document.getElementById('game-clock').style.color =
				this.analyzeTime(clock);
