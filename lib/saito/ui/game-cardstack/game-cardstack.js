...
class GameCardStack {
	...
	render() {
		...
		if (document.getElementById(selector)) {
			...
			let card_obj = document.querySelector(`#cardstack_${this.name} .card-slot`);
			if (card_obj) {

				this.card_width = Math.round(card_obj.getBoundingClientRect().width);
			}
		...
		if (this.orientation == "left" || this.orientation == "right") {
			if (container.width < (this.cards.length - 1) * gap + this.card_width) {
				gap = Math.ceil((container.width - this.card_width) / (this.cards.length - 1));
				console.log("Container: " + container.width, "Cards: " + this.cards.length, "New Gap: " + gap);
			}
			...
			let needed_height =
				(num_face_down * gap) / 4 + (this.cards.length - num_face_down - 2) * gap + card_height;
		...
		for (let card of this.cards) {
			...
			html += `<div id="cardstack_${this.name}_${index}" 
                  ...
                  style="z-index:${index};${shift}">${this.mod.returnCardImageHTML(card)}</div>`;
			index++;
		}
	...
	getTopCardElement() {
		if (this.cards.length > 0) {
			return document.getElementById(`cardstack_${this.name}_${this.cards.length - 1}`);
		}
