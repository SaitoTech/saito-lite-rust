...
class GamePlayerBox {
	...
	remove() {
		if (document.querySelector(`.game-playerbox-${this.player_number}`)) {
			document
				.querySelector(`.game-playerbox-${this.player_number}`)
				.remove();
		}
	}
    ...
    */
	...
	onclick(fn = null) {
		if (document.querySelector(`.game-playerbox-${this.player_number}`)) {
			document
				.querySelector(
					`.game-playerbox-head-${this.player_number} .saito-user`
				)
			...
			document
				.querySelector(
					`.game-playerbox-head-${this.player_number} .saito-user .saito-identicon`
				)
				.setAttribute('data-disable', true);
	...
	addClass(classname, target) {
		if (document.querySelector(`.${target}-${this.player_number}`)) {
			document
				.querySelector(`.${target}-${this.player_number}`)
	...
	updateBody(content) {
		if (
			document.querySelector(`.game-playerbox-body-${this.player_number}`)
		) {
			document.querySelector(
				`.game-playerbox-body-${this.player_number}`
			).innerHTML = content;
		}
	}
	...
	updateGraphics(content) {
		if (
			document.querySelector(
				`.game-playerbox-graphics-${this.player_number}`
			)
		) {
			document.querySelector(
				`.game-playerbox-graphics-${this.player_number}`
