...
class GamePlayerBox {
        ...
        for (let player = 1; player <= this.game_mod.game.players.length; player++) {
           ...
           if (pb_self.game_mod.game.players[player-1] !== pb_self.app.wallet.returnPublicKey()){
             $(`#player-box-head-${boxId}`).on("dblclick",function(){
              ...
              let newgroup = chatmod.createChatGroup(members, `Player ${player}: ${app.keys.returnUsername(pb_self.game_mod.game.players[player-1])}`);
                if (newgroup) {
    ...
    */
    playerBox(pnum){
      let player_box = (this.game_mod.game.players.includes(this.app.wallet.returnPublicKey())) ? this.returnPlayersBoxArray() : this.returnViewBoxArray();
    ...
    */
    returnPlayersBoxArray() {
      ...
      if(this.game_mod.seats) {
        ...
        if (this.game_mod.game.players.length == 4) { player_box = [1, 2, 4, 6]; }
