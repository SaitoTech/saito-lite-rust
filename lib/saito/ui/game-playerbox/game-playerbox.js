...
class GamePlayerboxManager {
  ...
  render() {
    ...
    if (this.playerboxes.length < this.mod.game.players.length) {
      for (let i = 0; i < this.mod.game.players.length; i++) {
        this.playerboxes.push(new GamePlayerbox(this.app, this.mod, this.container, this.mod.game.players[i], (i+1))); 
      }
    }
  ...
  setActive(player_number, deactivate_others=true) {
    ...
    // set as active
    //
    let qs = `.game-playerbox-${(player_number-1)}`;
  ...
  */ 
  alertPlayer(player_number=-1, alertType = "flash", stopTrigger = null, duration = 4000) {
    ...
    if (box) { box.classList.add(alertType); }
   
    if (stopTrigger) {
      window.addEventListener(stopTrigger, (e) => { box.classList.remove(alertType); })
    } else {
      setTimeout(() => { box.classList.remove(alertType); }, duration)
    } 
