...
class GamePlayerBox {
  ...
  async attachEvents() {
    ...
    for (let player = 1; player <= this.game_mod.game.players.length; player++) {
      ...
      if (pb_self.game_mod.game.players[player - 1] !== (await pb_self.app.wallet.getPublicKey())) {
        $(`#player-box-head-${boxId}`).on("dblclick", function () {
          pb_self.app.connection.emit("open-chat-with", {
            key: pb_self.game_mod.game.players[player - 1],
          });
        });
      }
    }
  ...
  async groupOpponents(onlyOpponents = true) {
    ...
    if (!this.game_mod.game.players.includes(await this.app.wallet.getPublicKey())) {
  ...
  async playerBox(pnum) {
    ...
    if (!this.game_mod.game.players.includes(await this.app.wallet.getPublicKey())) {
      ...
      let prank = 1 + this.game_mod.game.players.indexOf(await this.app.wallet.getPublicKey()); //Equivalent to game_mod.player ?
      let seat = pnum - prank;
   ...
   */
  async refreshName(pnum, name = "", nameClass = "") {
    let selector = "#player-box-head-" + (await this.playerBox(pnum));

    let playerKey =
      pnum >= 1 ? this.game_mod.game.players[pnum - 1] : await this.app.wallet.getPublicKey();
    ...
    if (name == "") {
      ...
      if (!name) {
        ...
        console.log(
          "Alt keys returns: " + this.app.keychain.returnIdentifierByPublicKey(playerKey)
        );
        name = playerKey;
      }
      if (name.includes("...")) {
  ...
  async refreshLog(html, pnum = -1) {
    this._updateDiv(`#player-box-log-${await this.playerBox(pnum)}`, html);
  }

  async appendLog(html, pnum = -1) {
    try {
      let div = document.getElementById(`player-box-log-${await this.playerBox(pnum)}`);
      if (div) {
        div.innerHTML += html;
        console.log(html);
      } else {
        console.log(`player-box-log-${await this.playerBox(pnum)} not found`);
      }
    } catch (err) {
  ...
  async refreshInfo(html, pnum = -1) {
    this._updateDiv(`#player-box-info-${await this.playerBox(pnum)}`, html);
  }
  ...
  async hideInfo(pnum = -1) {
    let selector = "#player-box-info-" + (await this.playerBox(pnum));
    try {
   ...
   */
  async alertNextPlayer(pnum, alertType = "flash", stopTrigger = null, duration = 4000) {
