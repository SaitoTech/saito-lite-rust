...
class GamePlayerBox {
  ...
  playerBox(pnum) {
    ...
    // some edge-cases / games init game playerbox after construction
    // on render, but there are display functions that might expect 
    // a response. game_mod will only be null if playerbox has not yet
    // rendered, in which case we return 1 to avoid erroring out below.
    if (this.game_mod == null) { return 1; }
