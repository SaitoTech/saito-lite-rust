...
class GamePlayerBox {
  ...
  render() {
    ...
    try {
      ...
      console.log("check active players inside game ////////////////");
      console.log(this.mod.game.players);
  ...
  refreshName(pnum, name = "", nameClass = "") {
    ...
    if (name.includes("...")) { name = `Player ${pnum}`; }
    if (name.includes("@")){ name = name.substring(0, name.indexOf("@")); }

    let html = `
      <div id="player-box-head-${this.playerBox(pnum)}" class="player-box-head-${this.playerBox(pnum)} player-box-head">
        <div class="saito-user saito-user-${publickey}" id="saito-user-${publickey}" data-id="${publickey}" data-disable="false">
          ...
          <div class="saito-userline" data-id="${publickey}"></div>
        </div>
      </div>
    `;


    this.app.browser.replaceElementBySelector(html, ("#player-box-head-" + this.playerBox(pnum)));
  ...
  refreshLog(html, pnum = -1) {
    ...
    console.log("LOG INFO IS: " + JSON.stringify(document.querySelector(`#player-box-body-${this.playerBox(pnum)}`).innerHTML));
  }

  appendLog(html, pnum = -1){
    try{
      let div = document.getElementById(`player-box-body-${this.playerBox(pnum)}`);
      if (div){
        div.innerHTML += html;
        console.log(html);
      }else{
        console.log(`player-box-body-${this.playerBox(pnum)} not found`);
      }
