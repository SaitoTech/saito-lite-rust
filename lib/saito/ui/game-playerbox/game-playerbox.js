...
class GamePlayerBox {
  ...
  attachEvents() {
    ...
    for (let player = 1; player <= this.game_mod.game.players.length; player++) {
      ...
      if (pb_self.game_mod.game.players[player - 1] !== pb_self.app.wallet.getPublicKey()) {
  ...
  groupOpponents(onlyOpponents = true) {
    ...
    if (!this.game_mod.game.players.includes(this.app.wallet.getPublicKey())) {
  ...
  playerBox(pnum) {
    ...
    if (!this.game_mod.game.players.includes(this.app.wallet.getPublicKey())) {
      ...
      let prank = 1 + this.game_mod.game.players.indexOf(this.app.wallet.getPublicKey()); //Equivalent to game_mod.player ?
      let seat = pnum - prank;
  ...
  refreshName(pnum, name = "", nameClass = "") {
    ...
    let playerKey = (pnum >= 1) ? this.game_mod.game.players[pnum - 1] : this.app.wallet.getPublicKey();
