...
class GameMenu {
  ...
  render() {
    ...
    html += `<li id="game-menu-toggle" class="game-menu-icon game-menu-mobile-toggle"><i class="fas fa-bars"></i></li></ul>`;
    ...
    if (this.game_mod?.game?.players) {
      if (this.game_mod.game.players.length > 2) {
        menu_self.addSubMenuOption("game-chat", {
          ...
          callback: function(app, game_mod) {
            menu_self.hideSubMenus();
            app.connection.emit("open-chat-with", {key: game_mod.game.players, name : (game_mod.returnName() + " Chat")});
          },
        });
      }
      ...
      for (let ii = 0; ii < this.game_mod.game.players.length; ii++) {
        if (this.game_mod.game.players[ii] != this.app.wallet.getPublicKey()) {
