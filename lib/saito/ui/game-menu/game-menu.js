...
class GameMenu {
  ...
  async render() {
    ...
    if (this.initialized == 0) {
     for (const menMod of this.app.modules.respondTo("game-menu")) {
      ...
      if (this.debug || this.game_mod.debug){
        this.addSubMenuOption("game-game", {
          ...
          callback: function (app, game_mod) {
            ...
            game_mod.menu.hideSubMenus();
            siteMessage("See console for game state details", 2000);
          }
        });
      }
  ...
  async addChatMenu(shortNames = null, longNames = null) {
    ...
    //
    let cm = this.app.modules.respondTo("chat-manager");
    if (!cm) {
      return;
    }
    ...
    if (this.game_mod?.game?.players) {
      if (this.game_mod.game.players.length > 2) {
        ...
        menu_self.addSubMenuOption("game-chat", {
          ...
          callback: function (app, game_mod) {
            menu_self.hideSubMenus();
            app.connection.emit("open-chat-with", { key: game_mod.game.players.slice(), name: (game_mod.returnName() + " Chat"), id: game_mod.game.id });
          },
        });
      }
