...
class GameMenu {
	...
	render() {
		...
		if (this.initialized == 0) {
			...
			for (const menMod of this.app.modules.respondTo(
				'game-menu',
				this.game_mod
			)) {
				...
				this.addSubMenuOption('game-game', {
					...
					callback: function (app, game_mod) {
						...
						menu_self.overlay.show(
							`<div class="debug_overlay"><button class="saito-button-primary" id="checkmoves">Check for missed transaction</button></div>`
						);
						...
						if (
							!game_mod.styles.includes(
								'/saito/lib/jsonTree/jsonTree.css'
							)
						) {
							game_mod.styles.push(
								'/saito/lib/jsonTree/jsonTree.css'
							);
		...
		let html = `<div id="game-menu-toggle" class="game-menu-mobile-toggle"><i class="fas fa-bars"></i></div><ul>`;
		for (let i = 0; i < this.icons.length; i++) {
			html += GameMenuIconTemplate(this.icons[i]);
		}
		for (let i = 0; i < this.options.length; i++) {
			html += GameMenuOptionTemplate(
				this.options[i],
				this.sub_options[i]
			);
		}
...
alert("not prepend Element to Selector...");
			this.app.browser.replaceElementById(
				GameMenuTemplate(html),
				'game-menu'
			);
		} else {
alert("prepend Element to Selector...");
...
alert("done!");
		...
		for (let i = 0; i < this.sub_options.length; i++) {
			for (let ii = 0; ii < this.sub_options[i].length; ii++) {
				...
				if (this.sub_options[i][ii].sub_menu) {
					for (
						let j = 0;
						j < this.sub_options[i][ii].sub_menu.length;
						j++
					) {
						let ssubmenu = document.querySelector(
							'#' + this.sub_options[i][ii].sub_menu[j].id
						);
						ssubmenu.onclick = (e) => {
							let menuObj = this.returnMenuFromID(
								e.currentTarget.id
							);
							if (menuObj?.callback) {
								menuObj.callback(this.app, this.game_mod);
		...
		for (let i = 0; i < this.sub_options.length; i++) {
			for (let j = 0; j < this.sub_options[i].length; j++) {
				...
				if (this.sub_options[i][j].sub_menu) {
					for (
						let k = 0;
						k < this.sub_options[i][j].sub_menu.length;
						k++
					) {
	...
	addChatMenu(shortNames = null, longNames = null) {
			...
			(chat_group = null) => {
				if (menu_self.initialized) {
					try {
						...
						document
							.getElementById('game-social')
							.classList.add('notification');
						document
							.getElementById('game-chat')
							.classList.add('notification');
						document
							.getElementById('game-menu-toggle')
							.classList.add('notification');
						document
							.getElementById('game-social')
						...
						if (chat_group.members.length == 1) {
							document
								.getElementById('game-chat-community')
								.classList.add('notification');
						} else if (chat_group.members.length >= 2) {
							document
								.getElementById('game-chat-group')
								.classList.add('notification');
						} else {
						}
						...
						setTimeout(() => {
							document
								.getElementById('game-social')
								.classList.remove('new-notification');
							document
								.getElementById('game-menu-toggle')
								.classList.remove('new-notification');
						}, 1000);
		...
		if (this.game_mod?.game?.players?.length > 1) {
			...
			if (!this.app.browser.isMobileBrowser()){
				this.app.connection.on('chat-ready', () => {
					...
					if (this.game_mod.game.players.length > 2) {
						...
						this.app.connection.emit('open-chat-with', {
							key: this.game_mod.game.opponents[0]
						});
					}
			...
			if (menu_self.game_mod.game.players.length > 2) { chat_title = "Game Chat"; }
				...
				callback: function (app, game_mod) {
					...
					if (game_mod.game.players.length > 2) {
						...
						app.connection.emit('open-chat-with', {
							key: game_mod.game.players.slice(),
