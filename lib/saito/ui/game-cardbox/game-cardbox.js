...
class GameCardbox {
  ...
  showCardboxHTML(card, html, action = "", mycallback = null) {
    ...
    document.getElementById("cardbox-card").innerHTML = sanitize(html);
    ...
    if (action) {
      ...
      document.getElementById("cardbox-menu").innerHTML = sanitize(action);
  ...
  show(card, action = "", mycallback = null) {
    ...
    console.log("halfway through show...");
    let html = this.game_mod.returnCardImage(card);
    console.log("halfway through show... 2");
