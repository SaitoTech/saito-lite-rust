...
class GameCardbox {
   ...
   */
  render() {
    if (!this.game_mod.browser_active) { return; }
   ...
   */
  attachEvents() { 
    console.log("For the Saito Cardbox, don't use attachEvents, use attachCardEvents");
  }
   ...
   */
  attachCardboxEvents() { this.attachCardEvents(); }
  attachCardEvents() {
    try {
	  ...
	  if (card.classList.contains("noclick")) { return; }
          ...
          card.onmouseup = async (e) => {
            ...
            if (this.cardbox_lock === 1) {
              ...
              await this.show(e.currentTarget.id, card_action, card_callback);
            }
          };
        });
      }
      ...
      //
      if (document.getElementById("cardbox-exit") != null) {
   ...
   */
  async show(card, action = "", mycallback = null) {
    if (!card) {
      return;
    }
    ...
    await this.showCardboxHTML(card, html, action, mycallback);
  }
