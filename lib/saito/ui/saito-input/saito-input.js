...
const SaitoInputIcons = require('./saito-input-icons');
const SaitoInputAddressAutocompleteBox = require('./saito-input-address-autocomplete-box');
...
class SaitoInput {
	...
	constructor(app, mod, container = '') {
		...
		this.quote = '';
		this.quote_src = '';
		this.icons = new SaitoInputIcons(app, mod, ".saito-input");
		this.address_autocomplete_box = new SaitoInputAddressAutocompleteBox(app, mod); // add to DOM
	}
	...
	render() {
		...
		if (!document.querySelector(this.container + ' .saito-input')) {
			this.app.browser.prependElementToSelector(
				SaitoInputTemplate(this, this.placeholder),
				this.container
			);
		}
				...
				if (document.querySelector('.saito-input-selection-box')) {
					...
					if (!document.querySelector('.saito-input-selection-box')) {
						this.app.browser.addElementToDom(
							SaitoInputSelectionBoxTemplate(this)
						);
			...
			if (this.display == "large") {
				msg_input.onkeydown = (e) => {
					...
					if (this.enable_mentions) {
						msg_input.onkeydown = this.processKeyDownEvents.bind(this);
					}
				};
			} else {
				if (this.enable_mentions) {
...
console.log("testing");
		...
		//
		// if typing an address / username
		//
		if (this.address_autocomplete_box.is_visible) {
			this.address_autocomplete_box.processKeyDownEvents(e);
		}
...
alert("a mention!");
			...
			this.address_autocomplete_box.render(obj);
	...
	getInput(as_html = true) {
		...
		document
			.querySelectorAll(
				`${this.container} .saito-input .text-input .saito-mention`
			)
			.forEach((mention) => {
		...
		//
		// content-editable (esp in Firefox) has a tendency to add a <br> tag if you hit the spacebar
	...
	findKeyOrIdentifier() {
		...
		for (let key of myKeys) {
			...
			if (key?.publicKey) {
				if (key.publicKey.toUpperCase().includes(this.searchString.toUpperCase())) {
			...
			if (!added && key?.identifier) {
				if (key.identifier.toUpperCase().includes(this.searchString.toUpperCase())) {
					matchingKeys.push(key);
				}
			}
		}
	...
	async addAtEvent() {
		...
		//
		// never remove focus from a textarea
		//
		let shift_focus = false;
