...
class SaitoInput {
  ...
  render(autofocus = true) {
    ...
    if (!document.querySelector(this.container + " .saito-input")) {
      if (this.display == "small") {
        ...
        this.position.top = reference.top;
        this.position.left = reference.left -1;
        ...
        let reference = si.getBoundingClientRect();
        this.position.top = reference.top;
        this.position.left = reference.right;
  ...
  liveSearch(e) {
    if (this.open_tab === "at-window") {
      ...
      if (!keys?.length > 0) {
        ...
        this.app.browser.replaceElementByIdOrAddToDom(SaitoInputSelectionBox(this), "saito-input-selection-box");
        this.addAtEvent();
      }
    }
  ...
  getInput() {
    ...
    if (inputBox) {
      ...
      input += sanitize(inputBox.innerHTML);
    }
  ...
  async addAtEvent(searchString) {
    document.querySelectorAll(".saito-input-contact").forEach((contact) => {
      contact.onclick = (e) => {
        ...
        let newText = `<span class="saito-mention saito-address" data-id="${pkey}" data-disable="true" contenteditable="false">${this.app.keychain.returnUsername(
          pkey
        )}</span>  `;
        ...
        if (this.searchString || this.open_tab == "at-window") {
          ...
          if (input.value) {
            ...
            input.innerHTML = input.innerHTML.replace(re, "").replace("<br>", "") + newText;
          }
        } else {
          if (input.value) {
            input.value += newText;
          } else {
            input.innerHTML = input.innerHTML.replace("<br>","") + newText;
          }
        }
