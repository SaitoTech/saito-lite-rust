        ...
        if (document.querySelector('.saito-input-selection-box')) {
          ...
          if (!document.querySelector('.saito-input-selection-box')) {
            ...
            Array.from(document.querySelectorAll('.saito-box-tab')).forEach(
              (tab) => {
                tab.onclick = (e) => {
                  Array.from(document.querySelectorAll('.active-tab')).forEach(
                    (tab2) => {
        ...
        switch (this.open_tab) {
          ...
          case 'photo-window':
            ...
            if (window.innerWidth <= 786) {
              if (
                document.querySelector('#hidden_file_element_tweet-overlay') !=
                null
              ) {
                document
                  .querySelector('#hidden_file_element_tweet-overlay')
                  .click();
              }
            } else {
    ...
    if (this.open_tab === 'at-window') {
      ...
      if (updatedList?.length > 0) {
        ...
        if (e.keyCode == 9) {
          ...
          document.querySelector('.saito-input-contact-list').onkeydown = (
            e
          ) => {
            if (e.keyCode == 38) {
        ...
        this.removeSelectionBox();
        //let obj = document.querySelector(`${this.container} .saito-input textarea`);
        //obj.value += e.key;
      ...
      if ((e.keyCode == 50 || e.charCode == 64) && e.key == '@') {
        ...
        <div class="saito-input-selection-box no-mouse" style="height: 0px; left: ${
          container.left
        }px; top: ${container.top - 1}px">
        ${SaitoInputContacts(this)}
        </div>`);
      ...
      for (let i = this.mentions.length - 1; i >= 0; i--) {
        if (
          !inputBox.value.includes(
            `[[${this.app.keychain.returnUsername(this.mentions[i])}]]`
          )
        ) {
