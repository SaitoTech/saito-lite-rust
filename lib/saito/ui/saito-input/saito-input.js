  ...
  liveSearch(e) {
    ...
    if (this.open_tab === "at-window") {
      ...
      if (updatedList?.length > 0) {
        this.app.browser.replaceElementBySelector(SaitoInputContacts(this), ".saito-input-contact-list");
  ...
  async addAtEvent(shift_focus = true) {
    document.querySelectorAll(".saito-input-contact").forEach((contact) => {
      contact.onclick = (e) => {
        ...
        if ((this.searchString || this.open_tab === "at-window") && !shift_focus) {
          let re = new RegExp("@" + this.searchString + "(?![^[]+\])", "gi");
          if (input.tagName == "INPUT" || input.tagName == "TEXTAREA") {
            input.value = input.value.replace(re, newText);
          } else {
            input.innerHTML = input.innerHTML.replace(re, newHTML).replace("<br>", "");
          }
        } else {
          if (input.tagName == "INPUT" || input.tagName == "TEXTAREA") {
            input.value += newText;
