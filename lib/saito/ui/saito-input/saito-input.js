	...
	processKeyDownEvents(e) {
		...
		if (this.open_tab === 'at-window') {
			...
			if (updatedList?.length > 0) {
				...
				if (e.keyCode == 9) {
					...
					let choices = document.querySelectorAll(
						'.saito-input-contact-list .saito-user'
					);
					let index = 0;
					choices[index].focus();
			...
			if ((e.keyCode == 50 || e.charCode == 64) && e.key == '@') {
				...
				this.app.browser.addElementToDom(`<div class="saito-input-selection-box no-mouse" style="height: 0px; left: ${container.left}px; top: ${container.top - 1}px"><div class="saito-input-contact-list"></div></div>`);
        			let keys = this.findKeyOrIdentifier();
			        for (let key of keys) {
					let x = new SaitoUser(this.app, this.mod, ".saito-input-selection-box .saito-input-contact-list", key.publicKey, "last seen on April 21");
					x.render();
