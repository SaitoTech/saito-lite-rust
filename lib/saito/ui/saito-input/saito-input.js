...
const SaitoInputSelectionBox = require('./saito-input-selection-box');
const SaitoInputLegacySelectionBoxTemplate = require('./saito-input-legacy-selection-box.template');
...
class SaitoInput {
	...
	constructor(app, mod, container = '') {
		...
		this.selection_box = new SaitoInputSelectionBox(app, mod); // add to DOM
	}
	...
	processKeyDownEvents(e) {
		...
		if (this.open_tab === 'at-window') {
			this.selection_box.processKeyDownEvents(e);
			...
			let updatedList = this.findKeyOrIdentifier();
			if (updatedList?.length > 0) {
				...
				if (e.keyCode == 9) {
					...
					let choices = document.querySelectorAll(
						'.saito-input-contact-list .saito-input-contact'
					);
					let index = 0;
					choices[index].focus();
