let SaitoHeaderTemplate = async (app, mod) => {
  ...
  for (const module of await app.modules.respondTo("header-menu")) {
    added_top_menu = 1;
    menu_insert_html += (await module.respondTo("header-menu")).returnMenu(app, mod);
  }
  if (added_top_menu == 1) {
    menu_insert_html += `
