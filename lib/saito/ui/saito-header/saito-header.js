...
class SaitoHeader extends UIModTemplate {
  ...
  addMenuItem(item, id) {
    ...
    if (typeof item.type != 'undefined' && item.type == 'wallet') {
      document.querySelector(".slidein-panel > .saito-header-menu-section > .saito-menu > ul").innerHTML += html;
    } else {
  ...
  attachEvents(){
    ...
    // let offsets = document.querySelector(".appspace-menu").getBoundingClientRect();
    // console.log("offsets /////////");
    // console.log(offsets);
    // document.querySelector(".slidein-panel").style.top = offsets.top + 'px';
  ...
  //  try {
      ...
      document.querySelector("#wallet-btn-history").onclick = (e) => {      
        ...
        let his_asset_id = event.currentTarget.getAttribute("data-assetid");
        let ticker = event.currentTarget.getAttribute("data-ticker");

        let obj = {};
        obj.ticker =  ticker;
        obj.his_exists = false;
        obj.his_asset_id = his_asset_id;
  ...
  async loadCryptoBalance(cryptoMod) {
    try {
      ...
      this.setAttributes(deposit, {
        ...
        "data-balance": cryptoMod.balance,
