...
console.log("our preferred crypto is: " + preferred_crypto.ticker);
...
console.log("IS THIS CRYPTO ACTIVATED? " + activation_check);
  ...
  async loadCryptoBalance(cryptoMod, preferred_crypto) {
    try {
      if (cryptoMod.returnIsActivated()) {
        ...
        // console.log("%%%");
        // console.log("%%%");
        let ac = await this.app.wallet.returnAvailableCryptosAssociativeArray();
        ...
        if (cryptoMod == preferred_crypto) {
          ...
          let initial_balance = preferred_crypto.balance;

          preferred_crypto.fetchBalance(preferred_crypto.asset_id, function(response){
            document.querySelector(`.balance-amount`).innerHTML = response.data.balance;

            if (response.data.balance > initial_balance) {
              let ticker = preferred_crypto.ticker;
              let deposit = response.data.balance - initial_balance;
              salert(Number(deposit).toFixed(2) + " " + ticker + " received in wallet");
            }
          });
        }
