...
class SaitoHeader extends UIModTemplate {
  ...
  async render() {
    ...
    for (const mod1 of mods) {
      console.log("rendering header : " + mod1.name);
  ...
  renderUsername() {
    ...
    if (username === "Anonymous Account" || username === "Anonymous") {
      el.onclick = async (e) => {
        await header_self.register_username_overlay.render();
      };
      ...
      if (key?.email) {
        //Launch profile
        el.onclick = async (e) => {
          await header_self.myUserMenu.render();
        };
      } else {
        //Prompt email registration
        el.onclick = async (e) => {
          await header_self.setup_recovery_overlay.render(username);
        };
      }
    }
  }
