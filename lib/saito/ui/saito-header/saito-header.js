...
class SaitoHeader extends UIModTemplate {
  ...
  initialize(app) {
    ...
    app.connection.on("update_identifier", (tmpkey) => {
      if (tmpkey.publickey === app.wallet.getPublicKey()) {
  ...
  attachEvents() {
    ...
    //
    try {
      let username = app.keychain.returnIdentifierByPublicKey(app.wallet.getPublicKey(), true);

      if (username == "" || username == app.wallet.getPublicKey()) {
      ...
      document.querySelector(".saito-header-show-qrcode").onclick = (e) => {
        ...
        app.browser.generateQRCode(app.wallet.getPublicKey());
      }
