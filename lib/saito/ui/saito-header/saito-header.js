...
class SaitoHeader extends UIModTemplate {
		...
		this.app.connection.on(
			...
			(callback = null) => {
				if (!document.querySelector('.saito-back-button')) {
					...
					const backFn = window.onpopstate;
					window.onpopstate = (e) => {
						console.log("POPSTATE EVENT", e?.state);
						if (e?.state){
							window.onpopstate = backFn;
							console.log("Running callback from popstate!");
							if (callback) {
								callback(e);
							}
							this.render();
						}else{
							console.log("ignore spa event");
						}
					}
					...
					document.querySelector(
						...
						window.onpopstate = backFn;
						console.log("Running callback from header click!");
						if (callback) {
							callback(e);
						}
	...
	async initialize(app) {
		...
		console.log("last_block_hash: ", await app.blockchain.getLatestBlockHash());
		...
		app.connection.on('create-mixin-account', (ticker) => {
			...
			this.loader.show();
			siteMessage(`Installing ${ticker} in Saito Multiwallet...`, 3000);
		});
	}
	...
	async checkBalanceUpdate(){
		try {
			...
			let preferred_crypto = await this.app.wallet.returnPreferredCrypto();
			await preferred_crypto.returnBalance();
			console.log("preferred_crypto: ", preferred_crypto);
