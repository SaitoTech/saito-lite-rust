...
class SaitoHeader extends UIModTemplate {
	...
	async initialize(app) {
		...
		app.connection.on('update_profile', (publicKey) => {
		...
		app.connection.on('update_balance', (wallet) => {
			console.log('update balance inside saito header');
			this.renderCrypto();
		});
		...
		app.connection.on('create-mixin-account', () => {
			...
			document.querySelector('#profile-public-key').innerHTML =
				'generating keys...';
			document
				.querySelector('#profile-public-key')
				.classList.add('generate-keys');
			this.loader.show();
		});
	}
	...
	async renderCrypto() {
			...
			if (preferred_crypto.destination) {
				if (
					...
					if (
						document
							.querySelector('#profile-public-key')
							.classList.contains('generate-keys')
					) {
						document
							.querySelector('#profile-public-key')
							.classList.remove('generate-keys');
					}
		...
		try {
			if (preferred_crypto.returnIsActivated()) {
				let balance = await preferred_crypto.formatBalance(4);
	...
	renderUsername() {
		...
		if (username === 'Anonymous Account' || username === 'Anonymous') {
			el.onclick = (e) => {
				header_self.app.connection.emit('register-profile-or-login', {
