...
class SaitoHeader extends UIModTemplate {
  ...
  constructor(app, mod) {
    ...
    this.app = app;
    this.mod = mod;
    this.setup_recovery_overlay = new SetupRecoveryOverlay(this.app, this.mod, (email, password) => {
      //window.location.reload();
    });
    this.register_username_overlay = new RegisterUsernameOverlay(this.app, this.mod, (identifier) => {
  ...
  attachEvents() {
    ...
    document.querySelector("#wallet-btn-scan").onclick = (e) => {
  ...
  async renderCrypto() {
    try {
      ...
      if (document.querySelector(".wallet-select-crypto").innerHTML != html) {
        document.querySelector(".wallet-select-crypto").innerHTML = html;
        for (let i = 0; i < available_cryptos.length; i++) {
  ...
  async loadCryptoBalance(cryptoMod) {
    try {
      ...
      if (document.querySelector(`.balance-amount`).innerHTML !== this.app.browser.sanitize(innerText)) {
