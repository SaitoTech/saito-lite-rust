...
class SaitoHeader extends UIModTemplate {
	constructor(app, mod) {
      ...
      if (this.is_open == false) {
		...
		this.app.connection.on('saito-header-update-message', (obj = {}) => {
			...
			console.log('msg, flash, callback: ', msg, flash, callback);
	...
	updateHeaderMessage(text = '', flash = false, callback = null){
		//console.log('inside header message function ////');
		let this_self = this;
	...
	async checkBalanceUpdate(){
		try {
			...
			// reset after a cool off period
			if (!this.withdraw.progress) {
				await preferred_crypto.returnBalance();
			} else {
				let balance = Number(previous_balance - this_self.withdraw.amount);
