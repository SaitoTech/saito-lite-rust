	...
	async initialize(app) {
		...
		app.connection.on('header-install-crypto', (ticker) => {
				...
				try {
					...
					if (addressContainer){
						addressContainer.data.add = "";
						addressContainer.innerHTML = 'generating keys...';
	...
	********************************************/
	...
	attachEvents() {
		...
		document.querySelector('.pubkey-containter').onclick = async (e) => {
			let public_key = document.getElementById('profile-public-key').data.add;
	...
	async renderCrypto(force = false) {
		...
		try {
			if (add && addressContainer) {
				if (addressContainer.innerHTML != add || force) {
					...
					addressContainer.innerHTML = `<div>${add.slice(0, -6)}</div><div>${add.slice(-6)}</div>`;
