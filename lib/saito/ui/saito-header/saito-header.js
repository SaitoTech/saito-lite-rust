...
class SaitoHeader extends UIModTemplate {
	constructor(app, mod) {
		...
		// Saito header shouldn't be using hacks to figure out if we have a deposit
	...
	async renderCrypto(force = false) {
		...
		try {
			if (add) {
				if (addressContainer.innerHTML != add || force) {
					...
					addressContainer.innerHTML = add;
		...
		try {
			if (preferred_crypto.isActivated()) {
				// Cache current balance for easy checking for updates
				this.current_balance = Number(preferred_crypto.returnBalance());
