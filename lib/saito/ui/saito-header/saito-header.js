...
class SaitoHeader extends UIModTemplate {
  ...
  renderUsername() {
    ...
    if (username === "Anonymous Account" || username === "Anonymous") {
      el.onclick = (e) => {
        header_self.app.connection.emit("register-username-or-login", {success_callback: (desired_identifier)=>{
          header_self.app.connection.emit("recovery-backup-overlay-render-request", {desired_identifier});
        }});
      }
