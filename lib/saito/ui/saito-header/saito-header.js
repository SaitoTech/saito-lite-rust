...
class SaitoHeader extends UIModTemplate {
  ...
  render() {
    ...
    if (mod == null || !document) {
      alert("MOD NULL IN RENDER HEADER");
      return;
    }
    ...
    mods.forEach((mod) => {
        ...
        console.log("HEADER appspace menu");
        console.log(this_header.callbacks);
        index++;
    })
  ...
  addMenuItem(item, id) {
    document.querySelector(".saito-header-menu-section.appspace-menu > .saito-menu > ul").innerHTML += `
      <li id="${id}" class="saito-header-appspace-option">
        <i class="${item.icon}"></i>
        <span>${item.text}</span>
      </li>
    `;
  ...
  attachEvents(){
    ...
    try {
      ...
      document.querySelector(".saito-header-show-qrcode").onclick = (e) => {
        ...
        this.overlay.show('<div id="qrcode" style="width: 80vw;height:auto;"></div>');
