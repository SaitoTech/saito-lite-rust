  ...
  async renderCrypto() {
    try {
      ...
      document.getElementById("profile-public-key").innerHTML = sanitize(
        await this.app.wallet.returnPreferredCrypto().returnAddress()
      );
  ...
  attachEvents(app, mod) {
    ...
    document.querySelectorAll("#profile-public-key").forEach((element, i) => {
      element.onmouseup = () => {
        ...
        if (selObj.toString() === element.innerHTML && element.innerHTML != "copied...") {
          // The user has selected this element
          document.execCommand("copy");
          let oldVal = element.innerHTML;
          element.innerHTML = "Copied";
          setTimeout(() => {
            element.innerHTML = sanitize(oldVal);
          }, 300);
        }
      };
    });
