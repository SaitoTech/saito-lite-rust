...
class SaitoHeader extends UIModTemplate {
	...
	async initialize(app) {
		...
		app.connection.on('wallet-updated', async () => {
			console.log("$$$$ wallet-updated --> renderCrypto");
		...
		app.connection.on('header-install-crypto', (ticker) => {
				...
				try {
					...
					document.querySelector(".balance-amount").style.display = "none";
					const addressContainer = document.querySelector('#profile-public-key');
					if (addressContainer){
						addressContainer.dataset.add = "";
						addressContainer.innerHTML = '<div>generating keys...</div><div></div>';
	...
	********************************************/
	...
	toggleMenu() {
		if (
			...
			console.log("$$$$ header.toggleMenu --> renderCrypto");
			this.renderCrypto();
	...
	async renderCrypto(force = false) {
		...
		try {
			...
			if (add && addressContainer) {
				if (addressContainer.dataset?.add != add || force) {
					...
					addressContainer.innerHTML = `<div>${add.slice(0, -8)}</div><div>${add.slice(-8)}</div>`;
				...
				console.log("$$$ header or crypto not rendered yet", add, addressContainer);
			}
		...
		try {
			if (preferred_crypto.isActivated()) {
				...
				//xxx this works to show balance
				this.showBalance(balance_as_string);
	...
	/**
	 * 
	 * Takes a string representation of a float and slots it into the html
	 * 
	 */	 
	showBalance(balance = '0.00') {
		
		balance = this.app.crypto.formatBalance(balance);
		...
		document.querySelector(".balance-amount").style.display = "flex";
	...
	initiatePendingDepositsCheck() {
		...
		const checkDeposits = async () => {
			...
			console.log("check pending deposits", intervalTime);
