...
class SaitoHeader extends UIModTemplate {
	...
	async initialize(app) {
		...
		app.connection.on('wallet-updated', () => {
			...
			setTimeout(async function(){
				await this_self.renderCrypto();
			}, 1000);
		});

		app.connection.on('update_balance', async() => {
	...
	async render() {
		...
		await this_header.renderCrypto();
	}
	...
	async renderCrypto() {
		...
		try {
			if (preferred_crypto.returnIsActivated()) {
				let balance = await preferred_crypto.returnBalance();
	...
	async checkBalanceUpdate(){
		// commenting out balance flashing effect because its causing confusion,
		// will add a better alternative
