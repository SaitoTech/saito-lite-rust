  ...
  async loadCryptoBalance(cryptoMod, preferred_crypto) {
    ...
    try {
      if (cryptoMod.returnIsActivated()) {
        ...
        balance = parseFloat(this_header.app.crypto.convertStringToDecimalPrecision(balance, 2));
        ...
        if (cryptoMod == preferred_crypto) {
          ...
          if (preferred_crypto.ticker != "SAITO") {
            preferred_crypto.fetchBalance(preferred_crypto.asset_id, function(response){
              let updated_balance = parseFloat(this_header.app.crypto.convertStringToDecimalPrecision(response.data.balance, 2));
              document.querySelector(`.balance-amount`).innerHTML = updated_balance;
