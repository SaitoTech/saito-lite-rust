...
class SaitoOverlay {
  ...
  show(app, mod, html = "", mycallback = null, eventCallback = null) {
    ...
    try {
      ...
      //DO NOT use sanitize!
      //We have contenteditable divs in some of our overlays
      if (this.closebox) {
  ...
  /**
   *  Destroy any content in the overlay 
   */ 
  clear(){
    let overlay_el = document.querySelector(".saito-overlay");
    if (overlay_el) {
   ...
   */
  blockClose() {
    const overlay_backdrop_el = document.querySelector("#saito-overlay-backdrop");
    if (overlay_backdrop_el) {
      overlay_backdrop_el.onclick = (e) => { };
    }
  }
