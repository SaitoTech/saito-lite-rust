class SaitoMentions {
  constructor(ref, menuRef, resolveFn, replaceFn, menuItemFn, inputType) {
    ...
    console.log("Create new SaitoMentions class for ", ref);
  ...
  selectItem(active) {
    ...
    console.log("selectItem", active, text);
  ...
  async onInput(ev) {
    ...
    */
    ...
    const tokens =
      typeof textBeforeCaret !== 'undefined' ? textBeforeCaret.split(/\s+/) : [];
  ...
  renderMenu(boundPos = null, coords = null) {
    ...
    if (this.options.length > 0) {
      this.menuRef.classList.remove('empty');
    } else {
      this.menuRef.classList.add('empty');
    }
