 ...
 * and add interactivity, all of the functioning is embedded in the showOverlay function
 * General usage: game_mod.overlay.showOverlay(...)
 * 
 *
 */
class GameOverlay {
  ...
  /**
   * Create the DOM elements if they don't exist. Called by showOverlay, so no need to include it in your code
   * @param app - the Saito Application
   ...
   */
  blockClose() {
    const overlay_backdrop_el = document.querySelector(
      "#game-overlay-backdrop"
    );
  ...
  showCardSelectionOverlay(
    ...
    for (let i in cards) {
      ...
      for (let p = 0; p < unselectable_cards.length; p++) {
        if (
          JSON.stringify(unselectable_cards[p]) === JSON.stringify(cards[i])
        ) {
    ...
    document.querySelector(".game-overlay-cardlist-container").style.height =
      "auto";
