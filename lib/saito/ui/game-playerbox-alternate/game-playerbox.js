...
class GamePlayerBox {
  ...
  refreshName(pnum, name = "", nameClass = "") {
    ...
    let html = `<p class="${nameClass}">${name}</p>`;
    html += identicon ? `<img  src="${identicon}">` : "";
 ...
 */
  alertNextPlayer(pnum, alertType = "flash", stopTrigger = null, duration = 3000) {
    console.log('adding next player: Player ', this.playerBox(pnum));
