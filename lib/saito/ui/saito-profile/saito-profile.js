...
class SaitoProfile {
	...
	resetMenuTabs(tabs) {
		...
		// check if blog mod exists and add the blog as a menu tab
		let blogWidget = this.app.modules.getRespondTos('blog-widget', { container: '.tweet-manager', publicKey: this.publicKey });
		console.log('blog widget', blogWidget[0]);
		if (blogWidget.length === 1) {
			this.menu['blog'] = [blogWidget[0]];
		}
