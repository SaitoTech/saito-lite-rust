...
class SaitoProfile {
  ...
  attachEvents() {
    if (document.querySelector(".saito-profile-follow-btn")) {
      document.querySelector(".saito-profile-follow-btn").onclick = (e) => {
        document.querySelector(".saito-profile-follow-btn").style.display = "none";
        this.app.connection.emit("encrypt-key-exchange", this.publicKey);
      };
    }

    if (document.querySelector(".saito-follow")) {
      document.querySelector(".saito-follow").onclick = (e) => {
        this.contactList.render();
      };
    }

    document.querySelectorAll(".redsquare-profile-menu-tabs").forEach((el) => {
      el.onclick = (e) => {
  ...
  loadProfile(mycallback = null) {
    if (this.profile_earliest_ts > 0) {
      //
      // field2 is publickey (the submitted not our own)
      //
      console.log("Check local archives");
      this.app.storage.loadTransactions(
        {
          field1: "RedSquare",
          field2: this.publicKey,
          created_earlier_than: this.profile_earliest_ts,
        },
        (txs) => {
          if (txs?.length == 0) {
            this.profile_earliest_ts = 0;
            this.loadProfile(mycallback);
            return;
          }

          for (let z = 0; z < txs.length; z++) {
            if (txs[z].timestamp < this.profile_earliest_ts) {
              this.profile_earliest_ts = txs[z].timestamp;
            }
          }
    ...
    } else if (this.profile_earliest_ts > -1) {
      console.log("Check network archives");
      this.app.storage.loadTransactions(
        {
          field1: "RedSquare",
          field2: this.publicKey,
      ...
      console.log("No more profile to load");
    }
  }
}
