...
const PhotoUploader = require('../photo-uploader/photo-uploader');
...
class SaitoProfile {
	constructor(app, mod, container = '') {
		...
		this.profile_mod =app.modules.returnFirstRespondTo('saito-profile');
	...
	render() {
		const myqs = '.saito-profile';
	
		if (!this.app || !this.app.keychain) {
			console.error('App configuration or keychain is missing');
			return;
		}
		
		this.keys = this.app.keychain.returnKeys();
	
		if (!this.keys || !this.publicKey) {
			console.error('No keys available or publicKey is missing');
			return;
		}
	
		console.log(this.publicKey, this.profile_mod, "profile mod");
	
		if (!this.profile_mod) {
			console.error('Profile module is not available');
			return;
		}

		if(this.publicKey === this.mod.publicKey){
			let key = this.app.keychain.returnKey(this.publicKey);
			this.bio = ""
			this.identifier = key.identifier || ""
			this.profile=""
			if(key.profile){
				this.bio = key?.profile.bio
				this.photo = key?.profile.photo
			}
		
			const profileElement = document.querySelector(myqs);
			const profileContent = SaitoProfileTemplate(this.app, this.mod, this);
	
			if (profileElement) {
				this.app.browser.replaceElementBySelector(profileContent, myqs);
			} else {
				this.app.browser.addElementToSelector(profileContent, this.container);
			}
			...
			this.profile_mod.fetchManyProfiles([this.publicKey], (profiles) => {
				if (!profiles || !profiles[this.publicKey]) {
					this.bio = ""
					this.identifier = ""
					this.photo = ""
				}else {
					this.bio = profiles[this.publicKey].bio;
					this.identifier = profiles[this.publicKey].identifier;
					this.photo = profiles[this.publicKey].photo;
				}
	...
	attachEvents() {
		...
		if (document.querySelector('.saito-profile-unfollow-contact-btn')) {
			document.querySelector(
				'.saito-profile-unfollow-contact-btn'
			).onclick = async (e) => {
				await this.mod.sendUnfollowTransaction(this.publicKey);
				this.render();
			};
		}

		if (document.querySelector('.saito-profile-follow-contact-btn')) {
			document.querySelector(
				'.saito-profile-follow-contact-btn'
			).onclick = async (e) => {
				await this.mod.sendFollowTransaction(this.publicKey);
				this.render();
			};
		}
		if (document.querySelector('.saito-profile-edit')) {
			document.querySelector(
				'.saito-profile-edit'
			).onclick = async (e) => {
				this.photoUploader = new PhotoUploader(this.app, this.mod)
				this.photoUploader.callbackAfterUpload = async (photo)=> {
					this.photo = await this.app.browser.resizeImg(photo);
				    this.profile_mod.updateProfile(this.identifier, "", this.photo, "")
				}
