module.exports = SaitoProfileTemplate = (app, mod, profile) => {
  ...
  let profileMod = app.modules.returnModule("Profile");
  let publicKey = profile.publicKey;
  let html = `
    <div class="saito-profile">
   
   
      <div id="${publicKey}-profile-banner" class="saito-profile-banner" data-id="${publicKey}">  ${profileMod && profile.publicKey === mod.publicKey ? `	<i class="saito-banner-edit fas fa-camera"> </i>` : ""}</div>
  ...
  html += `
     ...
     <div class="saito-profile-info">
      ...
      <div>
      <span id="${profile.publicKey}-profile-description" class="saito-profile-description" data-id="${profile.publicKey}"> </span>  ${profileMod && profile.publicKey === mod.publicKey ? `<span class="saito-description-edit"> <i class="fas fa-pen"></i></span>` : `<span> </span>`} 
      </div>
    </div>`;
