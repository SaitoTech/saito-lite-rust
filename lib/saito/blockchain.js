...
console.log("adding block to blockring!");
...
console.log("longest chain: " + am_i_the_longest_chain);
  ...
  async addBlockSuccess(block) {
    ...
    // generate fork_id
    //
    this.blockchain.fork_id = this.generateForkId(block_id);
...
console.log("UNFINISHED: deleteBlock");
...
console.log("NEW CHAIN: " + new_chain.length);
console.log("OLD CHAIN: " + old_chain.length);
...
console.log("block invalid because lack of adequate golden ticket support");
      return false;
    }

    if (old_chain.length == 0) {
console.log("wind in validate");
      return await this.windChain(new_chain, old_chain, new_chain.length-1, false);
    } else {
      if (new_chain.length > 0) {
console.log("unwind in validate");
        return await this.unwindChain(new_chain, old_chain, 0, true);
      } else {
console.log("unexpected in validate - old_chain but no new_chain");
	return false;
      }
    }
...
console.log("WIND CHAIN!");
      ...
      //this.onChainReorganization(block, true);
