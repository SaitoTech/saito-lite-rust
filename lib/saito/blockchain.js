...
class Blockchain {
  ...
  async addBlockSuccess(blockHash) {
    ...
    this.forkId = this.generateForkId(block.id);

    // ensure pruning of next block OK will have the right CVs
    if (this.getLatestBlockId() > this.genesis_period) {
      let prunedBlockHash = await this.blockring.returnLongestChainBlockHashByBlockId(this.getLatestBlockId() - this.genesis_period);
      ...
      await this.blocks[prunedBlockHash].upgradeBlockToBlockType(Block.BlockType.Full);
    }
  }
  ...
  generateForkId(blockId) {
    ...
    for (let i = 0; i < 16; ++i) {
      ...
      // index to update
      let idx = 2 * i;
      let blockHash = this.blockring.returnLongestChainBlockHashByBlockId(currentBlockId);
  ...
  getLatestBlockId() {
    return this.blockring.returnLatestBlockId();
  }
  ...
  async downgradeBlockchainData() {
    ...
    let blockHashesCopy = [];
    let blockHashes = this.blockring.returnBlockHashesAtBlockId(pruneBlocksAtBlockId);
