...
class InviteTemplate extends ModTemplate {
  ...
  addInvite(tx) {
    ...
    for (let i = 0; i < tx.transaction.to.length; i++) {
      if (tx.transaction.to[i].publicKey !== invite.creator) {
        if (!invite.adds.includes(tx.transaction.to[i].publicKey)) {
          invite.adds.push(tx.transaction.to[i].publicKey);
  ...
  returnUnacceptedInvitesFromMe() {
    ...
    for (let i = 0; i < this.app.options.invites.length; i++) {
      ...
      if (inv.transaction.from[0] === this.publicKey) {
        if (
          inv.transaction.msg &&
          ...
          for (let z = 0; z < this.app.options.invites.length; z++) {
            ...
            if (invz.transaction.msg.request === "accept") {
              if (invz.transaction.msg.invite_id == inv.transaction.signature) {
                accept_found = 1;
              }
            }
          }
  ...
  async createOpenTransaction(invite_obj = {}) {
    ...
    let newtx = this.app.wallet.createUnsignedTransaction(this.publicKey);
    newtx.msg = {
      module: "Invites",
      request: "open",
      invite: invite_obj
    ...
    for (let i = 0; i < invite_obj.adds.length; i++) {
      if (invite_obj.adds[i] !== "public") {
        if (!added_keys.includes(invite_obj.adds[i])) {
          ...
          slip.publicKey = invite_obj.adds[i];
          slip.amount = 0;
