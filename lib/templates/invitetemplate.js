...
console.log("and emitting menu notification request");
...
console.log("----------------");
    ...
    try {
      ...
      this.app.connection.emit('event-render-request', txmsg.invite);
    } catch (err) {
...
console.log("RECEIVED INVITE WITH: " + JSON.stringify(txmsg));
...
console.log("ignoring invite as not for us!");
	  return;
	}
...
console.log("receive ACCEPT transaction");
      ...
      for (let i = 0; i < invite_obj.adds.length; i++) {
        if (invite_obj.adds[i] !== "public") {
          relay_mod.sendRelayMessage(
            [invite_obj.adds[i], this.app.wallet.returnPublicKey()],
...
console.log("HOW MANY INVITES EXIST? " + this.app.options.invites.length);
...
console.log("does the invite exist? " + found_invite);
...
console.log("SENDING INVITE AS: " + JSON.stringify(invite_obj));
  ...
  receiveAcceptTransaction(tx) {
    ...
    let txmsg = tx.returnMessage();
    if (!txmsg.invite) {
console.log("error: no invite on tx");
      return;
    }
    let invite = txmsg.invite;
...
console.log("did we find the accept?");
  ...
  addPrivateSlotToInviteOptions(publickey) {
    ...
    if (!this.options.num) {
      ...
      this.options.num++;
console.log("INCREASING NUM TO: " + this.options.num);
    }
...
console.log("ADDING AN EXTRA NUM: " + this.options.adds.length + " -- " + this.options.num);
	this.options.num++;
      }
    }
...
console.log("NUM IS NOW: " + this.options.num);
