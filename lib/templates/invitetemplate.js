...
class InviteTemplate extends ModTemplate {
  ...
  onPeerHandshakeComplete(app, peer) {
    ...
    //
    console.log("and local invites? " + this.invites.length);
    for (let i = 0; i < this.invites.length; i++) {
      console.log("emitting: " + JSON.stringify(this.invites[i]));
  ...
  returnUnacceptedInvitesFromMe() {
    ...
    for (let i = 0; i < this.app.options.invites.length; i++) {
      ...
      if (inv.transaction.from[0] === this.app.wallet.getPublicKey()) {
        if (
          inv.transaction.msg &&
  ...
  createOpenTransaction(invite_obj = {}) {
    ...
    console.log("CREATING OPEN TX WITH THIS INVITE OBJ: ");
    ...
    if (!invite_obj.creator) {
      invite_obj.creator = this.app.wallet.getPublicKey();
    } // creator
    if (!invite_obj.adds) {
      invite_obj.adds = [this.app.wallet.getPublicKey()];
    } // addressees
    ...
    if (invite_obj.adds.length == 1 && invite_obj.num == 1) {
      if (invite_obj.adds[0] === this.app.wallet.getPublicKey()) {
    ...
    //
    let added_keys = [this.app.wallet.getPublicKey()];
    let newtx = this.app.wallet.createUnsignedTransaction(this.app.wallet.getPublicKey());
    newtx.msg = {
      module: "Invites",
      request: "open",
      invite: invite_obj,
      ...
      for (let i = 0; i < invite_obj.adds.length; i++) {
        if (invite_obj.adds[i] !== "public") {
          relay_mod.sendRelayMessage(
            [invite_obj.adds[i], this.app.wallet.getPublicKey()],
            `${this.returnSlug()} open`,
            newtx
          );
        }
      }
  ...
  signInvite(invite_obj, invite_id = "") {
    for (let i = 0; i < invite_obj.adds.length; i++) {
      if (invite_obj.adds[i] === this.app.wallet.getPublicKey()) {
        if (this.app.wallet.getPublicKey() == invite_obj.creator) {
  ...
  createJoinTransaction(invite_obj) {
    ...
    for (let i = 0; i < invite_obj.adds; i++) {
      if (invite_objs.adds[i] === this.wallet.getPublicKey()) {
