...
****/
	...
	async addNextMove(gametx) {
		...
		if (this.game.queue) {
			...
			console.info(`Add next move (${gametxmsg.step.game}): `, gametxmsg.turn);
			...
			if (this.game.player == 0 && this.gameBrowserActive()) {
				try {
					if (this.observerControls.is_paused || this.halted) {
						...
						console.log("Observer: addFutureMove, try to continue if not..." + this.gaming_active);
  ...
  */
	async processFutureMoves() {
		...
		console.log("UNLOCK GAME to process future move as next");
		...
		console.log(`Check ${this.game.future.length} future moves for next one...`);
		...
		if (this.game.future?.length > 0) {
			console.log(`We have ${this.game.future.length} future moves, but NOT the next one!`);
		}

		if (this.game.player == 0 && !this.archive_exhausted){
	...
	async sendGameMoveTransaction(
		...
		// Debugging
		if (this.halted){
			console.error("Attempting to send a game move from a halted state -- unhalting!!!!");
		}
...
console.log("#");
console.log("# sending MOVE w/ step: " + JSON.stringify(ns) + " - " + this.publicKey);
console.log("#");
...
//
	...
	async observerDownloadNextMoves(mycallback = null) {
		...
		if (!this.initial_connect){
			console.log("Haven't established peer yet, try again later...");
			setTimeout(()=>{ this.observerDownloadNextMoves(mycallback)}, 1000);
			return null;
		}

		console.log(`Load game moves: ${this.name}_${this.game.id} from ${this.game.originator}`);
		...
		return this.app.storage.loadTransactions(
			...
			async (txs) => {
				...
				console.log(`Checking ${txs.length} game moves for any later than step ${this.game.step.game}`);
				
				let new_moves = 0;
				this.gaming_active = true;
				...
				for (let tx of txs) {
					...
					if (game_move?.step){
						...
						if (
							...
							await this.addFutureMove(tx); //This will save future moves (so saveGame below doesn't overwrite them)
						}
					}else{
						console.log("Non game move: ", game_move);
					}

				}
				
				console.log(`Found ${new_moves} future moves in archives`);

				this.saveGame(this.game.id);

				if (new_moves == 0) {
					this.archive_exhausted = true;
					if (this.game.player == 0){
				...
				if (mycallback) {
					console.log("Run callback in observerDownloadNextMoves");
					mycallback();
				}
			}
		);
