...
class GameMoves {
	...
	async getPendingGameMoves() {
		...
		let pending = await this.app.wallet.getPendingTransactions();
		console.log("Pending TXS: ", JSON.parse(JSON.stringify(pending)));
		for (let i = 0; i < pending.length; i++) {
			let tx = pending[i];
			let txmsg = tx.returnMessage();
			if (txmsg && txmsg.module == this.name) {
...
console.log("^");
console.log("^");
console.log("^ into isUnprocessedMove() ");
...
console.log("... NO ... not an unprocessed move...");
...
console.info(" .");
console.info(" .");
console.info(" addNextMove -- triggering INITIALIZING GAME QUEUE: for " + this.game.id + " as move received");
...
console.log("is old move... splice out");
		...
		if (this.game.player == 0 && this.game.future.length == 0) {
			this.observerControls.updateStatus(
				'No future moves, click again to check the database'
			);
	...
	async endTurn(nextTarget = 0) {
		...
		await this.sendGameMoveTransaction('game', extra);
	}
...
console.log(">");
console.log(">");
console.log("into sendGameMoveTransaction...");

		let privateKey = await this.app.wallet.getPrivateKey();

console.log("we have just received our private key...");
...
console.log("sending MOVE 2");
			...
			if (newtx == null) {
console.log("newtx is null, returning...");
...
console.log("after add transaction to pending...");
			game_self.saveGame(game_self.game.id);
console.log("sending MOVE 4");
