...
console.log("into isUnprocessed Move");
console.log("now #");
console.log("now # asking if unprocessed move...?");
console.log(JSON.stringify(txmsg));

		let tx_step = parseInt(txmsg.step.game) - 1;

console.log("is this an unprocessed move?");
...
console.log("tx_step-1 is: " + tx_step + " which means we are 1 step ahead of game step... YES");
			return 1;
		}
...
console.log("player step: " + player + " ----> " + tx_step);
...
console.log("this move is at least 1 larger than the last move received by: " + player);
console.log("tx_step-1: " + tx_step + " -- general game step --> " + this.game.step.game)
			}
...
console.log("this move is at least 1 larger than the last move received by: " + player);
			...
			//
			// this is an idea for debugging -- faster_play seems to shunt current moves into
			// future moves when we shift into is_halted mode. is it possible this prevents
			// them from being retrieved ?
			//
			//if (
			//	tx_step >= this.game.step.players[player] &&
			//	tx_step < (this.game.step.game + (this.game.players.length))
			//) {
console.log("returning YES here...");
				return 1;
			}
		}
	...
	async addNextMove(gametx) {
		...
		if (
			...
			console.info(
				`This is next Move, but save as future move because halted (${this.halted}) or active (${this.gaming_active})`
			);
...
console.log("received move from " + gametx.from[0].publicKey);
console.log("adding next move - updating game step to " + gametxmsg.step.game);
		...
		console.info(`ANM (${gametxmsg.step.game}) to QUEUE: ${JSON.stringify(gametxmsg.turn)}`);
...
console.log("is unprocessed move... adding next move");
