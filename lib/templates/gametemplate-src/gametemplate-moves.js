...
console.log("STEP: " + txmsg.step.game + " --- " + this.game.step.players[tx.from[0].publicKey]);
	...
	*/
	...
	async addNextMove(gametx) {
		...
		if (
			...
			//
			// if HALTED is the last instruction on the game QUEUE
			//
                        // if we are halted and we receive a RESOLVE move, we want to 
                        // accept it and continue to process the game queue.
                        //                                              
                        let x = this.game.queue[this.game.queue.length-1].split("\t");
                        let queue_command = x[0];
                        let queue_publickey = x[2];                                     

			if (queue_command === "HALTED") {
console.log("MOVE RECEIVED WHILE HALTED: " + JSON.stringify(gametxmsg));
			}
...
console.log("we are now moving into START QUEUE");
			this.startQueue();
		} else {
  ...
  */
	async processFutureMoves() {
		...
		if (this.halted == 1 || this.initialize_game_run == 0) {
			console.info(
...
console.log(" we consider this the next move ... ");
				await this.addNextMove(ftx);
				return 1;
			} else if (this.isFutureMove(ftx.from[0].publicKey, ftxmsg)) {
console.log(" we consider this an unspecified future move...");
...
console.log(" we consider this an old move...");
				this.game.future.splice(i, 1);
