			...
			if (gmv[0] === 'STAKE') {
				...
				if (auths == game_self.game.players.length) {
					...
					if (game_self.game.player - 1 == first_non_verifier_idx) {
						...
						console.log("accept-game-stake gametemplate-queue ////");
						game_self.app.connection.emit("accept-game-stake", { 
							game_mod: this, 
							ticker, 
							stake, 
							accept_callback: (run = 1)=> { game_self.proposeGameStake(ticker, stake, sigs, ts); },
							reject_callback: (run = 2)=> {	game_self.refuseGameStake(ticker, stake); }
						});
					} else {
						this.hud.back_button = false;
