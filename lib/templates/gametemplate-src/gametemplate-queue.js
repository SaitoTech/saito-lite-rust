...
class GameQueue {
  ...
  /**
   * We use a flag game.halted to signal to the game engine that game execution
  ...
  initializeQueueCommands() {
        ...
        for (let i = 0; i < game_self.game.confirms_needed.length; i++) {
          if (game_self.game.confirms_needed[i] == 1) {
            notice +=
              ...
              game_self.app.keychain.returnUsername(game_self.game.players[i]) +
              "</li>";
    ...
    */
	...
	if (!game_self.game.cryptos) { game_self.game.cryptos = {}; }
        ...
        //
        if (Object.keys(game_self.game.cryptos).length === 0 && game_self.game.players.length > 0) {
          //
    ...
    */
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "REQUEST_AVAILABLE_CRYPTOS") {
        ...
        if (game_self.game.player == playerkey) {
          console.log(
            "BROADCASTING: " +
              `AVAILABLE_CRYPTOS\t${this.game.player}\t${JSON.stringify(
                game_self.app.wallet.returnAvailableCryptosAssociativeArray()
              )}`
          );
          game_self.addMove(
            `AVAILABLE_CRYPTOS\t${this.game.player}\t${JSON.stringify(
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "SEND") {
        ...
        //
        if (
          game_self.game.crypto_auto_settle == 1 ||
          game_self.game.state?.crypto_auto_settle == 1
        ) {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "RECEIVE") {
        ...
        //
        if (
          game_self.game_crypto_auto_settle == 1 ||
          game_self.game.state?.crypto_auto_settle == 1
        ) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
