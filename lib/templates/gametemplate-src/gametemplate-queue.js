...
class GameQueue {
	...
	async initializeGameQueue(game_id) {
		...
		if (this.can_bet && this.game.players.length > 1 && !this.game.options?.async_dealing) {
			//
...
//console.log("FM: " + this.game.future.length);
		...
		//
		// ADDED THIS CHECK ON JULY 30, 2024
		//
		// this runs if I am processing the queue outside the game, as happens with async gameplay. in 
		// that case we want to process the queue before we move on and handle future moves.
		//
                if (this.initialize_game_queue_run != 1) {
			this.initializeGameQueue(this.game.id);
			// run the queue before we 
			await this.runQueue();
		}
		...
		if ((await this.runQueue()) == 0) {
			...
			// call this function
			//
//console.log("PROCESS FUTURE MOVES: " + JSON.stringify(this.game.queue));
		...
		*/
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'DECKXOR') {
				...
				//
console.log("in DECKXOR - async dealing is : " + game_self.async_dealing + " // " + JSON.stringify(game_self.game.options));
				if (game_self.async_dealing == 1) { 
					game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
					return 1;
				}
