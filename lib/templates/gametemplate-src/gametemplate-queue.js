...
class GameQueue {
	...
	async initializeGameQueue(game_id) {
		...
		if (this.pending.length > 0 && this.browser_active) {
			...
			if (this.game.player == 0) {
				console.log("Observer.... check for additional moves...");
	 ...
	 */
	async startQueue() {
		console.log("start queue: ", this.halted, this.gaming_active);
		//
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] == 'READY') {
				...
				if (game_self.gameBrowserActive()) {
					...
					console.log("Halt game because ready");
					game_self.halted = 1;
					return 0;
				}
			}
...
console.log("into available cryptos 3 - clear queue");
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SEND') {
				...
				game_self.app.connection.emit('saito-crypto-send-render-request', {
					sender: [sender+'-'+sender_crypto_address],
					receiver: [receiver+'-'+receiver_crypto_address],
					amount: [amount],
					ts,
					ticker,
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'RECEIVE') {
				...
				const receiveWrapper = async function (divname = null, overlay = null, just_restart_queue=0) {
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'BALANCE') {
				...
				//
				game_self.saveGame(game_self.game.id);
				console.log("Halt game to check balance...why?");
				game_self.halted = 1;
				let my_specific_game_id = game_self.game.id;
				try {
					siteMessage(
