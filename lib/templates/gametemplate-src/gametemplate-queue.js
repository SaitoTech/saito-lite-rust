	...
	async startQueue() {
		...
		if ((await this.hasGameMovePending()) && this.game?.initializing == 0) {
			...
			if (await this.hasGameMovePending()) {
				...
				console.log("it's that we have a pending game move...");
				console.log("OUR FUTURE MOVES ARE WHAT? " + JSON.stringify(this.game.future));
				for (let z = 0; z < this.game.future.length; z++) {
					...
					console.log((z+1) + ": " + JSON.stringify(ftxmsg));
				}
			}
			if (await this.game?.initializing) { console.log("it's that the game is initializing..."); }
			if (await this.game?.initializing) { console.log("it's that the game is initializing..."); }
		...
		//
console.log("no pending moves found -- about to run queue");
		if ((await this.runQueue()) == 0) {
			...
			// call this function
			//
console.log("and pass through so running future moves!");
