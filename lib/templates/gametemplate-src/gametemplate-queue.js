...
class GameQueue {
	...
	async initializeGameQueue(game_id) {
		...
		if (this.initialize_game_queue_run == 1) { return; }
		this.initialize_game_queue_run = 1; 
		...
		if (this.can_bet && this.game.players.length > 1) {
			//
			// And we don't have a specified crypto yet
			// Add some commands to have players share what is available
			//
			if (!this.game.options?.crypto && !this.game.cryptos) {
				//
				// players one-by-one inform others of cryptos
				//
				for (let i = 0; i < this.game.players.length; i++) {
					this.game.queue.push(
						'REQUEST_AVAILABLE_CRYPTOS\t' + (i + 1)
					);
				}
			}
		}
...
console.log("@");
console.log("FM: " + this.game.future.length);

		//
		// for debugging purposes, this shows us what is queued on start
		// for future moves / processing.
		//
		for (let z = 0; z < this.game.future.length; z++) {
		  ...
		  console.log("TX: " +(z+1)+ " " + JSON.stringify(ftxmsg));
		}

		//
		// if we are initializing AND have pending transactions, our own move might
		// affect the state of the game queue, so we avoid starting the game queue
		//
		if ((await this.hasGameMovePending()) && this.game?.initializing == 0) {

		 	this.hud.back_button = false;
			await this.updateStatus('Rebroadcasting Last Move');
			//console.info('Should not start processing QUEUE because we have pending TXs');

			if (await this.hasGameMovePending()) {
		...
		if ((await this.runQueue()) == 0) {
			...
			if (this.game.player == 0 && this.browser_active) {
				...
				if (this.observerControls.is_paused) {
					...
					setTimeout(async () => {
						if ((await this.processFutureMoves()) == 0) {
	...
	async runQueue() {
		...
		while (game_self.game.queue.length > 0 && cont == 1) {
			...
			// 		game_self.game.queue[game_self.game.queue.length - 1]
			// );
			...
			for (let i = 0; i < game_self.commands.length; i++) {
				if ((await game_self.commands[i](game_self, gmv)) === 0) {
					...
					// console.info(
					// 	'GAME ENGINE STOPS FOR ' + JSON.stringify(gmv)
					// );
					// console.info(
					// 	'OUTSTANDING QUEUE: ' +
					// 		JSON.stringify(game_self.game.queue)
					// );
					return 0;
				}
			}
		...
		//
		// unset gaming active post-loop
		//
		this.gaming_active = 0; // prevents future moves from getting added to the queue while it is processing
		return 0;
	}
	...
	initializeQueueCommands() {
                ...
                if (this.queue_commands_initialized == 1) { return; }
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SETVAR') {
				...
				if (gmv[1]) { if (gmv[1] == "stake") { return 1; } }
				if (gmv[2]) { if (gmv[2] == "stake") { return 1; } }
				if (gmv[3]) { if (gmv[3] == "stake") { return 1; } }
				if (gmv[4]) { if (gmv[4] == "stake") { return 1; } }
				if (gmv[5]) { if (gmv[5] == "stake") { return 1; } }
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SAVE') {
				...
				let loadurl =
					this.app.browser.protocol + '//' + this.app.browser.host;
				game_self.updateLog(
					`<p></p>This game has been saved. To restore from this point, all players should visit the following URL:<p></p>&nbsp;<p></p><div style="overflow:auto;word-wrap:break-word">${loadurl}/${game_self.returnSlug()}?load=${game_self.game.id}</div><p></p>&nbsp;<p></p>`
				);
...
// console.log("Are more confirmations needed: " + this.areMoreConfirmsNeeded());
				if (this.areMoreConfirmsNeeded()) {
					return 0;
				}
// console.log("We believe not, so splicing out this command...");
// 				game_self.game.queue.splice(
// 					game_self.game.queue.length - 1,
// 					1
// 				);
				return 1;
			}
			...
			if (gmv[0] === 'ACKNOWLEDGE') {
				...
				game_self.playerAcknowledgeNotice(notice, async function () {
					...
					if (game_self.game.id != my_specific_game_id) {
						game_self.game =
							game_self.loadGame(my_specific_game_id);
					}
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'ROUNDOVER') {
				...
				if (game_self.game.players[0] == game_self.publicKey) {
					let newtx =
						await game_self.app.wallet.createUnsignedTransactionWithDefaultFee();
					...
					if (game_self.game.options?.league_id) {
						newtx.msg.league_id =
							game_self.game?.options?.league_id;
					}
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'OBSERVER') {
				...
				let msg = game_self.app.crypto.stringToBase64(
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'PLAY') {
				...
				for (
					let i = 0;
					i < game_self.game.confirms_needed.length;
					i++
				) {
				...
				for (let i = 0; i < players_to_go.length; i++) {
					if (game_self.game.player == parseInt(players_to_go[i])) {
						can_i_go = 1;
						if (
							game_self.game.confirms_needed[
								players_to_go[i] - 1
							] == 1
						) {
		...
		*/
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SHUFFLEDISCARDS') {
				...
				for (let i in game_self.game.deck[deckidx - 1].discards) {
					discarded_cards[i] =
						game_self.game.deck[deckidx - 1].cards[i];
				...
				game_self.game.queue.push(
					'DECK\t' + deckidx + '\t' + JSON.stringify(discarded_cards)
				);
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'LOGDECK') {
				let deckidx = parseInt(gmv[1]);
				game_self.updateLog(
					`Contents of Deck: ${JSON.stringify(
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'LOGHAND') {
				let deckidx = parseInt(gmv[1]);
				game_self.updateLog(
					`Contents of Hand: ${game_self.game.deck[deckidx - 1].hand}`
				);
				game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
			}
			return 1;
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'LOGPOOL') {
				...
				game_self.updateLog(
					`Contents of Pool: ${game_self.game.pool[poolidx - 1].hand}`
				);
				game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
			}
			return 1;
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] == 'DISCARD') {
				...
				let card = gmv[2];
				let handidx =
					game_self.game.deck[deckidx - 1].hand.indexOf(card);
				if (handidx > -1) {
				...
				if (game_self.game.player == recipient) {
					...
					if (
						...
						for (let i = 0; i < cards; i++) {
							...
							if (
								!game_self.game.deck[deckidx - 1].hand.includes(
									newcard
								)
							) {
							...
							//Sanity check
							if (
								!game_self.game.deck[deckidx - 1].cards[newcard]
							) {
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SIMPLEDEAL') {
				...
				for (let i = players; i >= 1; i--) {
					game_self.game.queue.push(
						'DEAL\t' + deckidx + '\t' + i + '\t' + cards
					);
				}
				for (let i = players; i >= 1; i--) {
					game_self.game.queue.push(
						'DECKENCRYPT\t' + deckidx + '\t' + i
					);
				}
				for (let i = players; i >= 1; i--) {
				...
				game_self.game.queue.push(
					'DECK\t' + deckidx + '\t' + JSON.stringify(deck)
				);
			}
			return 1;
		});
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SAFEDEAL') {
				...
				if (
					...
					let cards_to_deal_first =
						game_self.game.deck[deckidx - 1].crypt.length;
					let cards_to_deal_after =
						cards_to_deal - cards_to_deal_first;
					...
					if (Object.keys(discarded_cards).length <= 0) {
						...
						game_self.game.queue.push('DECKRESTORE\t' + deckidx);
						for (
							let z = game_self.game.players.length;
							z >= 1;
							z--
						) {
							game_self.game.queue.push(
...
// console.log("how many keys are in this deck: " + game_self.game.deck[deckidx-1].keys.length);
// console.log("DECK: " + JSON.stringify(game_self.game.deck[deckidx-1]));
				//
				// I sends keys
				//
				game_self.game.turn = [];
					...
					if (my_deck_length == opponent_deck_length) {
						for (let i = 0; i < cards; i++) {
							if (game_self.game.queue[keyidx + i] != null) {
								game_self.game.deck[deckidx - 1].crypt[i] =
									game_self.app.crypto.decodeXOR(
										game_self.game.deck[deckidx - 1].crypt[
											i
										],
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SECUREROLL') {
				...
				if (their_second_hash === '') {
					...
					if (
						game_self.app.crypto.hash(their_second_hash) ===
						originating_hash
					) {
						let new_random = game_self.app.crypto.hash(move);
						for (let z = 0; z < hashes.length; z++) {
							new_random = game_self.app.crypto.hash(
								new_random + hashes[z]
							);
						}
						...
						//
						for (
							let i = game_self.game.queue.length - 1;
							i >= 0;
							i--
						) {
							let sr = game_self.game.queue[i].split('\t');
...
console.log("all players? " + do_we_have_all_players);
...
console.log("not all players have submitted three sigs...");
					return 0;
				}
				...
				for (
					let player_id = 1;
					player_id <= game_self.game.players.length;
					player_id++
				) {
					let player_card = '';
					...
					for (let k = 0; k < players.length; k++) {
						if (players[k] == player_id) {
							...
							if (player_card_set == 0) {
								...
								player_card_set = 1;
								player_hash =
									game_self.app.crypto.hash(player_card);
							} else {
								if (player_hash_idx == 2) {
									player_hash = game_self.app.crypto.hash(
										hashes[k] + player_hash
									);
								} else {
									//
									...
									if (player_hash != hashes[k]) {
										...
										game_self.game.state.sp[player_id - 1] =
											player_card;
									}
								}
							}
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'DECKBACKUP') {
				...
				game_self.old_removed =
					game_self.game.deck[deckidx - 1].removed;
				game_self.old_cards = {};
				game_self.old_crypt = [];
				game_self.old_keys = [];
				game_self.old_hand = [];
				...
				for (
					let i = 0;
					i < game_self.game.deck[deckidx - 1].crypt.length;
					i++
				) {
					game_self.old_crypt[i] =
						game_self.game.deck[deckidx - 1].crypt[i];
					game_self.old_keys[i] =
						game_self.game.deck[deckidx - 1].keys[i];
				}
				for (var i in game_self.game.deck[deckidx - 1].cards) {
					game_self.old_cards[i] =
						game_self.game.deck[deckidx - 1].cards[i];
				}
				for (
					let i = 0;
					i < game_self.game.deck[deckidx - 1].hand.length;
					i++
				) {
					game_self.old_hand[i] =
						game_self.game.deck[deckidx - 1].hand[i];
				}
			...
			if (gmv[0] === 'DECKRESTORE') {
				...
				if (gmv[2] == 'push') {
					...
					for (let i = 0; i < game_self.old_crypt.length; i++) {
						game_self.game.deck[deckidx - 1].crypt.push(
							game_self.old_crypt[i]
						);
						game_self.game.deck[deckidx - 1].keys.push(
							game_self.old_keys[i]
						);
					}
				} else {
					if (game_self.old_fhand) {
						for (let i = game_self.old_fhand.length - 1; i >= 0; i--) {
							if (i == game_self.old_fhand.length - 1) { game_self.game.deck[deckidx - 1].fhand = []; }
					...
					for (let i = game_self.old_crypt.length - 1; i >= 0; i--) {
						game_self.game.deck[deckidx - 1].crypt.unshift(
							game_self.old_crypt[i]
						);
						game_self.game.deck[deckidx - 1].keys.unshift(
							game_self.old_keys[i]
						);
					}
				}
				...
				for (var b in game_self.old_cards) {
					game_self.game.deck[deckidx - 1].cards[b] =
						game_self.old_cards[b];
				}
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'CARDS') {
				...
				for (let i = 1; i <= cryptLength; i++) {
					...
					//if (game_self.game.player != 0) {
					game_self.game.deck[deckidx - 1].crypt[cryptLength - i] =
						card;
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SAFEPOOLDEAL') {
				...
				if (
					...
					game_self.game.queue.push(
						`POOLDEAL\t${gmv[1]}\t${
							cards_to_flip - cards_in_deck
						}\t${gmv[3]}`
					);
					game_self.game.queue.push(`SHUFFLEDISCARDS\t${deckidx}`);
					game_self.game.queue.push(
						`POOLDEAL\t${gmv[1]}\t${cards_in_deck}\t${gmv[3]}`
					);
				}
			}
			return 1;
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'POOLDEAL') {
				...
				for (let i = 1; i <= this.game.players.length; i++) {
					this.game.queue.push(
						`FLIPCARD\t${deckidx}\t${cards_to_flip}\t${poolidx}\t${i}`
					);
				}
    ...
    */
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'FLIPCARD') {
				...
				game_self.game.turn.push(
					`RESOLVEFLIP\t${deckidx}\t${cardnum}\t${poolidx}`
				);
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'RESOLVEFLIP') {
				...
				if (game_self.game.pool[poolidx - 1].crypt.length == 0) {
					...
					game_self.game.pool[poolidx - 1].cards =
						game_self.game.deck[deckidx - 1].cards;
					...
					for (let z = 0; z < cardnum; z++) {
						game_self.game.pool[poolidx - 1].crypt.push(
							game_self.game.deck[deckidx - 1].crypt[z]
						);
					}
				...
				if (
					game_self.game.pool[poolidx - 1].decrypted ==
					decryption_keys_needed
				) {
					for (let i = 0; i < cardnum; i++) {
						let newcard = game_self.game.pool[poolidx - 1].crypt[i];
						newcard = game_self.app.crypto.hexToString(newcard);
						if (
							!game_self.game.pool[poolidx - 1].hand.includes(
								newcard
							)
						) {
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'OBSERVER_CHECKPOINT') {
				...
				if (game_self.game.player == 0) {
					game_self.halted = 1;
					game_self.saveGame(game_self.game.id);
					game_self.observerControls.updateStatus(
						'Pause for Observer Checkpoint'
					);
					return 0;
				}
				return 1;
		...
		*/
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'DECK') {
				...
				for (var i in game_self.game.deck[deckidx - 1].cards) {
					game_self.game.deck[deckidx - 1].crypt.push(
						game_self.app.crypto.stringToHex(i)
					);
				}
			...
			if (gmv[0] === 'DECKXOR') {
				...
				if (game_self.game.deck[deckidx - 1].xor == '') {
					game_self.game.deck[deckidx - 1].xor =
						game_self.app.crypto.hash(Math.random());
				}
				...
				for (
					...
					game_self.game.deck[deckidx - 1].crypt[i] =
						game_self.app.crypto.encodeXOR(
							game_self.game.deck[deckidx - 1].crypt[i],
							game_self.game.deck[deckidx - 1].xor
						);
					game_self.game.deck[deckidx - 1].keys[i] =
						game_self.app.crypto.generateKeys();
				}
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'REQUEST_AVAILABLE_CRYPTOS') {
				...
				let ac =
					await game_self.app.wallet.returnAvailableCryptosAssociativeArray();

				if (game_self.game.player == playerkey) {
					// console.log(
					// 	`BROADCASTING: AVAILABLE_CRYPTOS\t${playerkey}\t${JSON.stringify(
					// 		ac
					// 	)}`
					// );
					game_self.addMove(
						`AVAILABLE_CRYPTOS\t${playerkey}\t${JSON.stringify(ac)}`
					);
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SEND') {
				...
				//initate inbound_trusted if its not set
				if (this.app.options.gameprefs.crypto_transfers_inbound_trusted == null) {
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'RECEIVE') {
				...
				//
				// sanity check so old wallets support
				//
				if (this.app.options.gameprefs.crypto_transfers_outbound_trusted == null) {
					this.app.options.gameprefs.crypto_transfers_outbound_trusted = 1;
				}

				game_self.halted = 1;

				let receive = await game_self.crypto_transfer_manager.receivePayment(
					this.app,
					this,
					[sender+'-'+sender_crypto_address],
					[receiver+'-'+receiver_crypto_address],
					...
					ts,
					ticker,
					async function (divname = null, overlay = null, just_restart_queue=0) {

						if ((divname == null && overlay == null) || just_restart_queue == 1) {
							if (game_self.game.id != my_specific_game_id) {
						...
						await game_self.app.wallet.receivePayment(
							...
							function (robj) {
								...
								if (this_self.app.options.gameprefs.crypto_transfers_inbound_trusted) {
									siteMessage(`Received ${amount} ${ticker}`, 3000);	
								}

								if (this_self.app.options.gameprefs.crypto_transfers_inbound_trusted == 0) {
									if (
										game_self.game.id != my_specific_game_id
									) {
										game_self.game =
											game_self.loadGame(
												my_specific_game_id
			...
			if (gmv[0] === 'STAKE') {
				...
				if (auths == game_self.game.players.length) {
					...
					if (game_self.game.player - 1 == first_non_verifier_idx) {
						...
						// auto-accept
						//
						await game_self.crypto_transfer_manager.approveGameStake(
							this.app,
							this,
							ticker,
							stake,
		...
		/**
		 *  This command ____SIMULATES_____ checking the player balances but does absolutely nothing,
		 *  other than slow the game down (or breaking the game!)
		 *
		 */
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'BALANCE') {
				...
				try {
					siteMessage(
						'checking player balance: ' + report_address,
						5000
					);
					game_self.hud.back_button = false;
				...
				let polling_timer = setTimeout(async () => {
					game_self.hud.back_button = false;
					game_self.updateStatus(
						'Checking Balance: ' + report_address
					);
					await game_self.app.wallet.returnPreferredCryptoBalances(
						addresses,
						function (robj) {
							...
							if (balance_adequate == 1) {
								...
								try {
									game_self.updateLog(
										report_address + ' balance adequate'
									);
									game_self.hud.back_button = false;
									game_self.updateStatus(
										report_address + ' balance adequate'
