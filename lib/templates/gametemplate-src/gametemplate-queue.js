...
class GameQueue {
	...
	async startQueue() {
		...
		//
		// We run commands from the queue until one returns a zero or we run out of queue (1/2)
		//
				...
				for (
					...
					if (game_self.game.confirms_needed[i] == 1) {
						notice +=
							...
							game_self.app.keychain.returnUsername(
		...
		*/
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'RECEIVE') {
				...
				//initate outbound_trusted if its not set
				...
				//if (app.crypto_transfers_inbound_trusted) {
				//	return 1;
				///} else {

					console.log("app.options.gameprefs //////", this.app.options.gameprefs);
					...
					let receive = await game_self.crypto_transfer_manager.receivePayment(
						...
						ts,
						ticker,
						async function (divname = null, overlay = null, skip = 0) {

							console.log('INSIDE CALLBACK ////');
							if (divname == null && overlay == null && skip == 0) {
								console.log("Skipping payment check ///////////");
								if (game_self.game.id != my_specific_game_id) {
									game_self.game =
										game_self.loadGame(my_specific_game_id);
								}
								game_self.updateLog('payment receiving in background...');
								game_self.restartQueue();

								return 1;
							}

							console.log("FETCHING RECEIVE PAYMENT ///////////////");
