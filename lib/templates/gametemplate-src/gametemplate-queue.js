...
console.log("is initialize game queue run? " + this.initialize_game_queue_run);

		if (this.initialize_game_queue_run == 1) { console.log("EXIT"); return; }
...
console.log("BEFORE INITIALIZE GAME");
		this.initializeGame(game_id);
console.log("AFTER INITIALIZE GAME");
...
console.log("BEFORE STEP 2 -- add crypto and save...");
...
console.log("gqe is: " + gqe);
console.log("queue is: " + JSON.stringify(game_self.game.queue));
	...
	initializeQueueCommands() {
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SETVAR') {
				...
				if (gmv[5]) { if (gmv[5] == "stake") { return 1; } }
                                ...
                                if (gmv[1] === "game") {
                                        if (gmv[2]) { gmv[1] = gmv[2]; }
                                        if (gmv[3]) { gmv[2] = gmv[3]; }
                                        if (gmv[4]) { gmv[3] = gmv[4]; }
                                        if (gmv[5]) { gmv[4] = gmv[5]; gmv[5] = null; }
                                }
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'SAVE') {
				...
				game_self.updateLog(
					`<p></p>This game has been saved. To restore from this point, all players should visit the following URL:<p></p>&nbsp;<p></p><div style="overflow:auto;word-wrap:break-word">${loadurl}/${game_self.returnSlug()}?load=${game_self.game.id}&noload=true</div><p></p>&nbsp;<p></p>`
				);
...
console.log("updated confs: " + JSON.stringify(game_self.game.confirms_needed) + " ("+resolve_queue+")");
