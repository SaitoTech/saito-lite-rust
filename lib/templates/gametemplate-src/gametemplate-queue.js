...
class GameQueue {
	...
	async initializeGameQueue(game_id) {
		...
		if (this.game.step.game < 2) {
			...
			if (this.game.crypto || (this.can_bet && this.game.players.length > 1 && !this.game.options?.async_dealing)) {
				//
	...
	async runQueue() {
		...
		while (game_self.game.queue.length > 0 && cont == 1) {
			...
			if (this.browser_active){
				console.log("clock: Update last received");
...
console.log("ok resetting... " + JSON.stringify(players_to_go));
		...
		this.commands.push(async (game_self, gmv) => {
			if (gmv[0] === 'RESOLVE') {
				...
				//
				if (gmv[1]) {
console.log("processing RESOLVE from " + gmv[1]);
					for (let i = 0; i < game_self.game.players.length; i++) {
						if (game_self.game.players[i] === gmv[1]) {
console.log("confirm needed set to zero for " + gmv[1]);
...
console.log("resolve queue: " + resolve_queue + " " + JSON.stringify(game_self.game.confirms_needed));
...
console.log("removing halted... and returning 1");
							return 1;
						} else {
							let gle = game_self.game.queue.length - 2;
							if (game_self.game.queue[gle] === 'RESOLVE') {
...
console.log("removing previous resolve... and returning 1: " + JSON.stringify(game_self.game.queue));
								return 1;
							} else {
								if (gle <= 0) {
									game_self.game.queue = [];
									...
									game_self.game.queue.splice(gle, 2);
console.log("splicing out stuff and returning 1 with queue like this: " + JSON.stringify(game_self.game.queue));
									return 1;
								}
							}
						}
				...
				//
				game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
console.log("default queue splice: " + JSON.stringify(game_self.game.queue));
				return 1;
			}
				...
				if (auths == game_self.game.players.length) {
					...
					if (game_self.game.player - 1 == first_non_verifier_idx) {
						...
						game_self.app.connection.emit("accept-game-stake", { 
							...
							ticker, 
							stake, 
							accept_callback: ()=> { game_self.proposeGameStake(ticker, stake, sigs, ts); },
							reject_callback: ()=> {	game_self.refuseGameStake(ticker, stake); }
						});
					} else {
						this.hud.back_button = false;
