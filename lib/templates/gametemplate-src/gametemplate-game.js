...
class GameGame {
	...
	newGame(game_id = null) {
		...
		game.clock = [];
		game.time = {};
		game.time.last_received = game.timestamp; //For when we last received control of game
		game.time.last_sent = game.timestamp; //For when we send our moves
	...
	async initializeGameFromAcceptTransaction(tx) {
		...
		if (!txmsg.options?.async_dealing){
			...
			if (this.game.step.game > 2) {
				console.info("ACCEPT IGNORED -- GAME IN PROGRESS");
				return false;
			}
  ...
  */
	async receiveStopGameTransaction(resigning_player, txmsg) {
		...
		return this.sendGameOverTransaction(winners, txmsg.reason);
	}
  ...
  */
	async receiveGameoverTransaction(blk, tx, conf, app) {
		...
		if (this.game.over == 0 && (!this.game?.canProcess && this.game.queue.length > 0)){
			this.gameOverCallback = () => {
