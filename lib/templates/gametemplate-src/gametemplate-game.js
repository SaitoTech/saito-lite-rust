...
class GameGame {
  ...
  */
  async sendStopGameTransaction(game_id = null, reason = "forfeit") {
  ...
  */
  async receiveStopGameTransaction(resigning_player, txmsg) {
    ...
    if (txmsg.reason == "cancellation") {
      this.updateLog(`${resigning_player} cancels the game, no one wins`);
    } else {
    ...
    if (this.game.players.includes(tx.from[0].publicKey) && this.game.over !== 1) {
      ...
      if (this.browser_active) {
        ...
        try {
          ...
          await this.updateStatusForGameOver(readable, txmsg.reason !== "cancellation");

          ////////////////////////////////////////
          // Attach Listeners for rematch actions
          ////////////////////////////////////////
          app.connection.on("arcade-reject-challenge", async (game_id) => {
            await this.updateStatusForGameOver(readable, false);
          });
