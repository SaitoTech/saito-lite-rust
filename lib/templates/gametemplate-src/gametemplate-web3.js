	...
	async payWinner(sender, receiver, amount) {
		...
		if (this.publicKey === sender) {
			...
			this.app.connection.emit('saito-crypto-send-render-request', {
					...
					// trusted: this.loadGamePreference("crypto_transfers_outbound_approved"),  
			...
			this.app.connection.emit('saito-crypto-receive-render-request', {
					...
					ts,
					ticker,
					mycallback: async function (divname = null, overlay = null) {
	...
	showStakeOverlay(){
		...
		if (typeof this.game.stake === "object"){
			...
			for (let i in this.game.stake){
				if (i !== 'min'){
					html += `<div>${this.app.keychain.returnUsername(i)}</div> <div>stakes</div> <div>${this.game.stake[i]} ${this.game.crypto}</div>`
				}
			}
			html += "</div>";
		}else{
