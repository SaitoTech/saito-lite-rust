...
class GameArcade {
	...
	returnOptions() {
		...
		if (this.minPlayers === this.maxPlayers) {
			...
			/*
			...
			Uncomment this when I have time to work out a better UX for joining table games mid-process that doesn't get confusing 
			with the game observer controls...
			...
				if (this.opengame) {
				html += `
				...
				if (i == 'stake') {
					...
					if (crypto){
						if (typeof options['stake'] === 'object') {
							sgo[crypto] = JSON.stringify(options['stake']);
