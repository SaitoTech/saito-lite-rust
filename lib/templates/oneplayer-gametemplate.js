...
class OnePlayerGameTemplate extends GameTemplate {
      ...
      <div class="saito-table">
        ...
        <div class="saito-table-body">
          ...
          <div class="saito-table-row">
            ...
            <div>${
              this.game.state.session.round > 0
                ? Math.round(
                    (1000 * this.game.state.session.wins) / this.game.state.session.round
                  ) / 10
                : 0
            }</div>
            <div>${
              this.game.state.lifetime.round + this.game.state.session.round > 0
                ? Math.round(
                    (1000 * (this.game.state.lifetime.wins + this.game.state.session.wins)) /
                      (this.game.state.lifetime.round + this.game.state.session.round)
                  ) / 10
                : 0
            }</div>
          </div>
  ...
  async render(app) {
    if (this.game.player == 0) {
      let c = await sconfirm("This is not a valid game! Return to Arcade");
      if (c) {
  ...
  async receiveStopGameTransaction(resigning_player = null, txmsg = null) {
    if (this.game.over > 0) {
      if (resigning_player == this.publicKey && txmsg){
        console.log("Receiving quit message from Arcade, delete game from wallet");
    ...
    if (this.game.state?.session?.wins >= 0 && this.game.state?.session?.losses >= 0) {
