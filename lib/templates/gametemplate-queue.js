  ...
  for (let i = 0; i < players_to_go.length; i++) {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "RESOLVE") {
        ...
        for (let i = 0; i < game_self.game.confirms_needed.length; i++) {
          if (game_self.game.confirms_needed[i] == 1) {
            notice += '<li class="option resolve">' + game_self.app.keychain.returnUsername(game_self.game.players[i]) + "</li>";
    ...
    */
	...
	//
	// before we process READY and STOP, COLLECT CRYPTO INFO
	//
console.log("do cryptos exist?");
	if (!game_self.game.cryptos) {
...
console.log("BACK TO READY");
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] == "DISCARD"){
        ...
        if (handidx > -1) { game_self.game.deck[deckidx-1].hand.splice(handidx,1); }
        game_self.game.deck[deckidx-1].discards[card] = game_self.game.deck[deckidx-1].cards[card];
    ...
    */
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "HANDBACKUP") {
        let deckidx = 1;
	if (gmv[1]) { deckidx = parseInt(gmv[1]); }          
    ...
    */
	...
	if (!game_self.game.deck[deckidx - 1].hand) { game_self.game.deck[deckidx - 1].hand = []; }
	if (!game_self.game.deck[deckidx - 1].crypt) { game_self.game.deck[deckidx - 1].crypt = []; }
	if (!game_self.game.deck[deckidx - 1].keys) { game_self.game.deck[deckidx - 1].keys = []; }
	if (!game_self.game.deck[deckidx - 1].cards) { game_self.game.deck[deckidx - 1].cards = {}; }
	...
	let cards = {};
        if (gmv[1]) { deckidx = parseInt(gmv[1]); };
        if (gmv[2]) { cards = JSON.parse(gmv[2]); }
	...
	for (let key in cards) {
	  if (!game_self.game.deck[0].cards[key]) { game_self.game.deck[0].cards[key] = cards[key]; }
	}
...
>>>>>>> staging
        ...
        console.log("~~~~~~~~~~~~~~~~~--------~");
        console.log("RECEIVED AVAILABLE CRYPTOS");
        console.log("~~~~~~~~~~~~~~~~~~--------");
...
=======
	if (!game_self.game.cryptos) { game_self.game.cryptos = {}; }
	game_self.game.cryptos[playerkey] = cryptos;

console.log("CRYPTOS LOCALLY: " + JSON.stringify(game_self.game.cryptos));
>>>>>>> staging
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "REQUEST_AVAILABLE_CRYPTOS") {
        ...
        console.log("~~~~~~~~~~~~~~~~~~~~");
        console.log("ASKED FOR MY CRYPTOS");
        console.log("~~~~~~~~~~~~~~~~~~~~");
        console.log(game_self.game.player + " -- " + playerkey);
	if (game_self.game.player == playerkey) {
	  console.log("BROADCASTING: " +`AVAILABLE_CRYPTOS\t${this.game.player}\t${JSON.stringify(game_self.app.wallet.returnAvailableCryptosAssociativeArray())}`);
	  game_self.addMove(`AVAILABLE_CRYPTOS\t${this.game.player}\t${JSON.stringify(game_self.app.wallet.returnAvailableCryptosAssociativeArray())}`);
console.log("1 MOVES ARE:" + JSON.stringify(game_self.moves));
	  game_self.endTurn();
console.log("SENT TO END-TURN");
	}
        game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
        return 0;
      }
