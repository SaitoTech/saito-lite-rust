...
const Slip = require("../saito/slip").default;
  ...
  for (let i = 0; i < players_to_go.length; i++) {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "ROUNDOVER") {
        ...
        if (game_self.game.players[0] == await game_self.app.wallet.getPublicKey()) {
          let newtx = await game_self.app.wallet.createUnsignedTransactionWithDefaultFee();
          let slip = new Slip();
          slip.publicKey = await game_self.app.wallet.getPublicKey();
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "OBSERVER") {
        let msgobj = {
          game_id: game_self.game.id,
          player: await game_self.app.wallet.getPublicKey(),
          module: game_self.game.module,
        };
  ...
  this.game.cryptos[playerkey] = cryptos;
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "SEND") {
        ...
        //
        if (await this.app.wallet.getPublicKey() !== sender) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
        }
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "RECEIVE") {
        ...
        // note: senders identified by Saito publickey not other in this function
        //
        if (await this.app.wallet.getPublicKey() !== receiver) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
        }
