...
class GameQueue {
  ...
  async initializeGameQueue(game_id) {
    ...
    console.log("beginning running queue", this.commands, this.game);
    //
  ...
  async runQueue() {
    ...
    while (game_self.game.queue.length > 0 && cont == 1) {
      ...
      for (let i = 0; i < game_self.commands.length; i++) {
        console.log("running command queue", gmv);
  ...
  initializeQueueCommands() {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "ROUNDOVER") {
        ...
        if (game_self.game.players[0] == game_self.app.wallet.publicKey) {
          let newtx = await game_self.app.wallet.createUnsignedTransactionWithDefaultFee();
          let slip = new Slip();
          slip.publicKey = game_self.app.wallet.publicKey;
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "OBSERVER") {
        let msgobj = {
          game_id: game_self.game.id,
          player: game_self.app.wallet.publicKey,
          module: game_self.game.module,
        };
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] == "READY") {
        ...
        if (game_self.browser_active) {
          ...
          console.log("emitting one in game template");
    ...
    */
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "SEND") {
        ...
        //
        if (this.app.wallet.publicKey !== sender) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
        }
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RECEIVE") {
        ...
        // note: senders identified by Saito publickey not other in this function
        //
        if (this.app.wallet.publicKey !== receiver) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
        }
