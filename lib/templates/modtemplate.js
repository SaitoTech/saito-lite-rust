...
class ModTemplate {
  ...
  async sendPeerDatabaseRequest(dbname, tablename, select = "", where = "", peer = null, mycallback = null) {
    ...
    if (peer == null) {
      this.app.network.sendRequestAsTransactionWithCallback(message.request, message.data, function (res) {
        mycallback(res);
      });
    } else {
  ...
  async sendPeerDatabaseRequestWithFilter(modname = "", sql = "", success_callback = null, peerfilter = null) {
    ...
    console.log("send peer db request with filter");
  ...
  attachScripts(target) {
    ...
    this.scripts.forEach(script => {
      ...
      document.querySelectorAll("script").forEach((el) => {
        if (el.attributes.src.nodeValue == script) { script_attached = true };
      });
      if (!script_attached) {
