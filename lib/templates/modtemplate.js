...
class ModTemplate {
  ...
  async attachScripts() {
    ...
    for (let script of this.scripts) {
      ...
      scriptCount++;
      if (!script_attached) {
        await this.attachScript(script);
      }
    }
  }
  ...
  async displayModal(modalHeaderText, modalBodyText = "") {
    await salert(`${modalHeaderText}${modalBodyText ? ": <br>" : ""}${modalBodyText}`);
  }
  ...
  onUpgrade(type="", privatekey="", walletfile=null) {
