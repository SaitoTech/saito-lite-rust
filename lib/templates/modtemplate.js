...
console.log("SPDRWF");
  ...
  async sendPeerRequestWithFilter(msg_callback = null, success_callback = null, peerfilter = null) {
    ...
    console.log("modtemplate.sendPeerRequestWithFilter");
    let peers = [];
    if (peerfilter == null) {
...
console.log("pf2");
      ...
      for (let i = 0; i < p.length; i++) {
        if (peerfilter(p[i])) {
console.log("peer passed filter");
          peers.push(p[i]);
        } else {
console.log("peer failed filter");
	}
      }
    }

    if (peers.length === 0) {
console.log("no peers, so done here...");
      return;
    }

    for (const peer of peers) {
console.log("send Request as Tx");
      await this.app.network.sendRequestAsTransaction(
        message.request,
        message.data,
        async function (res) {
console.log("and we got a reply!");
  ...
  async sendPeerDatabaseRequestRaw(db, sql, mycallback = null) {
    ...
    await this.app.network.sendRequestAsTransaction(message.request, message.data, function (res) {
