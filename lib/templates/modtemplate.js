...
var treatment = require('../saito/new-ui/browser-elements/browser-elements');
...
class ModTemplate {
  ...
  render(app, mod = "", selector = "") {
    ...
    for (let i = 0; i < this.components.length; i++) {
      //
      // this prevents us submitting selectors that over-ride the  
      // default for components if no selector is provided to 
      // added components.
      //
      if (selector !== "") {
        this.components[i].render(app, mod, selector);
      } else {
        this.components[i].render(app, mod);
      }
    }
    ...
    if (message.request == 'rawSQL') {
      ...
      if (message.data.module == this.name) {
        ...
        if (this.sqlcache[sql] && this.sqlcache_enabled == 1) {
          ...
          rows = await this.app.storage.queryDatabase(sql, params, message.data.dbname);
        }
