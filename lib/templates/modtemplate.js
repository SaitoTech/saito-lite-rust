...
class ModTemplate {
  ...
  onPeerHandshakeComplete(app, peer=null) {
    ...
    //
    if (peer.hasService("archive")) { this.onArchiveHandshakeComplete(app, peer); }
  ...
  //
  addScript(s) { for (let i = 0; i < this.scripts.length; i++) { if (this.scripts[i] === s) { return; } }; this.scripts.push(s); }
  addStyle(s) { for (let i = 0; i < this.styles.length; i++) { if (this.styles[i] === s) { return; } }; this.styles.push(s); }
  addComponent(obj) { for (let i = 0; i < this.components.length; i++) { if (this.components[i] === obj) { return; } }; this.components.push(obj); };
  removeComponent(obj) { for (let i = this.components.length; i >= 0; i--) { if (this.components[i] === obj) { this.components.splice(i, 1); } } };
  ...
  //
  renderInto(querySelector = "") { return null; }
...
console.log("PRE CACHE: " + new Date().getTime());
console.log(sql);
          rows = this.sqlcache[sql];
console.log("POST CACHE: " + new Date().getTime());
        } else {
console.log("PRE DB: " + new Date().getTime());
console.log(sql);
          rows = await this.app.storage.queryDatabase(sql, params, dbname);
console.log("POST DB: " + new Date().getTime());
...
console.log("rendering callback1");
        mycallback(res);
        return;
...
console.log("RAWSQL in HANDLE PEER TRANSACTION");
...
console.log("rendering callback2");
        mycallback(res);
        return;
  ...
  async sendPeerDatabaseRequestWithFilter(modname = "", sql = "", success_callback = null, peerfilter = null) {
    ...
    if (sql == "") { return; }
    if (modname == "") { return; }
    if (!this.app.modules.returnModule(modname)){console.error(modname + " not found!"); return; }
...
console.log("RAWSQL SEND: "+JSON.stringify(msg));
  ...
  attachPostScripts(target) {
    if (this.postScriptsAdded === true) { return; }
