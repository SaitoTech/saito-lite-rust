...
class GameTableTemplate extends GameTemplate {
  ...
  initializeHTML(app) {
    ...
    if (this.game.player == 0) {
      ...
      if (this.willleave || this.game?.eliminated?.includes(app.wallet.getPublicKey())) {
  ...
  sendMetaMessage(request) {
    ...
    newtx.msg = {
      ...
      request: request,
      my_key: this.app.wallet.getPublicKey()
    };
    ...
    this.app.connection.emit("relay-send-message", { recipient: this.game.accepted, request: "game relay update", data: newtx.transaction });
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (gametxmsg.request === "JOIN") {
            ...
            if (!this.toJoin.includes(gametxmsg.my_key)) {
              ...
              siteMessage(
                `${
                  app.wallet.getPublicKey() == gametxmsg.my_key
                    ? "You"
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "PLAYERS") {
        ...
        for (let pkey of this.toLeave) {
          ...
          if (pkey === this.app.wallet.getPublicKey()) {
