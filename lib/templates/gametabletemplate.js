    ...
    if (btn){
      btn.onclick = (e) => {
        switch (e.currentTarget.id){
          case "join":
                game_self.displayWarning("Join game","You will be dealt into the next hand (if there is a free seat)");
                game_self.willjoin = true;
                game_self.scoreboard.update(this.scoreFrame+`<div id="cancel" class="table_ctrl">CANCEL</div>`, game_self.controller.bind(game_self));
          ...
          case "cancel":
                ...
                if (game_self.willjoin){
                  ...
                  game_self.scoreboard.update(this.scoreFrame+`<div id="join" class="table_ctrl">JOIN</div>`, game_self.controller.bind(game_self));
                }else{
                  game_self.willleave = false;
                  game_self.scoreboard.update(this.scoreFrame+`<div id="leave" class="table_ctrl">LEAVE</div>`, game_self.controller.bind(game_self));
                }
                break;
          case "watch":
                //Add code to reload with observer mode
          default: console.log(e.currentTarget.id);
        }
      };
    }
  }
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (gametxmsg.request === "JOIN"){
            ...
            if (!this.toJoin.includes(gametxmsg.my_key)){
              ...
              siteMessage(`${(app.wallet.returnPublicKey() == gametxmsg.my_key)? "You":app.keys.returnUsername(gametxmsg.my_key)} will be dealt in next hand`, 2500);
            }
          ...
          if (gametxmsg.request === "LEAVE"){
            ...
            if (!this.toLeave.includes(gametxmsg.my_key)){
              ...
              siteMessage(`${(app.wallet.returnPublicKey() == gametxmsg.my_key)? "You":app.keys.returnUsername(gametxmsg.my_key)} will leave the table after this hand`, 2500);
            }
          }
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "PLAYERS") {
        ...
        for (let pkey of this.toLeave){
          ...
          this.updateLog(`Player ${i+1} (${pkey}) leaving the table.`);
        ...
        while (this.toJoin.length > 0 && this.game.players.length < this.maxPlayers){
          ...
          this.updateLog(`${this.getShortNames(pkey)} joining the table as Player ${this.game.players.length}`);
          if (pkey === this.app.wallet.returnPublicKey()){
