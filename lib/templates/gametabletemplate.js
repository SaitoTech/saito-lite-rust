...
class GameTableTemplate extends GameTemplate {
  ...
  render(app) {
    ...
    if (this.game.players.length < this.maxPlayers && this.game.options["open-table"]) {
      if (!this.game.eliminated || !this.game.eliminated.includes(app.wallet.publicKey)) {
        this.menu.addSubMenuOption("game-game", {
          text: "Join Table",
          id: "game-table",
          class: "game-table",
  ...
  sendMetaMessage(request) {
    ...
    newtx.msg = {
      ...
      request: request,
      my_key: this.app.wallet.publicKey,
    };
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (gametxmsg.request === "JOIN") {
            ...
            if (!this.toJoin.includes(gametxmsg.my_key)) {
              ...
              siteMessage(
                `${
                  app.wallet.publicKey == gametxmsg.my_key
                    ? "You"
  ...
  addPlayerLate(address) {
    ...
    // add key
    if (this.app.wallet.publicKey === address) {
      //this.game.live = 1;
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "PLAYERS") {
        ...
        for (let pkey of this.toLeave) {
          ...
          if (pkey === this.app.wallet.publicKey) {
        ...
        if (change) {
          ...
          console.log("!!!!!!!!!!!!!!!!!!!!");
          console.log("My Public Key: " + this.app.wallet.publicKey);
