/*********************************************************************************
 ...
 will expect denomination in NOLAN.
   ...
   // @return {Array} Array of {hash: {String}} where hash is the transaction_id
   //
   async receivePayment(amount=0, sender="", recipient="", timestamp=0, unique_hash="");
...
class CryptoModule extends ModTemplate {
  ...
  constructor(app, ticker) {
    ...
    this.optionsStorage.transfers_inbound = [];
    this.optionsStorage.transfers_outbound = [];
  ...
  initialize(app) {
    this.load();
console.log("IS ACTIVATED: " + this.optionsStorage.isActivated);
    if (this.optionsStorage.isActivated) {
      app.connection.emit('crypto_activated');
    }
  }

  isActivated() { return this.optionsStorage.isActivated; }
...
console.log("IS ACTIVATED ON ACTIVATE: " + this.optionsStorage.isActivated);
    if (this.optionsStorage.isActivated !== true) {
