...
class CryptoModule extends ModTemplate {
	...
	returnIsActivated() {
		...
		console.log("returnIsActivated this.is_initialized: ", this.is_initialized);
		console.log("returnIsActivated this.app.options.crypto[this.ticker]: ", this.ticker);
		console.log("returnIsActivated this.app.options.crypto[this.ticker]: ", this.app.options.crypto[this.ticker]);

		console.log('isActivated 1');
		if (this.is_initialized == 1 ) {
			console.log('isActivated 1.1');
			isActivatedCheck = this.options.isActivated;
		}

		console.log('isActivated 2');
	 ...
	 */
	async activate() {
		console.log("activate this.options.isActivated: ", this.options.isActivated);
		if (this.options.isActivated !== true) {
			this.options.isActivated = true;

			console.log("activate this.destination: ", this.destination);
			if (this.destination == "") {
				this.destination = await this.returnAddress();
			}

			console.log("activate saving ////");
			this.save();
		}

		console.log("activate returnBalance ////");
...
CryptoModule.prototype.renderModalSelectCrypto = async function (
	app,
	mod,
	cryptomod
) {
	let confirmations = this.confirmations;
	let address = await this.returnAddress();
