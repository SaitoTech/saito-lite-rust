...
class CryptoModule extends ModTemplate {
  ...
  initialize(app) {
    ...
    if (!this.options.transfers_inbound) { this.options.transfers_inbound = []; }
    if (!this.options.transfers_outbound) { this.options.transfers_outbound = []; }
  ...
  isActivated() { return this.returnIsActivated(); }
  returnIsActivated() {
console.log("are we activated? " + this.is_initialized + " / " + this.options.isActivated);
    if (this.is_initialized == 1) { return this.options.isActivated; }
...
console.log("does ticker exist?");
      if (this.app.options.crypto[this.ticker]) {
console.log("does ticker exist 2?");
console.log(JSON.stringify(this.app.options.crypto[this.ticker]));
        if (this.app.options.crypto[this.ticker].isActivated == true) {
console.log("does ticker exist 3?");
          return true;
        }
      }
    }
...
CryptoModule.prototype.renderModalSelectCrypto = function(app, mod, cryptomod) {
...
CryptoModule.prototype.attachEventsModalSelectCrypto = function(app, mod, cryptomod) {
  return null;
}
 ...
 * @return {Number}
 */
CryptoModule.prototype.sendPayment = async function(amount="", recipient="", unique_hash="") {
  for (let i = 0; i < this.options.transfers_outbound.length; i++) {
    if (this.options.transfers_outbound[i].unique_hash === unique_hash && unique_hash !== "") {
      return 1;
    }
...
CryptoModule.prototype.returnCachedAddress = function() { return this.address; }
CryptoModule.prototype.returnCachedBalance = function() { return this.balance; }
/**
...
CryptoModule.prototype.returnWithdrawalFeeForAddress = function(recipient = "", mycallback = null) {
  if (mycallback != null) { mycallback(0); }
};
