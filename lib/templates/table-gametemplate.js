...
class GameTableTemplate extends GameTemplate {
	...
	async render(app) {
		...
		this.maxPlayers =
			this.game.options['game-wizard-players-select-max'] ||
			this.maxPlayers;
			...
			this.menu.addSubMenuOption('game-game', {
				...
				callback: async function (app, game_mod) {
					...
					let c = await sconfirm(
						'Do you wish to leave the table at the end of this hand?'
					);
					if (c) {
		...
		if (
			this.game.players.length < this.maxPlayers &&
			this.game.options['open-table']
		) {
			if (
				!this.game.eliminated ||
				!this.game.eliminated.includes(this.publicKey)
			) {
				this.menu.addSubMenuOption('game-game', {
					text: 'Join Table',
					id: 'game-table',
					...
					callback: async function (app, game_mod) {
						game_mod.menu.hideSubMenus();
						let c = await sconfirm(
							'Request to be dealt into the next hand?'
						);
						if (c) {
							game_mod.sendMetaMessage('JOIN');
						}
					}
				});
			}
			...
			if (message?.data != undefined) {
				...
				if (this.name === gametxmsg.module) {
					...
					if (gametxmsg.request === 'CANCEL') {
						this.toJoin = this.toJoin.filter(
							(key) => key !== gametxmsg.my_key
						);
						this.toLeave = this.toLeave.filter(
							(key) => key !== gametxmsg.my_key
						);
						siteMessage(
							`${app.keychain.returnUsername(
	...
	removePlayerFromState(index) {
		console.error(
			'Did you define removePlayerFromState in your game module?'
		);
	}
	...
	initializeQueueCommands() {
		...
		this.commands.push((game_self, gmv) => {
			if (gmv[0] === 'PLAYERS') {
				...
				console.log(
					JSON.stringify(this.toLeave),
					JSON.stringify(this.toJoin)
				);
				for (let pkey of this.toLeave) {
					...
					this.updateLog(
						`Player ${i + 1} (${this.app.keychain.returnUsername(
							pkey
						)}) leaves the table.`
					);
					if (pkey === this.publicKey) {
						this.app.connection.emit(
							'arcade-gametable-removeplayer',
							this.game.id
						);
					}
				}
				...
				while (
					...
					this.updateLog(
						`${this.app.keychain.returnUsername(
							pkey
						)} joins the table as Player ${
							this.game.players.length
						}`
					);
					if (pkey === this.publicKey) {
						this.app.connection.emit(
							'arcade-gametable-addplayer',
							this.game.id
						);
					}
				}
				...
				if (change) {
					...
					console.log(
						'ALL PLAYERS: ' + JSON.stringify(this.game.players)
					);
