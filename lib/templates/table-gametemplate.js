...
class GameTableTemplate extends GameTemplate {
	...
	async render(app) {
		...
		if (this.game.player > 0 && this.game?.options["open-table"]) {
		...
		if (message?.request === 'game relay update') {
			if (message?.data != undefined) {
				...
				if (this.name === gametxmsg.module) {
					...
					if (gametxmsg.request === "SETTLEMENT"){
						console.log(`${gametx.from[0].publicKey} requested we settle at the end of the round`);
						this.settleNow = true;
	...
	exitConfirmationTemplate(){
		return `<div class="saito-modal saito-modal-menu game-exit-menu" id="saito-exit-menu">
     				<div class="saito-modal-title">Exit Game</div>
	...
	async exitGame() {
		if (this.game.over == 0 && this.game.player && this.game.options['open-table']) {
		    ...
		    $('.saito-modal-menu-option').on('click', async function () {
		      ...
		      if (choice == "forfeit"){
				await game_self.sendStopGameTransaction('forfeit');
		...
		this.commands.push((game_self, gmv) => {
			if (gmv[0] === 'ADDPLAYER') {
				...
				if (this.game.player == 0) {
					console.log("Send follow TX ^^^^^^^^^^^^^^^");
					this.sendFollowTx({game: this.name, game_id: this.game.id, players: [pkey]});
				}
			}
			return 1;
		});
