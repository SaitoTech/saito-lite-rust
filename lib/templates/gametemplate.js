...
let GameClock = require("../saito/new-ui/game-clock/game-clock");
let GameLog = require("../saito/new-ui/game-log/game-log");
let GameHud = require("../saito/new-ui/game-hud/game-hud");
let GameMenu = require("../saito/new-ui/game-menu/game-menu");
let SaitoOverlay = require("../saito/new-ui/saito-overlay/saito-overlay");
let GameCardbox = require("../saito/new-ui/game-cardbox/game-cardbox");
let GamePlayerbox = require("../saito/new-ui/game-playerbox/game-playerbox");
let GameCardfan = require("../saito/new-ui/game-cardfan/game-cardfan");
let GameHexGrid = require("../saito/new-ui/game-hexgrid/game-hexgrid");
let GameCryptoTransferManager = require("../saito/new-ui/game-crypto-transfer-manager/game-crypto-transfer-manager");
let GameBoardSizer = require("../saito/new-ui/game-board-sizer/game-board-sizer");
const GameScoreboard = require("../saito/ui/game-scoreboard/game-scoreboard");
const GameHammerMobile = require("../saito/new-ui/game-hammer-mobile/game-hammer-mobile");
...
class GameTemplate extends ModTemplate {
  ...
  displayWarning(warningTitle, warningText = "", time = 4000) {
    let html = `<div class="game_warning_overlay">
                  ...
                  <h2>${warningTitle}</h2>
                  <p ${(warningText.length == 0) ? "style='flex:1;'" : "style='flex:2;'"}>${warningText}</p>
                </div>`;
  ...
  onPeerHandshakeComplete(app, peer){
    ...
    if (this.hasGameMovePending() && this.game?.initializing == 0) {
      ...
      for (let i = 0; i < this.app.wallet.wallet.pending.length; i++) {
        let tx = new saito.default.transaction(JSON.parse(this.app.wallet.wallet.pending[i]));
          ...
          if (txmsg.game_id === this.game?.id) {
            if (txmsg?.step?.game) {
               if (this.game.step.players[tx.transaction.from[0].add] < txmsg.step.game) {
                ...
                if (relay_mod) {
                  relay_mod.sendRelayMessage(this.game.accepted, "game relay gamemove", tx);
                }
              }
            }
          }
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if ((!game_self.game && game_id) || (game_self.game.id !== game_id)) {
        console.log("Game engine, move received. Safety catch (1), loading game...");
      ...
      if (!txmsg?.step?.game) { //Not a game move
        console.log(`${this.name} skipping ${JSON.stringify(txmsg)}`);
        return;
      }
  ...
  sendGameObserverTx(initial_state){
    ...
    if (newtx) {
      ...
      if (relay_mod) {
        relay_mod.sendRelayMessage(this.game.accepted, "game relay observer", newtx);
      }*/
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    setTimeout(() => {
      ...
      console.log("Sending Move: ",JSON.parse(JSON.stringify(newtx)));
  ...
  async processAcceptRequest(tx, app) {
    let txmsg = tx.returnMessage();
    let game_id = txmsg.game_id;
  ...
  */
  initialize(app) {
    ...
    console.log("PRE INITIALIZE GAME FEEDER!: " + this.name);
  ...
  */
  ...
  initializeGameFeeder(game_id) {
    ...
    //
    if (!this.game || this.game.id !== game_id) {
      console.log("Loading game in GameFeeder!");
      this.loadGame(game_id);
    }
...
console.log("started queue done");
  ...
  startQueue() {
    ...
    if (this.hasGameMovePending() && this.game?.initializing == 0){
      ...
      console.log("Should not start processing QUEUE because we have pending TXs");
      this.gaming_active = 0;
      return 0;
    }
    ...
    if (this.runQueue() == 0) {
        ...
        if (this.game.player == 0 && this.observer){
          if (this.observer?.is_paused){
            console.log("Step wise");
            this.game.halted = 1;
          }else{
            setTimeout(()=>{ 
              if (this.processFutureMoves() == 0){
                //Try to download new moves
                console.log("Checking DB for missed future moves");
  ...
  */
  restartQueue() {
    ...
    console.log("Restarting queue: " + JSON.stringify(this.game.queue));
    this.saveGame(this.game.id);
  ...
  runQueue() {
    ...
    //
    console.log("START QUEUE: " + JSON.stringify(game_self.game.queue));
    ...
    while (game_self.game.queue.length > 0 && cont == 1) {
      ...
      for (let i = 0; i < game_self.commands.length; i++) {
        if (game_self.commands[i](game_self, gmv) === 0) { //Game engine requests queue processing pauses
          console.log("GAME ENGINE STOPS FOR " + JSON.stringify(gmv));
          console.log("OUTSTANDING QUEUE: " + JSON.stringify(game_self.game.queue));
          return 0;
        }
      }
      //if (this.game.player == 0) {
      ...
      //console.log("UPDATE QUEUE: " + JSON.stringify(game_self.game.queue));
    }
  ...
  */
  ...
  addNextMove(gametx) {
    ...
    if (this.game.halted == 1 || this.gaming_active == 1 || this.initialize_game_run == 0) {
      console.log(`Save as future move because halted (${this.game.halted}) or active (${this.gaming_active})`);
      this.addFutureMove(gametx);
      return;
    }
    ...
    console.log(`Add Next Move (${gametxmsg.step.game}) to QUEUE: ${JSON.stringify(gametxmsg.turn)}`);
    //console.log(JSON.parse(JSON.stringify(gametxmsg)));
  ...
  processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      ...
      if (this.isUnprocessedMove(ftx.transaction.from[0].add, ftxmsg)) {
        ...
        console.log("Is future move, leave on future queue");
        //This move (future[i]) is still for the future, so leave it alone
      } else { //Old move, can ignore
        console.log("Is old move, prune from future queue");
        this.game.future.splice(i, 1);
        ...
        try {
          ...
          document.getElementById("rematch").onclick = (e) => {
            ...
            timeout = setTimeout(async ()=>{
              let c = await sconfirm("It seems your opponent has logged off. Return to Arcade?");
              if (c){
                this.exitGame();
              }
            }, 10000);
          ...
          app.connection.on("arcade-reject-challenge", (game_id)=>{
            ...
            this.displayModal("Challenge declined");
          ...
          app.connection.on("arcade-game-loading" , () =>{
            ...
            this.displayModal("Challenge accepted", "Please wait while the new game initializes");
          });
  ...
  */
  endGame(winner = [], method = "") {
    this.game.winner = (method == "cancellation") ? [] : winner;
    ...
    if (player_to_send == this.app.wallet.returnPublicKey()) {
      ...
      newtx.msg = {
        ...
        //sig: this.game.id, //duplicate because Arcade uses "sig" and too lazy to change
        winner,
        module: this.game.module,
        reason: method,
      };

      if (this.game.options?.league){
        newtx.msg.league = this.game?.options?.league;
      }
  ...
  */
  resignGame(game_id = null, reason = "forfeit") {
    ...
    if (game_id && this.game.id != game_id) {
      console.log("Have to load game in order to resign");
      this.loadGame(game_id);
    }
  ...
  */
  processResignation(resigning_player, txmsg) {
    ...
    if (arcade) {
      if (arcade.debug) { console.log("Game resigned"); }
      arcade.removeGameFromOpenList(this.game.id);
    }
    ...
    if (txmsg.is_game_over == 0) {
      this.updateLog(`Player ${txmsg.loser} ${(txmsg.reason == "cancellation")? "cancels":"quits"} the game`);
    }
    ...
    if (game_id == null) {
      ...
      if (this.app.options.games.length > 0){
        for (let i = 0; i < this.app.options.games.length; i++) {
          ...
          if (this.name == this.app.options.games[i].module && this.app.options.games[i].ts > timeStamp) {
            game_to_open = i;
  ...
  updateStatus(str, force = 0) {
    ...
    try {
      if (this.useHUD) {
        ...
        let status_obj = document.getElementById("status") || document.querySelector(".status") || document.getElementById("obstatus");
        if (status_obj) {
  ...
  updateObserverStatus(str){
    ...
    try{
      ...
      if (statusBox){
        ...
        setTimeout(()=>{statusBox.innerHTML = "";}, 1500);
      }
    }catch(err){
      console.error(err);
    }
  ...
  returnPlayerSelectHTML(){
    ...
    if (this.minPlayers === this.maxPlayers) {
      html = `<div class="game-wizard-players-no-select" style="display:none" data-player="${this.minPlayers}">${this.minPlayers} player</div>`;
      html += this.returnSingularGameOption();
    } else {
      let defaultPlayers = this.minPlayers;
      if (this.opengame){
        defaultPlayers = this.maxPlayers;
        html = `<div class="game-wizard-players-no-select" style="display:none" data-player="${this.maxPlayers}">${this.maxPlayers} player</div>`;
      }else{
        html += `<select class="game-wizard-players-select" name="game-wizard-players-select">`;
        for (let p = this.minPlayers; p <= this.maxPlayers; p++) {
          html += `<option value="${p}" ${(p===defaultPlayers)?"selected default":""}>${p} player</option>`;
        }
        html += `</select>`;
      }
    }
  ...
  returnDefaultGameOptions(){
    ...
    let metaOverlay = new SaitoOverlay(this.app, false, false);
    metaOverlay.show(this.app, this, `<form>${playerOptions}${advancedOptions}</form>`);
    metaOverlay.hide();      
  ...
  initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RESOLVE") {
        ...
        console.log("ready to resolve queue: " + resolve_queue);
        console.log(JSON.stringify(game_self.game.confirms_needed));
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] == "READY") {
        ...
        if (game_self.browser_active) { 
          ...
          console.log("GAME READY, emit signal");
          game_self.app.connection.emit("game-ready", game_self.game.id);
          /*let arcade_self = game_self.app.modules.returnModule("Arcade");
          if (arcade_self.browser_active){
...
console.log("verifies 1");
          for (let i = 0; i < game_self.game.keys.length; i++) {
console.log("verifies 2: " + game_self.game.keys[i]);
            if (game_self.game.players[i] === playerkey) {
console.log("verifies 3");
...
console.log("moving into game crypto transfer manager...");

	//
	// permit users to opt-out of sanity checks
	//
	if (game_self.crypto_transfers_outbound_approved == 1) {
          game_self.app.wallet.sendPayment([sender_crypto_address], [receiver_crypto_address], [amount], ts, unique_hash, function (robj) {
            if (game_self.game.id != my_specific_game_id) {
              console.log("Have to reload game by ID when sending crypto");
              game_self.game = game_self.loadGame(my_specific_game_id);
            }
	...
	//
        if (game_self.crypto_transfers_inbound_trusted == 1) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
	  return 1;
	} else {
          game_self.crypto_transfer_manager.receivePayment(this.app, this, [sender_crypto_address.split("|")[0]], [receiver_crypto_address.split("|")[0]], [amount], ts, ticker, function (divname) {
            game_self.app.wallet.receivePayment([sender_crypto_address], [receiver_crypto_address], [amount], ts, unique_hash, function (robj) {
              ...
              if (game_self.game.id != my_specific_game_id) { 
                console.log("Have to reload game by ID when receiving crypto");
                game_self.game = game_self.loadGame(my_specific_game_id); 
              }
	...
	//
        ...
        for (let i = 0; i < game_self.game.players.length; i++) {
          ...
          if (game_self.app.crypto.verifyMessage(msg_to_verify, sigs[i], game_self.game.players[i]) === true) {
	    auths++;
	  } else {
...
console.log("non-verifier idx: " + first_non_verifier_idx + "  ---- my index is " + (game_self.game.player-1) );

	  if ((game_self.game.player-1) == first_non_verifier_idx) {
	    //
	    // auto-accept
	    //
            game_self.crypto_transfer_manager.approveGameStake(this.app, this, ticker, stake, sigs, ts, function() {
             // game_self.game.queue !=  game_mod.game_state_pre_move;
	    });
	  }
	  return 0;       
	}
...
console.log("PROPOSING GAME STAKE");

    while (sigs.length < this.game.players.length) { sigs.push(""); }
    sigs[this.game.player-1] = this.app.wallet.signMessage(`${ts} ${ticker} ${stake} ${this.game.id}`);
    ...
    if (this.game.queue.length) {
      ...
      let pm = pmv.split("\t");
      if (pm[0] === "STAKE") { this.game.queue.splice(this.game.queue.length - 1, 1); }
    }
