  ...
  onPeerHandshakeComplete(app, peer) {
    ...
    if (this.hasGameMovePending() && this.game?.initializing == 0) {
      ...
      for (let i = 0; i < this.app.wallet.wallet.pending.length; i++) {
        let tx = new saito.default.transaction(undefined, JSON.parse(this.app.wallet.wallet.pending[i]));
          ...
          if (txmsg.game_id === this.game?.id) {
            if (txmsg?.step?.game) {
              if (this.game.step.players[tx.transaction.from[0].add] < txmsg.step.game) {
                ...
                if (relay_mod) {
                  relay_mod.sendRelayMessage(this.game.accepted, "game relay gamemove", tx.transaction);
                }
              }
            }
          }
...
//console.log("GAME ADDING TX TO PENDING: " + newtx.returnMessage());
      ...
      if (this.relay_moves_offchain_if_possible && send_onchain_only == 0) {
        ...
        if (
          ...
          if (relay_mod) {
            relay_mod.sendRelayMessage(this.game.accepted, "game relay gamemove", newtx.transaction);
          }
        }
      }
    ...
    if (message?.request?.includes("game relay")) {
      if (message?.data != undefined) {
        ...
        if (!this.doesGameExistLocally(gametxmsg.game_id)) {
          console.info("Game does not exist locally. Not processing HPR message: waiting for on-chain");
          return;
        }
  ...
  */
  endGame(winner = [], method = "") {
    ...
    if (player_to_send == this.app.wallet.getPublicKey()) {
  ...
  */
  ...
  animateGameElementMove(target, destination, callback = null) {
    ...
    this.app.browser.addElementToSelector(`<div id="animation_helper${i}" class="animation_helper"></div>`, ".main");
    ...
    $(`#animation_helper${i}`).delay(50).queue(function() {
      $(this).css({
