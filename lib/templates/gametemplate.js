...
class GameTemplate extends ModTemplate {
  ...
  constructor(app) {
    ...
    //
    let temp_self = this;
    this.changeable_callback = function(card) { alert("CALLBACK MUST BE PROVIDED FOR CARDCLICK"); };
...
console.log("processing in gametemplate: " + txmsg.module);
...
console.log("AND HERE GAME ID: " + game_self.game.id);
...
console.log("start unprocessed move... 1");

    let game_self = this;
console.log("start unprocessed move... 2");

    if (!game_self.game) {
console.log("no game in game_self");
    }
console.log("start unprocessed move... 3");
    if (!game_self.game) {
console.log("no game in game_self");
    }
console.log("start unprocessed move... 4");
...
console.log("start unprocessed move... 5");

    if (!txmsg.step) {
console.log("no step in txmsg");
    }
console.log("start unprocessed move... 6");

console.log("txmsg: " + JSON.stringify(txmsg));
console.log("gameselfstep: " + JSON.stringify(game_self.game));
console.log("this in game step: " +txmsg.step.game);


    if (parseInt(txmsg.step.game) == (game_self.game.step.game+1)) { 
      return 1;
    }
console.log("start unprocessed move... 7");
    if (parseInt(txmsg.step.game) == (game_self.game.step.players[player]+1)) { 
      return 1; 
    }
console.log("start unprocessed move... 8");

 
    if (!game_self.game.step) {
console.log("no step in gameslef game");
    }
console.log("start unprocessed move... 9");

    // MAY 28 - player has an updated move, but not the next move
    if (game_self.game.step.players[player]) {
console.log("start unprocessed move... 10");
...
//console.log("Receive Move Offchain but neither future nor unprocessed: " + gametx.transaction.from[0].add + " -- " + JSON.stringify(gametxmsg.step));
  ...
  updateStatus(str, force=0) {
    ...
    } catch (err) { console.log("Error Updating Status: ignoring: " + err); }
  }
 ...
 */ 
  attachCardboxEvents(mycallback = null) {
    this.changeable_callback = (mycallback !== null)? mycallback : function(){};
    this.cardbox.hide(1);
    this.cardbox.attachCardEvents();
  }
  ...
  returnCardList(cardarray=[], deckid = 0) {
    if (cardarray.length === 0) { //Get the "hand"
    ...
    if (this.interface === 2){ //text
     ...
     for (i = 0; i < cardarray.length; i++) {
        html += `<div id="${cardarray[i]}" class="card hud-card">${this.returnCardImage(cardarray[i], deckid)}</div>`;
      }
      return html;  
     }
  }
   ...
   */ 
  returnCardImage(cardname, deckid) {
    let c = this.game.deck[deckid].cards[card];
    ...
    if (c.img.indexOf(suggested_img) != -1) {
      return `<img class="cardimg showcard" id="${cardname}" src="${c.img}" />`;
    }
...
/* two outdated jquery based functions to show detailed card on hover over */
/*  addShowCardEvents() {
    if (this.browser_active == 0) { return; }  
...
console.log("Here we are 1");
    ...
    let html = '<div class="status-header acknowledge-message">' + msg + "</div><ul>";
...
console.log("Here we are 4");
    ...
    try {
console.log("updating status with message: " + html);
      this.updateStatus(html);
console.log("done updating status: " + html);
...
console.log("Error with ACKWNOLEDGE notice!: " + err);
    }
...
console.log("it is not my turn!");
  ...
  hasPlayerConfirmed(pubkey) {
    if (this.game.tmp_confirms_players.includes(pubkey)) { return 1; }
    if (this.game.confirms_players.includes(pubkey)) { return 1; }
    return 0;
  }
  ...
  resetConfirmsNeeded(array_of_player_nums) {
    ...
    for (let i = 0; i < this.game.players.length; i++) {
      if (array_of_player_nums.includes((i+1)) || array_of_player_nums.includes(this.game.players[i])) { 
...
console.log("is anyone left to move: " + anyone_left_to_move);
	...
	for (let i = game_self.game.queue.length-1; i >= 0; i--) {
	  ...
	  if (sr[0] === "SECUREROLL") {
	    ...
	    if (game_self.app.crypto.verifyMessage(sr[2], sr[3], game_self.game.players[sr[1]-1]) == true) {
	      players.push(sr[1]);
	      hashes.push(sr[2]);
	    } else {
	...
	if (game_self.game.sroll_done == 0) {
	  if (game_self.game.player != initiating_player) {
	    ...
	    //
            ...
            let hash1_sig = game_self.app.crypto.signMessage(hash1, game_self.app.wallet.returnPrivateKey());
            game_self.addMove("SECUREROLL\t"+game_self.game.player+"\t"+hash1+"\t"+hash1_sig);
	...
	if (their_second_hash === "") {
	  ...
	  if (game_self.game.player == originating_player) {
            let hash_sig = game_self.app.crypto.signMessage(game_self.game.sroll_hash, game_self.app.wallet.returnPrivateKey());
            game_self.addMove("SECUREROLL\t"+game_self.game.player+"\t"+game_self.game.sroll_hash+"\t"+hash_sig);
	    game_self.game.sroll_done = 1;
	    game_self.endTurn();
	    return 0;
	  }
	 ...
	 if (game_self.app.crypto.hash(their_second_hash) === originating_hash) {
	   ...
	   alert("ERROR: the player which triggered the secure-dice-roll did not submit a second hash which hashed to their original commit. This invalidates the security of the random number generation. Halting.");
	  }
	   return 0;
	}
        //
	  ...
	  for (let k = 0; k < players.length; k++) {
		...
		if (player_hash_idx == 2) {
		  ...
		  if (player_hash != hashes[k]) {
		    alert("ERROR: Player " + player_id + " has submitted inconsistent hash values for their card selection. Halting.");
		    return 0;
		  } else {
