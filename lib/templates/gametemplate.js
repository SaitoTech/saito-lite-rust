...
class GameTemplate extends ModTemplate {
			...
			if (load_id) {
				for (let z = 0; z < this.app.options.games.length; z++) {
					if (this.app.options.games[z].id == load_id) {
						if (this.app.options.saves[load_id]) {
							this.app.options.games[z] =
								this.app.options.saves[load_id];
							this.app.options.games[z].timestamp =
								new Date().getTime();
							this.loadGame(load_id);
							let newUrl =
								this.app.browser.protocol +
								'://' +
								this.app.browser.host +
								'/' +
								this.returnSlug();
							try {
				...
				if (this.app.options.saves) {
					for (let key in this.app.options.saves) {
						let x =
							new Date().getTime() -
							this.app.options.saves[key].timestamp;
						if (x > 50000000) {
	...
	async attachEvents(app) {
		if (this?.game?.id) {
			...
			document.body.innerHTML = this.returnSplash();
