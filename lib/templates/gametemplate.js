      ...
      for (let i = 0; i < this.app.wallet.wallet.pending.length; i++) {
        ...
        if (txmsg.module === this.name) {
          if (txmsg.game_id === this.game?.id) {
            if (txmsg?.step?.game) {
              if (this.game.step.players[tx.transaction.from[0].publicKey] < txmsg.step.game) {
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if (txmsg.request === "stopgame") {
        game_self.processResignation(tx.transaction.from[0].publicKey, txmsg);
        return;
      }
      ...
      if (
        ...
        game_self.isFutureMove(tx.transaction.from[0].publicKey, txmsg)
      ) {
      ...
      } else if (game_self.isUnprocessedMove(tx.transaction.from[0].publicKey, txmsg)) {
      ...
      if (message?.data != undefined) {
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (message?.request === "game relay gamemove") {
            if (
              this.initialize_game_run == 0 ||
              this.isFutureMove(gametx.transaction.from[0].publicKey, gametxmsg)
            ) {
            ...
            await this.processResignation(gametx.transaction.from[0].publicKey, gametxmsg);
