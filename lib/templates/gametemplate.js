...
class GameTemplate extends ModTemplate {
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    setTimeout(() => {
      ...
      // saveGameState and observer_mode are both usually flagged at the same time
      // but legacy code in TS allows users to opt into observer mode in the middle
      // of the game, so we need to notify the opponent to also send the game states

      /*if (this.game.saveGameState == 1) {
        if (this.game.observer_mode == 0) {
          this.game.turn.push("OBSERVER\t" + this.game.player + "\t1");
          this.game.turn.push("SETVAR\tsaveGameState\t1");
        }
        mymsg.game_state = this.returnGameState(JSON.parse(JSON.stringify(this.game_state_pre_move)));
  ...
  addNextMove(gametx) {
    ...
    if (this.game.player == 0) {
      ...
      if (this.observer){
        ...
        //console.log(JSON.stringify(this.game_state_pre_move.queue),JSON.stringify(gametxmsg.turn));
      }
...
console.log("success ... restart game");
	...
	if (auths == game_self.game.players.length) {
	  ...
	  if ((game_self.game.player-1) == first_non_verifier_idx) {
	    ...
	    // auto-accept
	    //
console.log("ACCEPTING PROPOSED GAME STAKE AUTOMATICALLY!");
            game_self.crypto_transfer_manager.approveGameStake(this.app, this, ticker, stake, sigs, ts, function() {
console.log("WE FINISHED CRYPTO TRANSFER MANAGER");
	    });
	  }
	  return 0;       
	}
