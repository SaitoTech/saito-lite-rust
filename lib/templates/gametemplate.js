    ...
    */
	...
	async initializeObserverMode(tx, use_state = false) {
		...
		if (use_state){
			...
			console.log("B4 INITIALIZE GAME QUEUE -- OBSERVER");
			await this.initializeGameQueue(game_id);
			console.log("AFTER INITIALIZE GAME QUEUE -- OBSERVER");
		}
	}
...
/*	        if (this.browser_active && this.app.BROWSER) {
			setInterval( async () => {
console.log("rebroadcasting pending...");
...
*/
		...
		//
		// check if in-game crypto preferences loaded or not
		// if not loaded, reset to default
		if (this.app.options != undefined) {
			if (this.app.options.gameprefs != undefined) {
				let wallet_backup = false;

				if (this.app.options.gameprefs.crypto_transfers_inbound_approved == undefined) {
					this.app.options.gameprefs.crypto_transfers_inbound_approved = 1;
					wallet_backup = true;
				}

				if (this.app.options.gameprefs.crypto_transfers_outbound_approved == undefined) {
					this.app.options.gameprefs.crypto_transfers_outbound_approved = 1;
					wallet_backup = true;
				}

				if (this.app.options.gameprefs.crypto_transfers_inbound_trusted == undefined) {
					this.app.options.gameprefs.crypto_transfers_inbound_trusted = 0;
					wallet_backup = true;
				}

				if (this.app.options.gameprefs.crypto_transfers_outbound_trusted == undefined) {
		...
		if (conf == 0) {
			...
			if (current_game_id) {
				...
				if (game_id !== current_game_id) {
					...
					let player_step = 0;
					if (txmsg.step?.game) { player_step = txmsg.step.game; };
					...
					//
					console.log("Cache the game's state: ", this.halted, this.gaming_active, this.initialize_game_run);
			...
			} else if (txmsg.request == "follow game") {
				this.addFollower(txmsg.my_key);
				if (txmsg?.send_state){
					this.shareGameState(tx.from[0].publicKey);
				}
			}
			...
			if (current_game_id && current_game_id !== game_id) {
				...
				//
				if (game_halted != null) {
					console.log("Did we change the game's state? ", this.halted, this.gaming_active, this.initialize_game_run);
	...
	async handlePeerTransaction(app, tx = null, peer, mycallback = null) {
				...
				if (this.name === gametxmsg.module) {
					...
					if (message.request === 'game relay gamemove') {
						...
						if (gametxmsg.request == 'follow game') {
							...
							if (gametxmsg?.send_state){
								this.shareGameState(gametx.from[0].publicKey);
							}
						}
					} else {
		...
		//
		// Maybe better to be consistent with the other game moves because of the id protections...
		//
		if (message?.request ===  "report game cache"){
			if (tx.isTo(this.publicKey)){
				if (message.module === this.name){
					console.log("Player shared last game state", tx.from[0].publicKey, message.state);
					console.log(JSON.parse(JSON.stringify(this.game)));

					if (message?.state != "") {
						this.game = JSON.parse(message.state);
						this.game.player = 0;
						this.game.live = true;
						console.log(this.game.step, this.game.queue);
						this.saveGame(message.game_id);
						...
						this.initializeGameQueue(message.game_id);
					}
				}
			}
		}
			...
			for (let i = 0; i < this.playerRanks.length; i++) {
				...
				if (this.game.crypto && this.game.crypto !== "CHIPS" && this.game.stake){
					if (typeof this.game.stake === "object"){
						line += `<span class="player-stake">${this.game.stake[this.game.players[i]]} ${this.game.crypto}</span>`;
					}else{
	...
	async shareGameState(pkey) {
		...
		console.log("Share game state with observer");
		let tx = await this.app.wallet.createUnsignedTransactionWithDefaultFee(pkey);
		tx.msg = {
			module: this.name,
			game_id: this.game.id,
			request: 'report game cache',
			state
		};
