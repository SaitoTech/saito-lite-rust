...
class GameTemplate extends ModTemplate {
  ...
  async sendGameObserverTx(initial_state) {
    ...
    let newtx = await this.app.wallet.createUnsignedTransactionWithDefaultFee(this.publicKey, 0.0);
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    setTimeout(async () => {
      ...
      if (this.game.sroll == 1) {
        ...
        let hash1_sig = this.app.crypto.signMessage(hash1, privateKey);
        let hash2_sig = this.app.crypto.signMessage(hash2, privateKey);
        ...
        for (let i = 0; i < this.game.players.length; i++) {
          if (this.game.players[i] === this.publicKey) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
                `CRYPTOKEY\t${this.publicKey}\t${crypto_key}\t${this.app.crypto.signMessage(
                  crypto_key,
                  privateKey
                )}`
              );
