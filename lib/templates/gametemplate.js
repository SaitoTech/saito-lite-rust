...
class GameTemplate extends ModTemplate {
  ...
  initializeHTML(app) {
    ...
    //All games which have a menu will have a game-game menu item
    ...
    try {
      ...
      if (oldHash != "") {
        ...
        if (arcade_mod && this.game.id !== results.gid) {
          ...
          return;
          //arcade_mod.observeGame(results.gid);

          /*let game_tx = arcade_mod.returnGame(results.gid);
          if (!game_tx){
            salert("Invalid game");
            return;
          }
          ...
          arcade_mod.sendFollowTx(game_msg);
          this.initializeObserverMode(game_tx);
          */
        }
      }
...
try {
        ...
        game_self.updateStatusHeader(report_address + " balance check");
        game_self.updateLog(report_address + " balance check");
} catch (err) {}
        let polling_timer = setTimeout(() => {
          game_self.updateStatusHeader("Checking Balance: " + report_address);
		...
		try {
                  ...
                  game_self.updateStatusHeader(report_address + " balance adequate");
  ...
  depositGameStake(ticker = "", stake = "", sigs = [], ts = new Date().getTime()) {
    this.addMove("ACKNOWLEDGE\tOpponent Interested, considering deposit...");
    this.endTurn();
  }
