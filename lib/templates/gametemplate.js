...
alert("we are bug-hunting a game-ending bug -- please report this bug to developers - 582434");
		...
		app.connection.on("relay-notification", (mod, notice)=>{
			if (mod == this.name){
				if (this.gameBrowserActive()){
					siteMessage(notice, 5000);
				}
			}
		});
...
try {
  ...
  console.log("ERROR WITH LOAD:" + JSON.stringify(err));
		...
		//
		console.log("SET GAME HASH %%%%%%%%%%%%%%%%%%%", this.game.id);
	...
	gameBrowserActive(){
		if (this.browser_active){
			...
			try{
				...
				console.log("GameID: ", this?.game?.id, "ShortHash: ", short_game_id);
				return false;
			}
		}
				...
				if (this.name === gametxmsg.module) {
					...
					if (message.request === 'game relay gamemove') {
						...
						if (
							...
							//console.log("HPT: is future move " + gametxmsg.step.game);
							...
							console.log("HPT: is next move " + gametxmsg.step.game);
							await this.addNextMove(gametx);
							this.notifyMove();
						}else{
							//console.log("HPT: is old move " + gametxmsg.step.game);
						}
