...
class GameTemplate extends ModTemplate {
    ...
    */
	...
	async initializeObserverMode(tx) {
		...
		if (conf == 0) {
			...
			if (txmsg.request === 'gameover') {
				...
				if (!txmsg?.step?.game) {
					...
					// process game move
					//
					if (this?.treat_all_moves_as_future || this.game.future.length > 0|| this.isFutureMove(tx.from[0].publicKey, txmsg)) {
			...
			if (message?.data != undefined) {
				...
				if (this.name === gametxmsg.module) {
					...
					if (message.request === 'game relay gamemove') {
						if (
							this?.treat_all_moves_as_future || this.game.future.length > 0 ||
