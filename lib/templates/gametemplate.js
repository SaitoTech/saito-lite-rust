...
class GameTemplate extends ModTemplate {
  ...
  async onConfirmation(blk, tx, conf) {
    ...
    if (conf == 0) {
      ...
      if (txmsg.request === "gameover") {
        await game_self.receiveGameoverRequest(blk, tx, conf, this.app);
        return;
      }
      // stopgame requests
      ...
      if (
        ...
        if (game_self.game.player === 0 && game_self.game.future.length > 3) {
          await game_self.observerControls.next();
        }
  ...
  */
  ...
  async initializeGameFeeder(game_id) {
    ...
    try {
      ...
      if (this.game.status != "") {
        await this.updateStatusHeader(this.game.status);
      }
      ...
      console.log("error initializing game feeder ", error);
    }
  }

  async startQueue() {
    if ((await this.hasGameMovePending()) && this.game?.initializing == 0) {
      await this.updateStatusHeader("Rebroadcasting Last Move");
    ...
    if (this.game.players.includes(tx.from[0].publicKey) && this.game.over !== 1) {
      ...
      if (this.browser_active) {
        ...
        try {
          ...
          await this.endGameInterface(`Game Over: ${readable}`, txmsg.reason !== "cancellation");
  ...
  */
  ...
  // to make consistent.
  //
  async updateStatusHeader(message, include_back_button = false) {
    if (!this.useHUD) {
   ...
   *
   */
  async updateStatusWithOptions(message = "", optionHTML = "", include_back_button = false) {
    let html = `<div class="status-header">`;
  ...
  returnCardList(cardarray = [], deckid = 0) {
    ...
    if (this.interface === 2) {
      ...
      for (let i = 0; i < cardarray.length; i++) {
        ...
        html += `<div id="${cardarray[i]}" class="card hud-card ${
          cardarray[i]
  ...
  */
  ...
  async playerAcknowledgeNotice(msg, mycallback) {
      ...
      if (gmv[0] === "STATUS" || gmv[0] === "status") {
        ...
        for (let i = 0; i < players_to_go.length; i++) {
          if (game_self.game.player == i + 1) {
            await game_self.updateStatusHeader(gmv[1]);
          }
        }
        game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
      }
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "ACKNOWLEDGE") {
        ...
        game_self.playerAcknowledgeNotice(notice, async function () {
          ...
          await game_self.updateStatusHeader("acknowledged...");
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "RESOLVE") {
        ...
        if (am_i_still_to_move == 0) {
          ...
          await game_self.updateStatusWithOptions(msg, notice);
        }
      ...
      if (gmv[0] === "STAKE") {
        ...
        if (auths == game_self.game.players.length) {
          ...
          if (game_self.game.player - 1 == first_non_verifier_idx) {
            ...
            await this.updateStatusHeader("Waiting for Others to Accept");
          }
          return 0;
        }
      }
          ...
          await game_self.app.wallet.returnPreferredCryptoBalances(
            ...
            async function (robj) {
              ...
              if (balance_adequate == 1) {
                ...
                await game_self.updateStatusHeader("balance adequate ...");
                game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
                await game_self.restartQueue();
                return 0;
              }
