...
class GameTemplate extends ModTemplate {
  ...
  */
  initialize(app) {
    ...
    //
    if (this.game.players.length == 1) { this.cooperative = 0; }
    if (this.game.players.length == 1) { this.winnable = 0; }
    if (this.game.players.length == 1) { this.losable = 0; }
    if (this.losable == 0) {             this.winnable = 0; }
    if (this.winnable == 0) {            this.losable = 0; }
    if (this.cooperative == 1) {         this.winnable = 0; this.losable = 0; }
...
console.log("started queue done");
  ...
  */
  processResignation(resigning_player, txmsg) {
    ...
    this.app.connection.emit("arcade-remove-game", this.game.id);
    ...
    if (this.minPlayers === this.maxPlayers) {            
      ...
      if (this.opengame){
        ...
        for (let p = this.minPlayers; p <= this.maxPlayers; p++) {
          html += `<option value="${p}" ${(p===this.minPlayers)?"selected default":""}>${p} min</option>`;
        }
        html += `</select>`;
        ...
        html += `<select class="game-wizard-players-select" name="game-wizard-players-select">`;
        for (let p = this.minPlayers; p <= this.maxPlayers; p++) {
          html += `<option value="${p}" ${(p===this.minPlayers)?"selected default":""}>${p} player</option>`;
        }
        html += `</select>`;
      }
    }
  ...
  initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "ROUNDOVER") {
        ...
        console.log("ROUNDOVER : " + players);

        if (game_self.game.players[0] == game_self.app.wallet.returnPublicKey()) {

          var newtx = game_self.app.wallet.createUnsignedTransactionWithDefaultFee();
          newtx.transaction.to.push(new saito.default.slip(game_self.app.wallet.returnPublicKey(), 0.0));
          newtx.msg = {
            request: "roundover",
          ...
          if (game_self.game.options?.league){
            newtx.msg.league = game_self.game?.options?.league;
          }
...
console.log("EXECUTING SEND 1 -- auto send");
...
console.log("EXECUTING SEND 2 -- confirm send");
...
console.log("EXECUTING RECEIVE 1 -- not confirming receipt");
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
	  return 1;
	} else {
console.log("EXECUTING RECEIVE 2 -- confirming receipt");
