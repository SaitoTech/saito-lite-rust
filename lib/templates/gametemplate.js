...
async handlePeerRequest(app, message, peer, mycallback = null) {
    ...
    if (message.request === "game relay gamemove") {
	  ...
	  // check that the TX signature is valid
	  if (!gametx.validateSignature(this.app)) {
	    console.log("peer to peer game move but sig is invalid -- ignore");
	    return;
	  }
...
initializeQueueCommands() {
  ...
  //
	...
	if (game_self.old_fhand) { game_self.game.deck[deckidx-1].fhand = game_self.old_fhand; game_self.old_fhand = [];  }
