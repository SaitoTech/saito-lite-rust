...
let GameOverlay = require("../saito/new-ui/saito-overlay/saito-overlay");
  ...
  */
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if (this.waitingOnPendingTx){
        if (!this.hasGameMovePending()){
          //All the pending game moves have been rebroadcast and received, so maybe I can start up the queue again
          this.waitingOnPendingTx = false;  
          this.startQueue();
        }
      }
    }
  ...
  */
  ...
  runQueue() {
    ...
    this.gaming_active = 1; // gaming_active prevents future moves from getting added to the queue while it is processing
  	...
  	if (game_self.hasGameMovePending() && game_self.game?.initializing == 0) {
  	  ...
  	  //
  	  for (let i = 0; i < game_self.app.wallet.wallet.pending.length; i++) {
  	    let tx = new saito.default.transaction(JSON.parse(game_self.app.wallet.wallet.pending[i]));
  	    let txmsg = tx.returnMessage();
        if (txmsg.module === game_self.name) {
          if (txmsg.game_id === game_self.game?.id) {
      	    if (txmsg?.step?.game) {
  	           if (game_self.game.step.players[tx.transaction.from[0].add] < txmsg.step.game) {
      ...
      this.waitingOnPendingTx = true;
  	  //
  	  // unset gaming_active so our move processes when it arrives
  	  //
  	  this.gaming_active = 0;
  	  return 0;
  ...
  processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      ...
      if (this.isUnprocessedMove(ftx.transaction.from[0].add, ftxmsg)) {
        ...
        this.gaming_active = 0; //Make sure we can successfully push move to the run queue
  ...
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (this.game.players.includes(tx.transaction.from[0].add) && this.game.over == 0) {
      ...
      if (arcade) {
        ...
        if (arcade.debug) { console.log("Game over, removing game"); }
  ...
  initializeQueueCommands() {
    ...
    */
    ...
    this.commands.push((game_self, gmv) => {
      ...
      if (game_self.browser_active) { return 1; }else{return 0;}
      }
      return 1;
    });
