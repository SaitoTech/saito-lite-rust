  ...
  */
  ...
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (this.game.players.includes(tx.transaction.from[0].add) && this.game.over == 0) {
      ...
      //Crypto settlement???
      if (this.game.crypto && this.game?.stake > 0){
        if (Array.isArray(winner)){
          let num_winners = winner.length;
          let amount_to_send = parseFloat((this.game.stake/num_winners).toFixed(8));
          for (let w of winner){
            this.payWinner(w, amount_to_send);
          }  
        }else{
          this.payWinner(winner, this.game.stake);
        }
      }
  ...
  Upon receipt, the game is finalized
  */
...
initializeQueueCommands() {
  ...
  payWinner(receiver, amount){
      let sender = this.app.wallet.returnPublicKey();
      ...
      game_self.crypto_transfer_manager.sendPayment(this.app, this, [sender_crypto_address.split("|")[0]], [receiver_crypto_address.split("|")[0]], [amount], ts, ticker, function() {
        game_self.app.wallet.sendPayment([sender_crypto_address], [receiver_crypto_address], [amount], ts, unique_hash, null, ticker);
