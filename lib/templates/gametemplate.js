		...
		app.connection.on('league-leaderboard-loaded', (game, leaderboard) => {
			if (this.gameBrowserActive() && this.name == game) {
				...
				this.game.players.forEach((playerKey, i) => {
					...
					this.game.playerRanks.push(player);
				});
...
alert("we are bug-hunting a game-ending bug -- please report this bug to developers - 582434");
				}
			}
		});
				...
				if (this.gameBrowserActive(game_id)){
					...
					if (data?.code == "enter"){
						let data = {
							...
							notification: `${this.app.keychain.returnUsername(this.publicKey)} is already in the game`,
						};
		...
		if (this?.game?.id) {
			...
			this.app.connection.on('league-data-loaded', ()=> {
				...
				if (league_mod){
					...
					if (league?.description && league.description !== this.description){
						if (document.querySelector(".splash-page-game-description")){
							document.querySelector(".splash-page-game-description").innerHTML = league.description;
						}
				...
				if (document.querySelector(".splash-page-leaderboard")){
					...
					this.app.connection.emit('league-render-into', potential_league, ".splash-page-leaderboard");
				}
			...
			this.app.connection.on('arcade-data-loaded', () => {
				if (document.querySelector('.game-activity')){
					document.querySelector('.game-activity').innerHTML = `<div class="game-page-invites"></div><div class="league-overlay-games-list"></div>`;
				}
  ...
  */
			...
			if (message?.data != undefined) {
				...
				if (this.name === gametxmsg.module) {
					...
					if (!this.game?.id || gametxmsg.game_id != this.game.id) {
						console.warn(
							'ERROR SKIPPING HPT IN GAME: ' + gametxmsg.game_id
						);
						return;
					}
	...
	stopClock(player = this.game.target) {
		...
		if (this.time.last_sent > this.time.last_received + 1000) {
			 this.game.clock[player-1].spent +=	this.time.last_sent - this.time.last_received;
		...
		this.time.last_received = this.time.last_sent;
	...
	webServer(app, expressapp, express) {
		...
		expressapp.get(
			...
			async function (req, res) {
				...
				console.log(mod_self.name, mod_self.social);
