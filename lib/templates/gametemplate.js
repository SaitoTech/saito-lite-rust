...
class GameTemplate extends ModTemplate {
  constructor(app) {
    ...
    this.sizer = GameBoardSizer; //no constructor
    ...
    this.cardbox_callback = function (card) {
      console.log(temp_self.changeable_callback);
...
****/
  ...
  bindBackButtonFunction(mycallback) {
    document.getElementById("back_button").click = (e) => {
      mycallback();
    };
  }
  ...
  isUnprocessedMove(player, txmsg) {
    ...
    if (txmsg.game_id !== game_self.game.id && txmsg.id !== game_self.game.id) {
      return 0;
    }
    ...
    if (game_self.game.step.players[player]) {
      if (
        txmsg.step.game > game_self.game.step.players[player] &&
        txmsg.step.game <= game_self.game.step.game
      ) {
        return 1;
      }
    }
  ...
  createAcceptTransaction(game) {
    console.log("*****************");
    console.log("CREATING ACCEPT: " + JSON.stringify(game));
    console.log("*****************");
    ...
    for (let i = 0; i < players.length; i++) {
      tx.transaction.to.push(new saito.default.slip(players[i], 0.0));
    }
    tx.msg.ts = ts;
    tx.msg.module = this.name;
    ...
    tx.msg.players_sigs = [
      ...players_sigs,
      this.app.crypto.signMessage(`invite_game_${ts}`, this.app.wallet.returnPrivateKey()),
    ];
    //
  ...
  receiveInviteRequest(blk, tx, conf, app) {
    ...
    if (txmsg.invite_sig != "") {
      invite_sig = txmsg.invite_sig;
    }
    if (txmsg.players_needed > 1) {
      players_needed = txmsg.players_needed;
    }
    ...
    if (txmsg.accept_sig != "") {
      ...
      if (
        app.crypto.verifyMessage(msg_to_verify, txmsg.accept_sig, this.app.wallet.returnPublicKey())
      ) {
        create_new_game = 1;
        send_accept_tx = 1;
      }
    ...
    if (send_accept_tx == 1) {
      ...
      let relay_mod = this.app.modules.returnModule("Relay");
      if (relay_mod != null && this.initialize_game_offchain_if_possible == 1) {
  ...
  async receiveAcceptRequest(blk, tx, conf, app) {
    ...
    //
    if (!this.game) {
      console.log("loading the game 1: " + game_id);
      this.loadGame(game_id);
    }

    if (this.game.id != game_id) {
      console.log("loading the game 2: " + game_id);
      this.loadGame(game_id);
    }
    ...
    try {
      if (document.querySelector(".gameboard")) {
        ...
        if (gameWidth > window.innerWidth) {
          //Only needed in gameboardsizer
          this.screenRatio = window.innerWidth / gameWidth;
          //console.log("SCREEN RATIO:",this.screenRatio);
        }
  ...
  addNextMove(gametx) {
    ...
    if (game_self.game.halted == 1 || game_self.gaming_active == 1) {
      if (
        game_self.game.future == undefined ||
        game_self.game.future == "undefined" ||
        game_self.game.future == null
      ) {
        game_self.game.future = [];
      }
...
**/
    ...
    //
    if (gametxmsg.turn == undefined) {
      gametxmsg.turn = [];
    }
    if (gametxmsg.step == undefined) {
      gametxmsg.step = {};
    }
...
***/
  ...
  processFutureMoves() {
    ...
    while (
      loop_size > 0 &&
      game_self.gaming_active != 1 &&
      game_self.game.halted != 1 &&
      nothing_left == 0 &&
      num_txs_processed < max_loop_length
    ) {
      loops_through_queue++;
      loop_length = game_self.game.queue.length;
      loop_length_instruction = game_self.game.queue[game_self.game.queue.length - 1];
      ...
      if (
        loop_length == game_self.game.queue.length &&
        loop_length_instruction == game_self.game.queue[game_self.game.queue.length - 1] &&
        last_loop_instruction_processed != loops_through_queue
      ) {
        nothing_left = 1;
      } else {
      }
  ...
  resignGame(game_id = null, tiegame = 0) {
    ...
    newtx.transaction.to = game.players.map((player) => new saito.default.slip(player));
    ...
    if (game.over == 0) {
      ...
      if (this.game.winner > 0) {
        winner = this.game.players[this.game.winner - 1];
      }
      if (tiegame == 1) {
        winner = "";
      }
  ...
  loadGame(game_id = null) {
    console.log("===== LOADING GAME ID: "+game_id);
    if (this.app.options.games == undefined) {
      this.app.options.games = [];
    }
    if (this.app.options.gameprefs == undefined) {
    ...
    if (game_id == null) {
      ...
      if (this.app.options.games && this.app.options.games.length > 0){
        for (let i = 0; i < this.app.options.games.length; i++) {
    ...
    console.log(`Load failed, ${game_id} not found, creating new game`);
    //
    // otherwise subsequent save will be blank
    //
    this.game = this.newGame(game_id); //game_id may be null
    this.saveGame(this.game.id); 
    return this.game;
  }
    ...
    setTimeout(() => {
      ...
      if (this.time.last_sent > this.time.last_received + 1000) {
        ...
        if (time_left < 0 && this.game.clock_limit > 0) {
          ...
          try {
            this.displayModal(
              "<span>The Game is Over</span> - <span>Player " +
                this.game.player +
                " loses by timeout</span>"
            );
            this.updateStatus(
              "<span>The Game is Over</span> - <span>Player " +
                this.game.player +
            ...
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
                "CRYPTOKEY\t" +
                  ...
                  crypto_key +
                  "\t" +
                  this.app.crypto.signMessage(crypto_key, this.app.wallet.returnPrivateKey())
              );
            }
          }
        }
      ...
      for (let i = 0; i < this.game.opponents.length; i++) {
        newtx.transaction.to.push(new saito.default.slip(this.game.opponents[i], 0.0));
      }
        ...
        if (newtx.msg.request === "game" || this.initialize_game_offchain_if_possible == 1) {
          ...
          if (
            ...
            (game_self.game.initializing == 0 ||
              game_self.initialize_game_offchain_if_possible == 1)
          ) {
            console.log("sending mod relay message!");
            relay_mod.sendRelayMessage(this.game.players, "game relay gamemove", newtx);
          }
        }
      }

      console.log("sending onchain game move");
  ...
  async handlePeerRequest(app, message, peer, mycallback = null) {
    ...
    if (message.request === "game relay gamemove") {
      if (message.data != undefined) {
        ...
        /********** FAILS
        // acceptances
        if (gametx.isTo(app.wallet.returnPublicKey()) && gametxmsg.request === "accept") {
          if (game_self.doesGameExistLocally(gametxmsg.game_id)) { 
      return; 
    }
          tx_processed = 1;
          let accept_game = await game_self.receiveAcceptRequest(null, gametx, 0, app);
    return;
        }
*********/
        ...
        //
        //
        // this results in gametxmsg losing the state for moves broadcast by others
        //
        ...
        if (this.name === gametxmsg.module) {
          if (gametxmsg.game_id) {
            ...
            if (gametxmsg.id) {
              if (game_self.game.id !== gametxmsg.id) {
...
***/
  ...
  updateStatusAndListCards(message, cards = [], include_back_button = false) {
    ...
    let backBtn = include_back_button ? `<i id="back_button" class="fas fa-arrow-left"></i>` : "";
    html = `<div class="status-header">${backBtn}<span>${message}</span></div>`;
    ...
    //Do I need to attachCardBoxEvents here?
  }
  ...
  endGame(winner, method) {
    ...
    if (this.browser_active == 1) {
      try {
        this.displayModal(
          "<span>The Game is Over</span> - <span>" +
            winner.toUpperCase() +
            "</span> <span>wins by</span> <span>" +
            method +
            "<span>"
        );
        this.updateStatus(
  ...
  startClock() {
    ...
    this.clock_timer = setInterval(() => {
      ...
      let time_on_clock =
        this.game.clock_limit - (x - this.time.last_received) - this.game.clock_spent;
  ...
  if (!game_self.app.options.saves) {
    alert("You have no saved games which can be reloaded.");
  }

  let game_id = await sprompt("Enter Saved Game ID:");
        if (game_id != "") {
    let backup_game = game_self.app.options.saves[game_id];
    game_self.game = JSON.parse(JSON.stringify(backup_game));
          game_self.game.queue.splice(game_self.game.queue.length-1, 1);
    alert("Game restored... please reload from the Arcade");
...
****/
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "SAVE") {
        ...
        backup_game.queue.push(
          `ACKNOWLEDGE\tLast Warning -- we're not kidding -- your game will continue the next time you click to continue, but if not all players have restored things will break.`
        );
        backup_game.queue.push(
          `ACKNOWLEDGE\tGame Reloased. Note that this is BETA functionality -- do not click to continue your game until all players have confirmed they can see this message.`
        );
        game_self.app.options.saves[game_self.game.id] = JSON.stringify(backup_game);
        game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
      }
      return 1;
    });
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOGDECK") {
        ...
        game_self.updateLog(`Contents of Deck: ${game_self.game.deck[deckidx - 1].cards}`);
        game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
      }
      return 1;
    });
      ...
      if (gmv[0] === "RESOLVEDEAL") {
        ...
        if (game_self.game.player == recipient) {
          for (let i = 0; i < cards; i++) {
            // card must exist
            if (game_self.game.deck[deckidx - 1].crypt.length > i) {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RESOLVEFLIP") {
        ...
        if (game_self.game.pool[poolidx - 1].decrypted == decryption_keys_needed) {
          for (let i = 0; i < cardnum; i++) {
            game_self.game.pool[poolidx - 1].hand.push(
              game_self.app.crypto.hexToString(game_self.game.pool[poolidx - 1].crypt[0])
            );
          ...
          game_self.game.deck[deckidx - 1].keys = game_self.game.deck[deckidx - 1].keys.splice(
            cardnum,
            game_self.game.deck[deckidx - 1].keys.length - cardnum
          );
          game_self.game.deck[deckidx - 1].crypt = game_self.game.deck[deckidx - 1].crypt.splice(
            cardnum,
            game_self.game.deck[deckidx - 1].crypt.length - cardnum
          );
        } else {
          //
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "REQUESTKEYS") {
        ...
        if (game_self.game.player == sender) {
          ...
          for (let i = 0; i < cards; i++) {
            game_self.addMove(game_self.game.deck[deckidx - 1].keys[i]);
          }
          game_self.addMove(
            "ISSUEKEYS\t" +
              deckidx +
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "CARDS") {
        ...
        for (let i = 0; i < parseInt(gmv[2]); i++) {
          if (game_self.game.player != 0) {
            game_self.game.deck[deckidx - 1].crypt[parseInt(gmv[2]) - 1 - i] =
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "DECKENCRYPT") {
        ...
        if (game_self.game.player == playerid) {
...
console.log("receiver_crypto_address: " + receiver_crypto_address);
	...
	let polling_timer = setTimeout(() => {
	  ...
	  game_self.app.wallet.returnPreferredCryptoBalances(addresses, function(robj) {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RECEIVE") {
        ...
        game_self.crypto_transfer_manager.receive(
          this.app,
          this,
          [sender],
          [receiver],
