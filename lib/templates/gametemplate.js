...
class GameTemplate extends ModTemplate {
				...
				if (this.gameBrowserActive(game_id)) {
					...
					if (this.useClock && data?.ts) {
						if (this.game.target !== this.game.player) {
							console.log("Reset opponent's clock with their ts");
		...
		if (this.game?.options?.clock) {
			if (parseFloat(this.game.options.clock) == 0) {
				...
				if (!this.game.clock.length) {
					...
					console.log('Set clock limit to ', limit);
				}
				this.useClock = 1;
			}
		}
	...
	gameBrowserActive(game_id = null) {
		if (this.browser_active) {
			...
			try {
				...
				console.warn('Invalid game hash?', window.location.hash);
				console.log('GameID: ', game_id, 'ShortHash: ', short_game_id);
				return false;
			}
		}
	...
	async attachEvents(app) {
		if (this?.game?.id) {
			...
			console.log('ADDING GAME LANDING PAGE EVENTS!!!!');
		...
		if (service.service === 'relay') {
			...
			if (this.pending.length > 0) {
				...
				while (this.pending.length > 0) {
					...
					console.log('*** Pending move: ', tx.msg);
  ...
  */
	async initialize(app) {
		...
		console.log(this.name + ' Browser active!!!!!');
...
console.log("ON-CONFIRMATION: " +game_id);
...
console.log("ON-CONFIRMATION 2: " +game_id);
			...
			if (current_game_id) {
				...
				//
console.log("ON-CONFIRMATION 3");
				if (game_id !== current_game_id) {
console.log("ON-CONFIRMATION 3.5");
...
console.log("rapid check determines unneeded... skipping");
						return;
					}


					if (do_we_load_this_other_game == true) {
						...
						//
						game_halted = this.game.halted;
						game_gaming_active = this.game.gaming_active;
						game_initialize_game_run = this.game.initialize_game_run;
...
console.info('ON-CONFIRMATION 3.75 for this game...');
				}
			} else {
				//
				// No current game, just load the one to process the move
				//
console.log("ON-CONFIRMATION 5");
				this.loadGame(game_id);
			}
			...
			if (txmsg.request === 'gameover') {
				...
				if (!txmsg?.step?.game) {
					...
					if (this?.treat_all_moves_as_future || this.isFutureMove(tx.from[0].publicKey, txmsg)) {
						...
						console.log("is old move - ignoring: " + txmsg.step.game);
					}
				}
			}
			...
			if (current_game_id && current_game_id !== game_id) {
				...
				//
				if (game_halted != null) {
					this.game.halted = game_halted;
					this.game.gaming_active = game_gaming_active;
					this.game.initialize_game_run = game_initialize_game_run;
				}
