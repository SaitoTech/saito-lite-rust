...
console.log("onConf: game template");
  ...
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (game) {
      ...
      if (game_self.game.players.includes(tx.transaction.from[0].add)) {
        ...
        if (txmsg.reason != "") {
	  if (winner === game_self.app.wallet.returnPublicKey()) {
 	    game_self.updateStatus("You Win: " + txmsg.reason);
 	    game_self.updateLog("You Win: " + txmsg.reason);
	  } else {
 	    game_self.updateStatus("Game Over: " + txmsg.reason);
 	    game_self.updateLog("Game Over: " + txmsg.reason);
	  }
        } else {
 	  game_self.updateStatus("Game Over: Opponent Quit");
 	  game_self.updateLog("Game Over: Opponent Quit");
	}
  ...
  startClock() {
    ...
    if (this.game?.options.clock) {
      if (parseInt(this.game.options.clock) == 0) {
        ...
        if (this.game.clock_limit == 0 && this.game.clock_spent == 0) {
          this.game.clock_limit = parseInt(this.game.options.clock) * 60 * 1000;
        }
      }
    }
