        ...
        for (let i = 0; i < app.options.games.length; i++) {
          if (game_id == app.options.games[i].id) {
            if (tx.isTo(app.wallet.returnPublicKey())) {
                ...
                } else if (game_self.isFutureMove(tx.transaction.from[0].add, txmsg) || game_self.initialize_game_run == 0) {
                  console.log("is future move "+ txmsg.step.game);
                  game_self.addFutureMove(tx);
		  //
		  // just save un-inited games
		  // 
	          if (game_self.initialize_game_run == 0) { return; }
                } else {
                  //We just ignore this...
        ...
        if (this.name === gametxmsg.module) {
         //console.log("Game Peer Request",JSON.parse(JSON.stringify(gametxmsg)));
          ...
          if (message.request === "game relay gamemove") {
            if (this.isFutureMove(gametx.transaction.from[0].add, gametxmsg) || this.initialize_game_run == 0) {
              console.log("is future move " + gametxmsg.step.game);
              this.addFutureMove(gametx);
	      //
	      // nope out if not viewing game
	      //
              if (this.initialize_game_run == 0) { return; }
