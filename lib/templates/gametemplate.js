  ...
  async onConfirmation(blk, tx, conf) {
    ...
    if (conf == 0) {
      ...
      if (txmsg.request === "gameover") {
        await game_self.receiveGameoverTransaction(blk, tx, conf, this.app);
        return;
      }
      // stopgame requests
      if (txmsg.request === "stopgame") {
        await game_self.receiveStopGameTransaction(tx.from[0].publicKey, txmsg);
        return;
      }
  ...
  startClock() {
    ...
    this.clock_timer = setInterval(() => {
      ...
      if (time_on_clock <= 0) {
        ...
        this.sendStopGameTransaction(this.game.id, "time out").then(() => {});
      }
