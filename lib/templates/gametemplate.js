...
console.log(" 1> ");
	    ...
	    //
	    // avoid double-ending by saving after receipt
	    //
	    this.game.over = 1;
	    this.saveGame(this.game.id);
            return;
  ...
  */
  endGame(winner = [], method = "") {
    ...
    if (player_to_send == this.app.wallet.returnPublicKey()) {
      ...
      if (relay_mod) {
        this.app.connection.emit("relay-send-message", { recipient: this.game.accepted, request: "arcade spv update", data: newtx.transaction });
        let peers = [];
  ...
  */
  processResignation(resigning_player, txmsg) {
    ...
    //
    // if we have already resigned
    //
console.log("into processResignation()");
console.log("----");
console.log("----");
console.log("----");
...
if (this.app.BROWSER == 1) {
  alert("ENDGAME TRIGGERED 1");
}
      this.endGame(winners, txmsg.reason);
    } else {
if (this.app.BROWSER == 1) {
  alert("ENDGAME TRIGGERED 2");
}
