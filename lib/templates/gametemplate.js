...
console.log("*****************");
  ...
  async receiveAcceptRequest(blk, tx, conf, app) {
    ...
    //
    if (this.game.id != game_id) {
      console.log("loading the game: " + game_id);
      this.loadGame(game_id);
    }
...
console.log("received game relay gamemove: " + gametxmsg.module);
console.log("our loaded game id: " + this.game.id);
console.log("game tx: " + JSON.stringify(gametxmsg));
...
console.log("TESTING C");
	...
	//
	  ...
	  console.log("what module are we? " + game_self.name);
	  return;
	}
...
//console.log("Receive Move Offchain but neither future nor unprocessed: " + gametx.transaction.from[0].add + " -- " + JSON.stringify(gametxmsg.step));
  ...
  */
   ...
   *  
   */ 
  updateStatusAndListCards(message, cards=[], embedBackBtn = false) {
    //
    // OBSERVER MODE
    ...
    let backBtn = (embedBackBtn && this.menu_backup_callback != null)? `<i id="go-back-button" class="fas fa-arrow-left"></i>` : "";
    html = `
    ...
    if (embedBackBtn && this.menu_backup_callback){
      document.getElementById("go-back-button").onclick = this.menu_backup_callback;
    } 
  }
...
/**
   ...
   *  
   */ 
  updateStatusWithOptions(message = "", optionHTML = "", embedBackBtn = false){
    let backBtn = (embedBackBtn && this.menu_backup_callback != null)? `<i id="go-back-button" class="fas fa-arrow-left"></i>` : "";
    html = `
      <div class="status-message">
        ${backBtn}
 ...
 */ 
  ...
  returnCardList(cardarray=[], deckid = 0) {
    if (cardarray.length === 0) { //Get the "hand"
      let { cards, hand } = this.game.deck[deckid];
      cardarray = hand.map((key) => cards[key]);
    }
    ...
    if (this.interface === 2){ //text
      ...
      return `<ul id="status-cardbox">${html}</ul>`;
    }else{
     for (i = 0; i < cardarray.length; i++) {
