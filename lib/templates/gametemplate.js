...
//console.log("Receive Move Offchain but neither future nor unprocessed: " + gametx.transaction.from[0].add + " -- " + JSON.stringify(gametxmsg.step));
  ...
  */
   ...
   *  @param mycallback - optional callback for cardbox functionality
   */ 
...
/**
   ...
   *  @param mycallback - optional callback for cardbox functionality
   */ 
  updateStatusWithOptions(message = "", optionHTML = "", mycallback = null){
    html = `<div class="status-message">`;
    if (mycallback){
      html += `<i id="go-back-button" class="fas fa-arrow-left"></i><span> ${message}
      </span></div>
    ...
    this.updateStatus(html);
    if (mycallback){
      document.getElementById("go-back-button").onclick = mycallback;
    }
  }
