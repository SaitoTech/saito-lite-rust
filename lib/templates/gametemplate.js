...
class GameTemplate extends ModTemplate {
  ...
  async displayModal(modalHeaderText, modalBodyText = "") {
    salert(`${modalHeaderText}${modalBodyText ? ": " : ""}${modalBodyText}`);
  }
  ...
  async initializeHTML(app) {
    ...
    if (this.game.player == 0) {
      ...
      if (this.observer) {
        await this.observer.renderControls(app, this);
        if (this.game.live) {
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if (txmsg.request === "stopgame") {
        await game_self.processResignation(tx.from[0].publicKey, txmsg);
        return;
      }
      ...
      //
      if (
        game_self.initialize_game_run == 0 ||
        game_self.isFutureMove(tx.from[0].publicKey, txmsg)
      ) {
        //console.log("ONCHAIN: is future move " + txmsg.step.game);
        await game_self.addFutureMove(tx);
      ...
      } else if (game_self.isUnprocessedMove(tx.from[0].publicKey, txmsg)) {
        //console.log("ONCHAIN: is next move " + txmsg.step.game);
        await game_self.addNextMove(tx);
        if (document[this.hiddenTab]) {
  ...
  async sendGameObserverTx(initial_state) {
    let mymsg = {};
    ...
    mymsg.game_state.step.timestamp = new Date().getTime();
    ...
    if (newtx) {
      ...
      newtx.msg = mymsg;
      console.log("11111");
      await this.app.wallet.signTransaction(newtx);
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    let privateKey = await this.app.wallet.returnPrivateKey();
    setTimeout(async () => {
      //
      ...
      if (this.game.sroll == 1) {
        ...
        let hash1_sig = await this.app.crypto.signMessage(hash1, privateKey);
        let hash2_sig = await this.app.crypto.signMessage(hash2, privateKey);
      ...
      if (
        ...
        let crypto_key = await this.app.wallet.returnCryptoAddressByTicker(game_self.game.crypto);
        if (crypto_mod) {
        ...
        for (let i = 0; i < this.game.players.length; i++) {
          if (this.game.players[i] === this.publicKey) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
                `CRYPTOKEY\t${this.publicKey}\t${crypto_key}\t${await this.app.crypto.signMessage(
                  crypto_key,
                  privateKey
                )}`
              );
        ...
        for (let i = 0; i < this.game.players.length; i++) {
          if (this.game.players[i] === this.publicKey) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
                `CRYPTOKEY\t${crypto_key}\t${crypto_key}\t${await this.app.crypto.signMessage(
                  crypto_key,
                  privateKey
                )}`
              );
      ...
      await game_self.app.network.propagateTransaction(newtx);
    }, 100);
  }
  ...
  async handlePeerTransaction(app, tx = null, peer, mycallback = null) {
    ...
    console.log("gametemplate.handlePeerTransaction : ", message);
    ...
    if (message?.request?.includes("game relay")) {
      if (message?.data != undefined) {
        ...
        let gametxmsg = gametx.msg;
        console.log("gametxmsg 22222 : ", gametxmsg);
        //
        ...
        if (this.name === gametxmsg.module) {
          console.log("Game Peer Request", gametxmsg);
          ...
          if (message?.request === "game relay gamemove") {
            ...
            await this.receiveGameoverRequest(null, gametx, 0, app);
          } else if (message.request === "game relay stopgame") {
            await this.processResignation(gametx.from[0].publicKey, gametxmsg);
  ...
  async processAcceptRequest(tx) {
    let txmsg = tx.returnMessage();
    console.log("processAcceptRequest txmsg : ", txmsg);
    let game_id = txmsg.game_id;
    //
    ...
    if (!this.game || this.game.id != game_id) {
      ...
      console.log("this.game_id = " + this.game?.id);
    ...
    if (txmsg.players.length == txmsg.players_sigs.length) {
      for (let i = 0; i < txmsg.players.length; i++) {
        if (
          !this.app.crypto.verifyMessage(msg_to_verify, txmsg.players_sigs[i], txmsg.players[i])
        ) {
          console.warn("PLAYER SIGS do not verify for all players, aborting game acceptance");
          console.log("sig = ", txmsg.players_sigs[i]);
          console.log("key = ", txmsg.players[i]);
          console.log("key = ", msg_to_verify);
    ...
    if (this.game.players_set == 0) {
      ...
      for (let i = 0; i < players.length; i++) {
        ...
        if (players[i] === this.app.crypto.hash(this.publicKey + this.game.id)) {
      ...
      //
      await this.initializeGameFeeder(game_id);
    }
  ...
  */
  async initialize(app) {
    ...
    console.info("INITIALIZE GAME FEEDER!: " + this.name, this.game);
  ...
  */
  ...
  async initializeGameFeeder(game_id) {
    ...
    //
    await this.initializeHTML(this.app);
    ...
    if (this.game.initialize_game_run == 1 && this.initialize_game_run == 1) {
      console.log("initialize_game_run xxxxxxxxxxx");
      return 0;
    } else {
      this.game.initialize_game_run = 1;
      this.initialize_game_run = 1;
      console.log("initialize_game_run yyyyyyyyyyy");
    }
    ...
    await this.initializeGame(game_id);
  ...
  */
  ...
  async addNextMove(gametx) {
    ...
    console.info(
      `Add Next Move (${gametxmsg.step.game}) to QUEUE: ${JSON.stringify(gametxmsg.turn)}`
    );
  ...
  async addFutureMove(gametx) {
    ...
    if (!this.game.future.includes(JSON.stringify(gametx.toJson()))) {
      this.game.future.push(JSON.stringify(gametx.toJson()));
      this.saveFutureMoves(this.game.id);
  ...
  */
  async processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      let ftx = new Transaction(undefined, JSON.parse(this.game.future[i]));
  ...
  addPlayer(address) {
    console.log("ssssssssssss : addplayer : " + address);
    if (address === "") {
      return 0;
    }
    for (let i = 0; i < this.game.players.length; i++) {
  ...
  */
  ...
  loadGame(game_id = null) {
    console.log("ssssssssssss : " + game_id, this.app.options?.games);
    ...
    if (game_id != null) {
      console.log("game count : " + this.app.options?.games?.length);
      console.log("type = ", typeof this.app.options.games);
      for (let i = 0; i < this.app.options?.games?.length; i++) {
        if (this.app.options.games[i].id === game_id) {
          this.game = JSON.parse(JSON.stringify(this.app.options.games[i]));
          return this.game;
        } else {
          console.log("id 1 : ", this.app.options.games[i].id);
          console.log("id 2 : ", game_id);
        }
      }
    }
    ...
    console.info(this.app.options.games);
    ...
    try {
      if (this.useHUD) {
        ...
        if (status_obj) {
          ...
          console.warn("Unable to update status, msg: " + str);
        }
      }
    } catch (err) {
  ...
  async updateStatusWithNewDeal(message, deckid = 0) {
    ...
    let html = `<div class="status-header"><span id="status-content">${message}</span></div>`;
    html += `<div class="status-cardbox freshdeal" id="status-cardbox">${this.returnDoubleSidedCardList(
      [],
      deckid
    )}</div>`;
  ...
  returnCardItem(card, deckid = 0) {
    ...
    for (let z = 0; c == undefined && z < this.game.deck.length; z++) {
      c = this.game.deck[z].cards[card];
      if (c == undefined) {
        c = this.game.deck[z].discards[card];
      }
      if (c == undefined) {
        c = this.game.deck[z].removed[card];
      }
    }
  ...
  returnDiscardedCards(deckidx = 1) {
    ...
    for (const i in this.game.deck[deckidx].discards) {
  ...
  */
  ...
  copyGameElement(target) {
    ...
    target_obj.classList.add("hidden");
  ...
  */
  ...
  removeCardFromHand(card, moveToDiscard = false) {
    for (let z = 0; z < this.game.deck.length; z++) {
      for (let i = 0; i < this.game.deck[z].hand.length; i++) {
  ...
  initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "ACKNOWLEDGE") {
        ...
        console.info("about to ACKNOWLEDGE");
        ...
        game_self.playerAcknowledgeNotice(notice, async function () {
          ...
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          await game_self.restartQueue();

          return 1;
        });

        console.info("should halt game...");
        return 0;
      }
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "ROUNDOVER") {
        ...
        if (game_self.game.players[0] == this.publicKey) {
          let newtx = await game_self.app.wallet.createUnsignedTransactionWithDefaultFee();
          let slip = new Slip();
          slip.publicKey = game_self.publicKey;
          slip.amount = BigInt(0);
          newtx.addToSlip(slip);
          newtx.msg = {
            request: "roundover",
          ...
          console.log("555555");
          await game_self.app.wallet.signTransaction(newtx);
    ...
    */
      ...
      if (gmv[0] === "SAFEDEAL") {
        ...
        if (game_self.game.deck[deckidx - 1].crypt.length >= cards_to_deal) {
          ...
          if (Object.keys(discarded_cards).length <= 0) {
            ...
            //
            game_self.game.queue.push(
              "MARKRESHUFFLE\t" + deckidx + "\t" + JSON.stringify(discarded_cards)
            );
            game_self.game.queue.push("SHUFFLE\t" + deckidx);
            game_self.game.queue.push("DECKRESTORE\t" + deckidx);
            for (let z = game_self.game.players.length; z >= 1; z--) {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "DECKBACKUP") {
        ...
        for (const i in game_self.game.deck[deckidx - 1].cards) {
    ...
    */
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "DECKRESTORE") {
        ...
        for (const b in game_self.old_cards) {
    ...
    */
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "DECK") {
        ...
        for (let i in game_self.game.deck[deckidx - 1].cards) {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "DECKXOR") {
        console.info("DECKXOR HERE: " + JSON.stringify(gmv));
    ...
    if (Array.isArray(winner)) {
      ...
      for (let i = 0; i < this.game.players.length; i++) {
        if (!winner.includes(this.game.players[i])) {
          for (let w of winner) {
            await this.payWinner(this.game.players[i], w, amount_to_send);
          }
        }
      }
    } else {
      for (let i = 0; i < this.game.players.length; i++) {
        if (this.game.players[i] !== winner) {
          await this.payWinner(this.game.players[i], winner, this.game.stake.toString());
        }
      }
    }
  }
