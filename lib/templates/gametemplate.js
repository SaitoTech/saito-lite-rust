...
class GameTemplate extends ModTemplate {
    ...
    constructor(app) {
        ...
        this.changeable_callback = function (card) {
            alert("CALLBACK MUST BE PROVIDED FOR CARDCLICK");
        };
        let temp_self = this;
    ...
    attachCardEvents(app) {
        if (this.useHUD === 1) {
            this.hud.attachCardEvents(app, this);
        }
    }
    ...
    async onConfirmation(blk, tx, conf, app) {
        ...
        if (conf === 0) {
            ...
            //
            if ((game_self.game === "undefined" || game_self.game === undefined || game_self.game === "" || game_self.game === null) &&
                txmsg.game_id !==
                "") {
...
//
    ...
    runQueue() {
        ...
        if (game_self.game.queue.length > 0) {
            while (game_self.game.queue.length > 0 && cont === 1) {
                ...
                //
                if (loop_length ===
                    game_self.game.queue.length &&
                    cont ===
                    1 &&
                    loop_length_instruction ===
    ...
    addNextMove(gametx) {
        ...
        if (game_self.game.halted === 1 || game_self.gaming_active === 1) {
            if (game_self.game.future === undefined || game_self.game.future === "undefined" || game_self.game.future == null) {
        ...
        if (gametxmsg.last_move > 0) {
            ...
            try {
                window.location.hash =
                    `#gid=${this.game.id}&bid=${game_self.game.step.bid}&tid=${game_self.game.step.tid}&lm=${game_self.game.step.ts}`;
            } catch (err) {
        ...
        if (game_self.game.player === 0) {
            ...
            if (gametxmsg.game_state.deck) {
                if (gametxmsg.game_state.deck.length > 0) {
                    ...
                    //
                    if (game_self.game.player_decks) {
                    } else {
                ...
                if (gametxmsg.sharekey) {
                    ...
                    if (sharekey) {
                        for (let i = 0; i < gametxmsg.game_state.deck.length; i++) {
                            ...
                            game_self.game.player_decks[player_idx].deck[i].hand =
                                JSON.parse(this.app.crypto.hexToString(this.app.crypto.decodeXOR(gametxmsg.game_state.deck[i].hand, sharekey)));
                            game_self.game.player_decks[player_idx].deck[i].keys =
                                JSON.parse(this.app.crypto.hexToString(this.app.crypto.decodeXOR(gametxmsg.game_state.deck[i].keys, sharekey)));
                        }
                    }
                }
            }
...
//console.log("PLAYER: " + game_self.game.player);
        ...
        while (loop_size >
        0 &&
        game_self.gaming_active !==
        1 &&
        game_self.game.halted !==
        1 &&
        nothing_left ===
        0 &&
        num_txs_processed <
        max_loop_length) {
            ...
            if (loop_length ===
                ...
                loop_length_instruction ===
                game_self.game.queue[game_self.game.queue.length - 1] &&
                last_loop_instruction_processed !==
                loops_through_queue) {
                nothing_left = 1;
            } else {
            }
    ...
    async handlePeerRequest(app, message, peer, mycallback = null) {
        console.log("gametemplate.handlePeerRequest", message);
...
//console.log("Receive Move Offchain but neither future nor unprocessed: " + gametx.transaction.from[0].add + " -- " + JSON.stringify(gametxmsg.step));
    ...
    updateStatus(str, force = 0) {
        ...
        try {
            ...
            console.log("Error Updating Log: ignoring");
        }
        ...
        if (str) {
            if (!this.game.log.includes(str)) {
                ...
                if (this.game.log.length > length) {
                    if (this.game.log_length > length) {
                    } else {
...
//
    ...
    playerAcknowledgeNotice(msg, mycallback) {
        ...
        try {
            ...
            if (hudheader) {
                document.querySelector('#confirmit').style.width = hudheader.style.width;
            }
        } catch (err) {
        }
    ...
    endGame(winner, method) {
        ...
        if (this.browser_active === 1) {
            try {
                this.displayModal("<span>The Game is Over</span> - <span>" +
                                      winner.toUpperCase() +
                                      "</span> <span>wins by</span> <span>" +
                                      method +
                                      "<span>");
                this.updateStatus("<span>The Game is Over</span> - <span>" +
                                      winner.toUpperCase() +
                                      "</span> <span>wins by</span> <span>" +
                                      method +
                                      "<span>");
    ...
    initializeQueueCommands() {
            ...
            if (gmv[0] === "RESOLVEDEAL") {
                ...
                if (game_self.game.issued_keys_deleted === 0) {
                    if (game_self.game.deck[deckidx - 1].keys.length <= cards) {
                        ...
                        game_self.game.deck[deckidx - 1].keys =
                            game_self.game.deck[deckidx - 1].keys.splice(cards, game_self.game.deck[deckidx - 1].keys.length - cards);
                        game_self.game.deck[deckidx - 1].crypt =
                            game_self.game.deck[deckidx - 1].crypt.splice(cards, game_self.game.deck[deckidx - 1].crypt.length - cards);
                    }
                    game_self.game.issued_keys_deleted = 1;
                }
            }
        ...
        this.commands.push((game_self, gmv) => {
            if (gmv[0] === "RESOLVEFLIP") {
                ...
                if (game_self.game.pool[poolidx - 1].decrypted === decryption_keys_needed) {
                    ...
                    game_self.game.deck[deckidx - 1].keys =
                        game_self.game.deck[deckidx - 1].keys.splice(cardnum, game_self.game.deck[deckidx - 1].keys.length - cardnum);
                    game_self.game.deck[deckidx - 1].crypt =
                        game_self.game.deck[deckidx - 1].crypt.splice(cardnum, game_self.game.deck[deckidx - 1].crypt.length - cardnum);
                    ...
                    for (let i = 0; i < cards; i++) {
                        if (game_self.game.queue[keyidx + i] != null) {
                            game_self.game.deck[deckidx - 1].crypt[i] =
                                game_self.app.crypto.decodeXOR(game_self.game.deck[deckidx - 1].crypt[i], game_self.game.queue[keyidx + i]);
                        } else {
                        }
                    }
                }
        ...
        this.commands.push((game_self, gmv) => {
            if (gmv[0] === "DECKXOR") {
                ...
                for (let i = 0; i < game_self.game.deck[deckidx - 1].crypt.length; i++) {
                    game_self.game.deck[deckidx - 1].crypt[i] =
                        game_self.app.crypto.encodeXOR(game_self.game.deck[deckidx - 1].crypt[i], game_self.game.deck[deckidx - 1].xor);
        ...
        this.commands.push((game_self, gmv) => {
            if (gmv[0] === "DECKENCRYPT") {
                ...
                if (game_self.game.player === playerid) {
                    ...
                    for (let i = 0; i < game_self.game.deck[deckidx - 1].crypt.length; i++) {
                        game_self.game.deck[deckidx - 1].crypt[i] =
                            game_self.app.crypto.decodeXOR(game_self.game.deck[deckidx - 1].crypt[i], game_self.game.deck[deckidx - 1].xor);
                        game_self.game.deck[deckidx - 1].crypt[i] =
                            game_self.app.crypto.encodeXOR(game_self.game.deck[deckidx - 1].crypt[i], game_self.game.deck[deckidx - 1].keys[i]);
                    }
        ...
        this.commands.push((game_self, gmv) => {
            if (gmv[0] === "SEND") {
                ...
                console.log("SENDING FROM: " + sender_crypto_address + " to " + receiver_crypto_address);
                ...
                game_self.crypto_transfer_manager.send(this.app,
                                                       this,
                                                       ...
                                                       function () {
                                                           ...
                                                           game_self.app.wallet.sendPayment([sender_crypto_address],
                                                                                            [receiver_crypto_address],
                                                                                            [amount],
                                                                                            ts,
                                                                                            function (robj) {
                                                                                                ...
                                                                                                game_self.game.queue.splice(game_self.game.queue.length -
                                                                                                                                1, 1);
                                                                                                game_self.game.halted = 0;
