...
class GameTemplate extends ModTemplate {
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if (!txmsg?.step?.game){ //Not a game move
        console.log("skipping " + JSON.parse(txmsg));
        return;
      }
      //
      // process game move
  ...
  */
  resignGame(game_id = null, reason = "forfeit") {
    ...
    this.addMove(`RESIGN\t${this.game.player}\t${this.app.wallet.returnPublicKey()}\t${reason}`);
    this.endTurn();
  }
  ...
  */
  processResignation(resigning_player, player_key, reason){
    ...
    this.updateLog(`Player ${resigning_player} quits the game`);
    this.removePlayer(this.game.players[resigning_player]);
    
    if (this.game.step.game > this.grace_window){
      this.endGame(this.game.players, reason);
    }else{
...
initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RESIGN") {
        game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
        game_self.processResignation(parseInt(gmv[1]),gmv[2],gmv[3]);
      }
      return 1;
    });
