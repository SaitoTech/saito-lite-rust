...
class GameTemplate extends ModTemplate {
    ...
    if (this.hasGameMovePending() && this.game?.initializing == 0) {
      ...
      for (let i = 0; i < this.app.wallet.wallet.pending.length; i++) {
        let tx = new saito.default.transaction(
          undefined,
          JSON.parse(this.app.wallet.wallet.pending[i])
        );
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if (
        ...
        game_self.isFutureMove(tx.transaction.from[0].add, txmsg)
      ) {
  ...
  sendGameObserverTx(initial_state) {
    ...
    if (newtx) {
      newtx.transaction.to.push(new saito.default.slip(this.app.wallet.getPublicKey(), 0.0));
      newtx.msg = mymsg;
      ...
      if (relay_mod) {
        relay_mod.sendRelayMessage(this.game.accepted, "game relay observer", newtx.transaction);
      }*/
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        for (let i = 0; i < this.game.players.length; i++) {
          if (this.game.players[i] === this.app.wallet.getPublicKey()) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
                `CRYPTOKEY\t${this.app.wallet.getPublicKey()}\t${crypto_key}\t${this.app.crypto.signMessage(
                  crypto_key,
        ...
        let crypto_key = this.app.wallet.getPublicKey();
        for (let i = 0; i < this.game.players.length; i++) {
          if (this.game.players[i] === this.app.wallet.getPublicKey()) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
      ...
      console.info("Sending Move: ", JSON.parse(JSON.stringify(newtx)));
  ...
  async handlePeerTransaction(app, tx = null, peer, mycallback = null) {
    ...
    if (message?.request?.includes("game relay")) {
      if (message?.data != undefined) {
        let gametx = new saito.default.transaction(undefined, message.data);
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (!this.game?.id || gametxmsg.game_id != this.game.id) {
            console.log("ERROR SKIPPING HPT IN GAME: " + this.game.id);
            return;
          }
          ...
          if (message?.request === "game relay gamemove") {
            if (
              this.initialize_game_run == 0 ||
              this.isFutureMove(gametx.transaction.from[0].add, gametxmsg)
            ) {
  ...
  initializeSinglePlayerGame(game_data) {
    ...
    if (this.game.players_set == 0) {
      this.game.players = [];
      this.game.players.push(this.app.wallet.getPublicKey());
      this.game.accepted = this.game.players;
      this.game.step.players[this.app.wallet.getPublicKey()] = 1;
      ...
      console.log("!!!!!!!!!!!!!!!!!!!!");
      console.log("My Public Key: " + this.app.wallet.getPublicKey());
  ...
  async processAcceptRequest(tx, app) {
    ...
    //
    if (!txmsg.players.includes(app.wallet.getPublicKey())) {
    ...
    if (this.game.players_set == 0) {
      ...
      for (let i = 0; i < players.length; i++) {
        if (players[i] === this.app.crypto.hash(this.app.wallet.getPublicKey() + this.game.id)) {
  ...
  */
  initialize(app) {
    ...
    this.addFollower(this.app.wallet.getPublicKey());
  ...
  */
  processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      let ftx = new saito.default.transaction(undefined, JSON.parse(this.game.future[i]));
  ...
  addPlayer(address) {
    ...
    if (this.app.wallet.getPublicKey() !== address) {
  ...
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (this.game.players.includes(tx.transaction.from[0].add) && this.game.over == 0) {
      ...
      if (this.browser_active) {
        ...
        if (winner.includes(this.app.wallet.getPublicKey())) {
          readable = "You win";
        } else {
  ...
  */
  async endGame(winner = [], method = "") {
    ...
    if (player_to_send == (await this.app.wallet.getPublicKey())) {
      var newtx = await this.app.wallet.createUnsignedTransactionWithDefaultFee();
      ...
      let relay_mod = this.app.modules.returnModule("Relay");
      if (relay_mod) {
        relay_mod.sendRelayMessage(this.game.accepted, "game relay gameover", newtx.transaction);
        let peers = [];
        let p = await this.app.network.getPeers();
        for (let i = 0; i < p.length; i++) {
          peers.push(await p[i].returnPublicKey());
        }
        relay_mod.sendRelayMessage(peers, "arcade spv update", newtx.transaction);
      }
    }
  }
  ...
  */
  resignGame(game_id = null, reason = "forfeit") {
    ...
    console.info("Send quit message");
    console.info(JSON.parse(JSON.stringify(newtx)));
    ...
    let relay_mod = this.app.modules.returnModule("Relay");
    if (relay_mod) {
      relay_mod.sendRelayMessage(this.game.accepted, "game relay stopgame", newtx.transaction);
    }
  }
  ...
  */
  processResignation(resigning_player, txmsg) {
    ...
    if (txmsg.is_game_over == 0) {
      this.updateLog(
        `Player ${txmsg.loser} ${txmsg.reason == "cancellation" ? "cancels" : "quits"} the game`
      );
    }
        ...
        if (this.app.options.games) {
          for (let i = 0; i < this.app.options.games.length; i++) {
            if (this.app.options.games[i].id === game_id) {
              ...
              if (this.game.id != game_id) {
                console.warn("Saving Game Error: safety catch 2");
                return;
              }
  ...
  animateGameElementMove(target, destination, callback = null) {
    if (!document.querySelector(target) || !document.querySelector(destination)) {
      if (callback) {
        callback();
      }
      return;
    ...
    let source_stats = document.querySelector(target).getBoundingClientRect();
    let destination_stats = document.querySelector(destination).getBoundingClientRect();
    let game_self = this;

    let i = 0;

    for (let helper of document.querySelectorAll(".animation_helper")) {
      let temp_id = helper.id.replace("animation_helper", "");
      if (parseInt(temp_id) >= i) {
        i = parseInt(temp_id) + 1;
      }
    }

    //TODO -- fix for Selector Or DOM
    this.app.browser.addElementToSelector(
      `<div id="animation_helper${i}" class="animation_helper"></div>`,
      ".main"
    );

    let this_helper = document.getElementById(`animation_helper${i}`);
    ...
    this_helper.style.transition = "left 2s, top 2s, width 2s, height 2s";

    this_helper.append(document.querySelector(target));
    this_helper.firstElementChild.classList.add("animated_elem");
    ...
    $(`#animation_helper${i}`)
      ...
      .delay(1500)
      .queue(function () {
        document.querySelector(destination).append(this_helper.firstElementChild);
        this_helper.remove();
  ...
  the game will continue with the next player.
    ...
    game_self.addMove("RESOLVE\t" + game_self.app.wallet.getPublicKey());
  ...
  hasGameMovePending() {
    for (let i = 0; i < this.app.wallet.wallet.pending.length; i++) {
      //console.log("PENDING IS: " + this.app.wallet.wallet.pending[i]);
  ...
  initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "ROUNDOVER") {
        ...
        if (game_self.game.players[0] == game_self.app.wallet.getPublicKey()) {
          let newtx = game_self.app.wallet.createUnsignedTransactionWithDefaultFee();
          newtx.transaction.to.push(
            new saito.default.slip(game_self.app.wallet.getPublicKey(), 0.0)
          );
          newtx.msg = {
            request: "roundover",
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "OBSERVER") {
        let msgobj = {
          game_id: game_self.game.id,
          player: game_self.app.wallet.getPublicKey(),
          module: game_self.game.module,
        };
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RESOLVE") {
        ...
        if (gmv[1]) {
          ...
          if (game_self.app.wallet.getPublicKey() === gmv[1]) {
    ...
    */
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "SEND") {
        ...
        //
        if (this.app.wallet.getPublicKey() !== sender) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
        }
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "RECEIVE") {
        ...
        // note: senders identified by Saito publickey not other in this function
        //
        if (this.app.wallet.getPublicKey() !== receiver) {
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
          return 1;
        }
  ...
  payWinner(sender, receiver, amount) {
    ...
    } else if (this.app.wallet.getPublicKey() === receiver) {
      game_self.crypto_transfer_manager.receivePayment(
        this.app,
        this,
        [sender_crypto_address.split("|")[0]],
