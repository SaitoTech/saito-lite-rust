...
****/
  ...
  */
  ...
  initializeSinglePlayerGame(game_data) {
     console.log("TGD1: " + this.game.dice);
    //Initialize this.game and create ID
    this.game = this.newGame();
     console.log("TGD2: " + this.game.dice);
    ...
    if (this.game.players_set == 0) {
      ...
      this.game.step.players[this.game.players[0]] = 1;
    ...
    //
    console.log("TGD3: " + this.game.dice);
    if (this.game.dice === "dba5865c0d91b17958e4d2cac98c338f85cbbda07b71a020ab16c391b5e7af4b") {
      ...
      console.log("DICE IS NOW: " + this.game.dice);
    }
  ...
  */
  async receiveAcceptRequest(blk, tx, conf, app) {
    let txmsg = tx.returnMessage();
    //console.trace("game engine accept request");
  ...
  */
  initialize(app) {
    ...
    //this.startMoveRebroadcasting(); // loop to check for stalled txs //(empty interval)
  ...
  */
  ...
  runQueue() {
    ...
    while (game_self.game.queue.length > 0 && cont == 1) {
      ...
      //console.log("QUEUE ENTRIES: " + game_self.game.queue.length, "FULL QUEUE: " + JSON.stringify(game_self.game.queue));
      ...
      //
      // queue execution basically starts the timer -- why inside the loop?
      this.time.last_received = new Date().getTime();
    ...
    this.gaming_active = 0; 
    //this.processFutureMoves(); // Will run this once we return 0 
    return 0;
  }
  ...
  isFutureMove(playerpkey, txmsg) {
    ...
    console.log("Game Step:"+this.game.step.game,JSON.stringify(this.game.step.players));
  ...
  */
  isUnprocessedMove(player, txmsg) {
    ...
    if (this.game.step.players[player]) {
      ...
      if (tx_step >= this.game.step.players[player] && tx_step < this.game.step.game) {
        console.log("Player move update May 28");
        return 1;
      }
    }
  ...
  /*
  When receiving moves from other players, we almost always want to add them to the future moves array first
  until we have a break in the processing
  */
  addNextMove(gametx) {
    let game_self = this;
    ...
    // HALTED //
    ////////////
    if (this.game.halted == 1 || this.gaming_active == 1) {
...
***/
  ...
  **/
  ...
  initializeDice() {
    console.log("Initialize Dice 1:" + this.game.dice);
    if (this.game.dice === "") {
  ...
  /**
  *  Send game moves (called by endTurn() and some of the game engine commands) 
  */ 
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    //
    // CONFIRM MOVES -- Should be handled in the game module, no?
    /*
    ...
    //
    // adding a minor 100ms delay here allows JQUERY to execute
    ...
    setTimeout(() => {
      ...
      if (type == "game") {
        ns.game++;                //Message will indicate next game step
        mymsg.request = "game";
      }
      ...
      //
      // Check if game.keys has my correct key and update if necessary
      //
      if (this.game.crypto !== "SAITO" && this.game.crypto !== "") {
        let crypto_mod = this.app.wallet.returnCryptoModuleByTicker(this.game.crypto);
      ...
      } else { // revert keys to SAITO if necessary
      ...
      //Address it to the other players
      for (let i = 0; i < this.game.opponents.length; i++) {
      ...
      //console.log("SENDING THIS TX: " + JSON.stringify(newtx));

      this.app.wallet.wallet.pending.push(JSON.stringify(newtx.transaction));
  ...
  //
  // respond to on-chain game moves
  //
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      //
      // delete bad games -- necessary here???
      //
        ...
        /********** FAILS
            ...
            if (gametx.isTo(app.wallet.returnPublicKey()) && gametxmsg.request === "accept") {
              ...
              return; 
                  ...
                  tx_processed = 1;
                  let accept_game = await this.receiveAcceptRequest(null, gametx, 0, app);
            return;
                }
        *********/
        ...
        if (this.name === gametxmsg.module) {
         console.log("Game Peer Request",JSON.parse(JSON.stringify(gametxmsg)));
          ...
          if (!this.game?.id || gametxmsg.module != this.name) {
            console.log("No game id or wrong module");
            return;
          }
          ...
          //
          try {
                if ( ( (!this.game) || (this.game.id != gametxmsg.game_id ) ) && gametxmsg.game_id != "") {
                  for (let i = 0; i < gametxmsg.turn.length; i++) {
          ...
          ***/
          ...
          if (this.isUnprocessedMove(gametx.transaction.from[0].add, gametxmsg)) {
            console.log("is unprocessed move " + gametxmsg.step.game);
            this.addNextMove(gametx);
          } else if (this.isFutureMove(gametx.transaction.from[0].add, gametxmsg)) {
