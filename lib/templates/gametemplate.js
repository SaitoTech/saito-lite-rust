...
class GameTemplate extends ModTemplate {
  ...
  */
  ...
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (this.game.players.includes(tx.transaction.from[0].add) && this.game.over == 0) {
      ...
      if (this.browser_active){
        ...
        let readable = ""
        if (winner === this.app.wallet.returnPublicKey()) {
          readable = "You win";
        }else{
  ...
  tieGame(game_id = null) {
    ...
    if (this.browser_active){
      this.displayModal("Game Over", "It's a tie!");
      this.updateStatus("Game Over -- Tie!");
      this.updateLog("Game Over -- Tie!");
    }
  }
  ...
  endGame(winner, method = "") {
    ...
    if (this.game.winner == this.game.player) {
      this.resignGame(this.game.id, 0, method);
    }
  ...
  /*
  * General command to end the game. If we set game.winner ahead of time, we have a clear understanding of the game outcome
  * Otherwise, the player calling resignGame forfeits ....
  *
  */
  resignGame(game_id = null, tiegame = 0, reason = "") {
    ...
    newtx.transaction.to = this.game.players.map((player) => new saito.default.slip(player));
    ...
    if (this.game.over == 0) {
      ...
      if (this.game.winner > 0) {
        ...
        if (tiegame == 1) {
          ...
          winner = this.game.players.find((player) => player != this.app.wallet.returnPublicKey());
        }
      }
      ...
      //Send message to Arcade as well
      let arcadetx = this.app.wallet.createUnsignedTransactionWithDefaultFee();
      arcadetx.transaction.to = this.game.players.map((player) => new saito.default.slip(player));
      arcadetx.msg = {
        sig: game_id,
        status: "gameover",
        request: "close",
      ...
      arcadetx = this.app.wallet.signTransaction(arcadetx);

      //Send messages

      this.app.network.propagateTransaction(newtx);
      this.app.network.propagateTransaction(arcadetx);

      let relay_mod = this.app.modules.returnModule("Relay");
      if (relay_mod){
        relay_mod.sendRelayMessage(this.game.players, "game relay gameover", newtx);
        relay_mod.sendRelayMessage(this.game.players, "arcade spv update", arcadetx);
      }
