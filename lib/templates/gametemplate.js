...
console.log("LLI: " + loop_length_instruction);
  ...
  initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOAD") {
        ...
        let game_id = prompt("Enter Saved Game ID:");
        if (game_id != "") {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "SAVE") {
        ...
        backup_game.queue.push(
          `ACKNOWLEDGE\tPlease CONFIRM all players have reloaded -- your game will continue the next time you click to continue, but if not all players have restored things will break.`
        );
        backup_game.queue.push(
          `ACKNOWLEDGE\tGame Reloaded. Please wait until all players have confirmed they can see this message before clicking to continue.`
        );
	...
	if (this.app.browser.port) { loadurl = loadurl + ":" + this.app.browser.port; }
	game_self.updateLog(`<p></p>This game has been saved. To restore from this point, all players should visit the following URL:<p></p>&nbsp;<p></p>${loadurl}/loadgame?game_id=${game_self.game.id}<p></p>`);
      }
      return 1;
    });
