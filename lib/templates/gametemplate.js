  ...
  */
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (this.game.players.includes(tx.transaction.from[0].add) && this.game.over == 0) {
      this.game.over = 1; //Make sure this is only processed once
  ...
  */
  resignGame(game_id = null, reason = "forfeit") {
    ...
    newtx.msg = {
      ...
      module: this.game.module,
      is_game_over: 1,          //Game has reached natural conclusion
      reason,
    };

    console.info("Send quit message: ", JSON.parse(JSON.stringify(newtx.msg)));
