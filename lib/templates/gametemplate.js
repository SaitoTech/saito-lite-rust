...
class GameTemplate extends ModTemplate {
  ...
  render(app) {
    ...
    try {
      ...
      window.location.hash = app.browser.initializeHash(`#gid=${this.game.id}`, oldHash, {
  ...
  async initialize(app) {
    ...
    this.addFollower(this.app.wallet.publicKey);
  ...
  */
  ...
  endGameInterface(status, allowRematch) {
    ...
    if (document.querySelector(".status") && document.querySelector(".controls")) {
      ...
      this.overlay.show(
        `<div class="status-overlay"><div class="status-header">${status}</div><div class="status-text-menu">${options}</div></div>`
      );
  ...
  */
  endGame(winner = [], reason = "") {
    ...
    if (player_to_send == this.app.wallet.publicKey) {
