...
class GameTemplate extends ModTemplate {
    ...
    setTimeout(async () => {
      ...
      if (this.relay_moves_offchain_if_possible && send_onchain_only == 0) {
        ...
        if ((newtx.msg.request === "game" && this.game.initializing == 0) || this.initialize_game_offchain_if_possible == 1) {
          this.app.connection.emit("relay-send-message", { recipient: this.game.accepted, request: "game relay gamemove", data: newtx.toJson() });

          //An experiment to have game steps/ts update in arcade
          this.app.connection.emit("relay-send-message", { recipient: "PEERS", request: "arcade spv update", data: newtx.toJson() });
        if (
      ...
      if (message?.data != undefined) {
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (message.request === "game relay gamemove") {
            if (this.initialize_game_run == 0 || this.isFutureMove(gametx.transaction.from[0].publicKey, gametxmsg)) {
              //console.info("OFFCHAIN: is future move " + gametxmsg.step.game);
              await  this.addFutureMove(gametx);
            } else if (this.isUnprocessedMove(gametx.transaction.from[0].publicKey, gametxmsg)) {
  ...
  */
  ...
  async initializeGameFeeder(game_id) {
    ...
    if (this.game.status != "") {
      this.updateStatusHeader(this.game.status);
    }
  ...
  startQueue() {
    if (await this.hasGameMovePending() && this.game?.initializing == 0) {
      this.updateStatusHeader('Rebroadcasting Last Move');
  ...
  async runQueue() {
    ...
    while (game_self.game.queue.length > 0 && cont == 1) {
      ...
      for (let i = 0; i < game_self.commands.length; i++) {
        if (await game_self.commands[i](game_self, gmv) === 0) { //Game engine requests queue processing pauses
  ...
  async processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      ...
      if (this.isUnprocessedMove(ftx.from[0].publicKey, ftxmsg)) {
        ...
        return 1;
      } else if (this.isFutureMove(ftx.transaction.from[0].publicKey, ftxmsg)) {
  ...
  */
  async resignGame(game_id = null, reason = "forfeit") {
    ...
    //Send message
    await this.app.network.propagateTransaction(newtx);
    this.app.connection.emit("relay-send-message", { recipient: this.game.accepted, request: "game relay stopgame", data: newtx.toJson() });
    //Send message into the ether so that the arcade service can update the game status to "over"
    this.app.connection.emit("relay-send-message", { recipient: "PEERS", request: "arcade spv update", data: newtx.toJson() });
  }
  ...
  */
  ...
  initializeQueueCommands() {
	...
	for (let i = 0; i < players_to_go.length; i++) {
	    ...
	    game_self.updateStatusHeader(gmv[1]);
          }
        }
        game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
      }
    ...
    this.commands.push((game_self, gmv) => {
	  ...
	  if (parseInt(player) > 0) {
        ...
        game_self.playerAcknowledgeNotice(notice, async function () {
          ...
          game_self.updateStatusHeader("acknowledged...");
          game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
      ...
      if (gmv[0] === "STAKE") {
        ...
        if (auths == game_self.game.players.length) {
          ...
          if (game_self.game.player - 1 == first_non_verifier_idx) {
            ...
            this.updateStatusHeader("Waiting for Others to Accept");
          }
          return 0;
        }
      }
          ...
          await game_self.app.wallet.returnPreferredCryptoBalances(
            ...
            async function (robj) {
              ...
              if (balance_adequate == 1) {
                ...
                game_self.updateStatusHeader("balance adequate ...");
                game_self.game.queue.splice(game_self.game.queue.length - 1, 1);
                await game_self.restartQueue();
                return 0;
              }
