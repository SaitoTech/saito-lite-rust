...
class GameTemplate extends ModTemplate {
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      if (txmsg.request === "stopgame") {
        game_self.processResignation(tx.from[0].publicKey, txmsg);
        return;
      }
  ...
  */
  ...
  async addNextMove(gametx) {
    ...
    this.game.step.players[gametx.transaction.from[0].publicKey] = gametxmsg.step.game;
  ...
  */
  async processFutureMoves() {
    ...
    for (let i = 0; i < this.game.future.length; i++) {
      ...
      if (this.isUnprocessedMove(ftx.transaction.from[0].publicKey, ftxmsg)) {
  ...
  */
  async receiveGameoverRequest(blk, tx, conf, app) {
    ...
    console.info("Received Gameover Request from " + tx.transaction.from[0].publicKey);

    //
    // sender must be in game to end it (removed players no problem)
    // Make sure this is only processed once
    //
    if (this.game.players.includes(tx.transaction.from[0].publicKey) && this.game.over !== 1) {
    ...
    this.commands.push(async (game_self, gmv) => {
      if (gmv[0] === "ROUNDOVER") {
        ...
        if (game_self.game.players[0] == this.publicKey) {
          ...
          newtx = await game_self.app.wallet.signTransaction(newtx);
