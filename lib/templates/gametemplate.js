...
console.log("RECEIVED GAMEOVER REQUEST IN ON-CONF: ");
...
console.log("SENDING RELAY MESSAGE");
  ...
  */
  ...
  addNextMove(gametx) {
    ...
    console.log("Add Next Move to QUEUE");
    console.log(JSON.parse(JSON.stringify(gametxmsg)));
    ...
    if (this.game.player == 0) {
      ...
      if (gametxmsg.game_state.deck) {
        if (gametxmsg.game_state.deck.length > 0) {
          ...
          //
          if (this.game.player_decks) {} else {
            this.game.player_decks = [];
            if (this.game.player_decks.length < this.game.players) {
              for (let i = 0; i < this.game.players.length; i++) {
                this.game.player_decks.push = {};
                this.game.player_decks[i].deck = [];
              }
            }
          }
        }
        ...
        for (let i = 0; i < gametxmsg.game_state.deck.length; i++) {
          this.game.deck[i] = gametxmsg.game_state.deck[i];
        }
        if (gametxmsg.sharekey) {
          ...
          if (sharekey) {
            for (let i = 0; i < gametxmsg.game_state.deck.length; i++) {
              while (this.game.player_decks.length <= player_idx) {
                this.game.player_decks.push({});
                this.game.player_decks[this.game.player_decks.length-1].deck = [];
              }
              while (this.game.player_decks[player_idx].deck.length <= i) {
                this.game.player_decks[player_idx].deck.push({});
              }
              this.game.player_decks[player_idx].deck[i].hand = JSON.parse(this.app.crypto.hexToString(this.app.crypto.decodeXOR(gametxmsg.game_state.deck[i].hand, sharekey)));
              this.game.player_decks[player_idx].deck[i].keys = JSON.parse(this.app.crypto.hexToString(this.app.crypto.decodeXOR(gametxmsg.game_state.deck[i].keys, sharekey)));
            }
          }
        }
      }
  ...
  */
  processFutureMoves() {
      ...
      if (this.isUnprocessedMove(ftx.transaction.from[0].add, ftxmsg)) {
        ...
        if (this.game.player == 0){
          if (this.observer.is_paused){
            ...
            setTimeout(()=>{ this.startQueue(); }, this.observer.step_speed);
            return 1;
          }
        }
        this.startQueue();
    ...
    if (this.game.player == 0 && this.game.future.length == 0){
     this.updateObserverStatus("No pending moves");
  ...
  */
  ...
  updateObserverStatus(str){
    try{
      document.getElementById("obstatus").innerHTML = sanitize(str);
    }catch(err){
      console.error(err);
    }
  }
