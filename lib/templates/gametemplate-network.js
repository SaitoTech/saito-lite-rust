...
class GameNetwork extends ModTemplate {
  ...
  async onConfirmation(blk, tx, conf, app) {
    ...
    if (conf == 0) {
      ...
      // process game move
      //
      if (game_self.initialize_game_run == 0 || game_self.isFutureMove(tx.transaction.from[0].publicKey, txmsg)) {
      ...
      if (message?.data != undefined) {
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (message.request === "game relay gamemove") {
            ...
            this.processResignation(gametx.transaction.from[0].publicKey, gametxmsg);
