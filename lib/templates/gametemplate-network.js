...
class GameNetwork extends ModTemplate {
  async onConfirmation(blk, tx, conf) {
    ...
    console.log("txmsg inside onConfirmation ////");
    console.log(txmsg);
    console.log(txmsg.module);
    ...
    if (conf == 0) {
      ...
      if (txmsg.request === "gameover") {
        game_self.receiveGameoverRequest(blk, tx, conf, this.app);
        return;
      }
      // stopgame requests
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    setTimeout(async () => {
      ...
      if (this.game.sroll == 1) {
        ...
        let hash1_sig = this.app.crypto.signMessage(hash1, await this.app.wallet.getPrivateKey());
        let hash2_sig = this.app.crypto.signMessage(hash2, await this.app.wallet.getPrivateKey());
      ...
      if (message?.data != undefined) {
        ...
        if (this.name === gametxmsg.module) {
          ...
          if (message.request === "game relay gamemove") {
            ...
            this.processResignation(gametx.from[0].publicKey, gametxmsg);
