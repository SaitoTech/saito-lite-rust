...
console.log("GAME TURN AT SENDMESSAGE: "+JSON.stringify(this.game.turn));
...
console.log("hitting AA");
        return;
      }
      if (this.game.opponents == undefined) {
console.log("hitting AB");
        return;
      }
...
console.log("hitting A");
      ...
      if (!this.game.cryptos[this.publicKey]) {
      ...
      console.log("AFTER: "+ JSON.stringify(this.game.turn));
...
console.log("MESSAGE: " + JSON.stringify(mymsg));
  ...
  async processAcceptRequest(tx) {
    ...
    //
    for (let i = 0; i < txmsg.players.length; i++) {
      await this.addPlayer(txmsg.players[i]);
    }
    ...
    if (this.game.players.includes(tx.from[0].publicKey) && this.game.over !== 1) {
      ...
      if (this.browser_active) {
        ...
        try {
          ...
          app.connection.on("arcade-reject-challenge", async (game_id) => {
            await this.endGameInterface(`Game Over: ${readable}`, false);
          });
