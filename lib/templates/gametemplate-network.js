...
class GameNetwork extends ModTemplate {
  ...
  async sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    setTimeout(() => {
      ...
      if (
          ...
          if (this.game.players[i] === this.app.wallet.publicKey) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
                ...
                }\t${crypto_key}\t${this.app.crypto.signMessage(
                  crypto_key,
        ...
        let crypto_key = this.app.wallet.publicKey;
        for (let i = 0; i < this.game.players.length; i++) {
          if (this.game.players[i] === this.app.wallet.publicKey) {
            if (this.game.keys[i] !== crypto_key) {
              this.game.turn.push(
  ...
  async processAcceptRequest(tx) {
    ...
    //
    if (!txmsg.players.includes(this.app.wallet.publicKey)) {
    ...
    if (this.game.players_set == 0) {
      ...
      console.log("My PublicKey: " + this.app.wallet.publicKey);
  ...
  receiveGameoverRequest(blk, tx, conf, app) {
    ...
    if (this.game.players.includes(tx.transaction.from[0].add) && this.game.over !== 1) {
      ...
      if (this.browser_active) {
        ...
        if (winner.includes(this.app.wallet.publicKey)) {
          readable = "You win";
        } else {
