...
class DBModTemplate extends ModTemplate {
    ...
    if (txmsg.request == "delete") {
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        let sql =
          `SELECT deleted
           FROM ${dbtable}
           WHERE ${column}` + " = $value AND (admin = $admin OR $admin = $super)";
        let params = {
          $value: value,
          $admin: tx.from[0].publicKey,
          $super: this.app.modules.returnModule(txmsg.module).admin_pkey,
        };
  ...
  async deleteDatabase(values) {
    let newtx = await this.app.wallet.createUnsignedTransactionWithDefaultFee(this.admin_pkey);
