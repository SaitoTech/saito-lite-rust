class SettlersActions {
	...
	playerAcknowledgeNotice(msg, mycallback) {
		...
		try {
			this.updateStatusWithOptions(
				`${this.getLastNotice()}<div class="player-notice"><span>${msg}</span></div>`,
				html
			);
	...
	collectHarvest(value, player_who_rolled) {
		...
		for (let city of this.game.state.cities) {
			...
			for (let neighboringHex of city.neighbours) {
				if (
					...
					let resource =
						this.game.state.hexes[neighboringHex].resource;
		...
		let firstMsg =
			this.game.player == player_who_rolled
				? 'You'
				: this.game.playerNames[player_who_rolled - 1];
		...
		if (poor_harvest) {
			this.updateStatus(
				`<div class="persistent player-notice"><span>${firstMsg}: ${this.randomMsg()}</span></div>`
			);
		} else {
			this.updateStatus(
	...
	buildCity(player, slot) {
			...
			for (let road of newRoads) {
				...
				for (let i = 0; i < this.game.state.roads.length; i++) {
					if (this.game.state.roads[i].slot == 'road_' + road) {
						//console.log("exists");
						if (
							this.game.state.roads[i].player == this.game.player
						) {
							//console.log("is mine");
							blocks_me = true;
						}
		...
		let neighbours = this.hexgrid.hexesFromVertex(
			slot.replace('city_', '')
		); //this.returnAdjacentHexes(slot);
  ...
  */
	buildRoad(player, slot) {
		...
		if (this.game.player == player) {
			...
			for (let road of this.hexgrid.adjacentEdges(
				slot.replace('road_', '')
			)) {
				//console.log("road: ",road);
				...
				for (let i = 0; i < this.game.state.cities.length; i++) {
					if (
						...
						'city_' + intersection[0]
					) {
						if (
							this.game.state.cities[i].player !==
							this.game.player
						) {
							block_me = true;
						}
  ...
  */
	checkLargestArmy(player) {
		...
		if (vpChange) {
			this.updateLog(
				`${this.formatPlayer(player)} claims the LARGEST ARMY`
			);
			this.game.queue.push(
				`ACKNOWLEDGE\t${
					this.game.playerNames[player - 1]
				} forms the largest army ${this.largest.svg}`
			);
		}
	}
  ...
  */
	checkLongestRoad(player) {
		...
		for (let i = 0; i < playerSegments.length; i++) {
			...
			let bestPath = continuousRoad(
				this,
				Array(playerSegments[i]),
				remainder
			);
				...
				if (longest.length > this.game.state.longestRoad.size) {
					if (this.game.state.longestRoad.player != player) {
						...
						this.game.queue.push(
							`ACKNOWLEDGE\t${
								this.game.playerNames[player - 1]
							} claimed the longest road ${this.longest.svg}`
						);
					} else {
						//Increase size
						this.game.state.longestRoad.size = longest.length;
						this.game.state.longestRoad.path = longest;
						this.updateLog(
							`${this.formatPlayer(player)} extended the ${
								this.longest.name
							} to ${longest.length} segments.`
						);
					}
					return 1;
				}
	...
	wishListToImage(stuff) {
		...
		for (let resource in stuff) {
			for (let i = 0; i < stuff[resource]; i++) {
				offer += `<img class="icon" src="${this.returnCardImage(
					resource
				)}"/>`;
			}
		}
