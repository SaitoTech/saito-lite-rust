class SettlersPlayer {
		...
		if (thievingTargets.length > 0) {
			let robPlayer = (victim) => {
				...
				if (potentialLoot.length > 0) {
					let loot =
						potentialLoot[
							Math.floor(Math.random() * potentialLoot.length)
						];
					settlers_self.addMove(
						`steal_card\t${player}\t${victim}\t${loot}`
					);
				} else
					settlers_self.addMove(
						`steal_card\t${player}\t${victim}\tnothing`
					);
			...
			if (thievingTargets.length > 1) {
				...
				for (let i = 0; i < this.game.players.length; i++) {
					if (thievingTargets.includes(i + 1)) {
						html += `<li class="textchoice steal-player-choice" id="${
							i + 1
	...
	playerPlayBandit() {
		this.updateStatus(
			`<div class="player-notice"><span>Move the ${this.b.name}</span></div>`
		);
		this.updateControls('');
		let settlers_self = this;
	...
	canPlayerBuildTown(player) {
		...
		if (
			this.returnCitySlotsAdjacentToPlayerRoads(this.game.player)
				.length == 0
		)
			return false;
      ...
      */
		if (existing_cities < 2) {
			...
			$('.city.empty').on('mouseup', function (e) {
				...
				//Confirm this move
				let slot = $(this).attr('id');
				settlers_self.confirmPlacement(
					slot,
					settlers_self.c1.name,
					() => {
						$('.city.empty').removeClass('chover');
						$('.city.empty').off();
						settlers_self.game.state.placedCity = slot;
						settlers_self.buildCity(
			...
			this.updateStatus(
				`<div class="player-notice">You may build a ${this.c1.name}</div>`
			);
			if (canBackUp) {
				...
				$('.undo').on('click', function () {
					...
					settlers_self.addMove(
						`undo_build\t${settlers_self.game.player}\t1`
					);
					settlers_self.endTurn();
				});
			}
	...
	playerBuildRoad(player, canBackUp = false) {
        ...
        */
			...
			this.updateStatus(
				`<div class="player-notice">You may build a ${this.r.name}...</div>`
			);
			if (canBackUp) {
				this.updateControls('<ul><li class="undo">cancel</li></ul>');
				$('.undo').on('click', function () {
					...
					settlers_self.addMove(
						`undo_build\t${settlers_self.game.player}\t0`
					);
					settlers_self.endTurn();
				});
			}
	...
	playerBuildCity(player, canBackUp = 0) {
		...
		if (canBackUp) {
			...
			$('.undo').on('click', function () {
				...
				settlers_self.addMove(
					`undo_build\t${settlers_self.game.player}\t2`
				);
				settlers_self.endTurn();
			});
		}
		...
		$('.chover').on('click', function () {
			...
			settlers_self.confirmPlacement(slot, settlers_self.c2.name, () => {
				...
				for (
					let i = 0;
					i < settlers_self.game.state.cities.length;
					i++
				) {
					if (
    ...
    */
	playerPlayMove() {
		...
		if (can_do_something != true) {
			...
			this.addMove(
				'ACKNOWLEDGE\tYou cannot buy anything - end turn\t' +
					settlers_self.game.player
			);
			this.endTurn();
			return;
		...
		let statushtml =
			settlers_self.getLastNotice() ||
			'<div class="player-notice">YOUR TURN:</div>';
	...
	canPlayerTradeWithBank() {
		...
		for (let resource of this.returnResources()) {
			if (
				this.countResource(this.game.player, resource) >=
				minForTrade[resource]
			)
				return true;
		}
		return false;
	...
	playerTradeWithBank() {
		...
		if (Object.keys(my_resources).length > 0) {
			let html =
				'<div class=\'player-notice\'>Select Resource to Trade: <ul class=\'bank\'>';
			for (let i in my_resources) {
				html += `<li id="${i}" class="option">`;
				for (let j = 0; j < minForTrade[i]; j++) {
					html += `<img class="icon" src="${settlers_self.returnCardImage(
						i
					)}"/>`;
				}
			...
			$('.option').on('click', function () {
				...
				html =
					'<div class=\'player-notice\'>Select Desired Resource: <ul class=\'bank horizontal_list\'>';
