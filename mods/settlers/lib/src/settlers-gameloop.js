	...
	async handleGameLoop() {
		...
		if (this.game.queue.length > 0) {
			...
			if (mv[0] == 'winner') {
				...
				this.updateLog(
					`${this.formatPlayer(winner + 1)} is ${
						this.winState
					} and wins the game!`
				);

				this.stats_overlay.render();
				$('.settlers-stats-overlay h1').text(
					`Game Over: ${this.game.playerNames[winner]} wins!`
				);
			...
			if (mv[0] == 'buy_card') {
				...
				this.updateLog(
					`${this.formatPlayer(player)} bought a ${
						this.card.name
					} card`
				);
				...
				if (player != this.game.player) {
					...
					let html = `<span class="tip">${lastcard.card}
                        <div class="tiptext">${
	this.rules[lastcard.action]
}</div>
                      </span>`;

					console.log(
						'Current status: ' + this.game.state.canPlayCard
					);
					if (
						lastcard.action == 0 &&
					...
					this.updateStatus(
						`<div class="persistent player-notice"><span>You bought a ${html}</span></div>`
					);
				}
				return 1;
			}
			...
			if (mv[0] == 'vp') {
				...
				this.updateLog(
					`${this.formatPlayer(
						player
					)} played ${cardname} to gain 1 victory point`
				);
				return 1;
			}
			...
			if (mv[0] == 'road_building') {
				...
				this.game.state.players[player - 1].devcards--; //Remove card (for display)
				this.updateLog(
					`${this.formatPlayer(player)} played ${cardname}`
				);
				return 1;
			}
			...
			if (mv[0] == 'play_knight') {
				...
				this.updateLog(
					`${this.formatPlayer(player)} played a ${cardname}!`
				);
				this.game.state.players[player - 1].devcards--; //Remove card (for display)
				//Update Army!
			...
			if (mv[0] == 'monopoly') {
				...
				for (let i = 0; i < this.game.state.players.length; i++) {
					if (player != i + 1) {
						...
						for (
							...
							if (
								this.game.state.players[i].resources[j] ==
								resource
							) {
								am_i_a_victim = true;
								lootCt++;
								this.game.state.players[i].resources.splice(
									j,
									1
								);
								j--;
				...
				for (let i = 0; i < lootCt; i++)
					this.game.state.players[player - 1].resources.push(
						resource
					);

				if (this.game.player == player) {
					this.updateStatus(
						`<div class="persistent player-notice">You collected ${lootCt} ${this.formatResource(
							resource
						)}</div>`
					);
				}
				...
				this.updateLog(
					`${this.formatPlayer(
						player
					)} played ${cardname} for ${this.formatResource(
						resource
					)}, collecting ${lootCt}.`
				);
				return 1;
			...
			if (mv[0] == 'undo_build') {
				...
				this.updateLog(
					`${this.formatPlayer(
						player
					)} decided against building a ${key}`
				);
			...
			if (mv[0] == 'spend_resource') {
				...
				let target =
					this.game.state.players[player - 1].resources.indexOf(
						resource
					);
				if (target >= 0) {
					this.game.state.players[player - 1].resources.splice(
						target,
						1
					);
				} else {
			...
			if (mv[0] == 'player_build_road') {
				...
				if (this.game.player == player) {
					if (mv[2] == 1) {
						console.log(
							'Last Placed City: ' + this.game.state.last_city
						);
			...
			if (mv[0] == 'build_road') {
				...
				this.updateLog(
					`${this.formatPlayer(player)} built a ${this.r.name}`
				);
				if (this.checkLongestRoad(player)) {
					console.log(
						'Longest Road:',
						this.game.state.longestRoad.path
					);
				}
			...
			if (mv[0] == 'player_build_city') {
				...
				if (this.game.player == player) {
					...
					this.updateStatus(
						`<div class="player-notice">${
							this.game.playerNames[player - 1]
						} is building a ${this.c1.name}...</div>`
					);
				}
			...
			if (mv[0] == 'build_city') {
				...
				this.updateLog(
					`${this.formatPlayer(player)} built a ${this.c1.name}`
				);
			...
			if (mv[0] == 'player_upgrade_city') {
				...
				if (this.game.player == player) {
					...
					this.updateStatus(
						`<div class="player-notice">${
							this.game.playerNames[player - 1]
						} is upgrading to a ${this.c2.name}...</div>`
					);
				}
			...
			if (mv[0] == 'upgrade_city') {
				...
				this.updateLog(
					`${this.formatPlayer(player)} upgraded a ${
						this.c1.name
					} to a ${this.c2.name}`
				);
			...
			if (mv[0] === 'offer') {
				...
				console.log(
					'RECEIVED OFFER: ' + JSON.stringify(stuff_on_offer)
				);
				console.log(
					'RECEIVED OFFER: ' + JSON.stringify(stuff_in_return)
				);

				if (this.game.player == receiving_player) {
					this.game.state.ads[offering_player - 1].offer =
						stuff_on_offer;
					this.game.state.ads[offering_player - 1].ask =
						stuff_in_return;
					this.game.state.ads[offering_player - 1].ad = false;
					this.updateLog(
						`${this.formatPlayer(
							offering_player
						)} sent a trade offer to ${this.formatPlayer(
							receiving_player
						)}.`
					);
					this.displayPlayers();
      ...
      */
			...
			if (mv[0] === 'reject_offer') {
				...
				if (this.game.player == offering_player) {
					this.updateStatus(
						'<div class=\'persistent player-notice\'>Your offer has been rejected</div>'
					);
				}
				...
				this.updateLog(
					`${this.formatPlayer(
						refusing_player
					)} turned down a trade offer from ${this.formatPlayer(
						offering_player
					)}.`
				);
      ...
      */
			if (mv[0] === 'bank') {
				...
				if (this.game.player == player) {
					this.updateStatus(
						'<div class=\'persistent player-notice\'>Your trade is completed</div>'
					);
				} else {
					this.updateStatus(
						`<div class='persistent player-notice'>${
				...
				this.updateLog(
					`${this.formatPlayer(
						player
					)} traded ${outCount}x${this.formatResource(
						outResource
					)}<span> with the bank for </span>${this.formatResource(
						inResource
					)}.`
				);
      ...
      */
			...
			if (mv[0] == 'move_bandit') {
				...
				this.updateLog(
					`${this.formatPlayer(player)} moved the ${
						this.b.name
					} to ${hexName}`
				);
			...
			if (mv[0] == 'steal_card') {
				...
				if (victim > 0 && loot != 'nothing') {
					...
					this.game.queue.push(
						'spend_resource\t' + victim + '\t' + loot
					);
					this.game.state.players[thief - 1].resources.push(loot);
				}

				let x =
					loot == 'nothing' ? 'nothing' : this.formatResource(loot);
				...
				if (this.game.player === victim) {
					if (loot == 'nothing') {
						...
						this.updateStatus(
							`<div class="persistent player-notice"><span>${
								this.game.playerNames[thief - 1]
							} stole your</span>${this.formatResource(
								loot
							)}</div>`
						);
					}
				...
				let victim_name =
					victim > 0
						? `${this.formatPlayer(victim)}`
						: '<span>nobody</span>';
				this.updateLog(
					`${this.formatPlayer(
						thief
					)} stole ${x}<span> from </span>${victim_name}`
				);
				return 1;
			}
