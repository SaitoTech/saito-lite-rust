class SettlersState {
  ...
  */
	...
	returnResourceHTML(resource) {
		return `<div class="tip"><img class="icon" src="${this.returnCardImage(
			resource
		)}" /></div>`;
	}
    ...
    */
	...
	addSectorValueToGameboard(hex, sector_value) {
		...
		if (document.getElementById(svid)) {
			...
			let sector_value_obj =
				this.app.browser.htmlToElement(sector_value_html);
			if (hexobj) {
    ...
    */
	addPortsToGameboard() {
		if (Object.keys(this.game.state.ports).length == 9) {
			...
			for (let i = 0; i < 5; i++) {
				let r = resources.splice(
					this.rollDice(resources.length) - 1,
					1
				);
	...
	addPortToGameboard(hex, port, direction) {
		...
		if (!document.getElementById(port_id)) {
			let port_html = `<div class="port port${direction}" id="${port_id}">
                        <div class="ship hexTileCenter">${this.returnPortIcon(
		port
	)}</div>
    ...
    */
	...
	addRoadToGameboard(hex, road_component) {
		...
		if (hexobj && !document.getElementById(road_id)) {
			...
			$(hexobj).after(
				`<div class="road road${road_component} empty" id="${road_id}"></div>`
			);
		}
	}
