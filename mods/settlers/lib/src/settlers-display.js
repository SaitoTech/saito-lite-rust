class SettlersDisplay {
	renderTradeOfferInPlayerBox(
		...
		if (!can_accept) {
			...
			this.addMove(
				`reject_offer\t${this.game.player}\t${offering_player}`
			);
			this.endTurn();
			return;
		}
		...
		$(`${selector} .pboption`).on('click', function () {
			...
			if (choice == 'accept') {
				...
				settlers_self.addMove(
					`clear_advert\t${settlers_self.game.player}`
				);
				settlers_self.addMove(
					'accept_offer\t' +
			...
			if (choice == 'reject') {
				...
				settlers_self.addMove(
					`reject_offer\t${settlers_self.game.player}\t${offering_player}`
				);
				settlers_self.endTurn();
			}
		});
	...
	displayBoard() {
		...
		for (let i in this.game.state.hexes) {
			...
			if (
				this.game.state.hexes[i].resource != this.returnNullResource()
			) {
		...
		for (let i in this.game.state.cities) {
			...
			let classname =
				'p' + this.game.colors[this.game.state.cities[i].player - 1];
			...
			let ad = this.returnAdjacentCitySlots(
				this.game.state.cities[i].slot
			);
	...
	displayScore() {
		...
		for (let i = 0; i < this.game.state.players.length; i++) {
			if (
				this.game.state.players[i].vp !==
				this.racetrack.players[i].score
			) {
	...
	displayCardfan(deck = '') {
		try {
			...
			for (let r of this.game.state.players[this.game.player - 1]
				.resources) {
				//Show all cards
				cards += `<div class="card"><img src="${this.returnCardImage(
					r
				)}">
                  </div>`;
			}
	...
	formatPlayer(playerNumber) {
		return `<span class="p${
			this.game.colors[playerNumber - 1]
		}-lite display-name">${this.game.playerNames[playerNumber - 1]}</span>`;
	}

	formatResource(resource) {
		return `<div class="card tiny"><img src="${this.returnCardImage(
			resource
		)}" /></div>`;
	}
		...
		try {
			...
			for (let i = 1; i <= this.game.state.players.length; i++) {
				...
				if (this.game.state.players[i - 1].vpc > 0) {
					statshtml += `<div class="victory_point_card_points vproundel">${
				...
				for (
					let j = 0;
					j < this.game.state.players[i - 1].knights;
					j++
				) {
					...
					if (this.game.player != i) {
						if (
							...
							if (this.game.state.ads[i - 1].ad) {
								let offer = this.wishListToImage(
									this.game.state.ads[i - 1].offer
								);
								let ask = this.wishListToImage(
									this.game.state.ads[i - 1].ask
								);
								let id = `trade_${i}`;
								...
								$(id).on('click', function () {
									...
									settlers_self.showTradeOverlay(
										...
										settlers_self.game.state.ads[i - 1]
											.offer
									);
								});
							} else {
		...
		this.animationSequence.push({
			...
			params: [
				this.createGameElement(
					`<div class="card_holder"><img src="${this.returnCardImage(
						resource
					)}"/></div>`,
					tile,
					tile
  ...
  */
	getLastNotice(preserveLonger = false) {
		if (document.querySelector('.status .persistent')) {
			...
			return `<div class="${
				preserveLonger ? 'persistent' : 'player-notice'
