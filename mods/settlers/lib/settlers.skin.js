class SettlersSkin {
	...
	render(option = 'classic') {
		...
		case 'elements':
			this.c1 = {
				name: 'shrine',
				svg: `<svg viewbox="0 0 200 200" stroke="black"><line x1="25" x2="175" y1="30" y2="30" stroke-width="15px"/><line x1="45" x2="155" y1="70" y2="70"  stroke-width="9px"/><line x1="55" x2="30" y1="30" y2="200"  stroke-width="21px"/><line x1="145" x2="170" y1="30" y2="200"  stroke-width="21px"/>
	<line x1="100" x2="100" y1="30" y2="70"  stroke-width="18px"/></svg>`
			};
			this.c2 = {
				name: 'pagoda',
				svg: '<svg viewbox="0 0 200 200"><polygon points="100,0 160,30, 120,25 120,40 170,70 130,65 130,100 180,130 140,125 140,180 190,180 200,200 0,200 10,180 60,180 60,125 20,130 70,100 70,65 30,70 80,40 80,25 40,30"/></svg>'
			};
			this.r = { name: 'foot path' };
			this.b = { name: 'demon' };
			this.s = {
				name: 'yinyang master',
				img: '<i class="fas fa-magic"></i>'
			};
			this.t = { name: 'nature' };
			this.vp = {
				name: 'Mana',
				svg: '<svg viewbox="0 0 200 200"><circle fill="gold" cx="100" cy="100" r="95" stroke="goldenrod" stroke-width="5"/> <text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-size="132px" fill="saddlebrown">1</text></svg>'
			};
			this.longest = {
				name: 'Philosopher\'s Path',
				svg: `<svg viewbox="0 0 200 200"><circle fill="gold" cx="100" cy="100" r="95" stroke="goldenrod" stroke-width="5"/>
		      						<text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-size="132px" fill="saddlebrown">2</text></svg>`
			};
			this.largest = {
				name: 'Largest Yinyang Bureau',
				svg: `<svg viewbox="0 0 200 200"><circle fill="silver" cx="100" cy="100" r="95" stroke="goldenrod" stroke-width="5"/>
		      <text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-size="132px" fill="saddlebrown">2</text></svg>`
			};
			this.resources = [
				{
					...
					count: 3,
					ict: 1,
					tile: '/settlers/img/sectors/brick1.png',
					card: '/settlers/img/cards/brick.png'
				},
				{
					name: 'wood',
					count: 4,
					ict: 1,
					tile: '/settlers/img/sectors/wood1.png',
					card: '/settlers/img/cards/wood.png'
				},
				{
					name: 'earth',
					count: 4,
					ict: 1,
					tile: '/settlers/img/sectors/_wheat1.png',
					card: '/settlers/img/cards/wheat_old.png'
				},
				{
					name: 'water',
					count: 4,
					ict: 1,
					tile: '/settlers/img/sectors/wool1.png',
					card: '/settlers/img/cards/wool.png'
				},
				{
					name: 'metal',
					count: 3,
					ict: 1,
					tile: '/settlers/img/sectors/ore1.png',
					card: '/settlers/img/cards/ore.png'
				},
			...
			this.deck = [
				{
					card: 'Yinyang Shi',
					count: 14,
					img: '/settlers/img/cards/wizard.png',
					action: 1
				},
				{
					card: 'Elixir of Life',
		...
		default: //Classic
			...
			this.s = {
				name: 'knight',
				img: '<i class="fas fa-horse-head"></i>'
			};
			this.t = { name: 'bank' };
			this.vp = {
				name: 'VP',
				svg: '<svg viewbox="0 0 200 200"><circle fill="gold" cx="100" cy="100" r="95" stroke="goldenrod" stroke-width="5"/> <text x="50%" y="60%" text-anchor="middle" dominant-baseline="middle" font-size="132px" fill="saddlebrown">1</text></svg>'
			};
			this.longest = {
				name: 'Longest Road',
				svg: `<svg viewbox="0 0 200 200"><circle fill="gold" cx="100" cy="100" r="95" stroke="goldenrod" stroke-width="5"/>
			...
			this.largest = {
				name: 'Largest Army',
				svg: `<svg viewbox="0 0 200 200"><circle fill="gold" cx="100" cy="100" r="95" stroke="gold" stroke-width="5"/>
			...
			this.cardDir = '/settlers/img/cards/';
			this.back = '/settlers/img/cards/red_back.png';
			this.card = {
				name: 'development',
				back: '/settlers/img/cards/red_back.png'
			};
	...
	resourceArray() {
		...
		for (let i of this.resources) {
			if (i.count > 1) newArray.push(i.name);
		}
		return newArray;
	}
	...
	portIcon(res) {
		...
		for (let i of this.resources) {
			if (i.name == res) {
				if (i.icon) {
					return `<img class="icon" src="${i.icon.replace(
						'-icon',
						'-port'
					)}">`;
				}
  ...
  */
	nullResource() {
		for (let i of this.resources) if (i.count == 1) return i.name;
	}
	...
	isActionCard(cardname) {
		for (let c of this.deck) {
			if (cardname == c.card && c.action > 0) return true;
		}
		return false;
	}
}
