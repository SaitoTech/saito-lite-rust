	...
	attachEvents(app, mod) {
		...
		app.browser.addDragAndDropFileUploadToElement(
			...
			function (fileres) {
				...
				document.querySelector('.submit-file-btn-box').style.display =
					'block';
			},
			true
		);
		...
		document.getElementById('appstore-publish').onchange = async function (
			e
		) {
		...
		document.getElementById('appstore-publish-form').onsubmit = async (
			e
		) => {
			e.preventDefault();
			...
			if (mod.data.publish.zip) {
				...
				mod.uploading_application_id = app.crypto.hash(
					newtx.timestamp + '-' + newtx.signature
				);
