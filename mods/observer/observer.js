...
class Observer extends ModTemplate {
  ...
  async renderControls(app, game_mod) {
    if (!this.controls) {
      this.controls = new GameObserver(app);
    }
    await this.controls.render(app, game_mod);
  }
      ...
      async (res) => {
        if (res.rows) {
          for (const row of res.rows) {
            ...
            await this.updateGame(row.game_id);
          }
        }
      }
    );
  ...
  async notifyPeers(app, tx) {
    ...
    for (let i = 0; i < peers.length; i++) {
      if (peers[i].synctype == "lite") {
        ...
        message.data = {};
        message.data.tx = tx;
        await app.network.sendRequestAsTransaction(
          message.request,
          message.data,
          null,
          peers[i].peerIndex
    ...
    try {
      if (conf == 0) {
        if (txmsg.game_id != "") {
          ...
          await this.saveGameState(blk, tx, conf, app);
  ...
  async handlePeerTransaction(app, tx = null, peer, mycallback = null) {
    ...
    if (message.request === "observer spv update") {
      ...
      if (txmsg.game_id != "") {
        ...
        await this.saveGameState(null, tx, 0, app);
        if (txmsg.request == "observer") {
          this.addGameToObserverList(this.createGameFromTX(tx));
        } else {
          this.updateGameOnObserverList(txmsg);
  ...
  async observeGame(game_id, watch_live) {
    let msgobj = null;
    ...
    await this.sendFollowTx(msgobj);

    await this.initializeObserverMode(game_id, watch_live);
  }
  ...
  async sendFollowTx(game) {
    ...
    tx = await this.app.wallet.signTransaction(tx);
