    ...
    if (document.querySelector(".toggle-notifications")) {
      document.querySelector(".toggle-notifications").onclick = (e) => {
        if (this.mod.enable_notifications) {
          ...
          siteMessage("System Notifications turned off for Chat Messages", 3000)
        } else {
          Notification.requestPermission().then((result) => {
            if (result === "granted") {
              ...
              this.app.storage.saveOptions();
              siteMessage("System Notifications granted for Chat Messages", 3000)
            }else{
              siteMessage("Error enabling System Notifications", 3000)
            }
          });
        }
      };
