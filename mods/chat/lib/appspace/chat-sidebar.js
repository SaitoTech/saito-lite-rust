  ...
  render(chat = null) {
    ...
    if (document.getElementById('chat-sidebar')) {
      ...
      this.app.browser.addElementToSelector(
        ChatSidebarTemplate(this.app, this.mod),
        this.container
      );
    }
    ...
    if (chat.member_ids) {
      ...
      for (let m of chat.members) {
        ...
        if (chat.member_ids[m] == 'admin') {
          user.icon = `<i class="saito-overlaid-icon fa-solid fa-key"></i>`;
        }

        this.profile.menu.members.push(user);
      }
    } else if (
      chat.id == this.mod.communityGroup?.id ||
    ...
    this.app.browser.addElementToSelector(
      `<div id="chat-group-edit" class="saito-modal-menu-option"><i class="fa-solid fa-users-gear"></i><div>Manage Chat</div></div>`,
      '.chat-sidebar .saito-profile-controls'
    );
  ...
  attachEvents(chat) {
    ...
    Array.from(document.querySelectorAll('.remove_user')).forEach((member) => {
      member.onclick = async (e) => {
        ...
        if (person) {
          await this.mod.sendRemoveMemberTransaction(chat, person);
