...
module.exports = ChatBox = {
  render(app, mod) {
    ...
    document.querySelectorAll(".chat-box").forEach((box) => {
      ...
      for (let i = 0; i < message_blocks.length; i++) {
        let html = ChatBoxMessageBlockTemplate(
          app,
          mod,
          mod.groups[idx],
          message_blocks[i]
        ...
        if (i == message_blocks.length - 1) {
          ...
          if (message_blocks[i].length > 0) {
            first_comment_sig = app.crypto.hash(
              message_blocks[i][0].transaction.sig
            );
          }
          ...
          try {
            document
              .getElementById(`chat-message-set-${first_comment_sig}`)
              .destroy();
          } catch (err) {}
          html = ChatBoxMessageBlockTemplate(
            app,
            mod,
            mod.groups[idx],
            message_blocks[i]
  ...
  attachEvents(app, mod) {
    ...
    document.querySelectorAll(".chat-box").forEach((box) => {
      let group_id = box.getAttribute("data-id");
      let group = null;
      let msg_input = document.getElementById(
        `chat-box-new-message-input-${group_id}`
      );
      ...
      msg_input.addEventListener("keydown", (e) => {
        if ((e.which == 13 || e.keyCode == 13) && !e.shiftKey) {
          ...
          mod.sendMessage(app, newtx);
          mod.receiveMessage(app, newtx);
          chat_self.removeDefaultMessage(group_id);
          //chat_self.addMessage(app, mod, newtx);
          msg_input.value = "";
        }
        try{
    ...
    //
    // toggle chat window height -- TODO: change to minimize button
    //
    document.querySelectorAll(".chat-box-header").forEach((hdr) => {
      hdr.ontouch = (e) => {
        ...
        let chat_box_header = document.getElementById(
    ...
    for (let i = 0; i < mod.groups.length; i++) {
      if (document.getElementById(`chat-box-${mod.groups[i].id}`)) {
        ...
        pixen_consumed += document
          .getElementById(`chat-box-${mod.groups[i].id}`)
  ...
  addMessageToDOM(app, mod, msg) {
    try {
      if (document.getElementById(msg.sig)) {
        return;
      }
      ...
      let chat_box_main = document.getElementById(
        `chat-box-main-${message.group_id}`
      );

      console.log("MBL: " + this.message_blocks.length);

      if (this.message_blocks.length > 0) {
        let last_message_block = Object.assign(
          { messages: [] },
          this.message_blocks[this.message_blocks.length - 1]
        );
        let last_message = Object.assign(
          {},
          last_message_block.messages[last_message_block.messages.length - 1]
        );
        ...
        if (last_message.publickey == message.publickey) {
          last_message_block = Object.assign({}, last_message_block, {
            ...
            type:
              app.wallet.returnPublicKey() == message.publickey
                ? "myself"
                : "others",
          });
          chat_box_main.removeChild(chat_box_main.lastElementChild);
          chat_box_main.innerHTML += ChatBoxMessageBlockTemplate(
            last_message_block,
            mod
          );
          this.message_blocks[this.message_blocks.length - 1] =
  ...
  createMessageBlocks(app, mod, group) {
    ...
    while (idx < txs.length) {
      if (blocks.length == 0) {
        if (
          txs[idx].transaction.from[0].add != last_message_sender &&
          last_message_sender != ""
        ) {
          blocks.push(block);
        }
  ...
  removeDefaultMessage(group_id) {
    let default_message = document.getElementById(
      `chat-box-default-message-${group_id}`
    );
    if (default_message)
