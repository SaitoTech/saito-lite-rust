...
class ATR extends ModTemplate {
    ...
    async getBlockData(blk, type){
        ...
        if (type == 'old') {
            ...
            let data = (await S.getInstance().getBalanceSnapshot([])).toString();
    ...
    webServer(app, expressapp, express) {
        ...
        expressapp.get('/atr/cv/latest', async (req, res) => {
            try {
                let last_blk_hash = await this.app.blockchain.getLastBlockHash();
                const blk = await app.blockchain.getBlock(last_blk_hash);
                if (blk == null) {
                    let html_to_return = JSON.stringify({});
                    res.writeHead(200, {
                        'Content-Type': 'text/plain',
                ...
                let atr_obj = await atr_self.getBlockData(blk, 'server');
