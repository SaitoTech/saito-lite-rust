...
class Midnight extends OnePlayerGameTemplate {
  ...
  displayPlayer(){
    ...
    $(".potion").on("click", async ()=>{
      let c = await sconfirm(`Use your ${midnight_self.game.state.potion.name}?`);
      if (c){
  ...
  filterChoices(page_obj){
    ...
    if (page_obj.filter){
      ...
      if (page_obj.filter.field == "stats"){
        success = this.game.state[page_obj.filter.value] >= page_obj.filter.min;
    ...
    if (page_obj.choices?.length > 0){
      for (let i = 0; i < page_obj.choices.length; i++){
        if (apply_filter){
          if (page_obj.choices[i].filter !== undefined && page_obj.choices[i].filter !== success){
        ...
        if (page_obj.choices[i].gold){
          page_obj.choices[i].skipme = this.game.state.gold < page_obj.choices[i].gold;
        }

        if (page_obj.choices[i].event){
          if (this.game.state.events.includes(page_obj.choices[i].event)){
           page_obj.choices[i].skipme = true; 
          }
        }

        if (page_obj.choices[i].key !== undefined){
          if (page_obj.choices[i].key == "R" || page_obj.choices[i].key == "L"){
            page_obj.choices[i].skipme = !this.game.state.keys.includes(page_obj.choices[i].key);
          }else{
            page_obj.choices[i].skipme = page_obj.choices[i].key !== this.game.state.keys.length;
          }
        }
      }
    }
  ...
  simulCombat(params) {
    ...
    const combat = (target) =>{
      ...
      for (let i = 0; i < enemies.length; i++){
        let enemy = enemies[i];
        let myAttack = this.rollDice(6) + this.rollDice(6) + this.game.state.skill;
        let enemyAttack = this.rollDice(6) + this.rollDice(6) + enemy.skill;
  ...
  handleCombat(params) {
    ...
    const combat = () =>{
      ...
      let enemyAttack = this.rollDice(6) + this.rollDice(6) + enemy.skill + enemy_attack_modifier;
      rounds++;       
      ...
      if (myAttack >= enemyAttack){
        ...
        if (enemy.curse){
          for (let i in enemy.curse){
            this.game.state[i] = Math.max(0, this.game.state[i] + enemy.curse[i]);
          }
        }
      }
      combatHTML += "</div>";
  ...
  */
  roll4Damage(die, victory_page, success_message){
    ...
    $(".textchoice").on("click", function(){
      ...
      let action = $(this).attr("id");
      if (action == "luck"){
       let luck_roll = midnight_self.rollDice(6) + midnight_self.rollDice(6);
  ...
  returnState() {
    ...
    state.inventory = ["shortsword", "provisions", "potion", "lamp", "torch", "tinderbox"];
