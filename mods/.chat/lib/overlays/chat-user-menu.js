...
class ChatUserMenu {
	...
	attachEvents() {
		...
		if (
			this.chat_group.members.length == 2 &&
			!this.chat_group?.member_ids
		) {
			...
			document.getElementById('rename').onclick = async (e) => {
				...
				if (name) {
					...
					if (
						...
						thisobj.mod.sendUpdateGroupTransaction(
							thisobj.chat_group
						);
					}
		...
		if (document.getElementById('admin')) {
			document.getElementById('admin').onclick = (e) => {
				...
				contactList.render(
					thisobj.chat_group.members.filter(
						(x) => thisobj.chat_group.member_ids[x] == 1
					)
				);
			};
		}
		...
		if (document.getElementById('view')) {
			document.getElementById('view').onclick = async (e) => {
				...
				overlay.show(
					`<div class="chat-member-list-overlay saito-modal">${MemberList(
						this.app,
						this.mod,
						thisobj.chat_group
					)}</div>`
