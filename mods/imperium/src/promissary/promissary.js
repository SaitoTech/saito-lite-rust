this.importPromissary('ceasefire', {
	...
	activateSystemTriggers: function (imperium_self, attacker, player, sector) {
		let promissary_name =
			imperium_self.game.state.players_info[attacker - 1].faction +
			'-' +
			'ceasefire';
		...
		if (imperium_self.game.player != player) {
			imperium_self.updateStatus(
				imperium_self.returnFaction(player) +
					' is deciding whether to use Ceasefire'
			);
			return 0;
		}

		let html =
			'<div clss="sf-readable">Permit ' +
			imperium_self.returnFaction(attacker) +
			' to activate sector or use ceasefire? </div><ul>';
	...
	handleGameLoop: function (imperium_self, qe, mv) {
		if (mv[0] == 'ceasefire') {
			...
			for (let i = imperium_self.game.queue.length - 1; i >= 0; i--) {
				if (
					imperium_self.game.queue[i].indexOf('play') != 0 &&
					terminated_play == 0
				) {
...
this.importPromissary('trade', {
	...
	gainCommodities: function (imperium_self, player, amount) {
		let promissary_name =
			imperium_self.game.state.players_info[player - 1].faction +
			'-' +
			'trade';
				...
				if (
					...
					imperium_self.updateLog(
						imperium_self.returnFaction(i + 1) +
							' redeems their Trade Promissary from ' +
							imperium_self.returnFaction(pprom)
					);
					return 0;
				}
			}
...
this.importPromissary('political', {
	...
	menuOption: function (imperium_self, menu, player) {
		...
		if (menu == 'pre_agenda') {
			...
			x.html =
				'<li class="option" id="political-promissary">Political Promissary</li>';
		}
		return x;
	},
	menuOptionTriggers: function (imperium_self, menu, player) {
		...
		let playable_promissaries = imperium_self.returnPlayablePromissaryArray(
			player,
			'political'
		);
		for (let i = 0; i < playable_promissaries.length; i++) {
			if (
				...
				].promissary_notes.includes(playable_promissaries[i])
			) {
				return 1;
			}
		}
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (imperium_self.game.player == player) {
			let html =
				'<div class="sf-readable">Select a Specific Promissary: </div><ul>';
			let playable_promissaries =
				imperium_self.returnPlayablePromissaryArray(
					player,
					'political'
				);
			for (let i = 0; i < playable_promissaries.length; i++) {
				let tmpar = playable_promissaries[i].split('-');
				let pprom = imperium_self.returnPromissaryPlayer(
					playable_promissaries[i]
				);
				html += `<li class="option" id="${i}">${imperium_self.returnFactionName(
					pprom
				)} - ${imperium_self.promissary_notes[tmpar[1]].name}</li>`;
			}
			html += '</ul>';
			...
			$('.option').on('click', function () {
				...
				let pprom = imperium_self.returnPromissaryPlayer(
					playable_promissaries[parseInt(i)]
				);

				imperium_self.addMove(
					'rider\t' + pprom + '\tpolitical-promissary\t-1'
				);
				imperium_self.addMove(
					'give\t' +
...
this.importPromissary('throne', {
	...
	gainPromissary: function (imperium_self, gainer, promissary) {
		if (promissary.indexOf('throne') > -1) {
			...
			if (pprom != gainer) {
				imperium_self.game.state.players_info[gainer - 1][
					promissary
				] = 1;
				imperium_self.game.state.players_info[gainer - 1].vp++;
				imperium_self.updateLog(
					imperium_self.returnFaction(gainer) +
						' gains 1 VP from Support for the Throne'
				);
	...
	losePromissary: function (imperium_self, loser, promissary) {
		if (promissary.indexOf('throne') > -1) {
			let pprom = imperium_self.returnPromissaryPlayer(promissary);
			if (pprom != loser) {
				imperium_self.game.state.players_info[loser - 1][
					promissary
				] = 1;
				imperium_self.game.state.players_info[loser - 1].vp--;
				imperium_self.updateLog(
					imperium_self.returnFaction(loser) +
						' loses 1 VP from Support for the Throne'
				);
				imperium_self.updateLeaderboard();
			}
		}
		...
		for (let i = 0; i < imperium_self.game.state.players_info.length; i++) {
			if (i + 1 != activating_player) {
				if (imperium_self.doesPlayerHaveUnitsInSector(i + 1, sector)) {
					let faction_promissary =
						imperium_self.game.state.players_info[player - 1].id +
						'-' +
						'throne';
					if (
						imperium_self.doesPlayerHavePromissary(
						...
						imperium_self.game.state.players_info[
							activating_player - 1
						][faction_promissary] = 0;
						imperium_self.updateLog(
							imperium_self.returnFaction(activating_player) +
								' loses 1 VP from Support for the Throne'
						);
						imperium_self.game.state.players_info[
							activating_player - 1
