	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		...
		if (player == action_card_player) {
			...
			for (let i = 0; i < sys.s.units[attacker - 1].length; i++) {
				while (
					imperium_self.returnRemainingCapacity(
						sys.s.units[attacker - 1][i]
					) > 0 &&
					attacker_infantry.length > 0
				) {
					imperium_self.loadUnitByJSONOntoShip(
						attacker,
						sector,
						i,
						JSON.stringify(attacker_infantry[0])
					);
