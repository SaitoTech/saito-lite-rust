this.importActionCard('leadership-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			let choices =
				imperium_self.agenda_cards[active_agenda].returnAgendaOptions(
					imperium_self
				);
			let elect = imperium_self.agenda_cards[active_agenda].elect;
			let msg =
				'On which choice do you wish to place your Leadership rider?';
			imperium_self.playerSelectChoice(
				msg,
				choices,
				elect,
				function (choice) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							imperium_self.returnFactionNickname(
								imperium_self.game.player
							) +
							' has placed a Leadership Rider on ' +
							choices[choice]
					);
	...
	playActionCardEvent: function (
		...
		imperium_self.updateLog(
			imperium_self.returnFaction(action_card_player) +
				' gains 2 strategy tokens and 1 command token'
		);
		return 1;
	}
});
...
this.importActionCard('diplomacy-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			let msg =
				'On which choice do you wish to place your Diplomacy rider?';
			imperium_self.playerSelectChoice(
				msg,
				choices,
				elect,
				function (choice) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' has placed a Diplomacy Rider on ' +
							choices[choice]
					);
					imperium_self.endTurn();
				}
		...
		if (action_card_player == imperium_self.game.player) {
			imperium_self.playerSelectSectorWithFilter(
				...
				function (sector) {
					for (
						let i = 0;
						i < imperium_self.game.sectors[sector].planets.length;
						i++
					) {
						if (
							imperium_self.game.planets[
								imperium_self.game.sectors[sector].planets[i]
							].owner == imperium_self.game.player
						) {
							return 1;
						}
						return 0;
				...
				function (sector) {
					for (
						let b = 0;
						b < imperium_self.game.state.players_info.length;
						b++
					) {
						imperium_self.addMove(
							'activate\t' + (b + 1) + '\t' + sector
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' uses Diplomacy Rider to protect ' +
							sector
					);
					imperium_self.endTurn();
					return 0;
...
this.importActionCard('politics-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			let elect = imperium_self.agenda_cards[active_agenda].elect;
			let msg =
				'On which choice do you wish to place your Politics rider?';
			imperium_self.playerSelectChoice(
				msg,
				choices,
				elect,
				function (choice) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' has placed a Politics Rider on ' +
							choices[choice]
					);
					imperium_self.endTurn();
				}
	...
	playActionCardEvent: function (
		...
		if (imperium_self.game.player == action_card_player) {
			// three action cards
			imperium_self.addMove(
				'gain\t' + imperium_self.game.player + '\taction_cards\t3'
			);
			imperium_self.addMove(
				'DEAL\t2\t' + imperium_self.game.player + '\t3'
			);
			imperium_self.addMove(
				'NOTIFY\tdealing two action cards to ' +
					imperium_self.returnFactionNickname(
						imperium_self.game.player
					)
			);
...
this.importActionCard('construction-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (action_card_player == imperium_self.game.player) {
			...
			let msg =
				'On which choice do you wish to place the Construction rider?';
			imperium_self.playerSelectChoice(
				msg,
				choices,
				elect,
				function (choice) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' has placed a Construction Rider on ' +
							choices[choice]
					);
					imperium_self.endTurn();
				}
			...
			imperium_self.playerSelectPlanetWithFilter(
				...
				function (planet) {
					if (
						imperium_self.game.planets[planet].owner ==
							imperium_self.game.player &&
						imperium_self.doesPlanetHaveSpaceDock(planet) == 0
					) {
						return 1;
					}
					return 0;
				...
				function (planet) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							imperium_self.returnFaction(action_card_player) +
							' builds a Space Dock in ' +
							imperium_self.game.sectors[
								imperium_self.game.planets[planet].sector
							].name
					);
					imperium_self.endTurn();
...
this.importActionCard('trade-rider', {
	...
	playActionCardEvent(imperium_self, player, action_card_player, card) {
		imperium_self.game.queue.push(
			'purchase\t' + action_card_player + '\t' + 'goods' + '\t' + 5
		);
		imperium_self.game.queue.push(
			'NOTIFY\t' +
				imperium_self.returnFaction(action_card_player) +
				' gains 5 Trade Goods through their Trade Rider'
		);
		return 1;
	}
});
...
this.importActionCard('warfare-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			let msg =
				'On which choice do you wish to place your Warfare Rider?';
			let choices =
				imperium_self.agenda_cards[active_agenda].returnAgendaOptions(
					imperium_self
				);
	...
	playActionCardEvent: function (
		...
		if (imperium_self.game.player == action_card_player) {
			imperium_self.playerSelectSectorWithFilter(
				...
				function (sector) {
					return imperium_self.doesSectorContainPlayerShips(
						action_card_player,
						sector
					);
				},
				function (sector) {
					...
					imperium_self.addMove(
						'NOTIFY\tAdding dreadnaught to board'
					);
					imperium_self.endTurn();
					return 0;
				},
...
this.importActionCard('technology-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			let msg =
				'On which choice do you wish to place your Technology rider?';
			let choices =
				imperium_self.agenda_cards[active_agenda].returnAgendaOptions(
					imperium_self
				);
				...
				function (choice) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' has placed a Technology Rider on ' +
							choices[choice]
					);
					imperium_self.endTurn();
				}
...
this.importActionCard('imperial-rider', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			let msg =
				'On which choice do you wish to place the Imperial rider?';
			imperium_self.playerSelectChoice(
				msg,
				choices,
				elect,
				function (choice) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' has placed an Imperial Rider on ' +
							choices[choice]
					);
					imperium_self.endTurn();
				}
	...
	playActionCardEvent: function (
		...
		imperium_self.game.state.players_info[
			action_card_player - 1
		].objectives_scored.push('imperial-rider');
		return 1;
	}
});
