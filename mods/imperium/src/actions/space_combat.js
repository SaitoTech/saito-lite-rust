this.importActionCard('intercept', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		imperium_self.game.players[
			action_card_player - 1
		].temporary_opponent_cannot_retreat = 1;
		return 1;
	}
});
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (
			imperium_self.game.state.players_info[action_card_player - 1]
				.my_units_destroyed_last_combat_round.length > 0
		) {
			...
			for (
				...
				i <
				imperium_self.game.state.players_info[action_card_player - 1]
					.my_units_destroyed_last_combat_round[i];
				i++
			) {
				let unittype =
				...
				if (unit.combat < lowest_combat_roll_ship) {
					lowest_combat_roll_ship = unit.combat;
				}
			}
			...
			if (counterparty == player) {
				counterparty = imperium_self.game.state.space_combat_defender;
			}
...
this.importActionCard('salvage', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (player == action_card_player) {
			let a =
				imperium_self.game.state.players_info[
					imperium_self.game.state.space_combat_attacker
				];
			let d =
				imperium_self.game.state.players_info[
					imperium_self.game.state.space_combat_defender
				];
			...
			if (d.commodities > 0) {
				...
				imperium_self.updateLog(
					imperium_self.returnFaction(
						...
						' takes ' +
						d.commodities +
						' in trade goods from commodities lost in combat'
				);
				d.commodities = 0;
			}
...
this.importActionCard('emergency-repairs', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		...
		for (let i = 0; i < sys.s.units[action_card_player - 1].length; i++) {
			if (
				sys.s.units[action_card_player - 1][i].strength <
				sys.s.units[action_card_player - 1][i].max_strength
			) {
				sys.s.units[action_card_player - 1][i].strength =
					sys.s.units[action_card_player - 1][i].max_strength;
			}
...
this.importActionCard('moral-boost', {
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.state.space_combat_sector) {
			...
			for (
				let i = 0;
				i < sys.s.units[action_card_player - 1].length;
				i++
			) {
		...
		if (imperium_self.game.state.ground_combat_sector) {
			if (imperium_self.game.state.ground_combat_planet_idx) {
				...
				for (let p = 0; i < sys.p.length; p++) {
					for (
						let i = 0;
						i < sys.p[p].units[action_card_player - 1].length;
						i++
					) {
		...
		for (let n = 0; n < adjacent_sectors.length; n++) {
			...
			for (let p = 0; p < sys.p.length; p++) {
				if (sys.p[p].owner == imperium_self.game.player) {
					if (
						imperium_self.doesPlayerHaveSpaceDockOnPlanet(
						...
						imperium_self.game.state.players_info[
							action_card_player - 1
						].experimental_battlestation = sector;
						return 1;
					}
				}
			}
	...
	playActionCard: function (imperium_self, player, action_card_player, card) {
		if (imperium_self.game.player == action_card_player) {
			...
			imperium_self.playerSelectSectorWithFilter(
				...
				function (s) {
					if (
						imperium_self.areSectorsAdjacent(sector, s) &&
						s != sector
					) {
						if (!imperium_self.doesSectorContainNonPlayerShips(s)) {
							return 1;
						}
					}
					return 0;
				...
				function (s) {
					...
					imperium_self.addMove(
						'NOTIFY\t' +
							...
							' makes skilled retreat into ' +
							imperium_self.game.sectors[s].name
					);
					imperium_self.addMove(
						'activate\t' + action_card_player + '\t' + s
					);
					imperium_self.addMove(
						'activate\t' +
							action_card_player +
				...
				function () {
					imperium_self.addMove(
						'NOTIFY\tno suitable sectors available for skilled retreat'
					);
					imperium_self.endTurn();
				}
			);
