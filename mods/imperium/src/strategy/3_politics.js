this.importStrategyCard('politics', {
	...
	strategyPrimaryEvent: function (
		...
		if (
			...
			imperium_self.addMove(
				'DEAL\t2\t' + imperium_self.game.player + '\t2'
			);
			imperium_self.addMove(
				'NOTIFY\t' +
			...
			imperium_self.addMove(
				'strategy\t' + 'politics' + '\t' + strategy_card_player + '\t2'
			);
			imperium_self.addMove(
				'resetconfirmsneeded\t' +
	...
	strategySecondaryEvent: function (
		imperium_self,
		player,
		strategy_card_player
	) {
		if (imperium_self.game.player == player) {
			if (
				...
				imperium_self.addMove(
					'resolve\tstrategy\t1\t' + this.publicKey
				);
	...
	strategyTertiaryEvent: function (
		imperium_self,
		player,
		strategy_card_player
	) {
		...
		if (imperium_self.game.player === imperium_self.game.state.speaker) {
			...
			if (imperium_self.game.state.agendas_per_round == 1) {
				html +=
					'Select one agenda to advance for consideration in the Galactic Senate.<ul>';
			}
			if (imperium_self.game.state.agendas_per_round == 2) {
				html +=
					'Select two agendas to advance for consideration in the Galactic Senate.<ul>';
			}
			if (imperium_self.game.state.agendas_per_round == 3) {
				html +=
					'Select three agendas to advance for consideration in the Galactic Senate.<ul>';
			}

			for (
				i = 0;
				i < 3 && i < imperium_self.game.state.agendas.length;
				i++
			) {
				html +=
					'<li class="option" id="' +
			...
			let card_selection_function = function (cardkey) {
				...
				if (
					laws_selected >= imperium_self.game.state.agendas_per_round
				) {
					$(this).hide();
					imperium_self.hideAgendaCard(
						selected_agendas[selected_agendas.length - 1]
					);
					imperium_self.agenda_selection_overlay.hide();

					for (i = 1; i >= 0; i--) {
						if (
							imperium_self.game.state.agenda_voting_order ===
							'simultaneous'
						) {
							imperium_self.addMove(
								'resolve_agenda\t' + selected_agendas[i]
							);
							imperium_self.addMove(
								'post_agenda_stage_post\t' + selected_agendas[i]
							);
							imperium_self.addMove(
								'post_agenda_stage\t' + selected_agendas[i]
							);
							imperium_self.addMove(
								'simultaneous_agenda\t' +
									selected_agendas[i] +
							...
							imperium_self.addMove(
								'pre_agenda_stage_post\t' + selected_agendas[i]
							);
							imperium_self.addMove(
								'pre_agenda_stage\t' + selected_agendas[i]
							);
							imperium_self.addMove(
								'resetconfirmsneeded\t' +
									imperium_self.game.state.players_info.length
			...
			console.log(
				'SELECTED AGENDAS: ' + JSON.stringify(selected_agendas)
			);
			console.log(
				'SELECTEABLE AGENDAS: ' +
					JSON.stringify(imperium_self.game.state.agendas)
			);
			...
			imperium_self.updateStatus(
				'Speaker selecting Agendas for consideration by Senate'
			);
		}
	}
});
