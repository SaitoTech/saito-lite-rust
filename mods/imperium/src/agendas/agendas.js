this.importAgendaCard('homeland-defense-act', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (winning_choice === 'against') {
			for (
				let i = 0;
				i < imperium_self.game.state.players_info.length;
				i++
			) {
		...
		imperium_self.game.state.laws.push({
			agenda: 'homeland-defense-act',
			option: winning_choice
		});
	},
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda ===
...
this.importAgendaCard('new-constitution', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		for (let i = imperium_self.game.state.laws.length - 1; i >= 0; i--) {
			...
			imperium_self.agenda_cards[saved_agenda].repealAgenda(
				imperium_self
			);
		}
		...
		if (winning_choice === 'for') {
			...
			for (
				...
				imperium_self.game.state.players_info[
					i
				].must_exhaust_at_round_start.push('homeworld');
			}
		}
	...
	initialize: function (imperium_self, winning_choice) {
		if (imperium_self.space_cadet_initialized == undefined) {
			...
			if (imperium_self.game.state.space_cadet == 1) {
				imperium_self.returnFactionNamePreSpaceCadet =
					imperium_self.returnFactionName;
				imperium_self.returnFactionName = function (
					imperium_self,
					player
				) {
					let max_vp = 0;
					for (
						...
						if (
							max_vp < imperium_self.game.state.players_info[i].vp
						) {
							max_vp =
								imperium_self.game.state.players_info[i].vp;
						}
					}
					if (
						imperium_self.game.state.players_info[player - 1].vp <=
						max_vp - 3
					) {
						return 'Irrelevant Loser';
					}
					return imperium_self.returnFactionNamePreSpaceCadet(
						imperium_self,
						player
					);
				};
				imperium_self.returnFactionNameNicknamePreSpaceCadet =
					imperium_self.returnFactionNameNickname;
				imperium_self.returnFactionNameNickname = function (
					imperium_self,
					player
				) {
					let max_vp = 0;
					...
					return imperium_self.returnFactionNameNicknamePreSpaceCadet(
						imperium_self,
						player
					);
				};
	...
	initialize: function (imperium_self, winning_choice) {
		if (imperium_self.galactic_threat_initialized == undefined) {
			...
			if (imperium_self.game.state.galactic_threat == 1) {
				imperium_self.returnFactionNamePreGalacticThreat =
					imperium_self.returnFactionName;
				imperium_self.returnFactionName = function (
					imperium_self,
					player
					...
					if (
						imperium_self.game.state.galactic_threat_player ==
						player
					) {
						return 'The Galactic Threat';
					}
					return imperium_self.returnFactionNamePreGalacticThreat(
						imperium_self,
						player
					);
				};
				imperium_self.returnFactionNameNicknamePreGalacticThreat =
					imperium_self.returnFactionNameNickname;
				imperium_self.returnFactionNameNickname = function (
					imperium_self,
					player
					...
					return imperium_self.returnFactionNameNicknamePreGalacticThreat(
						imperium_self,
						player
					);
				};
...
this.importAgendaCard('minister-of-policy', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		].action_cards_bonus_when_issued++;
		let law_to_push = {};
	...
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda === 'minister-of-policy'
			) {
				winning_choice = imperium_self.game.state.laws[i].option;
				imperium_self.game.state.laws.splice(i, 1);
				i--;
...
this.importAgendaCard('executive-sanctions', {
	...
	onPass: function (imperium_self, winning_choice) {
		if (
			this.returnAgendaOptions(imperium_self)[winning_choice] == 'support'
		) {
			for (
				let i = 0;
				i < imperium_self.game.state.players_info.length;
				...
				imperium_self.game.state.players_info[i].action_card_limit = 3;
			}
		}
		let law_to_push = {};
	...
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.players_info.length; i++) {
			if (
				...
				imperium_self.game.state.players_info[i].action_card_limit = 7;
			}
		}
...
this.importAgendaCard('fleet-limitations', {
	...
	onPass: function (imperium_self, winning_choice) {
		if (
			...
			for (
				...
				imperium_self.game.state.players_info[i].fleet_supply_limit = 4;
				if (
					imperium_self.game.state.players_info[i].fleet_supply >= 4
				) {
					imperium_self.game.state.players_info[i].fleet_supply = 4;
				}
			}
	...
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda === 'fleet-limitations'
			) {
				winning_choice = imperium_self.game.state.laws[i].option;
				imperium_self.game.state.laws.splice(i, 1);
				i--;
		...
		for (let i = 0; i < imperium_self.game.state.players_info.length; i++) {
			imperium_self.game.state.players_info[i].fleet_supply_limit = 16;
		}
...
this.importAgendaCard('committee-formation', {
	...
	preAgendaStageTriggers: function (imperium_self, player, agenda) {
		if (
			...
			imperium_self.game.state.committee_formation_player == player
		) {
	...
	preAgendaStageEvent: function (imperium_self, player, agenda) {
		if (player != imperium_self.game.player) {
			let html =
				...
				' is deciding whether to Form a Committee';
			imperium_self.updateStatus(html);
			return 0;
		}

		let html =
			'Do you wish to use Committee Formation to select the winner yourself? <ul>';
		...
		$('.textchoice').on('click', function () {
			...
			for (
				...
				if (i != imperium_self.game.player - 1) {
					imperium_self.addMove(
						'rider\t' + (i + 1) + '\tassassinate-representative\t-1'
					);
				}
			}
			imperium_self.addMove(
...
this.importAgendaCard('shard-of-the-throne', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.state.players_info[
			imperium_self.game.state.shard_of_the_throne_player - 1
		].vp += 1;
		imperium_self.updateLeaderboard();
		imperium_self.updateLog(
			imperium_self.returnFaction(
				imperium_self.game.state.shard_of_the_throne_player
			) + ' gains the Shard of the Throne (1VP)'
		);
	},
	repealAgenda(imperium_self) {
		//
	...
	spaceCombatRoundEnd: function (imperium_self, attacker, defender, sector) {
		if (defender == imperium_self.game.state.shard_of_the_throne_player) {
			if (!imperium_self.doesPlayerHaveShipsInSector(defender, sector)) {
				...
				imperium_self.game.state.players_info[attacker - 1].vp += 1;
				imperium_self.game.state.players_info[defender - 1].vp -= 1;
				imperium_self.updateLeaderboard();
			}
		}
	},
	groundCombatRoundEnd: function (
		imperium_self,
		attacker,
		defender,
		sector,
		...
		if (defender == imperium_self.game.state.shard_of_the_throne_player) {
			if (
				!imperium_self.doesPlayerHaveInfantryOnPlanet(
					defender,
					sector,
					planet_idx
				)
...
this.importAgendaCard('holy-planet-of-ixth', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (owner != -1) {
			imperium_self.game.state.players_info[owner - 1].vp += 1;
			imperium_self.updateLeaderboard();
			imperium_self.updateLog(
				imperium_self.returnFaction(owner) +
					' gains 1 VP from Holy Planet of Ixth'
			);
		}
	},
	repealAgenda(imperium_self) {
...
this.importAgendaCard('research-team-biotic', {
	...
	menuOption: function (imperium_self, menu, player) {
		if (
			menu == 'main' &&
			imperium_self.game.planets[
				imperium_self.game.state.research_team_biotic_planet
			].owner == player
		) {
			return {
				event: 'research_team_biotic',
				html: '<li class="option" id="research_team_biotic">use biotic (green) tech-skip</li>'
			};
		}
		return {};
	},
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (menu == 'main') {
			imperium_self.game.state.players_info[player - 1]
				.temporary_green_tech_prerequisite++;
			imperium_self.game.planets[
				imperium_self.game.state.research_team_biotic_planet
			].exhausted = 1;
		}
...
this.importAgendaCard('research-team-cybernetic', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.state.research_team_cybernetic_planet =
			winning_choice;
		let law_to_push = {};
	...
	menuOption: function (imperium_self, menu, player) {
		if (
			...
			return {
				...
				html: '<li class="option" id="research_team_cybernetic">use cybernetic (yellow) tech-skip</li>'
			};
		}
		return {};
	},
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (menu == 'main') {
			imperium_self.game.state.players_info[player - 1]
				.temporary_yellow_tech_prerequisite++;
			imperium_self.game.planets[
				imperium_self.game.state.research_team_cybernetic_planet
			].exhausted = 1;
		}
...
this.importAgendaCard('research-team-propulsion', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.state.research_team_propulsion_planet =
			winning_choice;
		let law_to_push = {};
	...
	menuOption: function (imperium_self, menu, player) {
		if (
			...
			return {
				event: 'research_team_propulsion',
				html: '<li class="option" id="research_team_propulsion">use propulsion (blue) tech-skip</li>'
			};
		}
		return {};
	},
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (menu == 'main') {
			imperium_self.game.state.players_info[player - 1]
				.temporary_blue_tech_prerequisite++;
			imperium_self.game.planets[
				imperium_self.game.state.research_team_propulsion_planet
			].exhausted = 1;
		}
...
this.importAgendaCard('research-team-warfare', {
	...
	menuOption: function (imperium_self, menu, player) {
		if (
			menu == 'main' &&
			imperium_self.game.planets[
				imperium_self.game.state.research_team_warfare_planet
			].owner == player
		) {
			return {
				event: 'research_team_warfare',
				html: '<li class="option" id="research_team_warfare">use warfare (red) tech-skip</li>'
			};
		}
		return {};
	},
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (menu == 'main') {
			imperium_self.game.state.players_info[player - 1]
				.temporary_red_tech_prerequisite++;
			imperium_self.game.planets[
				imperium_self.game.state.research_team_warfare_planet
			].exhausted = 1;
		}
...
this.importAgendaCard('demilitarized-zone', {
	...
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda === 'demilitarized-zone'
			) {
				winning_choice = imperium_self.game.state.laws[i].option;
				imperium_self.game.state.laws.splice(i, 1);
				i--;
...
this.importAgendaCard('core-mining', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		for (
			let i = 0;
			i < imperium_self.game.planets[planetidx].units.length;
			i++
		) {
			let destroy = 1;
			for (
				let ii = 0;
				ii < imperium_self.game.planets[planetidx].units[i].length;
				ii++
			) {
				if (
					imperium_self.game.planets[planetidx].units[i][ii].type ==
					'infantry'
				) {
					if (destroy == 1) {
						imperium_self.game.planets[planetidx].units[i].splice(
							ii,
							1
						);
						ii--;
...
this.importAgendaCard('anti-intellectual-revolution', {
	...
	repealAgenda(imperium_self) {
		...
		if (techcard.strategyPrimaryEventBackup) {
			techcard.strategyPrimaryEvent = techcard.strategyPrimaryEventBackup;
		}
		if (techcard.strategySecondaryEventBackup) {
			techcard.strategySecondaryEvent =
				techcard.strategySecondaryEventBackup;
		}
	...
	initialize: function (imperium_self, winning_choice) {
			...
			let new_tech_sec = function (
				...
				if (
					...
					imperium_self.playerAcknowledgeNotice(
						'Anti-Intellectual Revolution is in play. Do you wish to destroy a capital ship to research technology?',
						function () {
							imperium_self.playerSelectUnitWithFilter(
								...
								function (unit_identifier) {
									...
									let unit = unit_identifier.unit;
									let sys =
										imperium_self.returnSectorAndPlanets(
											sector
										);
		...
		if (winning_choice === 'against') {
			...
			for (
				...
				imperium_self.game.state.players_info[
					i
				].must_exhaust_at_round_start.push('planet', 'planet');
			}
		}
	},
	onPass: function (imperium_self, winning_choice) {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (winning_choice === 'for') {
			for (
				...
				if (
					imperium_self.game.state.choices[
						imperium_self.game.state.how_voted_on_agenda[i]
					] == winning_choice
				) {
					imperium_self.game.queue.push(
						'gain\t2\t' + (i + 2) + '\taction_cards' + '\t' + 2
					);
					imperium_self.game.queue.push(
						'DEAL\t2\t' + (i + 1) + '\t2'
					);
					imperium_self.game.queue.push(
						'NOTIFY\tdealing two action cards to player ' + (i + 1)
					);
				}
			}
		}
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (winning_choice === 'for') {
			...
			for (let i = 0; i < io.length; i++) {
				if (
					highest_vp <
					imperium_self.game.state.players_info[io[i] - 1].vp
				) {
					highest_vp =
						imperium_self.game.state.players_info[io[i] - 1].vp;
				}
			...
			for (let i = 0; i < io.length; i++) {
				if (
					...
					imperium_self.game.queue.push(
						'NOTIFY\t' +
							imperium_self.returnFaction(io[i]) +
							' gains 1 VP from Seeds of an Empire'
					);
					imperium_self.game.state.seeds_of_an_empire = io[i];
					if (imperium_self.checkForVictory()) {
						return 0;
					}
				}
			}
		...
		if (winning_choice === 'against') {
			...
			for (let i = 0; i < io.length; i++) {
				if (
					...
					imperium_self.game.queue.push(
						'NOTIFY\t' +
							imperium_self.returnFaction(io[i] + 1) +
							' gains 1 VP from Seeds of an Empire'
					);
					imperium_self.game.state.seeds_of_an_empire = io[i];
					if (imperium_self.checkForVictory()) {
...
this.importAgendaCard('archived-secret', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.queue.push(
			'gain\t' +
				imperium_self.game.state.archived_secret_player +
				'\tsecret_objectives\t1'
		);
		imperium_self.game.queue.push(
			'DEAL\t6\t' +
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (winning_choice === 'for') {
			for (
				...
				imperium_self.game.state.players_info[i].goods = 0;
			}
		...
		if (winning_choice === 'against') {
			for (
				...
				imperium_self.game.state.players_info[i].goods = 5;
			}
		...
		if (winning_choice === 'for') {
			for (
				...
				if (
					...
					imperium_self.game.state.players_info[i].vp++;
					imperium_self.updateLog(
						imperium_self.returnFaction(i + 1) +
							' gains 1 VP from Mutiny'
					);
				}
			}
		}
...
this.importAgendaCard('conventions-of-war', {
	...
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda === 'conventions-of-war'
			) {
				winning_choice = imperium_self.game.state.laws[i].option;
				imperium_self.game.state.laws.splice(i, 1);
				i--;
		...
		if (winning_choice === 'against') {
			for (let i in imperium_self.game.planets) {
				if (imperium_self.game.planets[i].owner != -1) {
					imperium_self.game.planets[i].units[
						imperium_self.game.planets[i].owner - 1
					].push(
						imperium_self.returnUnit(
							'infantry',
							imperium_self.game.planets[i].owner
						)
					);
				}
			}
			...
			for (let k in imperium_self.game.planets) {
				if (imperium_self.game.planets[k].owner > 0) {
					...
					for (
						let m = 0;
						m <
						imperium_self.game.planets[k].units[owner - 1].length;
						m++
					) {
						if (
							imperium_self.game.planets[k].units[i][m].type ==
							'infantry'
						) {
					...
					for (
						let m = 0, n = 0;
						n < total_infantry_destroyed &&
						m < imperium_self.game.planets[k].units[i].length;
						m++
					) {
						if (
							imperium_self.game.planets[k].units[i][m].type ==
		...
		if (winning_choice === 'for') {
			for (let i in imperium_self.game.sectors) {
				if (imperium_self.game.sectors[i].wormhole != 0) {
					for (
						let k = 0;
						k < imperium_self.game.sectors[i].units.length;
						k++
					) {
			...
			for (let i = 0; i < players_to_research_tech.length; i++) {
				imperium_self.game.queue.push(
					'research\t' + players_to_research_tech[i]
				);
			}
		}
		...
		if (winning_choice === 'against') {
			for (
				...
				if (
					...
					imperium_self.game.state.players_info[i].command_tokens--;
					if (
						imperium_self.game.state.players_info[i]
							.command_tokens <= 0
					) {
						imperium_self.game.state.players_info[
							i
			...
			for (
				...
				imperium_self.game.queue.push(
					'activate\t' +
						(i + 1) +
						'\t' +
						imperium_self.returnPlayerHomeworld(i + 1)
				);
			}
		}
...
this.importAgendaCard('wormhole-reconstruction', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (winning_choice === 'against') {
			for (let i in imperium_self.game.sectors) {
				if (
					imperium_self.game.sectors[i].wormhole == 1 ||
					imperium_self.game.sectors[i].wormhole == 2
				) {
					for (
						let ii = 0;
						ii < imperium_self.game.state.players_info.length;
						ii++
					) {
...
this.importAgendaCard('crown-of-emphidia', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.updateLog(
			imperium_self.returnFaction(
				imperium_self.game.state.crown_of_emphidia_player
			) + ' gains 1 VP from Crown of Emphidia'
		);
	},
	repealAgenda(imperium_self) {
		let winning_choice = null;
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda === 'crown-of-emphidia'
			) {
				winning_choice = imperium_self.game.state.laws[i].option;
				imperium_self.game.state.laws.splice(i, 1);
				i--;
			...
			if (
				...
				if (
					...
					imperium_self.updateLog(
						imperium_self.returnFaction(
							imperium_self.game.state.crown_of_emphidia_player
						) + ' loses the Crown of Emphidia (-1VP)'
					);
					imperium_self.game.state.crown_of_emphidia_player =
						attacker;
					imperium_self.updateLog(
						imperium_self.returnFaction(
							imperium_self.game.state.crown_of_emphidia_player
						) + ' gains the Crown of Emphidia (+1VP)'
					);
					imperium_self.game.state.players_info[attacker - 1].vp += 1;
					imperium_self.game.state.players_info[defender - 1].vp -= 1;
					imperium_self.updateLeaderboard();
...
this.importAgendaCard('terraforming-initiative', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.state.terraforming_initiative_planet =
			winning_choice;
		let law_to_push = {};
		...
		imperium_self.updateLog(
			imperium_self.game.planets[winning_choice].name +
				' increases resource and influence through terraforming'
		);
...
this.importAgendaCard('senate-sanctuary', {
	...
	repealAgenda(imperium_self) {
		...
		for (let i = 0; i < imperium_self.game.state.laws.length; i++) {
			if (
				imperium_self.game.state.laws[i].agenda === 'senate-sanctuary'
			) {
				winning_choice = imperium_self.game.state.laws[i].option;
				imperium_self.game.state.laws.splice(i, 1);
				i--;
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.updateLog(
			imperium_self.game.planets[winning_choice].name +
				' increases influence value by 2'
		);
...
this.importAgendaCard('colonial-redistribution', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.state.colonial_redistribution_planet =
			winning_choice;
		imperium_self.game.queue.push(
			'colonial_redistribution\t' + winning_choice
		);

		imperium_self.game.state.laws.push({
			agenda: 'colonial-redistribution',
			option: winning_choice
		});
	...
	handleGameLoop: function (imperium_self, qe, mv) {
		if (mv[0] == 'colonial_redistribution') {
			...
			if (owner == -1) {
				owner = imperium_self.game.state.speaker;
			}
...
this.importAgendaCard('compensated-disarmament', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		imperium_self.game.state.compensated_disarmament_planet =
			winning_choice;
		...
		if (total_infantry > 0) {
			imperium_self.game.queue.push(
				'purchase\t' + owner + '\tgoods\t' + total_infantry
			);
		}
...
this.importAgendaCard('judicial-abolishment', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		if (repealed) {
			imperium_self.updateLog(
				imperium_self.agenda_cards[repealed].name + ' abolished'
			);
		}
...
this.importAgendaCard('public-execution', {
	...
	onPass: function (imperium_self, winning_choice) {
		...
		// lose action cards
		imperium_self.game.state.players_info[
			imperium_self.game.state.public_execution_player - 1
		].action_cards_in_hand = 0;
		if (
			imperium_self.game.player ==
			imperium_self.game.state.public_execution_player
		) {
			imperium_self.game.deck[1].hand = [];
		}
		...
		if (
			imperium_self.game.state.public_execution_player ==
			imperium_self.game.state.speaker
		) {
		...
		imperium_self.updateLog(
			imperium_self.returnFaction(
				imperium_self.game.state.public_execution_player
			) + ' representative publicly executed'
		);
		imperium_self.displayFactionDashboard();
	...
	onPass: function (imperium_self, winning_choice) {
		if (winning_choice == 'for') {
			...
			if (roll <= 5) {
				...
				for (let i = 0; i < as.length; i++) {
					for (
						...
						if (
							imperium_self.doesSectorContainPlayerUnits(
								ii + 1,
								as[i]
							)
						) {
			...
			if (roll >= 6) {
				...
				imperium_self.game.queue.push(
					'ACKNOWLEDGE\tThe Ixthian Artifact did not explode. All players may now research two technologies...'
				);
			}
		}
		return 1;
