this.importFaction('faction4', {
	...
	tech: [
		'faction4-unrelenting',
		'faction4-exotrireme-i',
		'faction4-flagship'
	],
	...
	onNewRound: function (imperium_self, player) {
		if (imperium_self.doesPlayerHaveTech(player, 'faction4-unrelenting')) {
			imperium_self.game.state.players_info[
				player - 1
			].faction4_unrelenting = 1;
		}
	},
	modifyCombatRoll: function (
		imperium_self,
		attacker,
		defender,
		player,
		...
		if (combat_type == 'space' || combat_type == 'ground') {
			if (
				imperium_self.doesPlayerHaveTech(
					attacker,
					'faction4-unrelenting'
				)
			) {
				imperium_self.updateLog(
					'Sardakk combat rolls +1 due to Sardakk'
				);
				roll += 1;
				if (roll > 10) {
					roll = 10;
		...
		if (combat_type == 'space') {
			...
			if (
				imperium_self.doesSectorContainPlayerUnit(
					attacker,
					imperium_self.game.state.activated_sector,
					'flagship'
				)
			) {
				imperium_self.updateLog(
					'Sardakk Flagship adds +1 to dice roll'
				);
				roll += 1;
				if (roll > 10) {
					roll = 10;
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction4_advanced_dreadnaught_i == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction4_advanced_dreadnaught_i = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		imperium_self.game.state.players_info[
			gainer - 1
		].faction4_advanced_dreadnaught_i = 1;
	},
	upgradeUnit: function (imperium_self, player, unit) {
		if (
			imperium_self.game.state.players_info[unit.owner - 1]
				.faction4_advanced_dreadnaught_i == 1 &&
			unit.type == 'dreadnaught'
		) {
			unit.cost = 4;
			unit.combat = 5;
			unit.move = 1;
			...
			unit.description =
				'The Exotrireme I is a more powerful dreadnaught not vulnerable to Direct Hit cards';
		}
...
this.importTech('faction4-exotrireme-ii', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction4_advanced_dreadnaught_ii == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction4_advanced_dreadnaught_ii = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		imperium_self.game.state.players_info[
			gainer - 1
		].faction4_advanced_dreadnaught_ii = 1;
		imperium_self.game.state.players_info[
			gainer - 1
		].faction4_advanced_dreadnaught_i = 0;
	},
	upgradeUnit: function (imperium_self, player, unit) {
		if (
			...
			unit.bombardment_combat = 4;
			unit.description =
				'The Exotrireme II is a more powerful dreadnaught not vulnerable to Direct Hit cards. It may be destroyed after a round of space combat to destroy up to two opponent ships.';
		}
		return unit;
	},
			...
			if (
				...
				if (
					...
					imperium_self.addMove(
						'faction4_exotrireme_ii_sacrifice\t' +
							attacker +
							'\t' +
							sector
					);
	...
	handleGameLoop: function (imperium_self, qe, mv) {
		if (mv[0] == 'faction4_exotrireme_ii_sacrifice') {
			...
			let opponent = imperium_self.returnOpponentInSector(
				player_to_go,
				mv[2]
			);
			...
			if (player_to_go == imperium_self.game.player) {
				if (opponent == -1) {
					imperium_self.addMove(
						'resolve\tfaction4_exotrireme_ii_sacrifice'
					);

					imperium_self.addMove(
						'NOTIFY\tNo target ships for Sardakk Exotrireme II faction ability'
					);
				...
				if (anything_to_kill == 0) {
					...
					imperium_self.addMove(
						'NOTIFY\tNo target ships for Sardakk Exotrireme II action ability'
					);
					imperium_self.endTurn();
					return 0;
				}
				...
				html =
					'<div class="sf-readable">Do you wish to sacrifice a Dreadnaught to destroy up to 2 opponent ships?</div><ul>';
				for (
					let i = 0;
					i < sys.s.units[imperium_self.game.player - 1].length;
					i++
				) {
					if (
						sys.s.units[imperium_self.game.player - 1][i].type ==
						'dreadnaught'
					) {
						html += `<li class="option" id="${i}">sacrifice ${imperium_self.returnShipInformation(
							sys.s.units[imperium_self.game.player - 1][i]
						)}</li>`;
					}
				}
				...
				$('.option').on('click', function () {
					...
					imperium_self.addMove(
						'faction4_exotrireme_ii_picktwo\t' +
							imperium_self.game.player +
							'\t' +
							mv[2]
					);
					imperium_self.addMove(
						'NOTIFY\tSardakk sacrifies Exotritreme II to destroy opponent ships'
					);
					imperium_self.addMove(
						'destroy_unit\t' +
							imperium_self.game.player +
		...
		if (mv[0] == 'faction4_exotrireme_ii_picktwo') {
			...
			if (player_to_go == imperium_self.game.player) {
				imperium_self.addMove(
					'resolve\tfaction4_exotrireme_ii_picktwo'
				);
				imperium_self.playerDestroyOpponentShips(
					player_to_go,
					2,
					mv[2]
				);
				...
				imperium_self.updateStatus(
					'Exotrireme II engaging in suicide assault'
				);
			}
...
this.importTech('faction4-particle-weave', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction4_particle_weave == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction4_particle_weave = 0;
			imperium_self.game.state.players_info[
				player - 1
			].faction4_particle_weave_opponent = 0;
			imperium_self.game.state.players_info[
				player - 1
			].faction4_particle_weave_my_forces = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'faction4-particle-weave') {
			imperium_self.game.state.players_info[
				gainer - 1
			].faction4_particle_weave = 1;
		}
	},
	groundCombatTriggers: function (imperium_self, player, sector, planet_idx) {
		if (
			imperium_self.doesPlayerHaveTech(player, 'faction4-particle-weave')
		) {
			//
			...
			imperium_self.game.state.players_info[
				player - 1
			].faction4_particle_weave_my_forces =
				planet.units[player - 1].length;
		}
		return 0;
	},

	groundCombatRoundEnd(
		imperium_self,
		attacker,
		defender,
		sector,
		...
		if (
			...
			if (
				current_forces <
				imperium_self.game.state.players_info[attacker - 1]
					.faction4_particle_weave_my_forces
			) {
				imperium_self.updateLog(
					'Sardakk Particle Weave vaporizes 1 opponent infantry...'
				);
		...
		if (
			...
			if (
				current_forces <
				imperium_self.game.state.players_info[defender - 1]
					.faction4_particle_weave_my_forces
			) {
				imperium_self.updateLog(
					'Sardakk Particle Weave vaporizes 1 opponent infantry...'
...
this.importPromissary('faction4-promissary', {
	...
	initialize: function (imperium_self, player) {
		imperium_self.game.state.players_info[
			player - 1
		].tekklar_legion_modifier = 0;
	},
	modifySpaceCombatRoll: function (imperium_self, attacker, defender, roll) {
		let tmod =
			imperium_self.game.state.players_info[attacker - 1]
				.tekklar_legion_modifier;
		if (tmod != 0) {
			roll += tmod;
			imperium_self.updateLog(
				'Tekklar Legion applies modifier on combat rolls'
			);
		}
		return roll;
	},
	...
	spaceCombatTriggers: function (imperium_self, player, sector) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.tekklar_legion_modifier == 1
		) {
			return 0;
		}
		if (imperium_self.hasUnresolvedSpaceCombat(player, sector)) {
			if (
				imperium_self.doesPlayerHavePromissary(
					player,
					'faction4-promissary'
				)
			) {
	...
	spaceCombatEvent: function (imperium_self, player, sector) {
		if (imperium_self.game.player == player) {
			let sardaak_player =
				imperium_self.returnPlayerOfFaction('faction4');
			...
			$('.option').on('click', function () {
				...
				if (id === 'yes') {
					...
					imperium_self.addMove(
						'NOTIFY' + '\t' + 'Sardaak Promissary redeemed'
					);
					imperium_self.endTurn();
				}
			});
	...
	groundCombatTriggers: function (imperium_self, player, sector, planet_idx) {
		...
		if (
			imperium_self.hasUnresolvedGroundCombat(player, sector, planet_idx)
		) {
			if (
				imperium_self.doesPlayerHavePromissary(
					player,
	...
	spaceCombatRoundEnd: function (imperium_self, attacker, defender, sector) {
		if (
			imperium_self.hasUnresolvedSpaceCombat(attacker, sector) ||
			imperium_self.hasUnresolvedSpaceCombat(defender, sector)
		) {
			imperium_self.game.state.players_info[
				attacker - 1
			].tekklar_legion_modifier = 0;
	...
	groundCombatRoundEnd: function (
		...
		if (
			imperium_self.hasUnresolvedGroundCombat(
				attacker,
				sector,
				planet_idx
			) ||
