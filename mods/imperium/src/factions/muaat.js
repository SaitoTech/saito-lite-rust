this.importFaction('faction7', {
	...
	tech: [
		'plasma-scoring',
		'faction7-star-forge',
		'faction7-gashlai-physiology',
		'faction7-advanced-warsun-i',
		'faction7-flagship'
	],
...
this.importTech('faction7-star-forge', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].star_forge ==
			undefined
		) {
	...
	menuOptionTriggers: function (imperium_self, menu, player) {
		if (
			imperium_self.doesPlayerHaveTech(player, 'faction7-star-forge') &&
			menu === 'main' &&
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (imperium_self.game.player == player) {
			...
			imperium_self.playerSelectSectorWithFilter(
				...
				function (sector) {
					return imperium_self.doesSectorContainPlayerUnit(
				...
				function (sector) {
					...
					imperium_self.addMove(
						'NOTIFY\tStar Forge adds destroyer to ' + sector
					);
					imperium_self.addMove(
						'expend\t' +
							imperium_self.game.player +
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.gashlai_physiology == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].gashlai_physiology = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'faction7-gashlai-physiology') {
			imperium_self.game.state.players_info[
				gainer - 1
			].gashlai_physiology = 1;
			imperium_self.game.state.players_info[
				gainer - 1
			].fly_through_supernovas = 1;
		}
	}
});
...
this.importTech('faction7-magmus-reactor', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].magmus_reactor ==
			undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
		...
		if (
			...
			if (give_bonus == 1) {
				imperium_self.updateLog(
					'Muatt gains 1 trade good from Magmus Reactor - producing in a sector with a Warsun or adjacent to a Supernova'
				);
...
this.importTech('faction7-advanced-warsun-i', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction7_advanced_warsun_i == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction7_advanced_warsun_i = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		imperium_self.game.state.players_info[
			gainer - 1
		].faction7_advanced_warsun_i = 1;
		imperium_self.game.state.players_info[
			gainer - 1
...
this.importTech('faction7-advanced-warsun-ii', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction7_advanced_warsun_ii == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction7_advanced_warsun_ii = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		imperium_self.game.state.players_info[
			gainer - 1
		].faction7_advanced_warsun_ii = 1;
	},
	upgradeUnit: function (imperium_self, player, unit) {
		if (
			imperium_self.game.state.players_info[unit.owner - 1]
...
this.importPromissary('faction7-promissary', {
	...
	menuOption: function (imperium_self, menu, player) {
		...
		if (menu == 'main') {
			...
			x.html =
				'<li class="option" id="faction7-promissary">Fires of the Gashlai (Muaat Promissary)</li>';
		}
		return x;
	},
	menuOptionTriggers: function (imperium_self, menu, player) {
		...
		if (imperium_self.game.player == player) {
			...
			imperium_self.addMove(
				'NOTIFY\t' +
					...
					' redeems Fires of the Gashlai (Muaat Promissary) from ' +
