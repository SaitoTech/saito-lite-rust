...
this.importTech('faction5-indoctrination', {
	...
	groundCombatTriggers: function (imperium_self, player, sector, planet_idx) {
		if (
			imperium_self.doesPlayerHaveTech(player, 'faction5-indoctrination')
		) {
	...
	groundCombatEvent: function (imperium_self, player, sector, planet_idx) {
		if (imperium_self.game.player == player) {
			let sys = imperium_self.returnSectorAndPlanets(sector);
			if (sys.p[planet_idx].units[player - 1].length > 0) {
				imperium_self.playIndoctrination(
					imperium_self,
					player,
					sector,
					planet_idx,
					function (imperium_self) {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction5_yin_spinner == null
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction5_yin_spinner = 0;
		}
	},
	...
	playerEndTurnEvent: function (imperium_self, player) {
		...
		imperium_self.playerSelectPlanetWithFilter(
			...
			function (planet) {
				...
				if (planet.owner == imperium_self.game.player) {
					return 1;
				}
				return 0;
			},
	...
	unitDestroyed: function (imperium_self, attacker, unit) {
		if (unit.type == 'flagship') {
			if (
				...
				if (active_sector === '') {
					active_sector =
						imperium_self.game.state.space_combat_sector;
				}
				...
				if (sys) {
					...
					imperium_self.updateLog(
						'The destruction of the Yin Flagship has caused a terrible calamity...'
					);
				}
			}
		}
...
this.playIndoctrination = function (
	imperium_self,
	player,
	sector,
	planet_idx,
	...
	if (
		imperium_self.returnNonPlayerInfantryOnPlanet(player, planet) <= 0 ||
		opponent == -1
	) {
		...
		if (action2 === 'yes') {
			imperium_self.playerSelectInfluence(2, function (success) {
				if (success == 1) {
					...
					imperium_self.addMove(
						'NOTIFY\tYin Indoctrination converts opposing infantry'
					);
					imperium_self.endTurn();
				} else {
					mycallback(imperium_self);
...
this.playDevotion = function (
	...
	let can_sacrifice_destroyer = imperium_self.doesSectorContainPlayerUnit(
		player,
		sector,
		'destroyer'
	);
	let can_sacrifice_cruiser = imperium_self.doesSectorContainPlayerUnit(
		player,
		sector,
		'cruiser'
	);
	...
	$('.textchoice').on('click', function () {
		...
		if (action2 === 'destroyer') {
			...
			imperium_self.playDevotionAssignHit(
				imperium_self,
				player,
				sector,
				mycallback,
...
this.playDevotionAssignHit = function (
	imperium_self,
	player,
	sector,
	mycallback,
	...
	for (let i = 0; i < sys.s.units[opponent - 1].length; i++) {
		...
		if (unit.destroyed != 1) {
			html +=
				...
				i +
				'">' +
				imperium_self.returnShipInformation(unit) +
				'</li>';
		}
	}
	html += '</ul>';
	...
	groundCombatEvent: function (imperium_self, player, sector, planet_idx) {
		if (imperium_self.game.player == player) {
			...
			$('.option').on('click', function () {
				...
				if (id === 'yes') {
					let yin_player =
						imperium_self.returnPlayerOfFaction('faction5');
					let sys = imperium_self.returnSectorAndPlanets(sector);
					let planet = sys.p[planet_idx];
					...
					imperium_self.addMove(
						'NOTIFY\tGreyfire Mutagen converts opposing infantry'
					);
					...
					imperium_self.addMove(
						'NOTIFY' + '\t' + 'Yin Promissary redeemed'
					);
					imperium_self.endTurn();
				}
			});
