...
this.importTech('faction1-flagship', {
	...
	playersChooseStrategyCardsBeforeTriggers: function (imperium_self, player) {
		if (!imperium_self.doesPlayerHaveTech(player, 'faction1-flagship')) {
			return 0;
		}
				...
				for (let k = 0; k < sec.units[player - 1].length; k++) {
					if (sec.units[player - 1][k].type == 'flagship') {
						...
						imperium_self.updateLog(
							'Faction Ability: infantry added to Sol Flagship...'
						);
						return 1;
					}
				}
...
this.importTech('faction1-orbital-drop', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].orbital_drop ==
			undefined
		) {
	...
	menuOptionTriggers: function (imperium_self, menu, player) {
		if (
			imperium_self.doesPlayerHaveTech(player, 'faction1-orbital-drop') &&
			menu === 'main'
		) {
			if (
...
this.importTech('faction1-advanced-carrier-ii', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction1_advanced_carrier_ii == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction1_advanced_carrier_ii = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		imperium_self.game.state.players_info[
			gainer - 1
		].faction1_advanced_carrier_ii = 1;
	},
...
this.importTech('faction1-advanced-infantry-ii', {
	...
	initialize: function (imperium_self, player) {
		imperium_self.game.state.players_info[
			player - 1
		].faction1_advanced_infantry_ii = 0;
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		imperium_self.game.state.players_info[
			gainer - 1
		].faction1_advanced_infantry_ii = 1;
	},
	upgradeUnit: function (imperium_self, player, unit) {
		if (
			imperium_self.game.state.players_info[unit.owner - 1]
...
this.importPromissary('faction1-promissary', {
	...
	menuOption: function (imperium_self, menu, player) {
		...
		if (menu == 'main') {
			...
			x.html =
				'<li class="option" id="faction1-promissary">Military Support (Sol Promissary)</li>';
		}
		return x;
	},
	menuOptionTriggers: function (imperium_self, menu, player) {
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		...
		if (imperium_self.game.player == player) {
			let my_planets = imperium_self.returnPlayerPlanetCards(
				imperium_self.game.player
			);
			imperium_self.playerSelectPlanetWithFilter(
				...
				function (p) {
					let sol_player =
						imperium_self.returnPlayerOfFaction('faction1');
