...
this.importTech('faction2-fragile', {
	...
	modifyCombatRoll: function (
		...
		if (combat_type == 'space' || combat_type == 'ground') {
			if (
				imperium_self.doesPlayerHaveTech(attacker, 'faction2-fragile')
			) {
				imperium_self.updateLog(
					'Jol Nar combat rolls -1 due to fragility'
				);
				roll -= 1;
				if (roll < 1) {
					roll = 1;
	...
	initialize: function (imperium_self, player) {
		if (imperium_self.faction2_brilliant_swapped == undefined) {
			...
			imperium_self.brilliant_original_event =
				imperium_self.strategy_cards[
					'technology'
				].strategySecondaryEvent;
			imperium_self.strategy_cards['technology'].strategySecondaryEvent =
				function (imperium_self, player, strategy_card_player) {
					if (
						...
						imperium_self.game.state.players_info[
							player - 1
						].cost_of_technology_secondary = 6;
						...
						imperium_self.playerAcknowledgeNotice(
							...
							function () {
								...
								$('.option').on('click', function () {
									...
									imperium_self.playerResearchTechnology(
										function (tech) {
											imperium_self.addMove(
												'resolve\tstrategy\t1\t' +
													this.publicKey
											...
											let available_resources =
												imperium_self.returnAvailableResources(
											...
											if (
												...
												html +=
													'<li class="option" id="yes">Yes</li>';
											}
											html +=
												'<li class="option" id="no">No</li>';
											html += '</ul>';
											...
											$('.option').on(
												...
												function () {
													...
													if (id === 'yes') {
														...
														imperium_self.playerSelectResources(
															resources_to_spend,
															function (success) {
																if (
																	success == 1
...
this.importTech('faction2-eres-siphons', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].eres_siphons ==
			null
		) {
	...
	activateSystemTriggers: function (
		...
		if (
			...
			if (
				imperium_self.doesSectorContainPlayerShips(player, sector) == 1
			) {
				return 1;
			}
		}
...
this.importTech('faction2-deep-space-conduits', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.deep_space_conduits == null
		) {
			imperium_self.game.state.players_info[
				player - 1
			].deep_space_conduits = 0;
			imperium_self.game.state.players_info[
				player - 1
	...
	activateSystemTriggers: function (
		...
		if (
			player == imperium_self.game.player &&
			activating_player == player
		) {
			if (
	...
	activateSystemEvent: function (
		...
		$('.textchoice').on('click', function () {
			...
			if (action == 'yes') {
				let sectors =
					imperium_self.returnSectorsWithPlayerUnits(
						activating_player
					);
				imperium_self.game.state.players_info[
					activating_player - 1
				...
				for (let i = 0; i < sectors.length; i++) {
					imperium_self.addMove(
						'adjacency\ttemporary\t' + sectors[i] + '\t' + sector
					);
				}
				imperium_self.endTurn();
			}
...
this.importPromissary('faction2-promissary', {
	...
	researchTechnologyEventTriggers: function (
		imperium_self,
		researcher,
		player,
		tech
	...
	researchTechnologyEvent: function (
		imperium_self,
		researcher,
		player,
		tech
		...
		if (imperium_self.game.player === player) {
			...
			$('.option').on('click', function () {
				...
				if (id === 'yes') {
					let jolnar_player =
						imperium_self.returnPlayerOfFaction('faction2');
					imperium_self.addMove(
						'purchase\t' +
							imperium_self.game.player +
							'\ttech\t' +
