...
this.importTech('faction3-flagship', {
	...
	returnPDSUnitsWithinRange: function (
		imperium_self,
		player,
		attacker,
		defender,
		...
		if (!imperium_self.doesPlayerHaveTech(player, 'faction3-flagship')) {
			return battery;
		}
		...
		if (player_fleet.flagship > 0) {
			...
			for (let i = 0; i < as.length; i++) {
				if (
					...
					pds1.unit = JSON.parse(
						JSON.stringify(imperium_self.returnUnit('pds', player))
					);
...
this.importTech('faction3-peace-accords', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].peace_accords ==
			undefined
		) {
	...
	strategyCardAfterTriggers: function (
		imperium_self,
		player,
		...
		if (
			imperium_self.game.state.players_info[player - 1].peace_accords == 1
		) {
			return 1;
		}
		return 0;
	},
	strategyCardAfterEvent: function (
		imperium_self,
		player,
		strategy_card_player,
		card
		...
		if (card == 'diplomacy') {
			...
			for (let i = 0; i < pcs.length; i++) {
				if (
					!sectors.includes(imperium_self.game.planets[pcs[i]].sector)
				) {
					sectors.push(imperium_self.game.planets[pcs[i]].sector);
					adjacent_sectors.push(
						imperium_self.game.planets[pcs[i]].sector
					);
				}
			}
			...
			for (let i = 0; i < sectors.length; i++) {
				...
				for (let z = 0; z < as.length; z++) {
					if (!adjacent_sectors.includes(as[z])) {
						adjacent_sectors.push(as[z]);
					}
				}
			}
			...
			for (let b = 0; b < adjacent_sectors.length; b++) {
				let sys = imperium_self.returnSectorAndPlanets(
					adjacent_sectors[b]
				);
				if (sys.p) {
					for (let y = 0; y < sys.p.length; y++) {
						...
						if (sys.p[y].owner != player) {
							if (
								!imperium_self.doesPlanetHaveInfantry(sys.p[y])
							) {
			...
			if (seizable_planets.length < 0) {
				imperium_self.updateLog(
					'XXCha cannot annex any unguarded planets via Peace Accords'
				);
				return 1;
			}
			...
			if (
				...
				imperium_self.updateStatus(
					'XXCha selecting planet to annex with Peace Accords'
				);
				...
				if (imperium_self.game.player == player) {
					imperium_self.playerSelectPlanetWithFilter(
						...
						function (planet) {
							if (seizable_planets.includes(planet)) {
								return 1;
							}
							return 0;
						},
						function (planet) {
							imperium_self.addMove(
								'annex\t' +
									...
									'\t' +
									imperium_self.game.planets[planet].sector +
									'\t' +
									imperium_self.game.planets[planet].idx
							);
							imperium_self.addMove(
								'NOTIFY\t' +
...
this.importTech('faction3-quash', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].quash == undefined
		) {
	...
	menuOptionTriggers: function (imperium_self, menu, player) {
		if (
			imperium_self.doesPlayerHaveTech(player, 'faction3-quash') &&
			menu == 'main'
		) {
			if (
				imperium_self.game.state.players_info[player - 1]
...
this.importTech('faction3-instinct-training', {
	...
	playActionCardTriggers: function (
		imperium_self,
		player,
		action_card_player,
		card
...
this.importTech('faction3-field-nullification', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.field_nullification == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].field_nullification = 0;
			imperium_self.game.state.players_info[
				player - 1
			].field_nullification_exhausted = 0;
		}
	},
	...
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'faction3-field-nullification') {
			imperium_self.game.state.players_info[
				gainer - 1
			].field_nullification = 1;
			imperium_self.game.state.players_info[
				gainer - 1
			].field_nullification_exhausted = 0;
		}
	},
	activateSystemTriggers: function (
		imperium_self,
		activating_player,
		player,
		sector
	...
	activateSystemEvent: function (
		...
		if (
			...
			if (
				imperium_self.game.state.players_info[player - 1]
					.field_nullification_exhausted == 1
			) {
				return 1;
			}
			...
			if (imperium_self.game.player != player) {
				imperium_self.updateStatus(
					imperium_self.returnFaction(player) +
						' is deciding whether to use Nullification Fields'
				);
				return 0;
			}
			...
			$('.textchoice').on('click', function () {
				...
				if (choice == 'yes') {
					...
					imperium_self.addMove(
						'setvar\tstate\t0\tactive_player_moved\t' +
							'int' +
							'\t' +
							'0'
					);
					imperium_self.addMove(
						'player_end_turn\t' + imperium_self.game.player
					);
					imperium_self.addMove(
						'field_nullification\t' +
							player +
		...
		if (mv[0] == 'field_nullification') {
			...
			imperium_self.updateLog(
				imperium_self.returnFactionNickname(player) +
					' uses Nullification Fields to end ' +
					imperium_self.returnFactionNickname(activating_player) +
					' turn'
			);
...
this.importPromissary('faction3-promissary', {
	...
	menuOption: function (imperium_self, menu, player) {
		...
		if (menu == 'main') {
			...
			x.html =
				'<li class="option" id="faction3-promissary">Political Favour (XXCha Promissary)</li>';
		}
		return x;
	},
	menuOptionTriggers: function (imperium_self, menu, player) {
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		...
		if (imperium_self.game.player == player) {
			...
			$('.option').on('click', function () {
				...
				imperium_self.addMove('quash\t' + agenda_to_quash + '\t' + '1'); // 1 = re-deal
				imperium_self.addMove(
					'expend\t' + xxcha_player + '\t' + 'strategy' + '\t' + '1'
				);
				imperium_self.addMove(
					'give' +
						'\t' +
