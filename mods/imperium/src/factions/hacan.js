...
this.importTech('faction8-flagship', {
	...
	postShipsFireEventTriggers: function (
		...
		if (player == attacker && combat_info.attacker == player) {
			if (
				imperium_self.doesPlayerHaveTech(attacker, 'faction8-flagship')
			) {
				if (
					...
					for (let i = 0; i < combat_info.modified_roll.length; i++) {
						if (combat_info.hits_or_misses[i] == 0) {
							costs_per_hit.push(
								2 *
									(parseInt(combat_info.hits_on[i]) -
										parseInt(combat_info.modified_roll[i]))
							);
						}
					}
		...
		if (player == defender && combat_info.attacker == player) {
			if (
				imperium_self.doesPlayerHaveTech(defender, 'faction8-flagship')
			) {
				if (
					imperium_self.doesSectorContainPlayerUnit(
						defender,
	...
	postShipsFireEvent: function (
		...
		if (player != imperium_self.game.player) {
			imperium_self.updateStatus(
				'Hacan considering using Flagship Ability to modify hits...'
			);
			return 0;
		} else {
			let costs_per_hit = [];
			...
			let available_trade_goods =
				imperium_self.game.state.players_info[player - 1].goods;
			for (
				let i = 0;
				i < costs_per_hit.length &&
				cumulative_cost <= available_trade_goods;
				i++
			) {
...
this.importTech('faction8-merchant-class', {
	...
	initialize: function (imperium_self, player) {
		if (imperium_self.faction8_merchant_class_swapped == undefined) {
			...
			imperium_self.strategy_cards['trade'].strategySecondaryEvent =
				async function (imperium_self, player, strategy_card_player) {
					if (
						...
						if (
							...
							imperium_self.addMove(
								'resolve\tstrategy\t1\t' +
									imperium_self.getPublicKey()
							);
							imperium_self.addPublickeyConfirm(
								imperium_self.getPublicKey(),
								1
							);
							imperium_self.endTurn();
						...
						html +=
							'<li class="option" id="yes">yes, of course</li>';
						html +=
							'<li class="option" id="no">no, perhaps not</li>';
						html += '</ul>';
...
this.importTech('faction8-guild-ships', {
	...
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'faction8-guild-ships') {
			imperium_self.game.state.players_info[
				gainer - 1
			].may_trade_with_non_neighbours = 1;
		}
	}
});
	...
	menuOption: function (imperium_self, menu, player) {
		...
		if (menu === 'main') {
			if (
				...
				x.html =
					'<li class="option" id="production_biomes">production biomes</li>';
			}
		}
		return x;
	},
...
this.importTech('faction8-quantum-datahub-node', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.faction8_quantum_datahub_node == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].faction8_quantum_datahub_node = 0;
			imperium_self.game.state.players_info[
				player - 1
	...
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'faction8-quantum-datahub-node') {
			imperium_self.game.state.players_info[
				gainer - 1
			].faction8_quantum_datahub_node = 1;
			imperium_self.game.state.players_info[
				gainer - 1
			].faction8_quantum_datahub_node_exhausted = 0;
		}
	...
	menuOption: function (imperium_self, menu, player) {
		...
		if (menu === 'main') {
			if (
				...
				x.html =
					'<li class="option" id="quantum_datahub_node">quantum datahub node</li>';
			}
		}
		return x;
	},
		...
		if (imperium_self.game.player == player) {
			imperium_self.playerSelectPlayerWithFilter(
				...
				function (pnum) {
					...
					let html =
						'<div>Select Strategy Card to Steal: </div><ul>"';
					for (
						let i = 0;
						i <
						imperium_self.game.state.players_info[pnum - 1].strategy
							.length;
						i++
					) {
						let s =
							imperium_self.game.state.players_info[pnum - 1]
								.strategy[i];
					...
					$('.option').on('click', function () {
						...
						if (id == 'skip') {
							imperium_self.updateLog(
								'Hacan skips Quantum Datahub Node'
							);
							imperium_self.endTurn();
							return;
						}

						let pull_strategy_card =
							imperium_self.game.state.players_info[pnum - 1]
								.strategy[id];
						let pull_strategy_card_from = pnum;

						let html =
							'<div>Select Your Strategy Card to Return: </div><ul>"';
						for (
							let i = 0;
							i <
							imperium_self.game.state.players_info[
...
this.importPromissary('faction8-promissary', {
	...
	activateSystemTriggers: function (
		...
		if (
			...
			if (
				imperium_self.doesSectorContainPlayerUnits(hacan_player, sector)
			) {
	...
	activateSystemEvent: function (
		...
		if (
			...
			if (imperium_self.game.player == player) {
				let hacan_player =
					imperium_self.returnPlayerOfFaction('faction8');
				imperium_self.addMove(
					'give' +
						'\t' +
						player +
		...
		if (promissary.indexOf('faction8-promissary') >= 0) {
			if (
				...
				if (loser !== imperium_self.returnPlayerOfFaction('faction8')) {
					imperium_self.game.state.players_info[
						loser - 1
					].may_trade_with_non_neighbours = 0;
				}
			}
		}
		return 1;
