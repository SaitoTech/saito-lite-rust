this.importTech('antimass-deflectors', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.antimass_deflectors == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].antimass_deflectors = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'antimass-deflectors') {
			imperium_self.game.state.players_info[
				gainer - 1
			].antimass_deflectors = 1;
			imperium_self.game.state.players_info[
				gainer - 1
			].fly_through_asteroids = 1;
		}
	}
});
...
this.importTech('gravity-drive', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].gravity_drive ==
			undefined
		) {
...
this.importTech('fleet-logistics', {
	...
	onNewRound: function (imperium_self, player) {
		if (imperium_self.doesPlayerHaveTech(player, 'fleet-logistics')) {
			imperium_self.game.state.players_info[
				player - 1
			].fleet_logistics_turn = 0;
		}
	},
	onNewTurn: function (imperium_self, player) {
		if (imperium_self.doesPlayerHaveTech(player, 'fleet-logistics')) {
			imperium_self.game.state.players_info[player - 1]
				.fleet_logistics_turn++;
		}
	},
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].fleet_logistics ==
			undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
	...
	menuOption: function (imperium_self, menu, player) {
		if (menu == 'main') {
			if (imperium_self.doesPlayerHaveTech(player, 'fleet-logistics')) {
				return {
					event: 'fleetlogistics',
					html: '<li class="option" id="fleetlogistics">use fleet logistics</li>'
				};
			}
		}
		return {};
	...
	menuOptionActivated: function (imperium_self, menu, player) {
		if (menu == 'main') {
			...
			imperium_self.addMove('play\t' + player);
			imperium_self.addMove(
				'NOTIFY\t' + player + ' activates fleet logistics'
			);
...
this.importTech('lightwave-deflector', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.lightwave_deflector == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].lightwave_deflector = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'lightwave-deflector') {
			imperium_self.game.state.players_info[
				gainer - 1
			].lightwave_deflector = 1;
			imperium_self.game.state.players_info[
				gainer - 1
			].move_through_sectors_with_opponent_ships = 1;
		}
	}
});
