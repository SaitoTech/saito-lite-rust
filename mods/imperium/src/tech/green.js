this.importTech('neural-motivator', {
	...
	gainTechnology: function (imperium_self, gainer, tech) {
		if (tech == 'neural-motivator') {
			...
			imperium_self.game.state.players_info[
				gainer - 1
			].action_cards_bonus_when_issued = 1;
		}
	}
});
...
this.importTech('dacxive-animators', {
	...
	groundCombatRoundEnd: function (
		...
		let attacker_forces = imperium_self.returnNumberOfGroundForcesOnPlanet(
			attacker,
			sector,
			planet_idx
		);
		let defender_forces = imperium_self.returnNumberOfGroundForcesOnPlanet(
			defender,
			sector,
			planet_idx
		);
			...
			if (attacker_forces < defender_forces && attacker_forces == 0) {
				...
				imperium_self.updateLog(
					imperium_self.returnFaction(defender) +
						' reinforces infantry with Dacxive Animators'
				);
			}
		}
	}
...
this.importTech('x89-bacterial-weapon', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.x89_bacterial_weapon == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].x89_bacterial_weapon = 0;
			imperium_self.game.state.players_info[
				player - 1
			].x89_bacterial_weapon_exhausted = 0;
		}
	},
	gainTechnology: function (imperium_self, gainer, tech) {
	...
	bombardmentTriggers: function (
		...
		if (
			imperium_self.game.state.players_info[bombarding_player - 1]
				.x89_bacterial_weapon == 1 &&
			imperium_self.game.state.players_info[bombarding_player - 1]
				.x89_bacterial_weapon_exhausted == 0
		) {
	...
	bombardmentEvent: function (
		...
		if (imperium_self.game.player != bombarding_player) {
			return 0;
		}
		...
		html =
			'<p>Do you wish to use Bacterial Weapons during Bombardment?</p><ul>';
		html +=
			'<li class="option textchoice" id="attack">use bacterial weapons?</li>';
		...
		$('.textchoice').on('click', function () {
			...
			if (action2 == 'attack') {
				...
				imperium_self.addMove(
					'setvar\tplayers\t' +
						player +
						'\t' +
						'x89_bacterial_weapon_exhausted' +
						'\t' +
						'int' +
						'\t' +
						'1'
			...
			if (action2 == 'skip') {
				imperium_self.addMove(
					'NOTIFY\t' +
						imperium_self.returnFaction(player) +
						' refrains from using X89 Bacterial Weapons'
				);
				imperium_self.endTurn();
			}
		});
