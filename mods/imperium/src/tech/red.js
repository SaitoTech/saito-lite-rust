this.importTech('plasma-scoring', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].plasma_scoring ==
			undefined
		) {
	...
	pdsSpaceAttackTriggers: function (imperium_self, attacker, player, sector) {
		if (imperium_self.doesPlayerHaveTech(player, 'plasma-scoring')) {
			if (
				imperium_self.doesPlayerHavePDSUnitsWithinRange(
					attacker,
					player,
					sector
				) == 1 &&
				...
				imperium_self.updateLog(
					imperium_self.returnFaction(player) +
						' gets +1 shot from Plasma Scoring'
				);
			}
		}
		//
	...
	pdsSpaceDefenseTriggers: function (
		imperium_self,
		attacker,
		player,
		sector
...
this.importTech('magen-defense-grid', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1]
				.magen_defense_grid == undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
			].magen_defense_grid = 0;
		}
	},
	onNewRound: function (imperium_self, player) {
		if (player == imperium_self.game.player) {
			imperium_self.game.state.players_info[
				player - 1
			].magen_defense_grid = 1;
		}
		return 1;
	},
	groundCombatTriggers: function (imperium_self, player, sector, planet_idx) {
		if (imperium_self.game.state.ground_combat_round >= 1) {
			return 0;
		} // starts at 0
		if (imperium_self.doesPlayerHaveTech(player, 'magen-defense-grid')) {
			...
			if (player == planet.owner) {
				...
				for (let i = 0; i < planet.units[player - 1].length; i++) {
					if (
						planet.units[player - 1][i].type == 'spacedock' ||
						planet.units[player - 1][i].type == 'pds'
					) {
						imperium_self.updateLog(
							'Magan Defense Grid triggers on ' + planet.name
						);
						return 1;
					}
				}
			...
			if (i != owner - 1 && i < planet.units.length) {
				for (let ii = 0; ii < ii_limit; ii++) {
					...
					if (attacker_unit.type == 'infantry') {
						...
						imperium_self.assignHitsToGroundForces(
							defender,
							attacker,
							sector,
							planet_idx,
						...
						imperium_self.updateLog(
							imperium_self.returnFaction(attacker) +
								' loses 1 infantry to Magan Defense Grid'
						);
...
this.importTech('duranium-armor', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].duranium_armor ==
			undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
	...
	onNewRound: function (imperium_self, player, mycallback) {
		if (player == imperium_self.game.player) {
			...
			imperium_self.game.state.players_info[
				player - 1
			].may_repair_damaged_ships_after_space_combat = 1;
		}
		return 1;
	},
	spaceCombatRoundOver: function (imperium_self, attacker, defender, sector) {
		...
		if (imperium_self.doesPlayerHaveTech(attacker, 'duranium-armor')) {
			for (let i = 0; i < sys.s.units[attacker - 1].length; i++) {
				...
				if (
					this_unit.last_round_damaged <
					imperium_self.game.state.space_combat_round
				) {
					this_unit.strength = this_unit.max_strength;
					imperium_self.updateLog(
						imperium_self.returnFaction(attacker) +
							' repairs ships with Duranium Armor'
					);
				}
			}
		}
		...
		if (imperium_self.doesPlayerHaveTech(defender, 'duranium-armor')) {
			for (let i = 0; i < sys.s.units[defender - 1].length; i++) {
				...
				if (
					...
					this_unit.strength = this_unit.max_strength;
					imperium_self.updateLog(
						imperium_self.returnFaction(defender) +
							' repairs ships with Duranium Armor'
					);
				}
			}
...
this.importTech('assault-cannon', {
	...
	initialize: function (imperium_self, player) {
		if (
			imperium_self.game.state.players_info[player - 1].assault_cannont ==
			undefined
		) {
			imperium_self.game.state.players_info[
				player - 1
	...
	onNewRound: function (imperium_self, player, mycallback) {
		if (player == imperium_self.game.player) {
			...
			imperium_self.game.state.players_info[
				player - 1
			].may_assign_first_round_combat_shot = 1;
		}
		return 1;
	},
	spaceCombatTriggers: function (imperium_self, player, sector) {
		...
		for (let i = 0; i < sys.s.units.length; i++) {
			if (
				i + 1 != player &&
				imperium_self.doesPlayerHaveTech(i + 1, 'assault-cannon')
			) {
				for (let ii = 0; ii < sys.s.units[i].length; ii++) {
					let thisunit = sys.s.units[i][ii];
					if (thisunit.type == 'destroyer') {
						capital_ships++;
					}
					if (thisunit.type == 'carrier') {
						capital_ships++;
					}
					if (thisunit.type == 'cruiser') {
						capital_ships++;
					}
					if (thisunit.type == 'dreadnaught') {
						capital_ships++;
					}
					if (thisunit.type == 'flagship') {
						capital_ships++;
					}
					if (thisunit.type == 'warsun') {
						capital_ships++;
					}
				}
	...
	spaceCombatEvent: function (imperium_self, player, sector) {
		imperium_self.game.state.players_info[player - 1].target_units = [
			'carrier',
			'destroyer',
			'cruiser',
			'dreadnaught',
		...
		imperium_self.game.queue.push(
			'ACKNOWLEDGE\t' +
				imperium_self.returnFaction(player) +
				' must destroy 1 ship from Assault Cannon'
		);
		return 1;
	}
});
