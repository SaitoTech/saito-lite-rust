this.importSecretObjective('military-catastrophe', {
	...
	spaceCombatRoundEnd: function (imperium_self, attacker, defender, sector) {
		if (
			...
			].units_i_destroyed_this_combat_round.includes('flagship')
		) {
...
this.importSecretObjective('flagship-dominance', {
	...
	spaceCombatRoundEnd: function (imperium_self, attacker, defender, sector) {
		if (
			...
			if (!imperium_self.doesPlayerHaveShipsInSector(defender, sector)) {
				if (
					attacker == imperium_self.game.player &&
					imperium_self.doesPlayerHaveShipsInSector(attacker, sector)
				) {
			...
			if (!imperium_self.doesPlayerHaveShipsInSector(attacker, sector)) {
				if (
					defender == imperium_self.game.player &&
					imperium_self.doesPlayerHaveShipsInSector(defender, sector)
				) {
					imperium_self.game.state.secret_objective_flagship_dominance = 1;
				}
			}
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		if (imperium_self.game.state.secret_objective_flagship_dominance == 1) {
			return 1;
		}
		return 0;
	},
...
this.importSecretObjective('nuke-them-from-orbit', {
		...
		for (let i = 0; i < planet.units.length; i++) {
			if (planet.units[i].length > 0) {
				if (i + 1 != bombarding_player) {
					defender = i + 1;
					imperium_self.game.state.secret_objective_nuke_from_orbit_how_many_got_nuked =
						infantry_on_planet;
				}
			}
		}
	...
	planetaryDefenseTriggers: function (
		imperium_self,
		player,
		sector,
		planet_idx
		...
		if (
			imperium_self.game.state
				.secret_objective_nuke_from_orbit_how_many_got_nuked > 0
		) {
			let sys = imperium_self.returnSectorAndPlanets(sector);
			let planet = sys.p[planet_idx];
			let infantry_on_planet =
				imperium_self.returnInfantryOnPlanet(planet);
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		if (imperium_self.game.state.secret_objective_nuke_from_orbit == 1) {
			return 1;
		}
		return 0;
	},
...
this.importSecretObjective('anti-imperialism', {
	...
	spaceCombatRoundEnd: function (imperium_self, attacker, defender, sector) {
		...
		if (
			imperium_self.game.player == attacker &&
			!imperium_self.doesPlayerHaveShipsInSector(defender, sector) &&
			imperium_self.doesPlayerHaveShipsInSector(attacker, sector)
		) {
			if (imperium_self.hasUnresolvedSpaceCombat(attacker, sector) == 0) {
				if (players_with_most_vp.includes(defender)) {
					imperium_self.game.state.secret_objective_anti_imperialism = 1;
				}
			}
		}
		if (
			imperium_self.game.player == defender &&
			!imperium_self.doesPlayerHaveShipsInSector(attacker, sector) &&
			imperium_self.doesPlayerHaveShipsInSector(defender, sector)
		) {
			if (imperium_self.hasUnresolvedSpaceCombat(defender, sector) == 0) {
				if (players_with_most_vp.includes(attacker)) {
					imperium_self.game.state.secret_objective_anti_imperialism = 1;
				}
			}
		}
	...
	groundCombatRoundEnd: function (
		...
		if (
			imperium_self.game.player == attacker &&
			planet.units[attacker - 1].length > 0
		) {
		...
		if (
			imperium_self.game.player == defender &&
			planet.units[defender - 1].length > 0
		) {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		if (imperium_self.game.state.secret_objective_anti_imperialism == 1) {
			return 1;
		}
		return 0;
	},
			...
			if (sys.s.units[defender - 1].length == 0) {
				if (players_with_lowest_vp.includes(defender)) {
					...
					for (let i in imperium_self.game.sectors) {
						if (
							imperium_self.game.sectors[i].units[defender - 1]
								.length > 0
						) {
							return;
						}
					...
					for (let i in imperium_self.game.planets) {
						if (
							imperium_self.game.planets[i].units[defender - 1]
								.length > 0
						) {
							return;
						}
		...
		if (imperium_self.game.player == defender) {
			if (sys.s.units[attacker - 1].length == 0) {
				if (players_with_lowest_vp.includes(attacker)) {
					for (let i in imperium_self.game.sectors) {
						if (
							imperium_self.game.sectors[i].units[attacker - 1]
								.length > 0
						) {
							return;
						}
					...
					for (let i in imperium_self.game.planets) {
						if (
							imperium_self.game.planets[i].units[attacker - 1]
								.length > 0
						) {
							return;
						}
				...
				for (let p = 0; p < sys.p.length; p++) {
					for (let b = 0; b < sys.p[p].units.length; b++) {
						if (b + 1 != player) {
							for (
								let bb = 0;
								bb < sys.p[p].units[b].length;
								bb++
							) {
								if (
									sys.p[p].units[b][bb].type === 'spacedock'
								) {
									return 1;
								}
							}
...
this.importSecretObjective('close-the-trap', {
	...
	spaceCombatRoundEnd: function (imperium_self, attacker, defender, sector) {
		...
		if (
			...
			if (imperium_self.hasUnresolvedSpaceCombat(attacker, sector) == 0) {
				if (
					imperium_self.game.state
						.secret_objective_close_the_trap_pds_fired == 1
				) {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		if (imperium_self.game.state.secret_objective_close_the_trap == 1) {
			return 1;
		}
		return 0;
	},
...
this.importSecretObjective('wormhole-administrator', {
	...
	type: 'secret',
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		let relevant_sectors =
			imperium_self.returnSectorsWithPlayerShips(player);
		let alpha = 0;
		let beta = 0;
		for (let i = 0; i < relevant_sectors.length; i++) {
			if (imperium_self.game.sectors[relevant_sectors[i]].wormhole == 1) {
				alpha = 1;
			}
			if (imperium_self.game.sectors[relevant_sectors[i]].wormhole == 2) {
				beta = 1;
			}
		}
		if (alpha == 1 && beta == 1) {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < relevant_sectors.length; i++) {
			for (
				let ii = 0;
				ii <
				imperium_self.game.sectors[relevant_sectors[i]].units[
					player - 1
				].length;
				ii++
			) {
...
this.importSecretObjective('cultural-diplomacy', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < planetcards.length; i++) {
			if (
				imperium_self.game.planets[planetcards[i]].type === 'cultural'
			) {
				cultural++;
			}
		}
...
this.importSecretObjective('act-of-espionage', {
	...
	type: 'secret',
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		if (imperium_self.returnPlayerActionCards(player).length >= 5) {
			return 1;
		}
		return 0;
	},
...
this.importSecretObjective('space-to-breathe', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < relevant_sectors.length; i++) {
			if (
				imperium_self.game.sectors[relevant_sectors[i]].planets
					.length == 0
			) {
				sectors_without_planets++;
			}
		}
		if (sectors_without_planets >= 3) {
			return 1;
		}
		return 0;
	},
...
this.importSecretObjective('ascendant-technocracy', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < techlist.length; i++) {
			if (imperium_self.tech[techlist[i]].color == 'blue') {
				bluetech++;
			}
			if (imperium_self.tech[techlist[i]].color == 'red') {
				redtech++;
			}
			if (imperium_self.tech[techlist[i]].color == 'yellow') {
				yellowtech++;
			}
			if (imperium_self.tech[techlist[i]].color == 'green') {
				greentech++;
			}
		}
...
this.importSecretObjective('penal-colonies', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < planetcards.length; i++) {
			if (
				imperium_self.game.planets[planetcards[i]].type === 'hazardous'
			) {
				hazardous++;
			}
		}
...
this.importSecretObjective('master-of-production', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < planetcards.length; i++) {
			if (
				imperium_self.game.planets[planetcards[i]].type === 'industrial'
			) {
				industrial++;
			}
		}
...
this.importSecretObjective('faction-technologies', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		...
		for (let i = 0; i < techlist.length; i++) {
			if (
				imperium_self.tech[techlist[i]].prereqs.length > 0 &&
				imperium_self.tech[techlist[i]].type == 'special' &&
				techlist[i].indexOf('faction') == 0
			) {
				factiontech++;
			}
		}
...
this.importSecretObjective('occupy-new-byzantium', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		if (imperium_self.game.planets['new-byzantium'].owner == player) {
			if (
				imperium_self.game.sectors['new-byzantium'].units[player - 1]
					.length >= 3
			) {
				return 1;
			}
...
this.importSecretObjective('cast-a-long-shadow', {
	...
	canPlayerScoreVictoryPoints: function (imperium_self, player) {
		// 1_1, 4_7, etc.
		let homeworlds = imperium_self.returnHomeworldSectors(
			imperium_self.game.state.players_info.length
		);
		let sectors = [];

		for (let i = 0; i < homeworlds.length; i++) {
			if (
				imperium_self.game.board[homeworlds[i]].tile !=
				imperium_self.game.board[
					imperium_self.game.state.players_info[player - 1].homeworld
				].tile
			) {
		...
		for (let i = 0; i < sectors.length; i++) {
			if (imperium_self.isPlayerAdjacentToSector(player, sectors[i])) {
				return 1;
			}
		}
