module.exports = ImperiumDashboardTemplate = (imperium_self, i=0, agenda_phase=0) => {
    ...
    html += `
      <div data-id="${(i+1)}" class="dash-faction-agenda">
        <div data-id="${(i+1)}" class="dash-item-agenda-influence agenda-influence">
          <span data-id="${(i+1)}" class="avail">${imperium_self.game.state.votes_available[i]}</span>
        </div> 
      </div>
    `;
  ...
  html += `
	...
	<div class="dash-faction-status-text">
      ...
      if (imperium_self.game.state.speaker == (i+1)) {  html += ' speaker">speaker'; } else { html += '">'; }
      html += `</div>
    </div>
  `;
