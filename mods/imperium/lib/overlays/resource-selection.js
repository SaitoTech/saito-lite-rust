...
class ResourceSelectionOverlay {
  ...
  render(resources_needed=1, cards=[], goods=0, submitResources=()=>{}) {
    ...
    this.overlay.show(ImperiumResourceSelectionOverlayTemplate(resources_needed, cards, goods));

    //
    // no trade goods
    //
    if (goods <= 0) {
      document.querySelector(".resource-selection-goods").style.display = "none";
    } 
  ...
  attachEvents() {
    ...
    document.querySelectorAll(".resource-selection-cards .planet-card").forEach((el) => {
      el.onclick = (e) => {
    ...
    document.querySelectorAll(".resource-selection-goods").forEach((el) => {
      el.onclick = (e) => {
	let amount = parseInt(e.currentTarget.getAttribute("data-amount"));
	if (amount > 0) {
	  amount--;
