...
class InfluenceSelectionOverlay {
  ...
  render(Influence_needed=1, cards=[], goods=0, submitInfluence=()=>{}) {
    ...
    //
    if (goods <= 0) {
      document.querySelector(".influence-selection-goods").style.display = "none";
    } 
  ...
  attachEvents() {
    ...
    document.querySelectorAll(".influence-selection-cards .planet-card").forEach((el) => {
      el.onclick = (e) => {
    ...
    document.querySelectorAll(".influence-selection-goods").forEach((el) => {
      el.onclick = (e) => {
