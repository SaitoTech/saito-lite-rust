...
class AgendaSelectionOverlay {
  ...
  render(available_agendas, selected_agendas, num_to_select, mycallback, removal_callback) {

    let agendas = this.mod.returnAgendaCards();

    this.overlay.show(ImperiumAgendaSelectionOverlayTemplate());
    this.overlay.setBackground("/imperium/img/backgrounds/agenda-selection-background.png", false);

    for (let i = 0; i < available_agendas.length; i++) {
      let a = agendas[available_agendas[i]];
      this.app.browser.addElementToSelector(a.returnCardImage(), ".agenda-selection-cards");
      if (selected_agendas.includes(available_agendas[i])) {
        ...
        let el = document.querySelector((".agenda-selection-cards ."+available_agendas[i]));
        el.classList.remove("opaque");
        el.classList.add("nonopaque");
      }
    }

    this.attachEvents(available_agendas, selected_agendas, num_to_select, mycallback, removal_callback);

  }

  attachEvents(available_agendas, selected_agendas, num_to_select, mycallback, removal_callback) {

    document.querySelectorAll(".agenda-selection-cards .card").forEach((el) => {
      el.onclick = (e) => {
