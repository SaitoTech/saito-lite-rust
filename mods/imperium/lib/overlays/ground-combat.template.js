module.exports = ImperiumGroundCombatOverlayTemplate = (imperium_self, attacker, defender, sector, planet_idx, overlay_html) => {
  ...
  let html = `
	...
	<div class="ground-combat-attacker">
	  <div class="ground-combat-attacker-name">${imperium_self.returnFactionName(imperium_self, attacker)}</div>
	  <div class="unit-table small">
  `;

  for (let i = 0; i < attacker_forces.length; i++) {
    if (attacker_forces[i].strength > 0 && attacker_forces[i].destroyed != 1) {
  ...
  html += `
	...
	<div class="ground-combat-defender">
	  <div class="ground-combat-defender-name">${imperium_self.returnFactionName(imperium_self, defender)}</div>
	  <div class="unit-table small">
  `;
  for (let i = 0; i < defender_forces.length; i++) {
    if (defender_forces[i].strength > 0 && defender_forces[i].destroyed != 1) {
