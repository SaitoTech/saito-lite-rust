...
console.log("Setting sector as: " + sector);
  ...
  render() {
    ...
    if (obj) {
      this.app.browser.replaceElementBySelector(SectorTemplate(this.mod, this.sector), myqs);
    } else {
      this.app.browser.addElementToSelector(SectorTemplate(this.mod, this.sector), this.container);
    }
  ...
  update() {
    ...
    for (let i = 0; i < imperium_self.game.state.players_info.length; i++) {
      if (imperium_self.game.queue.length > 0) {
        let lmv = imperium_self.game.queue[imperium_self.game.queue.length-1].split("\t");
        if (lmv[0] !== "destroy_unit" && lmv[0] !== "assign_hit") {
          imperium_self.eliminateDestroyedUnitsInSector((i+1), sector);
        }
      }
    }
