	  ...
	  if (num == 3) { num = "3rd"; }
		...
		return 1;
   	      ...
   	      let msg = "Choose Faction to Destroy Unit:";
              ...
              if (space.units["independent"].length) { html += '<li class="option" id="independent"></li>'; }
    	      html += '</ul>';
...
console.log("NEW WORLD PHASE!");
	...
	if (mv[0] === "reshuffle_diplomatic_deck") {
	  ...
	  this.game.queue.splice(qe, 1);
          ...
          for (let i = this.game.state.players_info.length; i > 0; i--) {
            this.game.queue.push("DECKXOR\tw\t"+(i));
          }
