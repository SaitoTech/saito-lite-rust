...
console.log("protestants = " + protestants_player);
	...
	if (stage == "papacy") {
	  ...
	  if (this.mod.game.player != papacy_player) {
	    document.querySelector(".council-of-trent-overlay .help").innerHTML = "Papacy select up to 4 Debaters.";
  	  } else {
	...
	if (stage == "protestants") {
	  ...
	  if (this.mod.game.player != protestants_player) {
	    document.querySelector(".council-of-trent-overlay .help").innerHTML = "Protestants select up to 2 Debaters.";
	  } else {
	    document.querySelector(".council-of-trent-overlay .help").innerHTML = `Protestants select up to 2 Debaters. (<span class="finish-selecting-debaters">click when done</span>)`;
	  }
	}
	if (stage == "results") {
...
alert(this.mod.game.state.council_of_trent.papacy.debaters.length + " -- " + this.mod.game.state.council_of_trent.protestants.debaters.length);
	  ...
	  for (let i = 0; i < this.mod.game.state.council_of_trent.protestants.debaters.length; i++) {
	    let d = this.mod.debaters[this.mod.game.state.council_of_trent.protestants.debaters[i]];
	  ...
	  for (let i = 0; i < papacy_rolls.length; i++) {
	    if (papacy_rolls[i] >= 5) { papacy_hits++; }
	  }
	  for (let i = 0; i < protestants_rolls.length; i++) {
	    if (protestants_rolls[i] >= 5) { protestants_hits++; }
	  }

	  let html  = 'Papacy rolls: ' + JSON.stringify(papacy_rolls) + "<br />";
	      html += 'Protestants roll: ' + JSON.stringify(protestants_rolls) + "<br />";
	  ...
	  if (protestants_hits < papacy_hits) {
	    ...
	    html += winner;
	    for (let i = 0; i < winner_converts; i++) {
	      this.mod.game.queue.push("catholic_counter_reformation\t"+papacy_player+"\tall");
	    }
	    this.mod.game.queue.push("ACKNOWLEDGE\tPapacy wins the Council of Trent");
	  }
	  if (protestants_hits === papacy_hits) {
    ...
    attachEvents(stage="papacy") {
      ...
      document.querySelector(".finish-selecting-debaters").onclick = (e) => {
        ...
        document.querySelector(".finish-selecting-debaters").onclick = (e) => {};
        if (this.mod.game.player == papacy_player && stage == "papacy") {
	  this.mod.addMove("council_of_trent_add_debaters\tpapacy\t"+JSON.stringify(this.selected));
	  this.mod.endTurn();
	}
        if (this.mod.game.player == protestants_player && stage == "protestants") {
	  this.mod.addMove("council_of_trent_add_debaters\tprotestants\t"+JSON.stringify(this.selected));
	  this.mod.endTurn();
	}
      ...
      try {
      document.querySelectorAll(".council-of-trent-overlay .debaters .debater-card").forEach((el) => {
	el.onclick = (e) => {
