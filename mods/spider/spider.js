...
class Spider extends OnePlayerGameTemplate {
  ...
  displayBoard() {
    ...
    for (let i = 0; i < this.game.state.draws_remaining; i++) {
      html += `<img style="bottom:${0.5 * i}vh; right:${
        0.5 * i
      }vh;" src="/spider/img/cards/red_back.png" />`;
    }
    if (!html) {
      html = "Start New Game";
    }
  ...
  attachEventsToBoardManual() {
    ...
    $(".card").on("click", function (e) {
      ...
      if (selected_stack) {
        //Deselect/Change
        if (card_pos[0] == selected_stack[0]) {
          // Same stack
          spider_self.untoggleAll();
          selected_stack = null;
        } else {
  ...
  revealCard(stackNum) {
    ...
    if (this.game.state.board[stackNum].length > 0) {
      ...
      if (this.isFaceDown(topCard)) {
        ...
        $(index)
          .append($(this.returnCardImageHTML(this.game.deck[0].cards[topCard])))
          .delay(30)
          .queue(function () {
  ...
  */
  ...
  async animateStackVictory(stackNo) {
    ...
    let newTop = Math.min(
      document.querySelector(".completed_stack_box").getBoundingClientRect().top,
