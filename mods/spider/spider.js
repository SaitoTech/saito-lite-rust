...
class Spider extends OnePlayerGameTemplate {
  ...
  async initializeGame(game_id) {
    ...
    await this.changeDifficulty(input_dif);
  ...
  async render(app) {
    ...
    this.menu.addSubMenuOption("game-difficulty", {
      ...
      callback: async function (app, game_mod) {
        await game_mod.changeDifficulty("easy");
      },
    });
    ...
    this.menu.addSubMenuOption("game-difficulty", {
      ...
      callback: async function (app, game_mod) {
        await game_mod.changeDifficulty("medium");
      },
    });
    ...
    this.menu.addSubMenuOption("game-difficulty", {
      ...
      callback: async function (app, game_mod) {
        await game_mod.changeDifficulty("hard");
      },
    });
  ...
  async updateScore(change = -1) {
    ...
    if (this.game.state.score <= 0) {
      await this.displayModal("You Lose!", "Too many moves");
      ...
      if (stackSize > 0) {
        for (let i = 1; i < 10; i++) {
          ...
          if (spider_self.canMoveStack(card_pos, colInd)) {
            await spider_self.commitMove(card_pos, colInd, stackSize);
            return;
          }
        }
      }
  ...
  async commitMove(source, target, stackSize) {
    ...
    if (!(await this.checkStack(target))) {
  ...
  attachEventsToBoardManual() {
    ...
    */
    ...
    $(".card-stack-array").on("click", async function (e) {
      if (selected_stack) {
  ...
  async checkStack(stackNum) {
    ...
    if (!success) {
      while (tempStack.length > 0) {
        let card = tempStack.pop();
        this.game.state.board[stackNum].push(card);
      }
      return false;
    } else {
      await this.updateScore(50);
      ...
      await this.runAnimationQueue(100);
      return true;
    }
  }
  ...
  async handleGameLoop(msg = null) {
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] === "win") {
        ...
        await this.animateFinalVictory();
      ...
      */
      if (mv[0] === "play") {
        if (this.browser_active) {
          ...
          await this.runAnimationQueue(120);
        }
        return 0;
      }
