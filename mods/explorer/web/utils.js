...
async function checkBalance(pubkey = '') {
	if (pubkey) {
		...
		if (balance.hasOwnProperty(pubkey)) {
			document.querySelector('.balance-search-input').placeholder =
				pubkey;
			let balance_nolan = balance[pubkey] || 0;
			let nolan_per_saito = 100000000;
			let balance_saito = formatNumberLocale(
				balance_nolan / nolan_per_saito
			);
			// draw
...
async function balanceAPI(pubkey = '') {
	...
	for (let i = 1; i < data.length; i++) {
		...
		if (balance_list.hasOwnProperty(row[0])) {
			balance_list[row[0]] =
				Number(balance_list[row[0]]) + Number(row[4]);
		} else {
