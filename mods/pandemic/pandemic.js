  ...
  playerMakeMove() {
    ...
    $(".city").on("mouseup", function (e) {
      ...
      if (hops <= pandemic_self.game.state.active_moves) {
        moveOptions.push(
          `Move to ${pandemic_self.skin.cities[selection].name} (${hops} move${
            hops > 1 ? "s" : ""
          })`
        );
        commands.push(() => {
      ...
      switch (action) {
        ...
        case "cards":
          ...
          if (can_play_event_card == 1) {
            html += '<li class="option" id="eventcard">play event card</li>';
          }
          if (can_share_knowledge == 1) {
            html += '<li class="option" id="shareknowledge">share knowledge</li>';
          }
          html += "</ul>";
    ...
    let offerCard = function (card) {
      ...
      for (let i = 0; i < pandemic_self.game.players_info.length; i++) {
        if (pandemic_self.game.players_info[i].city == city && i != pandemic_self.game.player - 1) {
          html += `<li class="nocard" id="${i + 1}">Player ${i + 1} (${
            pandemic_self.game.players_info[i].name
          })</li>`;
          numChoices++;
          choice = i + 1;
        }
  ...
  */
  playEvent(event) {
    ...
    if (event == "event1") {
      html = "<ul>";
      for (let i = 0; i < pandemic_self.game.players_info.length; i++) {
        html += `<li id="player${i + 1}" class="nocard">Player ${i + 1} (${
          pandemic_self.game.players_info[i].name
        })</li>`;
      }
      html += "</ul>";
        ...
        if (slot == 6) {
          ...
          for (let i = 0; i < slot; i++) {
            html += `<li class="nocard" id="${i}">${
              pandemic_self.skin.cities[pandemic_self.game.state.research_stations[i]].name
            }</li>`;
          }
          html += "</ul>";
  ...
  handleGameLoop() {
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] === "interrupt") {
        ...
        if (this.game.player === rudePlayer) {
          ...
          if (this.game.player === victim) {
            this.updateStatusAndListCards(
              `Hang on, ${this.app.keychain.returnUsername(this.game.players[rudePlayer - 1])} 
              (${this.game.players_info[rudePlayer - 1].name}) is playing an event card`,
              this.game.players_info[this.game.player - 1].cards
            );
          } else {
            this.updateStatusAndListCards(
              `Waiting for ${this.app.keychain.returnUsername(this.game.players[rudePlayer - 1])} 
      ...
      if (mv[0] === "move") {
        ...
        this.updateLog(
          `Player ${player} (${this.game.players_info[player - 1].name}) moves to ${
            this.skin.cities[mv[2]].name
          }`
        );
      ...
      if (mv[0] === "treatvirus") {
        ...
        this.updateLog(
          `Player ${player} (${
            this.game.players_info[player - 1].name
          }) removes ${numCubes} ${this.skin.getVirusName(virus)} cube${
            numCubes > 1 ? "s" : ""
          } from ${this.skin.cities[city].name}`
        );
  ...
  triggerOutbreak(city, virus) {
    ...
    let msg = `Outbreak! ${this.skin.getVirusName(virus).toUpperCase()} spreads from ${
      this.skin.cities[city].name
    } to ...`;
        ...
        switch (this.game.state.cities[i].virus[color]) {
          ...
          case 2:
            cubedeath = `<img class="cube" src="${this.skin.returnDiseaseImg(
             ...
             <img class="cube" src="${this.skin.returnDiseaseImg(color)}" style="top:${this.scale(
              10
            )}px;left:${this.scale(0)}px;"/>`;
            break;
          case 3:
            cubedeath = `<img class="cube" src="${this.skin.returnDiseaseImg(
              color
