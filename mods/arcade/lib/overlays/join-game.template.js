module.exports = JoinGameOverlayTemplate = (app, mod, invite) => {
  ...
  for (let i = 0; i < invite.empty_slots; i++) {
    html += `
	        <div class="arcade-game-playerbox saito-table-row${
            app.wallet.publicKey === invite.originator ? " available_slot" : ""
          }">  
  ...
  if (!invite.time_finished) {
    if (mod.isAcceptedGame(invite.game_id)) {
      ...
      if (invite.players.includes(app.wallet.publicKey)) {
        if (app.wallet.publicKey === invite.originator) {
        ...
        invite.empty_slots > 0 ||
        invite.desired_opponent_publickeys.includes(app.wallet.publicKey)
      ) {
