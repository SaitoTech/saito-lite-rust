  ...
  handleGameLoop() {
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] == "init") {
        ...
        // observer skips
        if (this.game.player === 0 || !this.game.players.includes(this.app.wallet.publicKey)) {
          return 1;
        }
            ...
            if (valid_targets == 0) {
              ...
              if (twilight_self.game.player == 1) {
                ...
                twilight_self.updateStatus(
                  `Waiting for USSR to play second ${twilight_self.cardToText("che")} coup`
                );
                return 0;
              }
            }
      ...
      if (mv[0] == "latinamericandebtcrisis") {
        ...
        if (this.game.player == 1) {
          ...
          this.updateStatus(
            "Select " + countries_to_double + " countries in South America to double USSR influence"
          );
      ...
      if (mv[0] == "dynamic_deck_management") {
        ...
        if (
          ...
          (this.game.saito_cards_removed.length > 0 && this.game.options.deck === "saito")
        ) {
      ...
      if (mv[0] === "play_card_or_hand_to_opponent") {
        ...
        if (this.game.player == player) {
          ...
          let html = `<ul>
            <li class="option" id="play">play card</li>
            <li class="option" id="hand">hand to opponent</li>
            </ul>`;
          twilight_self.updateStatusWithOptions(user_message, html, function (action2) {
            if (action2 === "play") {
          ...
          try {
            ...
            if (available_cards.length == 0) {
              ...
              for (let i = 0; i < available_cards.length; i++) {
                html += `<li class="option" id="${available_cards[i]}">${
    ...
    if (stage == "headline6") {
      ...
      if (uscard == "defectors" || this.game.state.defectors_pulled_in_headline == 1) {
        ...
        if (this.game.state.player_to_go == 1) {
          ...
          statusMsg = `US headlines ${this.cardToText(uscard)}. USSR headlines ${this.cardToText(
            ussrcard
          )}`;
          this.updateLog(`US headlines ${this.cardToText(uscard)}.`);
          this.updateLog(`USSR headlines ${this.cardToText(ussrcard)}`);
    ...
    if (twilight_self.game.player == 1) {
      ...
      twilight_self.updateStatusWithOptions(
        ...
        function (action2) {
          ...
          twilight_self.addMove("setvar\tgame\tstate\tevents\tcubanmissilecrisis_cancelled\t1");
          twilight_self.addMove(
            "setvar\tgame\tstate\tevents\tcubanmissilecrisis_removal_country\t" + action2
          );
  ...
  playEvent(player, card) {
    ...
    if (card == "easteuropean") {
      if (this.game.player == 2) {
        ...
        if (options_purge.length <= countries_to_purge) {
          ...
          twilight_self.updateStatus(
            "Remove " + ops_to_purge + " from 3 countries in Eastern Europe"
          );
    ...
    if (card == "marine") {
      ...
      if (this.game.player == 1) {
        ...
        $(".easterneurope").on("click", function () {
          ...
          twilight_self.updateStatus(
            "Remove " + ops_to_purge + " US influence from the Middle East"
          );
        });
      }
      return 0;
    ...
    if (card == "socgov") {
      ...
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          "Remove " + ops_to_purge + " US influence from Western Europe (max 2 per country)"
        );
    ...
    if (card == "summit") {
      ...
      if (us_roll === ussr_roll) {
        ...
        if (this.game.player === winner) {
          ...
          if (this.game.player == 0) {
            this.updateStatus(
              `${this.roles[winner].toUpperCase()} won the ${this.cardToText(card)}`
            );
          } else {
            this.updateStatus(
    ...
    if (card == "august1968") {
      ...
      if (i_played_the_card) {
        ...
        twilight_self.hud.attachControlCallback(function (action2) {
          twilight_self.addMove(
            "NOTIFY\t" + player.toUpperCase() + " purged " + twilight_self.cardToText(action2)
          );
    ...
    if (card == "bayofpigs") {
      if (this.game.player == 1) {
        ...
        for (let i = 0; i < this.game.deck[0].hand.length; i++) {
          if (this.game.deck[0].hand[i] == "grainsales") {
            holding_gs = true;
          }
          if (this.game.deck[0].hand[i] == "voiceofamerica") {
            holding_voa = true;
          }
        }
        ...
        let html = `<ul>`;
        if (holding_voa == true) {
          html += `<li class="option" id="voa">discard Voice of America</li>`;
        }
        if (holding_gs == true) {
          html += `<li class="option" id="gs">discard Grain Sales</li>`;
        }
    ...
    if (card == "energycrisis") {
      ...
      if (this.game.player == 2) {
        ...
        for (let i = 0; i < twilight_self.game.deck[0].hand.length; i++) {
          if (
            this.game.deck[0].cards[this.game.deck[0].hand[i]].ops >= 2 &&
            this.game.deck[0].cards[this.game.deck[0].hand[i]].player === "us"
          ) {
        ...
        twilight_self.updateStatusWithOptions(
          ...
          function (action2) {
            ...
            if (action2 === "discard") {
              ...
              for (let i = 0; i < twilight_self.game.deck[0].hand.length; i++) {
                if (
                  twilight_self.game.deck[0].cards[twilight_self.game.deck[0].hand[i]].ops >= 2 &&
                  twilight_self.game.deck[0].cards[twilight_self.game.deck[0].hand[i]].player ===
                    "us"
                ) {
                  let card_in_hand = twilight_self.game.deck[0].hand[i];
                  if (
                    card_in_hand != "china" &&
                    card_in_hand != twilight_self.game.state.headline_opponent_card &&
                    card_in_hand != twilight_self.game.state.headline_card
                  ) {
                    cardList.push(card_in_hand);
                  }
                }
                    ...
                    if (me === "us") {
                      if (twilight_self.isControlled("ussr", c)) {
                        let max_influence_possible =
                          ...
                          twilight_self.game.countries[c].control -
                          1;
                        while (
                          twilight_self.game.countries[c].us + max_influence_to_add >
                    ...
                    if (me === "ussr") {
                      if (twilight_self.isControlled("us", c)) {
                        let max_influence_possible =
                          ...
                          1;
                        while (
                          twilight_self.game.countries[c].ussr + max_influence_to_add >
                          max_influence_possible
                        ) {
                          max_influence_to_add--;
                        }
    ...
    if (card == "revolutionsof1989") {
      ...
      if (this.game.player == 2) {
        ...
        this.updateStatusWithOptions(
          `${this.cardToText(card)}: do you want to trigger Final Scoring?`,
          choicehtml,
          function (action2) {
            if (action2 == "endgame") {
    ...
    if (card == "poliovaccine") {
      ...
      if (my_go == 1) {
        ...
        for (let i = 0; i < this.game.deck[0].hand.length; i++) {
          if (this.game.deck[0].hand[i] != "china") {
            html += `<li class="option" id="${this.game.deck[0].hand[i]}">${
    ...
    if (card == "eurocommunism") {
      ...
      if (this.game.player == 2) {
        ...
        if (available_targets < 4) {
          ...
          this.updateStatus(
            "Remove " + ops_to_purge + " USSR influence from Western Europe (max 2 per country)"
          );
        }
