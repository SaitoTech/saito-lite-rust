...
class Twilight extends GameTemplate {
  ...
  handleDisplayMenu() {
    ...
    if (twilight_self.useClock == 1) {
      use_clock_html = '<li class="menu-item" id="move_clock">Move Clock</li>';
    }
    ...
    $(".menu-item").on("click", async function () {
      ...
      if (action2 === "text") {
        await twilight_self.displayModal("Card Menu options changed to text-mode. Please reload.");
      ...
      if (action2 === "graphics") {
        await twilight_self.displayModal(
  ...
  async initializeHTML(app) {
    ...
    this.menu.addSubMenuOption("game-confirm", {
      ...
      callback: async function (app, game_mod) {
        if (game_mod.confirm_moves == 0) {
          await game_mod.displayModal("Game Settings", "Will confirm moves before committing");
    ...
    this.menu.addSubMenuOption("game-confirm", {
      ...
      callback: async function (app, game_mod) {
        if (game_mod.confirm_moves == 1) {
          await game_mod.displayModal("Game Settings", "No need to confirm moves");
    ...
    this.menu.addSubMenuOption("game-language", {
      ...
      id: "game-language-en",
      callback: async function (app, game_mod) {
        await game_mod.displayModal("Language Settings", "Card settings changed to English");
        game_mod.lang = "en";
    ...
    this.menu.addSubMenuOption("game-language", {
      ...
      id: "game-language-zh",
      callback: async function (app, game_mod) {
        await game_mod.displayModal("语言设定", "卡牌语言改成简体中文");
        game_mod.lang = "zh";
  ...
  async handleGameLoop() {
    ...
    if (this.game.over == 1) {
      ...
      if (gid === this.game.id) {
        this.updateStatus(
          "<div class='status-message' id='status-message'><span>Game Over:</span> " +
            winner.toUpperCase() +
            "</span> <span>wins</span></div>"
        );
      }
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] == "wargames") {
        ...
        if (activate == 0) {
          ...
          await this.updateVictoryPoints();
          ...
          if (this.game.options.deck === "late-war") {
            if (this.game.state.vp < 20) {
              await this.endGame(this.game.players[0], "Wargames");
            } else {
              await this.endGame(this.game.players[1], "Wargames");
            }
            return 0;
          }
      ...
      if (mv[0] === "grainsales") {
        ...
        if (mv[1] == "ussr") {
          ...
          if (this.game.player == 2) {
            ...
            let user_message = `${this.cardToText(mv[0])} pulls ${this.cardToText(
              mv[2]
      ...
      if (mv[0] == "checoup") {
        ...
        // results.
        //
        await twilight_self.playCoup("ussr", mv[2], couppower, function () {
              ...
              if (twilight_self.game.player == 1) {
                ...
                twilight_self.attachCardboxEvents(function (action2) {
                  if (action2 == "skipche") {
                    ...
                    twilight_self.updateStatus(
                      "<div class='status-message' id='status-message'>Skipping Che coups...</div>"
                    );
                  }
                });
                ...
                $(".easterneurope").on("click", function () {
                  ...
                  twilight_self.addMove(
                    "NOTIFY\tChe launches coup in " + twilight_self.countries[c].name
                  );
                  twilight_self.endTurn();
                });
              } else {
                twilight_self.updateStatus(
                  `<div class="status-message" id="status-message">Waiting for USSR to play second ${twilight_self.cardToText(
                    "che"
                  )} coup</div>`
                );
      ...
      if (mv[0] === "missileenvy") {
        ...
        let opponent_card = 0;
        if (this.game.deck[0].cards[card].player == "us" && sender == 2) {
          opponent_card = 1;
        }
        if (this.game.deck[0].cards[card].player == "ussr" && sender == 1) {
          opponent_card = 1;
        }
        ...
        if (opponent_card == 1) {
          ...
          this.game.queue.push(
            "ops\t" + receiver + "\t" + card + "\t" + this.game.deck[0].cards[card].ops
          );
          this.game.queue.push(
            "NOTIFY\t" +
      ...
      if (mv[0] === "quagmire") {
        ...
        this.updateLog(
          `${this.cardToText(sticker)}: ${mv[1].toUpperCase()} discards ${this.cardToText(
            discard
          )} and rolls a ${roll}`
        );
        ...
        if (roll < 5) {
          ...
          await this.displayModal(
            `${mv[1].toUpperCase()} extricates itself from ${this.cardToText(sticker)}`
          );
        } else {
          this.updateLog(`${this.cardToText(sticker)} continues...`);
          await this.displayModal(
            `${mv[1].toUpperCase()} remains stuck in the ${this.cardToText(sticker)}`
          );
        }
          ...
          if (this.game.player == 1) {
            ...
            for (let i = 0; i < keysnum; i++) {
              cardoptions[i] = this.app.crypto.decodeXOR(
                cardoptions[i],
                this.game.queue[this.game.queue.length - keysnum + i]
              );
      ...
      if (mv[0] == "latinamericandebtcrisis") {
        ...
        if (this.game.player == 1) {
          ...
          this.updateStatus(
            "<div class='status-message' id='status-message'>Select " +
              countries_to_double +
              " countries in South America to double USSR influence</div>"
          );
      ...
      if (mv[0] == "space") {
        await this.playerSpaceCard(mv[2], mv[1]);
        if (mv[2] != "china") {
      ...
      if (mv[0] == "chernobyl") {
        ...
        if (this.game.player == 1) {
          await this.displayModal(
            "Chernobyl",
            `Influence placements restricted in ${this.region_key[mv[1]]}`
          );
        }
      }
          ...
          if (this.game.player == 1) {
            ...
            this.updateStatus(
              `<div class="status-message" id="status-message">${this.cardToText(
                "aldrichames"
              )}: USSR choosing card to discard</div>`
            );
          }
      ...
      if (mv[0] === "cambridge") {
        if (this.game.player == 1) {
          ...
          let placetxt = `<div class="status-message" id="status-message">${this.cardToText(
            "cambridge"
      ...
      if (mv[0] === "teardownthiswall") {
        if (this.game.player == 2) {
          ...
          twilight_self.attachCardboxEvents(function (action2) {
            if (action2 == "skiptear") {
              twilight_self.updateStatus(
                "<div class='status-message' id='status-message'>Skipping Tear Down this Wall...</div>"
              );
          ...
          this.updateStatus(
            "<div class='status-message' id='status-message'>US playing Tear Down This Wall</div>"
          );
        }
        shd_continue = 0;
      }
      ...
      if (mv[0] === "reshuffle") {
        ...
        if (this.game.deck[0].crypt.length < reshuffle_limit) {
          ...
          if (this.game.state.round != 4 && this.game.state.round != 8) {
            ...
            this.updateStatus(
              "<div class='status-message' id='status-message'>Dealing remaining cards from draw deck before reshuffling...</div>"
            );
            this.updateLog(
      ...
      if (mv[0] === "ops") {
        ...
        if (mod_ops > orig_ops) {
          this.updateLog(
            mv[1].toUpperCase() +
              ` plays ${this.game.state.event_name} for ${mv[3]} OPS (+${mod_ops - orig_ops} bonus)`
          );
          ...
          this.updateLog(
            mv[1].toUpperCase() + ` plays ${this.game.state.event_name} for ${mv[3]} OPS`
          );
        }
      ...
      if (mv[0] === "coup") {
        ...
        this.updateLog(
          mv[1].toUpperCase() + " coups " + this.countries[mv[2]].name + " with " + mv[3] + " OPS"
        );
        ...
        if (this.game.state.limit_milops != 1) {
          ...
          //
          if (mv[1] == "us") {
            this.game.state.milops_us += this.modifyOps(parseInt(mv[3]), card, "us");
          }
          if (mv[1] == "ussr") {
            this.game.state.milops_ussr += this.modifyOps(parseInt(mv[3]), card, "ussr");
          }
        ...
        //
        await this.playCoup(mv[1], mv[2], parseInt(mv[3]));
        this.game.queue.splice(qe, 1);
      }

      if (mv[0] === "realign") {
        if (mv[1] != player) {
          //Other player needs to "check the math"
      ...
      if (mv[0] === "placement") {
        ...
        if (this.game.player == mv[1]) {
          ...
          this.updateStatusAndListCards(
            `${mv[1] == 1 ? "USSR" : "US"} is making its initial placement of influence:`
          );
        }
      ...
      if (mv[0] === "placement_bonus") {
        ...
        if (this.game.player == mv[1]) {
          ...
          this.updateStatusAndListCards(
            `${mv[1] == 1 ? "USSR" : "US"} is making its bonus placement of ${mv[2]} influence`
          );
        }
      ...
      if (mv[0] === "round") {
          ...
          if (this.game.state.us_defcon_bonus == 1) {
            ...
            if (this.isControlled("us", "canada") == 1) {
              ...
              if (this.game.player == 2) {
                ...
                this.updateStatus(
                  "<div class='status-message' id='status-message'>Place your NORAD bonus: (1 OP)</div>"
                );
                ...
                this.updateStatus(
                  "<div class='status-message' id='status-message'>NORAD triggers: US places 1 influence in country with US influence</div>"
                );
              }
              return 0;
            }
        ...
        if (this.game.state.events.northseaoil_bonus == 1) {
          ...
          if (this.game.player == 2) {
            ...
            this.updateStatus(
              "<div class='status-message' id='status-message'>US is deciding whether to take extra turn</div>"
            );
          }
          ...
          try {
            ...
            if (this.game.player != bonus_player) {
              this.updateStatus(
                '<div class="status-message" id="status-message">' +
                  ...
                  " is deciding whether to discard a card</div>"
              );
              return 0;
            }
            ...
            if (available_cards.length == 0) {
              ...
              for (let i = 0; i < available_cards.length; i++) {
                html += `<li class="card" id="${available_cards[i]}">${
                  twilight_self.game.deck[0].cards[available_cards[i]].name
                }</li>`;
              }
              ...
              twilight_self.attachCardboxEvents(function (action2) {
                if (action2 == "nope") {
                  ...
                  twilight_self.updateStatus(
                    "<div class='status-message' id='status-message'>Discarding...</div>"
                  );
                  twilight_self.removeCardFromHand(action2);
                  twilight_self.addMove(
                    "discard\t" + twilight_self.game.state.eagle_has_landed + "\t" + action2
                  );
                  twilight_self.addMove(
                    "NOTIFY\t" +
                      twilight_self.game.state.eagle_has_landed.toUpperCase() +
                      ` discards ${twilight_self.cardToText(action2)}`
                  );
                  twilight_self.endTurn(1);
                  return 0;
                }
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Preparing for round " +
          ...
          if (this.isControlled("us", "canada") == 1) {
            ...
            */
            if (this.game.player == 2) {
              ...
              this.updateStatus(
                "<div class='status-message' id='status-message'>US place NORAD bonus: (1 OP)</div>"
              );
      ...
      if (mv[0] === "modal") {
        this.game.queue.splice(qe, 1);
        await this.displayModal(mv[1], mv[2]);
        return 1;
      }
      ...
      if (mv[0] === "sharehandsize") {
        ...
        if (this.game.player == player) {
          ...
          this.addMove(
            "setvar\t" + this.game.player + "\topponent_cards_in_hand\t" + cards_in_hand
          );
          this.endTurn();
        }
  ...
  playHeadlinePostModern(stage, hash = "", xor = "", card = "") {
    ...
    if (this.game.state.man_in_earth_orbit == "") {
      ...
      if (stage == "headline2") {
        this.updateLog(
          this.game.state.man_in_earth_orbit.toUpperCase() + " selecting headline card second"
        );
    ...
    if (stage == "headline6") {
      ...
      if (uscard == "defectors") {
        ...
        this.updateStatus(
          `<div class="status-message" id="status-message">${this.cardToText(
            "defectors"
          )} cancels USSR headline. Moving into first turn...</div>`
        );
      } else {
        let statusMsg = "";
        if (this.game.state.player_to_go == 1) {
          statusMsg = `<div class="status-message" id="status-message">USSR headlines ${this.cardToText(
            ussrcard
          )}. US headlines ${this.cardToText(uscard)}</div>`;

          this.updateLog(`USSR headlines ${this.cardToText(ussrcard)}.`);
          this.updateLog(`US headlines ${this.cardToText(uscard)}`);
        } else {
          statusMsg = `<div class="status-message" id="status-message">US headlines ${this.cardToText(
            uscard
          )}. USSR headlines ${this.cardToText(ussrcard)}</div>`;
          this.updateLog(`US headlines ${this.cardToText(uscard)}.`);
          this.updateLog(`USSR headlines ${this.cardToText(ussrcard)}`);
        }
    ...
    if (stage == "headline7") {
      ...
      let statusMsg = "";
      if (this.game.state.player_to_go == 1) {
        statusMsg = `<div class="status-message" id="status-message">Resolving USSR headline: ${this.cardToText(
          ussrcard
        )}</div>`;
      } else {
        statusMsg = `<div class="status-message" id="status-message">Resolving US headline: ${this.cardToText(
          uscard
        )}</div>`;
      }
  ...
  playerPickHeadlineCard() {
    ...
    twilight_self.attachCardboxEvents(async function (card) {
      if (twilight_self.confirm_moves == 1) {
  ...
  async playerTurnHeadlineSelected(card, player) {
    ...
    if (card == "china") {
      await this.displayModal("Invalid Headline", "You cannot headline China");
      return;
    }
    if (card == "unintervention") {
      await this.displayModal("Invalid Headline", "You cannot headline UN Intervention");
      return;
    }
    ...
    twilight_self.game.state.headline_hash = twilight_self.app.crypto.encodeXOR(
      twilight_self.app.crypto.stringToHex(twilight_self.game.state.headline_card),
    ...
    if (this.game.state.man_in_earth_orbit) {
      ...
      let card_sig = twilight_self.app.crypto.signMessage(
        card,
        twilight_self.app.wallet.returnPrivateKey()
      );
      ...
      twilight_self.addMove(
        "SIMULTANEOUS_PICK\t" + twilight_self.game.player + "\t" + hash3 + "\t" + hash3_sig
      );
    }
    ...
    twilight_self.endTurn();
    twilight_self.updateStatus(
      "<div class='status-message' id='status-message'>simultaneous blind pick... encrypting selected card</div>"
    );
  ...
  playerTurn(selected_card = null) {
      ...
      if (this.game.deck[0].hand.length == 1) {
        if (this.game.deck[0].hand[0] == "china") {
          if (is_player_skipping_playing_china_card) {
            ...
            let html =
              '<ul><li class="card" id="play">play card</li><li class="card" id="skipturn">skip turn</li></ul>';
            this.updateStatusWithOptions(user_message, html, false);
    ...
    if (
      ...
      user_message = `Select a card for ${
        this.game.player == 1 ? this.cardToText("beartrap") : this.cardToText("quagmire")
      }: `;
    ...
    if (selected_card == null && this.game.deck[0].hand.length == 0) {
      ...
      this.endTurn();
      this.updateStatus(
        "<div class='status-message' id='status-message'>Skipping turn... no cards left to play</div>"
      );
      return;
    }

    twilight_self.attachCardboxEvents(async function (card) {
      await twilight_self.playerTurnCardSelected(card, player);
    });
  }
  ...
  async playerTurnCardSelected(card, player) {
    ...
    if (
      ...
      twilight_self.game.deck[0].cards[card]?.scoring == 0
    ) {
    ...
    if (
      ...
      ((twilight_self.game.state.events.quagmire == 1 && twilight_self.game.player == 2) ||
        (twilight_self.game.state.events.beartrap == 1 && twilight_self.game.player == 1))
    ) {
      //
    ...
    if (twilight_self.game.state.events.unintervention === 1) {
      ...
      twilight_self.addMove(
        "ops\t" + player + "\t" + card + "\t" + twilight_self.game.deck[0].cards[card].ops
      );
      twilight_self.removeCardFromHand(card);
      twilight_self.endTurn();
      return;
    ...
    if (twilight_self.game.deck[0].cards[card]?.scoring == 1) {
      ...
      let ops = twilight_self.modifyOps(
        twilight_self.game.deck[0].cards[card].ops,
        card,
        player,
        0
      );
      ...
      if (card == "unintervention") {
        ...
        for (let b = 0; b < twilight_self.game.deck[0].hand.length; b++) {
          ...
          if (tmpc != "china") {
            if (twilight_self.game.player == 1) {
              if (twilight_self.game.deck[0].cards[tmpc].player === "us") {
                opponent_event_in_hand = 1;
              }
            } else {
              if (twilight_self.game.deck[0].cards[tmpc].player === "ussr") {
                opponent_event_in_hand = 1;
              }
            }
          }
      ...
      if (card == "missileenvy" && is_this_missile_envy_noneventable == 1) {
        can_play_event = 0;
      }
      ...
      if (
        ...
        twilight_self.game.state.events.cubanmissilecrisis === twilight_self.game.player
      ) {
      ...
      if (can_play_event == 1) {
        announcement += '<li class="card" id="event">play event</li>';
      }
      ...
      let header_msg = `${player.toUpperCase()} playing <span>${
        twilight_self.game.deck[0].cards[card].name
      }</span>`;
    ...
    twilight_self.attachCardboxEvents(async function (action) {
      ...
      //
      if (
        (twilight_self.game.player == 2 && twilight_self.game.state.events.missile_envy == 2) ||
        (twilight_self.game.player == 1 && twilight_self.game.state.events.missile_envy == 1)
      ) {
        if (card == "missileenvy") {
      ...
      if (action == "cancel_cmc") {
        ...
        await twilight_self.playerTurnCardSelected(card, player); //And shortcut back to our position
        return;
      }
      ...
      if (action == "event") {
        ...
        //
        if (
          twilight_self.game.deck[0].cards[card].player != "both" &&
          twilight_self.game.deck[0].cards[card].player != player
        ) {
          let fr_header =
      ...
      if (action == "ops") {
        ...
        if (
          ...
          (card != "missileenvy" || is_this_missile_envy_noneventable == 0)
        ) {
    ...
    if (player === me) {
      ...
      twilight_self.attachCardboxEvents(async function (action2) {
        ...
        if (action2 == "cancel_cmc") {
          ...
          if (
            twilight_self.game.queue[twilight_self.game.queue.length - 1].split("\t")[0] === "ops"
          ) {
        ...
        if (action2 == "coup") {
          ...
          //
          if (
            (twilight_self.game.state.events.cubanmissilecrisis == 2 && player == "us") ||
        ...
        if (action2 == "realign") {
          ...
          twilight_self.attachCardboxEvents(function (action2) {
            if (action2 == "cancelrealign") {
              twilight_self.addMove(
                "NOTIFY\t" + player.toUpperCase() + " opts to end realignments"
              );
              twilight_self.endTurn();
              return;
            }
          ...
          $(".country").on("click", async function () {
            ...
            //
            if (
              twilight_self.game.state.limit_region.indexOf(twilight_self.countries[c].region) > -1
            ) {
            ...
            if (
              ...
              if (
                twilight_self.countries[c].region == "mideast" &&
            ...
            if (
              ...
              if (twilight_self.isControlled("us", c) == 1) {
                if (
                  (c == "westgermany" && twilight_self.game.state.events.nato_westgermany == 0) ||
                  (c == "france" && twilight_self.game.state.events.nato_france == 0)
                ) {
            ...
            if (
              (player == "us" && twilight_self.countries[c].ussr <= 0) ||
              (player == "ussr" && twilight_self.countries[c].us <= 0)
            ) {
              failureReason = "No enemy influence";
            }

            if (failureReason.length > 0) {
              await twilight_self.displayModal(coupHeader, failureReason);
            } else {
              //
              // vietnam revolts and china card bonuses
              //
              if (twilight_self.countries[c].region !== "seasia") {
              ...
              twilight_self.updateStatusWithOptions(
                ...
                `<ul><li class="card" id="cancelrealign">end turn</li></ul>`,
                false
              );
              twilight_self.attachCardboxEvents(function (action2) {
                if (action2 == "cancelrealign") {
      ...
      this.updateStatusAndListCards(
        `${player.toUpperCase()} playing ${this.game.state.event_name} for ${ops} OPS`
      );
      this.attachCardboxEvents();
    }
  ...
  formatPlayOpsStatus(player, ops) {
    ...
    if (this.game.state.limit_placement == 0) {
      html += '<li class="card" id="place">place influence</li>';
    }
    if (this.game.state.limit_coups == 0) {
      html += '<li class="card" id="coup">launch coup</li>';
    }
    if (this.game.state.limit_realignments == 0) {
      html += '<li class="card" id="realign">realign country</li>';
    }
    if (this.game.state.events.unintervention == 1) {
  ...
  */
  ...
  revertTurn() {
    ...
    if (start_turn_game_state == null || start_turn_game_state == undefined) {
    } else {
  ...
  playerTriggerOps(player, card) {
    ...
    if (twilight_self.game.state.events.flowerpower == 1 && player === "us") {
      if (
        (card == "arabisraeli" && twilight_self.game.state.events.campdavid == 0) ||
        card == "koreanwar" ||
        card == "brushwar" ||
        card == "indopaki" ||
        card == "iraniraq"
    ...
    if (twilight_self.game.deck[0].cards[card].player == opponent) {
      let html =
        '<ul><li class="card" id="before_ops">event before ops</li><li class="card" id="after_ops">event after ops</li></ul>';
      twilight_self.updateStatusWithOptions("Playing opponent card:", html, true);

      twilight_self.bindBackButtonFunction(async () => {
        await twilight_self.playerTurnCardSelected(card, player);
      });
  ...
  */
  ...
  removeInfluence(country, inf, player, mycallback = null) {
    ...
    this.updateLog(
      player.toUpperCase() + " removes " + inf + " from " + this.countries[country].name
    );
  ...
  placeInfluence(country, inf, player, mycallback = null) {
    ...
    this.updateLog(
      player.toUpperCase() + " places " + inf + " in " + this.countries[country].name,
      1
    );
  ...
  playerPlaceInfluence(player, mycallback = null) {
    ...
    for (var i in this.countries) {
      ...
      if (this.game.state.limit_region.indexOf(this.countries[i].region) > -1) {
        restricted_country = 1;
      }
        ...
        if (player == "us") {
          ...
          $(divname).on("mouseup", async function (e) {
            ...
            if (twilight_self.countries[countryname].place == 1) {
              ...
              //
              twilight_self.game.state.events.vietnam_revolts_eligible = 0;
              if (twilight_self.countries[countryname].region.indexOf("asia") < 0) {
                twilight_self.game.state.events.china_card_eligible = 0;
              }
                  ...
                  if (twilight_self.countries[countryname].us >= 1) {
                    ...
                    let removeinf = await sconfirm(
                      "You are placing 1 influence in " +
                        twilight_self.countries[countryname].name +
              ...
              await twilight_self.displayModal("you cannot place there...");
              return;
            }
          });
        } else {
          ...
          $(divname).on("mouseup", async function (e) {
            ...
            if (twilight_self.countries[countryname].place == 1) {
              ...
              //
              if (twilight_self.countries[countryname].region !== "seasia") {
                twilight_self.game.state.events.vietnam_revolts_eligible = 0;
              }
              if (twilight_self.countries[countryname].region.indexOf("asia") < 0) {
                twilight_self.game.state.events.china_card_eligible = 0;
              ...
              await twilight_self.displayModal("Invalid Target");
              return;
            }
          });
  ...
  isSpaceRaceAvailable(ops) {
    ...
    if (this.game.player == 2 && this.game.state.space_race_us_counter >= 1) {
      if (this.game.state.animal_in_space == "us" && this.game.state.space_race_us_counter < 2) {
      } else {
        sre = 0;
      }
    }
    ...
    if (sre == 1) {
      let myspacerace =
        this.game.player == 1 ? this.game.state.space_race_ussr : this.game.state.space_race_us;
  ...
  async playerSpaceCard(card, player) {
    let card_ops = this.game.deck[0].cards[card].ops;

    // stats
    if (player == "us") {
      this.game.state.stats.us_ops_spaced += this.modifyOps(card_ops, card, player);
    }
    if (player == "ussr") {
      this.game.state.stats.ussr_ops_spaced += this.modifyOps(card_ops, card, player);
    }
    ...
    if (successful == 1) {
      ...
      await this.advanceSpaceRace(player);
    } else {
      this.updateLog(player.toUpperCase() + ` fails in the Space Race with roll ${roll}`);
      await this.displayModal(player.toUpperCase() + " fails to advance in the Space Race");
    }
  }
  ...
  playerCoupCountry(player, ops, card) {
    ...
    $(".country").on("click", async function () {
      ...
      if (twilight_self.game.state.limit_ignoredefcon == 0) {
        ...
        if (
          (twilight_self.countries[countryname].region == "asia" ||
            twilight_self.countries[countryname].region == "seasia") &&
          twilight_self.game.state.defcon < 4
        ) {
      ...
      */
        ...
        if (twilight_self.isControlled("us", countryname) == 1) {
          if (
            (countryname == "westgermany" &&
              twilight_self.game.state.events.nato_westgermany == 0) ||
            (countryname == "france" && twilight_self.game.state.events.nato_france == 0)
          ) {
            //If West Germany or France have been removed from Nato (by Degaulle or WillyBrandt), then one can coup there....
          } else {
      ...
      if (
        (player == "us" && twilight_self.countries[countryname].ussr <= 0) ||
        (player == "ussr" && twilight_self.countries[countryname].us <= 0)
      ) {
        failureReason = "No enemy influence";
      }
      ...
      if (failureReason.length > 0) {
        ...
        if (
          card == "china" &&
          (twilight_self.game.countries[countryname].region == "asia" ||
  ...
  async playCoup(player, countryname, ops, mycallback = null) {
    let roll = this.rollDice(6);
    let modifier = 0;
    ...
    if (player == "ussr" && this.game.state.events.cubanmissilecrisis == 1) {
      await this.endGame(this.game.players[1], "Cuban Missile Crisis");
      return;
    }
    if (player == "us" && this.game.state.events.cubanmissilecrisis == 2) {
      await this.endGame(this.game.players[0], "Cuban Missile Crisis");
      return;
    }
    ...
    if (this.game.state.events.deathsquads != 0) {
      if (this.game.state.events.deathsquads <= -1) {
        ...
        if (
          ...
          if (player == "ussr") {
            this.updateLog(
              `${this.cardToText("deathsquads")} triggers: USSR +${roll_modifier} modifier`
            );
          ...
          if (player == "us") {
            if (
              ...
              this.updateLog(
                `${this.cardToText("deathsquads")} triggers: US -${roll_modifier} modifier`
              );
              modifier -= roll_modifier;
            }
          }
    ...
    if (winning > 0) {
      ...
      let gained = 0;
      this.updateLog(
        `${player.toUpperCase()}'s coup succeeds in ${this.countries[countryname].name}.`
      );
      ...
      if (
        ...
        await this.displayModal(`COUP FAILS!`);
      } else {
        await this.displayModal(
          `COUP in ${this.countries[countryname].name}!`,
  ...
  playRealign(player, country) {
    ...
    if (roll_us > roll_ussr) {
      ...
      this.updateLog(
        `Realignment roll is a draw! US [${roll_us - bonus_us} + ${bonus_us}] = USSR [${
  ...
  endTurn(nextTarget = 0) {
    ...
    this.updateStatus(
      "<div class='status-message' id='status-message'>Submitting moves... awaiting response from peers...</div>"
    );
  ...
  returnCountries() {
    ...
    countries["westgermany"] = {
      ...
      control: 4,
      bg: 1,
      neighbours: ["austria", "france", "benelux", "denmark", "eastgermany"],
      region: "europe",
      name: "West Germany",
    };
  ...
  returnMidWarCards() {
    ...
    if (this.game.options != undefined) {
      for (var key in this.game.options) {
        ...
        // ABSURDUM
        if (key === "kissingerisawarcriminal") {
          deck["kissingerisawarcriminal"] = {
            img: "TNRnTS-502png",
  ...
  isRegionBonus(card = "") {
    ...
    if (
      ...
      // https://boardgamegeek.com/thread/1136951/red-scarepurge-and-vietnam-revolts
      if (card != "") {
        if (this.returnOpsOfCard(card) == 1 && this.game.state.events.redscare_player1 >= 1) {
          return 0;
        }
      }

      this.updateStatus(
        "<div class='status-message' id='status-message'>Extra 1 OP Available for Southeast Asia</div>"
      );
    ...
    if (
      ...
      this.updateStatus(
        "<div class='status-message' id='status-message'>Extra 1 OP Available for Asia</div>"
      );
  ...
  */
  ...
  async finalScoring() {
    ...
    if (this.whoHasTheChinaCard() == "ussr") {
      ...
      if (this.game.state.vp <= -20) {
        await this.endGame(this.game.players[0], "victory points");
        return;
      }
    } else {
      this.game.state.vp++;
      ...
      if (this.game.state.vp >= 20) {
        await this.endGame(this.game.players[1], "victory points");
        return;
      }
    }
    ...
    if (this.game.options.deck === "late-war") {
      if (this.game.state.vp < 20) {
        await this.endGame(this.game.players[0], "final scoring");
      } else {
        await this.endGame(this.game.players[1], "final scoring");
      }
    }
  ...
  */
  doesPlayerDominateRegionForSummit(player, region) {
    ...
    if (region === "europe") {
      ...
      if (bg_ussr > bg_us && total_ussr > bg_ussr && total_ussr > total_us) {
        vp_ussr = 7;
      }
  ...
  lowerDefcon() {
    ...
    //
    if (
      (this.game.state.events.manwhosavedtheworld === "us" && this.game.state.turn == 2) ||
      (this.game.state.events.manwhosavedtheworld === "ussr" && this.game.state.turn == 1)
    ) {
  ...
  async advanceSpaceRace(player, withOSS = false) {
    await this.displayModal(`${player.toUpperCase()} advances in the Space Race`);
  ...
  async updateVictoryPoints() {
    ...
    if (this.game.state.vp > 19) {
      await this.endGame(this.game.players[1], "victory point track");
    }
    if (this.game.state.vp < -19) {
      await this.endGame(this.game.players[0], "victory point track");
    }
  }
  ...
  cardToText(cardname, textonly = false) {
    let card =
      this.game.deck[0].cards[cardname] ||
      this.game.deck[0].discards[cardname] ||
      this.game.deck[0].removed[cardname];
    try {
      if (textonly) {
        return card.name;
      } else {
  ...
  // queued up to go.
  //
  async playEvent(player, card) {
    if (this.game.deck[0].cards[card] != undefined) {
      this.updateStatus(
        `<div class="status-message" id="status-message">${player.toUpperCase()} triggers ${this.cardToText(
          card
        )}</div>`
      );
      this.attachCardboxEvents();
    ...
    if (card == "aldrichames") {
      ...
      if (this.game.player == 2) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>USSR is playing Aldrich Ames</div>"
        );
        ...
        for (let i = 0; i < this.game.deck[0].hand.length; i++) {
          if (
            ...
            this.game.deck[0].hand[i] !== "china" &&
            this.game.deck[0].hand[i] !== this.game.state.headline_opponent_card &&
            this.game.deck[0].hand[i] !== this.game.state.headline_card
          ) {
            cards_to_reveal++;
            if (revealed != "") {
              revealed += ", ";
    ...
    if (card == "armsrace") {
      ...
      await this.displayModal(this.cardToText(card), `${player.toUpperCase()} gains ${bonus} VP`);
      if (!i_played_the_card) {
        this.game.queue.push(
          `ACKNOWLEDGE\t${player.toUpperCase()} plays ${this.cardToText(card)}.`
        );
      }
    ...
    if (card == "asknot") {
      if (this.game.player == 2) {
        ...
        for (let i = 0; i < this.game.deck[0].hand.length; i++) {
          ...
          if (
            card_in_hand != "china" &&
            card_in_hand != this.game.state.headline_opponent_card &&
          ...
          if (action2 == "finished") {
            ...
            if (cards_to_deal_before_reshuffle > twilight_self.game.deck[0].crypt.length) {
              ...
              if (Object.keys(discarded_cards).length > 0) {
                ...
                twilight_self.addMove("DECKBACKUP\t1");
                twilight_self.addMove(
                  "NOTIFY\tcards remaining: " + twilight_self.game.deck[0].crypt.length
                );
    ...
    if (card == "blockade") {
      ...
      if (this.game.state.events.optional.berlinagreement == 1) {
        this.updateLog(
          `${this.cardToText("berlinagreement")} prevents ${this.cardToText("blockade")}.`
        );
        return 1;
      }
      ...
      if (this.game.player == 2) {
        ...
        this.updateStatusWithOptions(
          ...
          '<ul><li class="card" id="discard">discard 3 OP card</li><li class="card" id="remove">remove all US influence in W. Germany</li></ul>',
          false
        );
        twilight_self.attachCardboxEvents(function (action) {
          if (action == "discard") {
            ...
            twilight_self.attachCardboxEvents(function (card) {
              ...
              twilight_self.addMove(
                `NOTIFY\tUS discarded ${twilight_self.cardToText(
                  card
                )} to resolve ${twilight_self.cardToText("blockade")}`
              );
              twilight_self.endTurn();
              return 0;
            });
          ...
          if (action == "remove") {
            twilight_self.addMove(
              "remove\tus\tus\twestgermany\t" + twilight_self.countries["westgermany"].us
            );
            ...
            $(divname).on("click", async function () {
              ...
              if (c === "italy" || c === "greece" || c === "spain" || c === "turkey") {
                if (twilight_self.game.state.events.nato == 1) {
                  if (twilight_self.isControlled("us", c) == 1) {
                    await twilight_self.displayModal("NATO prevents Brush War in Europe");
                    return;
                  }
                }
              }
              ...
              for (let v = 0; v < twilight_self.countries[c].neighbours.length; v++) {
                if (
                  twilight_self.isControlled(opponent, twilight_self.countries[c].neighbours[v]) ==
                  1
                ) {
                  modifications++;
                }
              ...
              if (die >= 3 + modifications) {
                ...
                twilight_self.addMove(
                  "NOTIFY\tBrush War in " + twilight_self.countries[c].name + " succeeded."
                );
              } else {
                ...
                twilight_self.addMove(
                  "NOTIFY\tBrush War in " + twilight_self.countries[c].name + " failed."
                );
              }
              ...
              twilight_self.addMove(
                "NOTIFY\t" +
                  player.toUpperCase() +
                  ` rolls for Brush War: ${die}, adjusted: ${die - modifications}`
              );
              twilight_self.addMove(`war\t${card}\t${winner}\t${die}\t${modifications}\t${player}`);
              twilight_self.endTurn();
            });
    ...
    if (card == "cambridge") {
      if (this.game.state.round > 7) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'><span>The Cambridge Five cannot be played as an event in Late War</span></div>"
        );
        return 1;
      }
      ...
      if (this.game.player == 2) {
        ...
        if (revealed.length == 0) {
          ...
          this.endTurn();
          this.updateStatus(
            `<div class="status-message" id="status-message">USSR is placing influence for ${this.cardToText(
              card
            )}</div>`
          );
        }
      ...
      if (this.game.player == 1) {
        ...
        twilight_self.updateStatus(
          "<div class='status-message' id='status-message'>Place " +
            ops_to_place +
            " influence in non-US controlled countries in Eastern Europe (1 per country)</div>"
        );
    ...
    if (card == "easteuropean") {
      if (this.game.player == 2) {
        ...
        if (options_purge.length <= countries_to_purge) {
          ...
          twilight_self.updateStatus(
            "<div class='status-message' id='status-message'>Remove " +
              ops_to_purge +
              " from 3 countries in Eastern Europe</div>"
          );
      ...
      if (this.game.player == 1) {
        ...
        for (let i = 0; i < twilight_self.game.deck[0].hand.length; i++) {
          ...
          if (
            ...
            !(
              this.game.state.headline == 1 &&
              (thiscard == this.game.state.headline_opponent_card ||
        ...
        if (available_cards.length == 0) {
          ...
          twilight_self.rollDice(available_cards.length, function (roll) {
            ...
            if (twilight_self.game.deck[0].cards[card].player == "us") {
              ...
              twilight_self.addMove(
                "modal\tFive Year Plan\tUSSR triggers " + twilight_self.cardToText(card)
              );
              twilight_self.addMove(
                "NOTIFY\tFive Year Plan triggers US event: " + twilight_self.cardToText(card)
              );
              twilight_self.endTurn();
            } else {
              //twilight_self.displayModal("You have rolled: " + card);
              twilight_self.addMove(
                "modal\tFive Year Plan\tUSSR discards " + twilight_self.cardToText(card)
              );
      ...
      if (i_played_the_card) {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          $(".set_defcon_box_" + i).css(
            "top",
            twilight_self.scale(twilight_self.game.state.defcon_ps[5 - i].top) + "px"
          );
          $(".set_defcon_box_" + i).css(
            "left",
            twilight_self.scale(twilight_self.game.state.defcon_ps[5 - i].left) + "px"
          );
        }
        ...
        twilight_self.updateStatusWithOptions(
          "Set DEFCON to:",
          '<ul><li class="card" id="5">five</li><li class="card" id="4">four</li><li class="card" id="3">three</li><li class="card" id="2">two</li><li class="card" id="1">one</li></ul></div>',
          false
        );
        twilight_self.attachCardboxEvents(function (action2) {
    ...
    if (card == "indopaki") {
      ...
      if (this.playerRoles[this.game.player] == player) {
        ...
        twilight_self.updateStatusWithOptions(
          ...
          `<ul><li class="card" id="pakistan">Pakistan</li><li class="card" id="india">India</li></ul>`,
          false
        );
        ...
        twilight_self.attachCardboxEvents(function (invaded) {
          ...
          let die = twilight_self.rollDice(6);
          twilight_self.addMove(
            "NOTIFY\t" + player.toUpperCase() + ` rolls: ${die}, adjusted: ${die - modifications}`
          );
    ...
    if (card == "iraniraq") {
      ...
      if (this.playerRoles[this.game.player] == player) {
        ...
        twilight_self.updateStatusWithOptions(
          ...
          `<ul><li class="card" id="iraq">Iraq</li><li class="card" id="iran">Iran</li></ul>`,
          false
        );
        ...
        twilight_self.attachCardboxEvents(function (invaded) {
          ...
          if (die >= target + modifications) {
            ...
            winner =
              invaded == "iran"
                ? "Iran repels Iraqi aggression!"
                : "Iraq repels Iranian aggression!";
            if (player == "us") {
              twilight_self.addMove("milops\tus\t2");
            } else {
              twilight_self.addMove("milops\tussr\t2");
    ...
    if (card == "junta") {
      ...
      if (i_played_the_card) {
        ...
        $(divname).on("click", function () {
          ...
          let confirmoptional =
            '<ul><li class="card" id="conduct">coup or realign</li><li class="card" id="skip">skip</li></ul>';
          twilight_self.updateStatusWithOptions(
    ...
    if (card == "kitchendebates") {
      ...
      if (us_bg > ussr_bg) {
        ...
        this.updateLog(
          "US controls " + us_bg + " battlegrounds and USSR controls " + ussr_bg + " battlegrounds"
        );
    ...
    if (card == "liberation") {
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>USSR places three influence in Central America (max 2 per country)</div>"
        );
        for (var i in this.countries) {
    ...
    if (card == "marine") {
      ...
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Remove</span> <span>" +
            ops_to_purge +
            " </span>US influence from the Middle East</div>"
        );
    ...
    if (card == "marshall") {
      ...
      if (this.game.player == 2) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Place 1 influence in each of " +
            ops_to_place +
            " non USSR-controlled countries in Western Europe</div>"
        );
      ...
      if (this.game.player == respondant) {
        ...
        if (multiple_cards == 0) {
          ...
          for (let i = 0; i < available_cards.length; i++) {
            console.log(
              "OPS: " +
                this.game.deck[0].cards[available_cards[i]].ops +
                " -- " +
                this.modifyOps(
                  this.game.deck[0].cards[available_cards[i]].ops,
                  available_cards[i],
            ...
            if (
              ...
              html += `<li class="card" id="${available_cards[i]}">${
                this.game.deck[0].cards[available_cards[i]].name
              }</li>`;
            }
          }
          html += "</ul>";
        ...
        this.updateStatus(
          `<div class="status-message" id="status-message">${this.playerRoles[
            respondant
          ].toUpperCase()} is returning card for ${this.cardToText(card)}</div>`
        );
      }
      return 0;
    }
    ...
    if (card == "muslimrevolution") {
      if (this.game.state.events.awacs == 1) {
        this.updateLog(
          `${this.cardToText("muslimrevolution")} prevented by ${this.cardToText("awacs")}}`
        );
        return 1;
      }
      ...
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Remove All US influence from 2 countries among: Sudan, Egypt, Iran, Iraq, Libya, Saudi Arabia, Syria, Jordan.</div>"
        );
    ...
    if (card == "nasser") {
      ...
      this.updateStatus(
        "<div class='status-message' id='status-message'>Nasser - Soviets add two influence in Egypt. US loses half (rounded-up) of all influence in Egypt.</div>"
      );
    ...
    if (card == "oas") {
      if (this.game.player == 2) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>US places two influence in Central or South America</div>"
        );
        for (var i in this.countries) {
          if (this.countries[i].region == "samerica" || this.countries[i].region == "camerica") {
            this.countries[i].place = 1;
    ...
    if (card == "olympic") {
      ...
      if (player == opponent) {
        ...
        twilight_self.updateStatusWithOptions(
          `${opponent.toUpperCase()} hosts the ${this.cardToText(card)}:`,
          '<ul><li class="card" id="boycott">boycott</li><li class="card" id="participate">participate</li></ul>',
          false
        );
    ...
    if (card == "ortega") {
      ...
      if (this.game.state.events.cubanmissilecrisis == 1) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>USSR is under Cuban Missile Crisis and cannot coup. Skipping Ortega coup.</div>"
        );
      ...
      if (this.game.player == 1) {
        ...
        twilight_self.updateStatusWithOptions(
          ...
          '<ul><li class="card" id="skiportega">or skip coup</li></ul>',
          false
        );
        ...
        twilight_self.attachCardboxEvents(function (action2) {
          if (action2 == "skiportega") {
            twilight_self.updateStatus(
              "<div class='status-message' id='status-message'>Skipping Ortega coup...</div>"
            );
    ...
    if (card == "pershing") {
      if (this.game.player == 1) {
        ...
        if (valid_targets.length == 0) {
          twilight_self.addMove(
            "NOTIFY\tUS does not have any targets for " + twilight_self.cardToText(card)
          );
          twilight_self.endTurn();
          return;
        }
    ...
    if (card == "puppet") {
      if (this.game.player == 2) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>US place three influence in countries without any influence</div>"
        );
    ...
    if (card == "reagan") {
      ...
      if (x >= 2) {
        ...
        this.updateLog(
          "<span>Reagan bombs Libya and US scores</span> " + us_vp + " <span>VP</span>"
        );
    ...
    if (card == "saltnegotiations") {
      ...
      if (i_played_the_card) {
        ...
        for (var i in this.game.deck[0].discards) {
          ...
          try {
            if (this.game.deck[0].discards[i].scoring == 0) {
              if (
                this.game.state.events.shuttlediplomacy == 0 ||
                (this.game.state.events.shuttlediplomacy == 1 && i != "shuttle")
              ) {
                discard_deck.push(i);
                console.log(i);
        ...
        twilight_self.attachCardboxEvents(function (action2) {
          ...
          twilight_self.addMove(
            "NOTIFY\t" + player.toUpperCase() + " retrieved " + twilight_self.cardToText(action2)
          );
    ...
    if (card == "socgov") {
      ...
      if (this.game.player == 1) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Remove " +
            ops_to_purge +
            " US influence from Western Europe (max 2 per country)</div>"
        );
    ...
    if (card == "solidarity") {
      if (this.game.state.events.johnpaul == 0) {
        this.updateLog(
          `${this.cardToText(card)} does not trigger because no ${this.cardToText("johnpaul")}`
        );
        return 1;
      }
    ...
    if (card == "southafrican") {
      if (this.game.player == 1) {
        ...
        twilight_self.updateStatusWithOptions(
          `${twilight_self.cardToText(card)}: `,
          '<ul><li class="card" id="southafrica">2 Influence in South Africa</li><li class="card" id="adjacent">1 Influence in South Africa and 2 Influence in an adjacent country</li></ul>',
          false
        );
        ...
        twilight_self.attachCardboxEvents(function (action2) {
          ...
          if (action2 == "adjacent") {
            ...
            twilight_self.updateStatus(
              "<div class='status-message' id='status-message'>Place two influence in a neighboring country</div>"
            );
      ...
      if (this.isControlled("us", "uk") == 1) {
        if (this.game.player == 2) {
          ...
          if (this.game.state.events.nato == 1) {
            ...
            this.updateStatus(
              "<div class='status-message' id='status-message'>US is playing Special Relationship. Place 2 OPS anywhere in Western Europe."
            );
          } else {
            this.updateStatus(
              "<div class='status-message' id='status-message'>US is playing Special Relationship. Place 1 OP adjacent to the UK.</div>"
            );
            placeable.push("canada");
            placeable.push("france");
            placeable.push("norway");
    ...
    if (card == "starwars") {
      ...
      if (discard_deck.length == 0) {
        twilight_self.addMove(
          `NOTIFY\tUS cannot use ${this.cardToText(card)} as no cards to retrieve`
        );
        twilight_self.endTurn();
        return 1;
      }
      ...
      if (this.game.player == 2) {
        ...
        twilight_self.updateStatusAndListCards(
          `${this.cardToText(card)}: Choose card to play immediately:`,
          discard_deck,
          false
        );
        twilight_self.attachCardboxEvents(function (action2) {
          ...
          twilight_self.addMove(
            "NOTIFY\t" + player + " retrieved " + twilight_self.cardToText(action2)
          );
          twilight_self.addMove("undiscard\t" + action2);
          twilight_self.endTurn();
        });
    ...
    if (card == "suezcrisis") {
      if (this.game.player == 1) {
        ...
        twilight_self.updateStatus(
          "<div class='status-message' id='status-message'>Remove four influence from Israel, UK or France</div>"
        );
    ...
    if (card == "summit") {
      ...
      this.updateLog(
        `${this.cardToText(card)}: USSR result ${ussr_roll} (+${ussr_roll - ussrbase} bonus)`
      );
      ...
      if (us_roll === ussr_roll) {
        ...
        if (this.game.player === winner) {
          ...
          twilight_self.updateStatusWithOptions(
            ...
            '<ul><li class="card" id="raise">raise DEFCON</li><li class="card" id="lower">lower DEFCON</li><li class="card" id="same">do not change</li></ul>',
            false
          );
          ...
          if (this.game.player == 0) {
            this.updateStatus(
              `${this.playerRoles[winner].toUpperCase()} won the ${this.cardToText(card)}`
            );
          } else {
            this.updateStatus(
              `You lost the ${this.cardToText(card)}, waiting for opponent to change DEFCON`
            );
          }
        }
        return 0;
      ...
      if (target == this.playerRoles[this.game.player]) {
        ...
        for (let z = 0; z < this.game.deck[0].hand.length; z++) {
          if (
            ...
            this.game.deck[0].hand[z] == this.game.state.headline_opponent_card ||
            this.game.deck[0].hand[z] == this.game.state.headline_card
          ) {
          } else {
        ...
        if (cards_to_discard == 0) {
          this.addMove(
            `NOTIFY\t${this.cardToText(card)}: ${target.toUpperCase()} has no cards to discard`
          );
        }
            ...
            this.rollDice(cards_for_select.length, function (roll) {
              ...
              twilight_self.addMove(
                `modal\t${twilight_self.cardToText(
                  card
                )}\t${target.toUpperCase()} discarded ${twilight_self.cardToText(victim)}`
              );
    ...
    if (card == "truman") {
      ...
      if (this.game.player == 2) {
        ...
        twilight_self.updateStatus(
          "<div class='status-message' id='status-message'>Select a non-controlled country in Europe to remove all USSR influence: </div>"
        );
        ...
        $(".westerneurope").on("click", function () {
          ...
          twilight_self.addMove(
            `NOTIFY\t${twilight_self.cardToText(card)}: US removes all USSR influence from ` +
              twilight_self.countries[c].name
          );
    ...
    if (card == "ussuri") {
      ...
      if (us_cc == 1) {
        ...
        if (this.game.player == 2) {
          ...
          this.updateStatus(
            "<div class='status-message' id='status-message'>US place four influence in Asia (2 max per country)</div>"
          );
    ...
    if (card == "voiceofamerica") {
      ...
      if (this.game.player == 2) {
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Remove 4 USSR influence from non-European countries (max 2 per country)</div>"
        );
    ...
    if (card == "wargames") {
      ...
      if (i_played_the_card) {
        ...
        let choicehtml =
          '<ul><li class="card" id="endgame">end the game</li><li class="card" id="cont">continue playing</li></ul>';
        ...
        twilight_self.attachCardboxEvents(function (action2) {
          if (action2 == "endgame") {
            twilight_self.updateStatus(
              "<div class='status-message' id='status-message'>Triggering Wargames...</div>"
            );
          ...
          if (action2 == "cont") {
            twilight_self.updateStatus(
              "<div class='status-message' id='status-message'>Discarding Wargames...</div>"
            );
            twilight_self.addMove("resolve\twargames");
              ...
              for (let i = 0; i < options_purge.length; i++) {
                twilight_self.addMove(
                  "remove\tussr\tus\t" +
                    options_purge[i] +
                    "\t" +
                    twilight_self.countries[options_purge[i]].us
                );
                twilight_self.removeInfluence(
                  options_purge[i],
                  twilight_self.countries[options_purge[i]].us,
    ...
    if (card == "berlinagreement") {
      ...
      for (var i in twilight_self.countries) {
        ...
        if (this.countries[countryname].region == "europe") {
          if (this.countries[countryname].us > this.countries[countryname].ussr) {
            us_predominates = 1;
          }
          if (this.countries[countryname].us < this.countries[countryname].ussr) {
            ussr_predominates = 1;
      ...
      if (placeable.length == 0) {
        ...
        if (player == opponent) {
          this.updateStatus(
            "<div class='status-message' id='status-message'>Opponent is placing 1 influence in a European country in which they have a predominance of influence</div>"
          );
          return 0;
        }
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Place 1 influence in a European country in which you have a predominance of influence</div>"
        );
        ...
        for (let i = 0; i < placeable.length; i++) {
          ...
          $(divname).on("click", function () {
            twilight_self.placeInfluence(countryname, 1, player, function () {
              twilight_self.addMove(
                "place\t" + player + "\t" + player + "\t" + countryname + "\t1"
              );
              twilight_self.playerFinishedPlacingInfluence();
              twilight_self.endTurn();
            });
    ...
    if (card == "culturaldiplomacy") {
      ...
      if (
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Place one influence two hops away from a country in which you have existing influence. You cannot break control with this influence.</div>"
        );
          ...
          $(divname).on("click", function () {
            ...
            for (let z = 0; z < neighbours.length; z++) {
              ...
              for (let zz = 0; zz < neighbours2.length; zz++) {
                ...
                if (player == "us") {
                  if (twilight_self.countries[two_hopper].us > 0) {
                    is_this_two_hops = 1;
                  }
                }
                if (player == "ussr") {
                  if (twilight_self.countries[two_hopper].ussr > 0) {
                    is_this_two_hops = 1;
                  }
                }
            ...
            if (player == "us") {
              if (twilight_self.isControlled("ussr", selected_countryname) == 1) {
                opponent_controlled = 1;
              }
            }
            if (player == "ussr") {
              if (twilight_self.isControlled("us", selected_countryname) == 1) {
                opponent_controlled = 1;
              }
            }
            ...
            if (is_this_two_hops == 1 && opponent_controlled == 0) {
              twilight_self.addMove(
                "place\t" + player + "\t" + player + "\t" + selected_countryname + "\t1"
              );
    ...
    if (card == "poliovaccine") {
      ...
      if (my_go == 0) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Waiting for Opponent to play Polio Vaccine</div>"
        );
        return 0;
      }
      if (my_go == 1) {
        ...
        for (let i = 0; i < this.game.deck[0].hand.length; i++) {
          if (this.game.deck[0].hand[i] != "china") {
            html += `<li class="card" id="${this.game.deck[0].hand[i]}">${
          ...
          if (action2 == "finished" || cards_discarded == 3) {
            ...
            if (cards_discarded > twilight_self.game.deck[0].crypt.length) {
              ...
              if (Object.keys(discarded_cards).length > 0) {
                ...
                twilight_self.addMove("DECKBACKUP\t1");
                twilight_self.updateLog(
                  "cards remaining: " + twilight_self.game.deck[0].crypt.length
                );
                twilight_self.updateLog("Shuffling discarded cards back into the deck...");
              }
            }
    ...
    if (card == "breakthroughatlopnor") {
      ...
      if (this.game.player == 1) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Remove 3 US influence from Southeast Asia or North Korea (max 2 per country)</div>"
        );
    ...
    if (card == "eurocommunism") {
      if (this.game.player == 1) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Eurocommunism: US is removing 4 USSR influence from Western Europe (max 2 per country)</div>"
        );
        return 0;
      }
      if (this.game.player == 2) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Remove 4 USSR influence from Western Europe (max 2 per country)</div>"
        );
        ...
        if (available_targets < 4) {
          ...
          this.updateStatus(
            "<div class='status-message' id='status-message'>Remove " +
              ops_to_purge +
              " USSR influence from Western Europe (max 2 per country)</div>"
          );
        }
    ...
    */
    ...
    if (card == "inftreaty") {
      ...
      if (my_go == 0) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Opponent is playing INF Treaty</div>"
        );
        return 0;
      }
    ...
    if (card == "nationbuilding") {
      ...
      if (player != me) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Opponent is playing Nation Building</div>"
        );
        return 0;
      } else {
        ...
        for (let i = 0; i < this.game.deck[0].hand.length; i++) {
          if (
            me === "ussr" &&
            this.game.deck[0].cards[this.game.deck[0].hand[i]].player === "ussr"
          ) {
            eligible_cards.push(this.game.deck[0].hand[i]);
          }
          if (me === "us" && this.game.deck[0].cards[this.game.deck[0].hand[i]].player === "us") {
            eligible_cards.push(this.game.deck[0].hand[i]);
          }
        }
        ...
        if (eligible_cards.length == 0) {
          this.playerFinishedPlacingInfluence();
          this.addMove(
            "notify\t" + player.toUpperCase() + " has no valid cards for Nation Building"
          );
          this.endTurn();
          return 0;
        }
        ...
        this.updateStatus(
          "<div class='status-message' id='status-message'>Select any country in Africa, Central America or South America that is not controlled by the opposing player and in which you have at least 1 influence: </div>"
        );
        for (var i in this.countries) {
          let divname = "#" + i;
          let is_eligible = 0;
        ...
        if (eligible_countries == 0) {
          this.playerFinishedPlacingInfluence();
          this.addMove(
            "notify\t" + player.toUpperCase() + " has no eligible countries for Nation Building"
          );
          this.endTurn();
          return 0;
        }
    ...
    if (card == "perestroika") {
      if (this.game.player == 2) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>USSR is playing Perestroika</div>"
        );
        return 0;
      }
      if (this.game.player == 1) {
        ...
        twilight_self.updateStatusWithOptions(
          ...
          '<ul><li class="card" id="skip">or skip...</li></ul>'
        );
        twilight_self.attachCardboxEvents(function (action2) {
          twilight_self.playerFinishedPlacingInfluence();
          twilight_self.endTurn();
    ...
    if (card == "peronism") {
      ...
      if (player != me) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>Opponent deciding to add influence to or coup or realign Argentina</div>"
        );
        return 0;
      } else {
        let html = `<ul>
        ...
        twilight_self.attachCardboxEvents(function (action2) {
          if (action2 == "place") {
            twilight_self.placeInfluence("argentina", 1, player, function () {
              ...
              twilight_self.addMove(
                "notify\t" + player + " places an extra influence in Argentina"
              );
              twilight_self.endTurn();
            });
          }
          if (action2 == "couporrealign") {
            ...
            twilight_self.attachCardboxEvents(function (action2) {
              ...
              if (action2 == "coup") {
                twilight_self.addMove("resolve\tperonism");
                twilight_self.addMove(
                  "coup\t" + player + "\targentina\t" + modified_ops + "\tperonism"
                );
                twilight_self.endTurn();
              }
              if (action2 == "realign") {
      ...
      if (me == "ussr") {
        ...
        for (var i in twilight_self.countries) {
          ...
          $(divname).on("click", function () {
            ...
            twilight_self.addMove(
              "SETVAR\tcountries\t" + c + "\t" + "control" + "\t" + modified_stability
            );
            twilight_self.endTurn();
          });
        }
      ...
      if (me == "us") {
        ...
        for (var i in twilight_self.countries) {
          ...
          $(divname).on("click", function () {
            ...
            if (us_influence <= 0) {
              alert("Invalid Choice - country must have US influence");
              return;
            }
      ...
      if (me == player) {
        ...
        if (die >= target) {
          if (player == "us") {
            ...
            twilight_self.addMove(
              "remove\tus\tussr\tindia\t" + twilight_self.countries["india"].ussr
            );
            twilight_self.addMove("milops\tus\t2");
            ...
            twilight_self.addMove(
              "place\tussr\tussr\tindia\t" + twilight_self.countries["india"].us
            );
            twilight_self.addMove(
              "remove\tussr\tus\tindia\t" + twilight_self.countries["india"].us
            );
            twilight_self.addMove("milops\tussr\t2");
    ...
    if (card == "sovietcoup") {
      ...
      if (1) {
        ...
        twilight_self.updateStatusWithOptions(
          ...
          '<ul><li class="card" id="zero">0 VP</li><li class="card" id="one">1 VP</li><li class="card" id="two">2 VP</li><li class="card" id="three">3 VP</li></ul>'
        );
        twilight_self.attachCardboxEvents(function (action) {
          let modifier = 0;
          ...
          if (roll < 4) {
            ...
            twilight_self.addMove(
              "NOTIFY\tUSSR rolls " + roll + " for Soviet Coup and loses the game."
            );
            twilight_self.endTurn();
            return;
          }
          ...
          this.updateStatus(
            "<div class='status-message' id='status-message'>Place eight influence in non-US controlled countries:</div>"
          );
    ...
    if (card == "brexit") {
      ...
      this.updateLog(
        "Wargames now requires " + this.game.state.wargames_concession + " VP concession."
      );
    ...
    if (card == "kissingerisawarcriminal") {
      if (this.game.player == 1) {
        this.updateStatus(
          "<div class='status-message' id='status-message'>US is playing Kissinger:</div>"
        );
        return 0;
      }
      ...
      twilight_self.attachCardboxEvents(function (action2) {
        ...
        for (let i in twilight_self.countries) {
          if (
            twilight_self.countries[i].region.indexOf(action2) != -1 &&
            twilight_self.countries[i].control == 1
          ) {
            twilight_self.addMove("SETVAR\tcountries\t" + i + "\tbg\t" + 1);
            twilight_self.addMove(
              "notify\t" + twilight_self.countries[i].name + " is now a battleground country"
            );
          }
        }
