...
class Mixin extends ModTemplate {
	...
	async handlePeerTransaction(app, tx = null, peer, mycallback) {
		...
		if (message.request === 'mixin create account') {
			await this.receiveCreateAccountTransaction(
				app,
				tx,
				peer,
				mycallback
		...
		if (message.request === 'mixin fetch address') {
			await this.receiveFetchAddressTransaction(
				app,
				tx,
				peer,
				mycallback
		...
		if (message.request === 'mixin fetch user by publickey') {
			await this.receiveFetchUserByPublickeyTransaction(
				app,
				tx,
				peer,
				mycallback
		...
		if (message.request === 'mixin fetch address by user id') {
			await this.receiveFetchAddressByUserIdTransaction(
				app,
				tx,
				peer,
				mycallback
	...
	async sendCreateAccountTransaction(callback = null) {
		...
		mixin_self.app.network.sendRequestAsTransaction(
			...
			function (res) {
				console.log(
					'Callback for sendCreateAccountTransaction request: ',
					res
				);
	...
	async bare_register(m) {
		try {
			...
			const { seed: sessionSeed, publicKey: sessionPublicKey } =
				getED25519KeyPair();
	...
	async updateTipPin(first_pin, second_pin, counter) {
		try {
			if (this.user) {
				let update = await this.user.pin.updateTipPin(
					first_pin,
					second_pin,
					counter
				);
		...
		try {
			...
			const chain =
				asset.chain_id === asset.asset_id
					? asset
					: await user.safe.fetchAsset(asset.chain_id);
	...
	async fetchSafeTransaction(transaction_id) {
		...
		try {
			...
			const transaction = await user.utxo.fetchTransaction(
				transaction_id
			);
	...
	async createWithdrawalAddress(
		...
		try {
			...
			let address_data = await user.address.create(
	...
	async sendExternalNetworkTransferRequest(
		asset_id,
		destination,
		amount,
		unique_hash = ''
	...
	async sendSaveUserTransaction(params = {}) {
		...
		await this.app.network.sendRequestAsTransaction(
			...
			function (res) {
				console.log(
					'Callback for sendSaveUserTransaction request: ',
					res
				);
			},
			peers[0].peerIndex
	...
	async sendFetchUserTransaction(params = {}, callback) {
		...
		await this.app.network.sendRequestAsTransaction(
			...
			function (res) {
				console.log(
					'Callback for sendFetchUserTransaction request: ',
					res
				);
				return callback(res);
			},
	...
	async sendFetchUserByPublicKeyTransaction(params = {}, callback) {
		...
		await this.app.network.sendRequestAsTransaction(
			...
			function (res) {
				console.log(
					'Callback for sendFetchUserByPublicKeyTransaction request: ',
					res
				);
				return callback(res);
			},
	...
	async receiveFetchUserByPublickeyTransaction(
		app,
		tx,
		peer,
		callback = null
	...
	async receiveFetchAddressByUserIdTransaction(
		app,
		tx,
		peer,
		callback = null
	...
	async load() {
		if (this.app?.options?.mixin) {
			...
			console.log(
				'MIXIN OPTIONS: ' + JSON.stringify(this.app.options.mixin)
			);
