...
class Encrypt extends ModTemplate {
			...
			if (tx.isTo(this.publicKey)) {
				...
				if (txmsg.request == 'key exchange request') {
					if (sender == this.publicKey) {
						console.log(
							'ENCRYPT: You have sent an encrypted channel request to ' +
								receiver
						);
					}
					if (receiver == this.publicKey) {
						console.log(
							'ENCRYPT: You have received an encrypted channel request from ' +
								sender
						);
				...
				if (txmsg.request == 'key exchange confirm') {
					if (sender !== this.publicKey) {
						console.log(
							`ENCRYPT: ${sender} has accepted your encrypted channel request`
						);
						this.confirm_key_exchange(txmsg.bob, sender);
					} else {
						console.log(
							'ENCRYPT: You have accepted an encrypted channel request from ' +
								sender
						);
					}
				}
	...
	async handlePeerTransaction(app, newtx = null, peer, mycallback) {
		...
		if (message.request === 'diffie hellman key exchange') {
			...
			if (txmsg.request == 'key exchange request') {
				if (receiver == this.publicKey) {
					console.log(
						'\n\n\nYou have accepted an encrypted channel request from ' +
							sender
					);
	...
	async onPeerHandshakeComplete(app, peer) {
		...
		for (let key of this.app.keychain.returnKeys()) {
			if ((key.aes_privatekey || key.aes_publicKey) && !key.aes_secret) {
				console.log(
					'ENCRYPT: attempt key exchange again for: ' + key.publicKey
				);
	...
	async initiate_key_exchange(recipients, offchain = 0, peer = null) {
		...
		if (!tx) {
			...
			tx = await this.app.wallet.createUnsignedTransaction(
				recipient,
				BigInt(0),
				BigInt(0)
			);
	...
	async accept_key_exchange(tx, offchain = 0, peer = null) {
		...
		let bob_publicKey = bob
			.getPublicKey(null, 'compressed')
			.toString('hex');
		let bob_privatekey = bob
			.getPrivateKey(null, 'compressed')
			.toString('hex');
		...
		this.sendEvent('encrypt-key-exchange-confirm', {
			members: [remote_address, our_address]
		});
	}
	...
	async confirm_key_exchange(bob, sender) {
		...
		if (senderkeydata == null) {
			if (this.app.BROWSER == 1) {
				alert(
					'Cannot find original diffie-hellman keys for key-exchange'
				);
				return;
			}
		}
		...
		siteMessage(
			`Successfully added ${this.app.keychain.returnUsername(
				sender,
				8
			)} as a friend`,
			5000
		...
		let alice = this.app.crypto.createDiffieHellman(
			alice_publicKey,
			alice_privatekey
		);
