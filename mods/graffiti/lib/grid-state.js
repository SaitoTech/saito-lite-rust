class GridState {
	...
	prettyPrint(nbColumns, nbRows) {
		...
		const cellWidth =
			...
			nbColorCharacters +
			1 +
			nbOrdinalDigitsToPrint +
			1;
		const cellHorizontalBorder = '-'.repeat(cellWidth);
		const gridHorizontalBorder = new Array(nbColumns + 1)
			.fill('+')
			.join(cellHorizontalBorder);
		...
		const alignedText = (text, alignementType, width) => {
			...
			const rightPaddingLength =
				leftRightPaddingLength - leftPaddingLength;
				...
				for (let i = 0; i < nbColumns; i++) {
					...
					if (this.state[i][j][status] !== null) {
						line +=
							' ' +
							alignedText(
								colorName(this.state[i][j][status].color),
								'left',
								nbColorCharacters
							) +
							' ' +
							ordinalToPrintedString(
								this.state[i][j][status].ordinal
							) +
							' ';
					} else {
						line += alignedText(
							'null',
							'centered',
							cellWidth - (statusHeaderLength + 1)
						);
					}
					line += '|';
				}
