...
class GraffitiUI {
	constructor(mod) {
		...
		this.foregroundLeftMarginRatio = 0.0;
		this.foregroundRightMarginRatio = 0.0;
	...
	renderForeground() {
		this.foregroundMaxWidth =
			(1 -
				this.foregroundLeftMarginRatio -
				this.foregroundRightMarginRatio) *
			window.innerWidth;
		this.foregroundMaxHeight =
			(1 -
				this.foregroundTopMarginRatio -
				this.foregroundBottomMarginRatio) *
			window.innerHeight;
		this.foregroundLeftMargin =
			this.foregroundLeftMarginRatio * window.innerWidth;
		this.foregroundTopMargin =
			this.foregroundTopMarginRatio * window.innerHeight;

		this.foregroundWidth = Math.min(
			this.foregroundMaxWidth,
			(this.foregroundMaxHeight * this.gridWidth) / this.gridHeight
		);
		this.foregroundHeight = Math.min(
			this.foregroundMaxHeight,
			(this.foregroundMaxWidth * this.gridHeight) / this.gridWidth
		);
		this.foregroundLeft =
			this.foregroundLeftMargin +
			(this.foregroundMaxWidth - this.foregroundWidth) / 2;
		this.foregroundTop =
			this.foregroundTopMargin +
			(this.foregroundMaxHeight - this.foregroundHeight) / 2;
		...
		this.gridRenderingLeft =
			this.gridApparentLeft -
			(this.gridRenderingWidth - this.gridApparentWidth) / 2;
		this.gridRenderingTop =
			this.gridApparentTop -
			(this.gridRenderingHeight - this.gridApparentHeight) / 2;
		...
		this.gridContainer.style.left = `${
			this.gridRenderingLeft - this.foregroundLeft
		}px`;
		this.gridContainer.style.top = `${
			this.gridRenderingTop - this.foregroundTop
		}px`;
		this.gridContainer.style.transform = `scale(${
		...
		this.backgroundLayerCtx.fillRect(
			0,
			0,
	...
	attachEventsToForeground() {
		...
		this.draftedTiles = new RBTree((a, b) => {
			return this.tileHash(a) - this.tileHash(b);
		});
	...
	renderSubmitButton() {
		...
		this.submitButton.style.left = `${
			this.foregroundLeft + this.foregroundWidth
		}px`;
		const submitButtonHeight = window.getComputedStyle(
			this.submitButton
		).height;
		this.submitButton.style.top = `calc(${
			this.foregroundTop + this.foregroundHeight
		}px - ${submitButtonHeight})`;
	...
	attachEventsToButtons() {
		...
		this.colorPreview.addEventListener('mouseup', () => {
			this.onMouseupOverColorPreview();
		});
		this.clearDraftButton.addEventListener('mouseup', () => {
			this.onMouseupOverClearDraftButton();
		});

		this.submitButton.addEventListener('mouseup', () => {
			this.onMouseupOverSubmitButton();
		});
	}
	...
	attachEventsToRedoButton() {
		...
		this.redoButton.addEventListener('mouseup', () => {
			clearInterval(redoIntervalId);
		});
	...
	onWheelOverForeground(event) {
		if (this.zoomable) {
			...
			const zoomFactor = this.setScale(
				event.deltaY < 0
					? this.zoomAbsoluteFactor
					: 1 / this.zoomAbsoluteFactor
			);
			this.gridApparentLeft =
				event.clientX +
				zoomFactor * (this.gridApparentLeft - event.clientX);
			this.gridApparentTop =
				event.clientY +
				zoomFactor * (this.gridApparentTop - event.clientY);
	...
	onMousedownOverForeground(event) {
		...
		this.lastMousedownInsideGrid = this.isInsideGrid({
			x: event.clientX,
			y: event.clientY
		});
		if (this.lastMousedownButton === 0 || this.lastMousedownButton === 2) {
			this.mousedown = true;
			const i = Math.floor(
				(event.clientX - this.gridApparentLeft) / this.currentScale
			);
			const j = Math.floor(
				(event.clientY - this.gridApparentTop) / this.currentScale
			);
	...
	onMousemoveOverForeground(event) {
		...
		if (this.mousedown) {
			if (!this.lastMousedownInsideGrid) {
				...
				this.isInsideGrid({ x: event.clientX, y: event.clientY })
			) {
	...
	onMouseupOverColorPreview() {
		this.palette.style.display =
			this.palette.style.display === 'none' ? 'flex' : 'none';
	}
	...
	onMouseupOverSubmitButton() {
		if (this.draftedTiles.size !== 0) {
			const tilesToSend = [];
			const it = this.draftedTiles.iterator();
			let draftedTile;
	...
	moveForeground(event) {
		...
		if (this.gridApparentLeft > window.innerWidth / 2) {
			...
			this.gridApparentLeft <
			window.innerWidth / 2 - this.gridApparentWidth
		) {
			this.gridApparentLeft =
				window.innerWidth / 2 - this.gridApparentWidth;
		} else if (
			this.gridApparentTop <
			window.innerHeight / 2 - this.gridApparentHeight
		) {
			this.gridApparentTop =
				window.innerHeight / 2 - this.gridApparentHeight;
		}
	...
	isInsideGrid(position) {
		return (
			position.x >= this.gridApparentLeft &&
	...
	actOnTile(i, j) {
		if (this.lastMousedownButton === 0) {
			...
			console.error(
				'Unexpected mousedown button: ' + this.lastMousedownButton
			);
		}
	}
	...
	undo() {
		...
		const reversedLastActionType = this.actionType(
			this.reversedAction(lastAction)
		);
		const tile = { i: lastAction.i, j: lastAction.j };
		if (
			reversedLastActionType === 'undraft' ||
			reversedLastActionType === 'redraft'
		) {
			this.removeFromDraftedTiles(tile);
		}
		if (
			reversedLastActionType === 'draft' ||
			reversedLastActionType === 'redraft'
		) {
	...
	redo() {
		...
		if (
			lastUndoneActionType === 'undraft' ||
			lastUndoneActionType === 'redraft'
		) {
			this.removeFromDraftedTiles(tile);
		}
		if (
			lastUndoneActionType === 'draft' ||
			lastUndoneActionType === 'redraft'
		) {
	...
	actionType(action) {
		if (action.oldDraftColor === null && action.newDraftColor !== null) {
			...
			action.oldDraftColor !== null &&
			action.newDraftColor !== null
		) {
			return 'redraft';
		} else if (
			action.oldDraftColor !== null &&
	...
	reversedAction(action) {
		return {
			i: action.i,
			j: action.j,
			oldDraftColor: action.newDraftColor,
			newDraftColor: action.oldDraftColor
		};
	}
	...
	clearDraft() {
		...
		while ((draftedTile = it.next()) !== null) {
			this.mod.setTile(
				{ i: draftedTile.i, j: draftedTile.j, color: null },
				'drafted'
			);
		}
	...
	draftTile(i, j, oldDraftColor, newDraftColor) {
		this.stackPush(this.undoStack, {
			i: i,
			j: j,
			oldDraftColor: oldDraftColor,
			newDraftColor: newDraftColor
		});
		this.clearStack(this.redoStack);
	...
	tileHash(tile) {
		...
		return (
			parseInt(
				createHash('md5').update(id.toString()).digest('hex'),
				16
			) %
			10 ** 15
		);
