...
class President extends GameTemplate {
	constructor(app) {
		...
		this.name = 'President';
		this.description =
			'Play cards in singles or sets in increasing value. Last player starts when all are done. The first player who gets rid of their cards is the President. The last?';
		...
		if (this.game.queue.length > 0) {
			...
			if (mv[0] === 'cards') {
				...
				if (cards_left == 0) {
					...
					this.game.players_info[player - 1].rank =
						this.game.state.players_out + 1;
			...
			if (mv[0] === 'turn') {
				...
				for (let i = 0; i < this.game.players_info.length; i++) {
					if (
						...
						this.game.players_info[i].out != 1
					) {
						has_everyone_passed = 0;
					}
				}
				...
				if (this.game.players_info[player_to_go - 1].out == 1) {
					...
					if (parseInt(mv[1]) == this.game.player) {
						...
						this.updateStatusAndListCards(
							'Waiting for ' + this.game.players[player_to_go - 1]
						);
						return 0;
					}
					shd_continue = 0;
			...
			if (mv[0] === 'round') {
				...
				let has_everyone_passed = 1;
				for (let i = 0; i < this.game.players_info.length; i++) {
					if (this.game.players_info[i].passed == 0) {
						has_everyone_passed = 0;
					}
				}
				if (has_everyone_passed == 1) {
					...
					this.game.state.first_player_in_round =
						this.game.state.last_player_to_play;
				}
				...
				for (
					let i = player_to_go + 1;
					i <= player_to_go + this.game.players.length;
					i++
				) {
					let player_to_turn = i % this.game.players.length;
					if (player_to_turn == 0) {
						player_to_turn = this.game.players.length;
					}
	...
	playerTurn() {
		...
		this.attachCardboxEvents(function (card) {
			if (president_self.isValidPlay(this.hud.cardbox.cards)) {
				president_self.addMove(
					'cards\t' +
						president_self.game.player +
						'\t' +
						(president_self.game.deck[0].hand.length - 1)
				);
				president_self.addMove(
					'play\t' +
						president_self.game.player +
						'\t' +
						JSON.stringify(this.hud.cardbox.cards)
				);
	...
	returnCardItem(card) {
		if (this.interface == 1) {
			if (this.game.deck[0].cards[card] == undefined) {
				return `<div id="${card.replace(
					/ /g,
					''
				)}" class="card hud-card cardbox-hud-status">${this.returnCardImage(
					card,
					1
				)}</div>`;
			}
	...
	isValidPlay(cards) {
		...
		if (hand1.length > 1) {
			if (hand1.length == 2 && hand1.pair == 0) {
				return 0;
			}
			if (hand1.length == 3 && hand1.triple == 0) {
				return 0;
			}
			if (hand1.length == 4 && hand1.four_of_a_kind == 0) {
				return 0;
			}
		}
	...
	convertHand(cards) {
		...
		for (let i = 0; i < cards.length; i++) {
			...
			// aces high
			if (card_suite == 'J' && cva == 0) {
				cva = 1;
				x.val.push('16');
			}
			if (card_value == 2 && cva == 0) {
				cva = 1;
				x.val.push('15');
			}
			if (card_value == 1 && cva == 0) {
				cva = 1;
				x.val.push('14');
			}
			if (cva == 0) {
				cva = 1;
				x.val.push(card_value);
			}
		}
