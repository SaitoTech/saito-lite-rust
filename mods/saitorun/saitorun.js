...
class SaitoRun extends OnePlayerGameTemplate {
  ...
  returnStatsHTML(title = "Saito Run") {
    let avg_points =
      Math.round((10 * this.game.state.lifetime.total_points) / this.game.state.lifetime.round) /
        10 || 0;
    let avg_distance =
      Math.round((10 * this.game.state.lifetime.total_distance) / this.game.state.lifetime.round) /
        10 || 0;
  ...
  checkForGameOver(log_msg) {
    if (log_msg.includes("SAITORUN:")) {
      ...
      this.game.state.lifetime.max_level = Math.max(scores[0], this.game.state.lifetime.max_level);
