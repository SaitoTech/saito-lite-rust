...
class ShogunBoard {
    ...
    renderCardPile() {
      ...
      if (this.mod.game.deck.length == 0) { return; }
    ...
    renderDiscardPile() {
      ...
      discardhtml = `<div id="discardpile" class="cardpile">
                <div>Discards: ${Object.keys(this.mod.game.deck[this.mod.game.player-1].discards).length}</div>`;
      let shift = 0;
      for (let card in this.mod.game.deck[this.mod.game.player-1].discards){
    ...
    renderDrawPile() {
      ...
      let drawhtml = `<div id="drawpile" class="cardpile">
                    <div>Draw: ${this.mod.game.deck[this.mod.game.player-1].crypt.length}</div>`;
      for (let i = 0; i < this.mod.game.deck[this.mod.game.player-1].crypt.length; i++){
        if (this.mod.game.deck[this.mod.game.player-1].crypt.length - i <= 5){
          drawhtml +=   `<img id="draw${this.mod.game.deck[this.mod.game.player-1].crypt.length - i}" src="${this.mod.card_img_dir}/blank.jpg" >`;
        } else {
