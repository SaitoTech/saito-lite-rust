<<<<<<< HEAD:mods/monarchy/monarchy.js
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
...
=======
        ...
        supply.sort((a,b) =>{
          let c_a = this.deck[a];
          let c_b = this.deck[b];
          if (c_a.type < c_b.type) { return -1; }
          if (c_a.type > c_b.type) { return 1; }
          if (c_a.cost < c_b.cost) { return -1; }
          if (c_a.cost > c_b.cost) { return 1; }
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
          return 0;
        });
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
	  ...
	  this.board.render();
          ...
          let displayedCards = document.querySelector("#status-cardbox").children;
          let slot = 1;
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
          ...
          for (let p = 1; p <= this.game.players.length; p++) {
            ...
            this.game.queue.push(
              `SAFEDEAL\t${p}\t${p}\t${
                Object.keys(this.game.deck[p - 1].discards).length +
                this.game.deck[p - 1].crypt.length
              }`
            );
          }
...
<<<<<<< HEAD:mods/monarchy/monarchy.js
            for (let j = this.game.deck[player - 1].hand.length - 1; j >= 0; j--) {
              //Will add animation(s) to sequence
              this.discardCard(
                this.game.deck[player - 1].cards[this.game.deck[player - 1].hand[j]]
              );
=======
            for (let j = this.game.deck[player-1].hand.length - 1; j >= 0; j--){
...
<<<<<<< HEAD:mods/monarchy/monarchy.js
        ...
        */
        ...
        if (player !== this.game.player) {
          this.game.halted = 1;
          let animator;
          if (document.querySelector(`.cardstacks #${card_to_buy}`)) {
            animator = this.copyGameElement(`.cardstacks #${card_to_buy} img`);
          } else {
            animator = this.createGameElement(
              this.returnCardImage(card_to_buy),
              "#active_card_zone",
              ".purchase_zone"
            );
          }
          this.moveGameElement(animator, ".purchase_zone", { insert: 1 }, () => {
            $(".purchase_zone img").fadeOut(1800);
            this.finishAnimation();
          });
        } else {
          let animationOrigin =
            card_to_buy === "curse" || !document.querySelector(`.cardstacks #${card_to_buy}`)
              ? `.cardstacks .cardpile`
              : `.cardstacks #${card_to_buy}`;
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
      ...
      if (mv[0] == "play") {
        ...
        if (this.game.player !== player) {
          msg = `Player ${player}`;
          $(this.returnCardImage(card_to_play)).hide().appendTo("#active_card_zone").fadeIn();
        }
      ...
      if (mv[0] == "hand") {
        ...
        if (this.game.player == player) {
          ...
          this.hud.updateStatusHeaderOnly(`<div class="status-header">${
          ...
          this.attachCardboxEvents(function (card) {
            ...
            switch (target) {
              case "deck":
                ...
                we_self.addMove(
                  `PUSHONDECK\t${mv[1]}\t${JSON.stringify(we_self.returnLastCard())}`
                );
          ...
          if (victim != player) {
            if (this.hasCardInHand("moat")) {
              ...
              switch (card_to_play) {
                ...
                case "militia":
                  this.addMove(
                    `hand\t${victim}\t${this.game.deck[victim - 1].hand.length - 3}\tdiscards`
                  );
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
        ...
        if (
          ...
          (this.game.state.shuffled && this.game.deck[player - 1].crypt.length == 0)
        ) {
          this.game.queue.splice(qe, 1);
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
      ...
      if (mv[0] == "bureaucrat") {
        ...
        if (vpcards.length > 0) {
          ...
          this.addMove(
            `NOTIFY\t${this.cardToText("bureaucrat")}: Player ${victim} puts a ${this.cardToText(
              vpcards[0]
      ...
      if (mv[0] == "sentry") {
        ...
        if (this.game.player == player) {
          ...
          this.updateStatusAndListCards(
            ...
            this.game.pool[player - 1].hand.map((x) => this.game.pool[player - 1].cards[x]),
            false
          );
          this.attachCardboxEvents(function (card) {
...
<<<<<<< HEAD:mods/monarchy/monarchy.js
        let card =
          this.game.pool[victim - 1].hand.length > 0
            ? this.game.pool[victim - 1].cards[this.game.pool[victim - 1].hand[0]]
            : "Deck empty";
        let player_display =
          this.game.player === victim ? "My top card:" : `Player ${victim}'s top card:`;
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
          ...
          we_self.attachCardboxEvents(function (action) {
            ...
            if (action == "deck") {
              ...
              we_self.addMove(
                `NOTIFY\t${we_self.cardToText(
                  "spy"
                )}: Player ${victim} discards a ${we_self.cardToText(card)}.`
              );
            }
      ...
      if (mv[0] == "thief") {
        ...
        this.addMove(
          `NOTIFY\t${this.cardToText("thief")}: Player ${victim} reveals ${this.cardToText(
            cards[0]
          )} and ${this.cardToText(cards[1])}`
        );
        this.endTurn();
        this.updateStatusAndListCards(
          `A ${this.cardToText("thief")} is rummaging through your deck...`
        );
        return 0;
      }
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
      ...
      if (mv[0] == "thief3") {
        ...
        if (this.game.player == player) {
          if (card) {
            this.updateStatusWithOptions(
              `Would you like to keep the ${this.cardToText(card)} from Player ${victim}?`,
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
        ...
        if (this.game.player == player) {
          ...
          if (this.deck[cname].type.includes("action")) {
            ...
            we_self.updateStatusWithOptions(
              `You are discarding a ${we_self.cardToText(cname)}, play it first?`,
              html
            );
            we_self.attachCardboxEvents(function (action) {
...
<<<<<<< HEAD:mods/monarchy/monarchy.js
  ...
  playerTurn() {
    ...
    if ((this.game.state.throneroom || this.game.state.actions > 0) && this.hasActionCard()) {
      this.hideBank();
      this.updateStatusAndListCards(
        `Pick a card to play${this.game.state.throneroom ? " twice" : ""}`,
        [],
        true
      );
      this.attachCardboxEvents(function (action) {
    ...
    if (this.game.state.buys > 0) {
      ...
      this.updateStatusAndListCards(
        `You have ${available_coins} coins and ${this.game.state.buys} card${
          this.game.state.buys > 1 ? "s" : ""
        } to buy`,
        [],
        true
      );
      ...
      this.cardbox.addCardType("buyme", "buy", function (newcard) {
        ...
        if (we_self.deck[newcard].cost <= available_coins) {
          ...
          we_self.addMove(`buy\t${we_self.game.player}\t${newcard}`);
          we_self.addMove(
            `NOTIFY\tPlayer ${we_self.game.player} bought a ${we_self.cardToText(newcard)}.`
          );
          we_self.game.state.buys--;
          available_coins -= we_self.deck[newcard].cost;
          we_self.spendMoney(we_self.deck[newcard].cost);
  ...
  acquireCard(max_value, target = "") {
    ...
    this.cardbox.addCardType("buyme", "take", function (newcard) {
      ...
      if (we_self.deck[newcard].cost <= max_value) {
        ...
        we_self.addMove(
          `NOTIFY\tPlayer ${we_self.game.player} acquired a ${we_self.cardToText(newcard)}.`
        );
        we_self.endTurn();
      }
    });
  ...
  clearActiveCards(player) {
    ...
    if (this.game.player === player) {
      Array.from(document.querySelectorAll("#active_card_zone div.card")).forEach((card) => {
        this.animationSequence.push({
          callback: this.moveGameElement,
          params: [this.copyGameElement(card), "#discardpile", { resize: 1 }, resetHTML],
        });
      });
    } else {
      $("#active_card_zone").children().fadeOut("slow", resetHTML);
    }
  }
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
      ...
      for (let i = 0; i < this.game.deck[this.game.player - 1].crypt.length; i++) {
        if (this.game.deck[this.game.player - 1].crypt.length - i <= 5) {
          html += `<img id="draw${this.game.deck[this.game.player - 1].crypt.length - i}" src="${
            this.card_img_dir
          }/blank.jpg" >`;
        } else {
      ...
      html = `<div id="discardpile" class="cardpile">
                <div>Discards: ${
                  Object.keys(this.game.deck[this.game.player - 1].discards).length
                }</div>`;
      let shift = 0;
...
>>>>>>> d78b646660d92a43b6b603e94e8e9f5ce5b2f4b0:mods/shogun/shogun.js
  ...
  returnPlayerVP(player) {
    ...
    return (
      ...
      this.game.state.players[player - 1].gardens * Math.floor(numCards / 10) -
      this.game.state.players[player - 1].curses
    );
  }
  ...
  discardCard(card, pause) {
    ...
    if (pause) {
      this.animationSequence.push({
        callback: this.moveGameElement,
        params: [
          this.copyGameElement(`#status-cardbox #${card}:not(.hidden)`),
          "#discardpile",
          {},
          () => {
            this.finishAnimation();
...
<<<<<<< HEAD:mods/monarchy/monarchy.js
      for (let card in discards) {
        this.animationSequence.push({
          callback: this.moveGameElement,
          params: [
            this.createGameElement(this.flipCardHTML(discards[card], false), "#discardpile"),
            "#drawpile",
            {
              callback: (id) => {
                $(`#${id} .flippable-card`).toggleClass("flipped");
              },
            },
            () => {
              this.finishAnimation();
...
<<<<<<< HEAD:mods/monarchy/monarchy.js
  async flipCardAnimation() {
    for (card of $("#status-cardbox").children()) {
...
=======
  returnDeck() {
	...
	name : "Moneylender", 
	type : "action" , 
	cost : 4 , text: "You may trash a Copper from your hand for +3 Coin"
    };
    deck['poacher']     = { 
	img : "poacher.jpg" , 
	name : "Poacher", 
      ...
      if (gmv[0] === "DEAL") {
        ...
        if (recipient == game_self.game.player && game_self.browser_active) {
          ...
          for (let i = 0; i < cards; i++) {
            ...
            game_self.animationSequence.push({
              callback: game_self.moveGameElement,
              params: [
                game_self.copyGameElement(target),
                destination,
                { insert: 1 },
                () => {
