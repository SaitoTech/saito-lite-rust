...
class Tweet {
  ...
  async render(prepend = false) {
    ...
    if (this.text == "" && this.retweet_tx != null) {
      ...
      //
      this.retweet.notice =
        "retweeted by " + this.app.browser.returnAddressHTML(this.tx.transaction.from[0].publicKey);
      ...
      if (t) {
        ...
        await t.render(prepend);
      } else {
        await this.retweet.render(prepend);
      }
      return;
    }
  ...
  async renderWithCriticalChild(prepend = false) {
    ...
    if (this.children.length > 0) {
      if (
        ...
        if (this.children[0].children.length > 0) {
          ...
          await this.children[0].renderWithChildren();
        } else {
  ...
  async renderWithParentAsCritical() {
    let parent = this.mod.returnTweet(this.parent_id);

    if (parent) {
      parent.critical_child = this;
      await parent.renderWithCriticalChild(true);
    } else {
      await this.render();
    }
  }

  async renderWithParentAndChildren() {
    //
  ...
  isCriticalChild(tweet) {
    ...
    for (let i = 0; i < tweet.tx.to.length; i++) {
      if (tweet.tx.to[i].publicKey === this.app.wallet.returnPublicKey()) {
        if (this.critical_child == null) {
          return true;
        }
        if (tweet.tx.timestamp > this.critical_child.tx.timestamp) {
          return true;
        }
      }
    }
