...
class RedSquare extends ModTemplate {
	...
	loadSettings(container = null) {
		if (!container) {
			...
			overlay.show(
				`<div class="module-settings-overlay"><h2>Redsquare Settings</h2></div>`
			);
				...
				x.push({
					...
					event: function (id) {
						this_mod.app.connection.on(
							'redsquare-update-notifications',
							(unread) => {
								this_mod.app.browser.addNotificationToId(
									unread,
									id
								);
								this_mod.app.connection.emit(
	...
	async render() {
		...
		if (this.main == null) {
			...
			this.menu = new RedSquareNavigation(
				this.app,
				this,
				'.saito-sidebar.left'
			);
		...
		if (service.service === 'relay') {
			if (this.following.length > 0) {
				...
				this.app.connection.emit('relay-send-message', {
					recipient: this.app.options.redsquare.following,
					request: 'ping',
					data: {}
				});
			...
			if (tx.isTo(this.publicKey)) {
				...
				if (txmsg.module == this.name) {
					if (this.black_list.includes(tx.from[0].publicKey)) {
						console.log(
							"Don't process transactions from blacklisted keys"
						);
						return;
					}
					...
					if (txmsg.request == 'loadTweets') {
						console.log(
							'Our colleague asked us to return tweets!!!!'
						);
						...
						this.app.storage.loadTransactions(
							...
							async (txs) => {
								...
								response_tx.msg = {
									...
									created_at:
										txmsg.data?.updated_earlier_than >= 0
											? 'earlier'
											: 'later',
									data: []
								};
								...
								for (let tx of txs) {
									response_tx.msg.data.push(
										tx.serialize_to_web(this.app)
									);
								}
					...
					if (txmsg.request == 'returnTweets') {
						...
						if (!colleague) {
							console.warn(
								"We got tweets back from someone we weren't following"
							);
							return;
						}
						...
						console.log(
							`REDSQUARE (${colleague.publicKey}): returned ${txs.length} ${created_at} tweets, ${count} are new to the feed. `
						);
						if (created_at === 'earlier') {
			...
			if (txmsg.request === 'follow') {
				if (this.app.BROWSER) {
					if (tx.isFrom(this.publicKey)) {
						...
						this.app.options.redsquare.followers.push(
							tx.from[0].publicKey
						);
			...
			if (txmsg.request === 'unfollow') {
				if (this.app.BROWSER) {
					if (tx.isTo(this.publicKey)) {
						...
						for (
							let i = 0;
							i < this.app.options.redsquare.followers.length;
							i++
						) {
							if (
								this.app.options.redsquare.followers[i] ==
								tx.from[0].publicKey
							) {
								this.app.options.redsquare.followers.splice(
									i,
									1
								);
								break;
		...
		for (let i = 0; i < this.peers.length; i++) {
			...
			if (
				...
				if (created_at == 'earlier') {
					...
					console.log(
						`REDSQUARE: fetch earlier tweets from ${this.peers[i].publicKey}`
					);
				...
				this.app.storage.loadTransactions(
					...
					(txs) => {
						...
						if (txs.length > 0) {
							count = this.processTweetsFromPeer(
								this.peers[i],
								txs
							);
						} else {
						...
						if (created_at === 'later') {
							if (this.peers[i].peer !== 'localhost') {
								this.app.connection.emit(
									...
									`Processing ${
										txs.length
									} tweets returned from ${this.app.keychain.returnUsername(
										this.peers[i].publicKey
									)}`
								);
							} else {
	...
	async loadTweets2(created_at = 'earlier', mycallback) {
		...
		for (let i = 0; i < this.following.length; i++) {
			if (this?.stun.hasConnection(this.following[i].publicKey)) {
				if (
					!(
						this.following[i].tweets_earliest_ts == 0 &&
						created_at == 'earlier'
					)
				) {
					peer_count++;
					...
					if (created_at == 'earlier') {
						obj.updated_earlier_than =
							this.following[i].tweets_earliest_ts;
					} else if (created_at == 'later') {
						obj.updated_later_than =
							this.following[i].tweets_latest_ts;
					} else {
						console.error('Unsupported time restraint in rS');
					}
	...
	loadNotifications(mycallback = null) {
		...
		const middle_callback = () => {
			...
			if (notifications.length > 0) {
				...
				console.log(
					'Redsquare: last notification fetch returned nothing'
				);
				this.notifications_earliest_ts = 0;
			}

			console.log(
				`Appending ${new_notifications.length} new notification notices to the page`
			);
	...
	addTweet(tx, source) {
		...
		if (
			this.mute_list.includes(tx.from[0].publicKey) &&
	...
	addNotification(tx) {
		if (tx.isTo(this.publicKey)) {
			if (
				...
				!this.black_list.includes(tx.from[0].publicKey)
			) {
				//
				...
				if (this.notifications_sigs_hmap[tx.signature] != 1) {
					...
					for (let i = 0; i < this.notifications.length; i++) {
						if (
							tweet.created_at > this.notifications[i].created_at
						) {
							insertion_index = i;
							break;
						}
	...
	loadOptions() {
		...
		if (this.app.options.redsquare) {
			...
			if (this.app.options.redsquare.following) {
				this.app.options.redsquare.following.forEach((key) =>
	...
	async fetchRecentTweets() {
		...
		return this.app.storage.loadTransactions(
			...
			(txs) => {
				console.log('loaded local tweets', txs);
				let cnt = 0;
