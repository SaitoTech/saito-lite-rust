...
class CombatOverlay {
    ...
    renderPreCombat(res={}) {
      this.render(res, 1);
      let obj = document.querySelector(".combat-overlay");
      obj.style.backgroundImage = "url(/his/img/backgrounds/field_battle.jpg)";
      obj.style.backgroundSize = "contain";
      this.updateInstructions("Combat imminent in " + this.mod.game.spaces[res.spacekey].name);
    }
...
console.log("Assign Hits Manually!");
	...
	let factionspace = el.querySelector(".combat-desc").innerHTML;
...
console.log("RES: " + JSON.stringify(res));
	  ...
	  for (let i = 0; i < res.attacker_modified_rolls.length; i++) {
		...
		if (am_i_attacker) {
	      ...
	      if (pre_battle) { roll = "?"; rrclass = ""; }
              ...
              let html = `
                <div class="combat-row ${assignable}" data-unit-type="${unit_type}" data-faction="${faction_name}">
                	<div class="combat-unit">${unit_type}<div class="combat-desc">${faction_name}</div></div>
                	<div class="combat-roll ${rrclass}">${roll}</div>
                </div>
              `;
              this.app.browser.addElementToSelector(html, ".combat-grid .attacker");
	  ...
	  for (let i = 0; i < res.defender_modified_rolls.length; i++) {
		...
		if (am_i_defender) {
	      ...
	      if (pre_battle) { roll = "?"; rrclass = ""; }
              ...
              `;
              this.app.browser.addElementToSelector(html, ".combat-grid .defender");
	  }
	}
    ...
    updateInstructions(help="") {
      let x = document.querySelector(".combat-overlay .help");
