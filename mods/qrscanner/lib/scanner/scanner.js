		...
		try {
			for (var x = 0; x < max; x += 2) {
				...
				var bit =
					image[
						Math.floor(points[x]) +
							qrcode.width * Math.floor(points[x + 1])
					];
	...
	this.buildFunctionPattern = function () {
		...
		for (var x = 0; x < max; x++) {
			...
			for (var y = 0; y < max; y++) {
				...
				bitMatrix.setRegion(
					this.alignmentPatternCenters[y] - 2,
					i,
					5,
					5
				);
...
PerspectiveTransform.quadrilateralToQuadrilateral = function (
	...
	var sToQ = this.squareToQuadrilateral(
		x0p,
		y0p,
		x1p,
		y1p,
...
function Detector(image) {
	...
	this.processFinderPatternInfo = function (info) {
		...
		var moduleSize = this.calculateModuleSize(
			topLeft,
			topRight,
			bottomLeft
		);
...
FormatInformation.doDecodeFormatInformation = function (maskedFormatInfo) {
	...
	for (var i = 0; i < FORMAT_INFO_DECODE_LOOKUP.length; i++) {
		...
		var bitsDifference = this.numBitsDiffering(
			maskedFormatInfo,
			targetInfo
		);
		if (bitsDifference < bestDifference) {
	...
	this.runEuclideanAlgorithm = function (a, b, R) {
		...
		while (r.Degree >= Math.floor(R / 2)) {
			...
			while (r.Degree >= rLast.Degree && !r.Zero) {
				...
				q = q.addOrSubtract(
					this.field.buildMonomial(degreeDiff, scale)
				);
				r = r.addOrSubtract(
...
function GF256Poly(field, coefficients) {
	...
	this.divide = function (other) {
		...
		while (remainder.Degree >= other.Degree && !remainder.Zero) {
			...
			var iterationQuotient = this.field.buildMonomial(
				degreeDifference,
				scale
			);
	...
	if (qrcode.debug) {
		for (var y = 0; y < qrcode.height; y++) {
			for (var x = 0; x < qrcode.width; x++) {
				...
				qrcode.imagedata.data[point] = image[x + y * qrcode.width]
					? 0
					: 0;
				qrcode.imagedata.data[point + 1] = image[x + y * qrcode.width]
					? 0
					: 0;
	...
	if (qrcode.debug) {
		for (var y = 0; y < qRCodeMatrix.bits.Height; y++) {
			for (var x = 0; x < qRCodeMatrix.bits.Width; x++) {
				...
				qrcode.imagedata.data[point] = qRCodeMatrix.bits.get_Renamed(
					x,
					y
				)
					? 0
				...
				qrcode.imagedata.data[point + 1] =
					qRCodeMatrix.bits.get_Renamed(x, y) ? 0 : 0;
				qrcode.imagedata.data[point + 2] =
					qRCodeMatrix.bits.get_Renamed(x, y) ? 255 : 0;
			}
		}
		ctx.putImageData(qrcode.imagedata, 0, 0);
	}
...
qrcode.orderBestPatterns = function (patterns) {
	...
	if (
		oneTwoDistance >= zeroOneDistance &&
...
function FinderPattern(posX, posY, estimatedModuleSize) {
	...
	this.aboutEquals = function (moduleSize, i, j) {
		if (
			...
			var moduleSizeDiff = Math.abs(
				moduleSize - this.estimatedModuleSize
			);
			return (
				moduleSizeDiff <= 1.0 ||
...
function FinderPatternFinder() {
	...
	this.foundPatternCross = function (stateCount) {
		...
		var moduleSize = Math.floor(
			(totalModuleSize << INTEGER_MATH_SHIFT) / 7
		);
					...
					if ((currentState & 1) == 0) {
						...
						if (currentState == 4) {
							...
							if (this.foundPatternCross(stateCount)) {
								// Yes
								var confirmed = this.handlePossibleCenter(
									stateCount,
									i,
									j
								);
								if (confirmed) {
									...
									if (this.hasSkipped) {
											...
											this.haveMultiplyConfirmedCenters();
									} else {
...
function AlignmentPatternFinder(
	...
	this.handlePossibleCenter = function (stateCount, i, j) {
		...
		if (!isNaN(centerI)) {
			...
			var point = new AlignmentPattern(
				centerJ,
				centerI,
				estimatedModuleSize
			);
