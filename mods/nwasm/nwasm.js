...
class Nwasm extends GameTemplate {
  ...
  initialize(app) {
    ...
    if (library_mod) {
      let collection = library_mod.returnCollection("Nwasm", app.wallet.getPublicKey());
      if (collection.length > 0) {
        this.addCollectionToLibrary(app.wallet.getPublicKey(), collection);
          ...
          this.app.storage.loadTransactions("Nwasm" + this.active_rom_sig, 5, function (txs) {
            try {
              for (let z = 0; z < txs.length; z++) {
                let newtx = new saito.default.transaction(undefined, txs[z].transaction);
  ...
  loadGameFile() {
    ...
    this.app.storage.loadTransactions("Nwasm" + this.active_rom_sig, 1, function (txs) {
      try {
        ...
        let newtx = new saito.default.transaction(undefined, txs[0].transaction);
        let txmsg = newtx.returnMessage();
