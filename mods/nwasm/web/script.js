...
class MyClass {
	constructor() {
		...
		document
			.getElementById('file-upload-eep')
			.addEventListener('change', this.uploadEep.bind(this));
		document
			.getElementById('file-upload-sra')
			.addEventListener('change', this.uploadSra.bind(this));
		document
			.getElementById('file-upload-fla')
			.addEventListener('change', this.uploadFla.bind(this));
		...
		rivets.bind(document.getElementById('topPanel'), {
			data: this.rivetsData
		});
		rivets.bind(document.getElementById('bottomPanel'), {
			data: this.rivetsData
		});
		rivets.bind(document.getElementById('loginModal'), {
			data: this.rivetsData
		});
		rivets.bind(document.getElementById('buttonsModal'), {
			data: this.rivetsData
		});
		rivets.bind(document.getElementById('lblError'), {
			data: this.rivetsData
		});
			...
			if (keymappings) {
				...
				for (let [key, value] of Object.entries(keymappings_object)) {
					if (key in this.rivetsData.inputController.KeyMappings) {
						this.rivetsData.inputController.KeyMappings[key] =
							value;
					}
				}
			}
	...
	async LoadEmulator(byteArray) {
		if (this.rom_name.toLocaleLowerCase().endsWith('.zip')) {
			...
			this.initAudio();
			this.rivetsData.beforeEmulatorStarted = false;
			this.showToast = Module.cwrap('neil_toast_message', null, [
				'string'
			]);
	...
	AudioProcessRecurring(audioProcessingEvent) {
		...
		for (let sample = 0; sample < AUDIOBUFFSIZE; sample++) {
			if (this.audioWritePosition != this.audioReadPosition) {
				...
				outputData2[sample] =
					this.audioBufferResampled[this.audioReadPosition + 1] /
					32768;
	...
	WriteConfigFile() {
		...
		//gamepad
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Up + '\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Down +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Left +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Right +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Action_A +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Action_B +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings
				.Joy_Mapping_Action_Start + '\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Action_Z +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Action_L +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Action_R +
			'\r\n';
		configString +=
			this.rivetsData.inputController.KeyMappings.Joy_Mapping_Menu +
			'\r\n';
	...
	createDB() {
		...
		request.onupgradeneeded = function (ev) {
			...
			let objectStore = db.createObjectStore('N64WASMSTATES', {
				autoIncrement: true
			});
	...
	ExportSraEvent() {
		...
		var file = new File([filearray], 'game.sra', {
	...
	ExportFlaEvent() {
		...
		var file = new File([filearray], 'game.fla', {
			type: 'text/plain; charset=x-user-defined'
		});
		saveAs(file);
	}
	...
	showRemapModal() {
		...
		this.rivetsData.remappings = JSON.parse(
			JSON.stringify(this.rivetsData.inputController.KeyMappings)
		);
	...
	saveRemap() {
		...
		else this.rivetsData.inputController.Gamepad_Process_Axis = false;

		this.rivetsData.inputController.KeyMappings = JSON.parse(
			JSON.stringify(this.rivetsData.remappings)
		);
	...
	restoreDefaultKeymappings() {
		this.rivetsData.remappings =
			this.rivetsData.inputController.defaultKeymappings();
	}
	...
	remapPressed() {
		if (this.rivetsData.remapMode == 'Key') {
			...
			if (this.rivetsData.currKey == 20)
				this.rivetsData.remappings.Mapping_Action_Analog_Right =
					keyLast;
		}
		if (this.rivetsData.remapMode == 'Button') {
			var joyLast = this.rivetsData.inputController.Joy_Last;
			if (this.rivetsData.currJoy == 1)
				this.rivetsData.remappings.Joy_Mapping_Up = joyLast;
			if (this.rivetsData.currJoy == 2)
				this.rivetsData.remappings.Joy_Mapping_Down = joyLast;
			if (this.rivetsData.currJoy == 3)
				this.rivetsData.remappings.Joy_Mapping_Left = joyLast;
			if (this.rivetsData.currJoy == 4)
				this.rivetsData.remappings.Joy_Mapping_Right = joyLast;
			if (this.rivetsData.currJoy == 5)
				this.rivetsData.remappings.Joy_Mapping_Action_A = joyLast;
			if (this.rivetsData.currJoy == 6)
				this.rivetsData.remappings.Joy_Mapping_Action_B = joyLast;
			if (this.rivetsData.currJoy == 8)
				this.rivetsData.remappings.Joy_Mapping_Action_Start = joyLast;
			if (this.rivetsData.currJoy == 9)
				this.rivetsData.remappings.Joy_Mapping_Menu = joyLast;
			if (this.rivetsData.currJoy == 10)
				this.rivetsData.remappings.Joy_Mapping_Action_Z = joyLast;
			if (this.rivetsData.currJoy == 11)
				this.rivetsData.remappings.Joy_Mapping_Action_L = joyLast;
			if (this.rivetsData.currJoy == 12)
				this.rivetsData.remappings.Joy_Mapping_Action_R = joyLast;
		}
		this.rivetsData.remapWait = false;
	}
	...
	postLoginProcess() {
		...
		this.rivetsData.n64SaveStates.sort((a, b) => {
