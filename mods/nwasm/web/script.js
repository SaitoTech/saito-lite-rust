...
class MyClass {
  ...
  showRemapModal() {
    ...
    if (this.rivetsData.inputController.Gamepad_Process_Axis) this.rivetsData.chkUseJoypad = true;
  ...
  saveRemap() {
    if (this.rivetsData.chkUseJoypad) this.rivetsData.inputController.Gamepad_Process_Axis = true;
  ...
  async loginToServer() {
    let result = await $.get(
      this.rivetsData.settings.CLOUDSAVEURL + "/Login?password=" + this.rivetsData.password
    );
  ...
  async getSaveStates() {
    let result = await $.get(
      this.rivetsData.settings.CLOUDSAVEURL + "/GetSaveStates?password=" + this.rivetsData.password
    );
    console.log("getSaveStates result: ", result);
    this.allSaveStates = result;
    result.forEach((element) => {
      if (element.Name == this.rom_name + ".n64wasm") this.rivetsData.noCloudSave = false;
    });
    return result;
  }
