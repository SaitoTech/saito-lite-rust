...
cycloa.Tracer = function (machine) {
	...
	this.decode = function () {
		...
		for (var i = 0, max = 3; i < max; ++i) {
			inst +=
				i < this.code_idx_
					? cycloa.util.formatHex(this.code_[i]) + ' '
					: '   ';
		}
		inst += ' ' + inst_repr;
		var regstr = '';
	...
	this.addrImmediate = function () {
		...
		this.addr_repr_ =
			'#$' + cycloa.util.formatHex(this.m.read(this.m.PC + 1));
		this.addr_resolved_repr_ = '';
	};
	...
	this.addrZeropageX = function () {
		...
		this.addr_repr_ =
			'$' +
			cycloa.util.formatHex(base) +
			',X @ $' +
			cycloa.util.formatHex(this.addr_);
		this.addr_resolved_repr_ = this.formatResolvedAddr_();
	};
	...
	this.addrAbsolute = function () {
		this.readCode_(3);
		this.addr_ =
			this.m.read(this.m.PC + 1) | (this.m.read(this.m.PC + 2) << 8);
	...
	this.addrAbsoluteX = function () {
		...
		this.addr_repr_ =
			'$' +
			cycloa.util.formatHex(base, 16) +
			',X @ $' +
			cycloa.util.formatHex(this.addr_, 16);
		this.addr_resolved_repr_ = this.formatResolvedAddr_();
	};
	...
	this.addrIndirect = function () {
		...
		this.addr_ =
			this.m.read(base) |
			(this.m.read((base & 0xff00) | ((base + 1) & 0x00ff)) << 8); //bug of NES
		this.addr_repr_ =
			'($' +
			cycloa.util.formatHex(base, 16) +
			') @ $' +
	...
	this.addrIndirectY = function () {
		...
		var base = this.m.read(this.m.PC + 1);
		this.addr_ =
			(this.m.read(base) | (this.m.read((base + 1) & 0xff) << 8)) +
			this.m.Y;
		this.addr_repr_ =
			'($' +
			cycloa.util.formatHex(base) +
	...
	this.addrRelative = function () {
		...
		this.addr_ =
			((offset >= 128 ? offset - 256 : offset) + this.m.PC + 2) & 0xffff;
		this.addr_repr_ = '$' + cycloa.util.formatHex(this.addr_, 16);
		this.addr_resolved_repr_ = '';
	};
	this.LDA = function () {
...
cycloa.ScriptMachine = function (videoFairy, audioFairy, pad1Fairy, pad2Fairy) {
	...
	this.__triangle__waveForm = [
		...
		0xe, 0xf, 0xf, 0xe, 0xd, 0xc, 0xb, 0xa, 0x9, 0x8, 0x7, 0x6, 0x5, 0x4,
		0x3, 0x2, 0x1, 0x0
	];
...
cycloa.ScriptMachine.prototype.run = function () {
	 ...
	 * @type {number}
	 */
	var __cpu__ZNFlagCache = this.__cpu__ZNFlagCache;
	var __cpu__TransTable = this.__cpu__TransTable;
	var __cpu__rom = this.__cpu__rom;
	var __cpu__ram = this.__cpu__ram;

	var __video__palette = this.__video__palette;
	var __video__vramMirroring = this.__video__vramMirroring;
	var __video__pattern = this.__video__pattern;
	var __video__screenBuffer8 = this.__video__screenBuffer8;
	var __video__screenBuffer32 = this.__video__screenBuffer32;
	var __audio__audioFairy = this.__audio__audioFairy;
	var __audio__enabled = __audio__audioFairy.enabled;
	var __audio__data = __audio__audioFairy.data;
	var __audio__data__length = __audio__audioFairy.dataLength;
	var __vm__run = true;
	...
	while (__vm__run) {
		...
		__vm__clockDelta = __vm__reservedClockDelta;
		__vm__reservedClockDelta = 0;
		++__video__nowY;
		...
		if (__video__nowY <= 240) {
			...
			if (
				...
				this.__video__vramAddrRegister =
					(this.__video__vramAddrRegister & 0x7be0) |
					(this.__video__vramAddrReloadRegister & 0x041f);
				this.__video__buildBgLine();
				this.__video__buildSpriteLine();
				var __video__vramAddrRegister =
					this.__video__vramAddrRegister + (1 << 12);
				__video__vramAddrRegister +=
					(__video__vramAddrRegister & 0x8000) >> 10;
			...
			this.__video__videoFairy.dispatchRendering(
				__video__screenBuffer8,
				this.__video__paletteMask
			);
			...
			this.NMI =
				this.__video__executeNMIonVBlank; /* reserve NMI if emabled */
			this.onVBlank();
			...
			if (
				...
				this.__video__vramAddrRegister =
					(this.__video__vramAddrRegister & 0x041f) |
					(this.__video__vramAddrReloadRegister & 0x7be0);
			}
			__vm__run = false;
		} else {
			throw new cycloa.err.CoreException(
				'Invalid scanline: ' + this.__video__nowY
			);
		}
	}
	...
	while (this.__audio__clockCnt >= 1786840) {
		/*unsigned int*/ var __audio__processClock =
			1786840 + this.__audio__leftClock;
		/*unsigned int*/ var __audio__delta =
			(__audio__processClock / 22050) | 0;
		...
		if (
			...
			this.__rectangle1__frequency <= 0x7ff
		) {
			/*
			 * @type {number} unsigned int
			 * @const
			 */
			var __rectangle1__nowCounter =
				this.__rectangle1__freqCounter + __audio__delta;
			this.__rectangle1__freqCounter =
				__rectangle1__nowCounter % (this.__rectangle1__frequency + 1);
			this.__rectangle1__dutyCounter =
				(this.__rectangle1__dutyCounter +
					__rectangle1__nowCounter /
						(this.__rectangle1__frequency + 1)) &
				15;
			if (this.__rectangle1__dutyCounter < this.__rectangle1__dutyRatio) {
				__audio__sound += this.__rectangle1__decayEnabled
					? this.__rectangle1__decayVolume
					: this.__rectangle1__volumeOrDecayRate;
			} else {
				__audio__sound += this.__rectangle1__decayEnabled
		...
		if (
			...
			this.__rectangle0__frequency <= 0x7ff
		) {
			/*
			 * @type {number} unsigned int
			 * @const
			 */
			var __rectangle0__nowCounter =
				this.__rectangle0__freqCounter + __audio__delta;
			this.__rectangle0__freqCounter =
				__rectangle0__nowCounter % (this.__rectangle0__frequency + 1);
			this.__rectangle0__dutyCounter =
				(this.__rectangle0__dutyCounter +
					__rectangle0__nowCounter /
						(this.__rectangle0__frequency + 1)) &
				15;
			if (this.__rectangle0__dutyCounter < this.__rectangle0__dutyRatio) {
				__audio__sound += this.__rectangle0__decayEnabled
					? this.__rectangle0__decayVolume
					: this.__rectangle0__volumeOrDecayRate;
			} else {
				__audio__sound += this.__rectangle0__decayEnabled
		...
		if (
			...
			/* unsigned int */ var __triangle__nowCounter =
				this.__triangle__freqCounter + __audio__delta;
			var __triangle__freq = this.__triangle__frequency + 1;
			this.__triangle__freqCounter =
				__triangle__nowCounter % __triangle__freq;
			__audio__sound +=
				this.__triangle__waveForm[
					(this.__triangle__streamCounter =
						(this.__triangle__streamCounter +
							__triangle__nowCounter / __triangle__freq) &
						31)
				];
		...
		if (this.__noize__lengthCounter != 0) {
			/* unsigned int */ var __noize__nowCounter =
				this.__noize__freqCounter + __audio__delta;
			/* const uint16_t */ var __noize__divFreq =
				this.__noize__frequency + 1;
			/* const uint8_t */ var __noize__shiftAmount = this
				.__noize__modeFlag
				? 6
				: 1;
			...
			while (__noize__nowCounter >= __noize__divFreq) {
				...
				__noize__shiftReg =
					(__noize__shiftReg >> 1) |
					(((__noize__shiftReg ^
						(__noize__shiftReg >> __noize__shiftAmount)) &
						1) <<
						14);
			}

			if ((__noize__shiftReg & 1) == 1) {
				__audio__sound += this.__noize__decayEnabled
					? -this.__noize__decayVolume
					: -this.__noize__volumeOrDecayRate;
			} else {
				__audio__sound += this.__noize__decayEnabled
		...
		if (this.__digital__sampleLength != 0) {
			/*unsigned int*/ var __digital__nowCounter =
				this.__digital__freqCounter + __audio__delta;
			/*const uint16_t*/ var __digital__divFreq =
				this.__digital__frequency + 1;
			while (__digital__nowCounter >= __digital__divFreq) {
			   ...
			   If an interrupt flag was set at the same moment of the read, it will read back as 1 but it will not be cleared. */
				...
				this.__digital__sampleBuffer =
					this.__digital__sampleBuffer >> 1;
		...
		if (__audio__enabled) {
			__audio__data[__audio__audioFairy.dataIndex++] =
				__audio__sound / 100;
...
cycloa.ScriptMachine.TransTable = new Uint32Array([
	459499, 393688, 255, 255, 255, 197073, 327905, 255, 197195, 131536, 131323,
	255, 255, 262612, 393444, 255, 131994, 328153, 255, 255, 255, 262610,
	393442, 255, 131707, 262614, 255, 255, 255, 262613, 393445, 255, 394036,
	393432, 255, 255, 196865, 196817, 328161, 255, 262763, 131280, 131579, 255,
	262404, 262356, 393700, 255, 131962, 327897, 255, 255, 255, 262354, 393698,
	255, 131771, 262358, 255, 255, 255, 262357, 393701, 255, 394011, 393592,
	255, 255, 255, 196977, 328113, 255, 197179, 131440, 131531, 255, 197412,
	262516, 393652, 255, 132010, 328057, 255, 255, 255, 262514, 393650, 255,
	131739, 262518, 255, 255, 255, 262517, 393653, 255, 393995, 393416, 255,
	255, 255, 196801, 328193, 255, 262747, 131264, 131611, 255, 328487, 262340,
	393732, 255, 132026, 327881, 255, 255, 255, 262338, 393730, 255, 131803,
	262342, 255, 255, 255, 262341, 393733, 255, 255, 393272, 255, 255, 196689,
	196657, 196673, 255, 131435, 255, 131227, 255, 262228, 262196, 262212, 255,
	131914, 327737, 255, 255, 262226, 262194, 262211, 255, 131259, 262198,
	131243, 255, 255, 262197, 255, 255, 131104, 393224, 131088, 255, 196641,
	196609, 196625, 255, 131195, 131072, 131179, 255, 262180, 262148, 262164,
	255, 131930, 327689, 255, 255, 262178, 262146, 262163, 255, 131755, 262150,
	131211, 255, 262181, 262149, 262166, 255, 131376, 393496, 255, 255, 196913,
	196881, 328001, 255, 131499, 131344, 131419, 255, 262452, 262420, 393540,
	255, 131978, 327961, 255, 255, 255, 262418, 393538, 255, 131723, 262422,
	255, 255, 255, 262421, 393541, 255, 131360, 393768, 255, 255, 196897,
	197153, 328065, 255, 131483, 131616, 131835, 255, 262436, 262692, 393604,
	255, 131946, 328233, 255, 255, 255, 262690, 393602, 255, 131787, 262694,
	255, 255, 255, 262693, 393605, 255
]);
...
cycloa.ScriptMachine.prototype.__video__buildBgLine = function () {
	...
	var __video__screenBuffer8 = this.__video__screenBuffer8;
	var __video__screenBuffer32 = this.__video__screenBuffer32;
	var _color = 0 | __video__palette[32];
	if (!this.__video__backgroundVisibility) {
		var _color32 = (_color << 24) | (_color << 16) | (_color << 8) | _color;
		for (var i = (nowY - 1) << 6, max = i + 64; i < max; ++i)
			screenBuffer32[i] = _color32;
		return;
	}
	/**
	 * @type {number} uint8_t
	...
	while (true) {
		 ...
		 */
		var palAddr =
			(nameTableAddr & 0x2f00) |
			0x3c0 |
			((tileYofScreen & 0x1c) << 1) |
			((nameTableAddr >> 2) & 7);
		/**
		 * @type {number} uint8_t
		 * @const
		...
		for (var x = offX; x < 8; x++) {
			...
			var color =
				...
				(((secondPlane >> (7 - x)) & 1) << 1);
			if (color !== 0) {
				__video__screenBuffer8[buffOffset + renderX] =
					__video__palette[paletteOffset + color] | 128;
			} else {
	...
	for (var i = 0; i < _spriteHitCnt; i++) {
		...
		if (slot.flipHorizontal) {
			for (var x = 0; x < endX; x++) {
				 ...
				 */
				var color =
					((firstPlane >> x) & 1) | (((secondPlane >> x) & 1) << 1); //ここだけ違います
				/**
				 * @type {number} uint8_t
				 */
				...
				if (
					color != 0 &&
					((!slot.isForeground && isEmpty) ||
						(slot.isForeground && isSpriteNotDrawn))
				) {
					__video__screenBuffer8[buffOffset + slot.x + x] =
						__video__palette[(slot.paletteNo << 2) + color] |
						layerMask;
				}
			}
		} else {
...
cycloa.ScriptMachine.prototype.__video__writeReg = function (
	/* uint16_t */ addr,
	/* uint8_t */ value
) {
	var __video__palette = this.__video__palette;
...
cycloa.ScriptMachine.prototype.__video__readReg = function (
	/* uint16_t */ addr
) {
	var __video__palette = this.__video__palette;
	var __video__vramMirroring = this.__video__vramMirroring;
...
cycloa.ScriptMachine.prototype.__video__writeVramExternal = function (
	/* uint16_t */ addr,
	/* uint8_t */ value
) {
	if (addr < 0x2000) {
...
cycloa.ScriptMachine.prototype.__video__writeVram = function (
	/* uint16_t */ addr,
	/* uint8_t */ value
) {
 ...
 */
cycloa.ScriptMachine.prototype.__video__changeMirrorType = function (
...
cycloa.ScriptMachine.prototype.ASCII_CHR = new Uint8Array([
	,
	...
	,
...
VideoFairy.prototype.dispatchRendering = function (
	/* const uint8_t*/ nesBuffer,
...
function AudioFairy() {
	...
	if (audioContext) {
		...
		this.initBuffer = function () {
			this.buffer_ = this.context_.createBuffer(
				1,
				this.dataLength,
				this.SAMPLE_RATE_
			);
...
function NesController() {
	...
	this.machine_ = new cycloa.ScriptMachine(
		this.videoFairy_,
		this.audioFairy_,
		this.padFairy_
	);
...
NesController.prototype.start = function () {
	if (this.running_) {
		$('#state').text(
			'VM already running! Please stop the machine before loading another script.'
		);
		return false;
	}
	if (!this.loaded_) {
	...
	var loop = function () {
		...
		if (cnt >= 30) {
			...
			if (cycloa.calc_fps_mode) {
				str +=
					' / avg: ' +
					((self.total_frame_ * 1000) / (now - startTime)).toFixed(2);
			}
			state.text(str);
			beg = now;
			cnt = 0;
	...
	$(document).ready(function () {
		...
		$.ajax({
			...
			error: function (XMLHttpRequest, textStatus, errorThrown) {
				$('#state').text(
					'Oops. failed to load sample script. code:' + textStatus
				);
			}
		});
