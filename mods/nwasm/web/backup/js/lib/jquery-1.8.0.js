...
(function (window, undefined) {
	...
	jQuery.extend({
		...
		data: function (elem, name, data, pvt /* Internal Use Only */) {
			...
			if (!id) {
				...
				if (isNode) {
					elem[internalKey] = id =
						jQuery.deletedIds.pop() || ++jQuery.uuid;
				} else {
					id = internalKey;
				}
			}
		...
		var Expr = (Sizzle.selectors = {
			...
			*/
				...
				PSEUDO: function (match) {
					...
					if (
						unquoted &&
						(argument = rselector.exec(unquoted)) &&
						argument.pop()
					) {
			...
			filter: {
				...
				CHILD: function (type, argument, first, last) {
					if (type === 'nth') {
						...
						return function (elem) {
							...
							if (
								parent &&
								(parent[expando] !== doneName || !elem.sizset)
							) {
								for (
				...
				PSEUDO: function (pseudo, argument, context, xml) {
					...
					var fn =
						Expr.pseudos[pseudo] ||
						Expr.pseudos[pseudo.toLowerCase()];
			...
			setFilters: {
				...
				lt: function (elements, argument, not) {
					return not
						? elements.slice(+argument)
						: elements.slice(0, +argument);
				},
		...
		function addMatcher(higher, deeper) {
			return higher
				? function (elem, context) {
					...
					return (
						result &&
							higher(result === true ? elem : result, context)
					);
				  }
				: deeper;
		}
					...
					if ((match = matchExpr[type].exec(token))) {
						...
						if (token === notTokens) {
							selector =
								selector.slice(
									0,
									selector.length - notTokens.length
	...
	jQuery.buildFragment = function (args, context, scripts) {
		...
		context = context || document;
		context =
			(context[0] || context).ownerDocument || context[0] || context;
