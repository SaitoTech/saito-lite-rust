  ...
  initializeHTML(app) {
    ...
    try {
      document.querySelector(
        "#game-scoreboard #round"
      ).innerHTML = `Round: ${this.game.state.round}`;
      if (
        this.game.state.button_player <= this.game.players.length &&
        this.game.state.button_player > 0
      ) {
        document.querySelector(
          "#game-scoreboard #dealer"
        ).innerHTML = `Button: ${this.getShortNames(
          this.game.players[this.game.state.button_player - 1],
          6
        )}`;
      }
    } catch (err) {
  ...
  displayPlayers(preserveLog = false) {
    ...
    try {
      ...
      elem = document.querySelector(
        `#player-box-head-${this.playerbox.playerBox(this.game.state.button_player)}`
      );
      if (elem) {
