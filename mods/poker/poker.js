 ...
 */
      ...
      if (mv[0] === "raise") {
        ...
        if (call_portion > 0) {
          if (raise_portion > 0) {
            ...
            if (this.game.player !== player && this.browser_active) {
              this.playerbox.refreshLog(`<div class="plog-update">raises ${this.formatWager(raise_portion)}</div>`, player);
            }
          } else {
          ...
          this.updateLog(`${this.game.state.player_names[player - 1]} ${raise_message}to ${this.formatWager(this.game.state.player_pot[player - 1])}`);
          if (this.game.player !== player && this.browser_active) {
            this.playerbox.refreshLog(`<div class="plog-update">raises ${this.formatWager(raise)}</div>`, player);
          }
        }
        this.game.queue.splice(qe, 1);
  ...
  async clearTable() {
    ...
    if (document.querySelector(".flipped")) {
      $(".flipped").removeClass("flipped").delay(200).queue(function() {
        $("#deal").children().animate({ left: "1000px" }, 1200, "swing", function() {
          $(this).remove();
        }).dequeue();
      });
    } else {
  ...
  processResignation(resigning_player, txmsg) {
    ...
    if (this.browser_active) {
      if (this.app.wallet.getPublicKey() !== resigning_player) {
