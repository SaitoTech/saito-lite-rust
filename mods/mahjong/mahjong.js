...
class Mahjong extends OnePlayerGameTemplate {
  ...
  async displayBoard(timeInterval = 1) {
    ...
    try {
      ...
      await this.displayUserInterface();
    } catch (err) {
      console.log(err);
    }
  }
        ...
        if (mahjong_self.game.selected === "") {
          ...
          if (
            ...
            if (mahjong_self.game.cardsLeft === 0) {
              ...
              await mahjong_self.endTurn();
              return;
            }
            await mahjong_self.displayUserInterface();
            mahjong_self.game.selected = "";
            return;
          } else {
            // no match
    ...
    if (this.game.queue.length > 0) {
      ...
      if (mv[0] === "lose") {
        ...
        this.game.queue.push(
          `ROUNDOVER\t${JSON.stringify([])}\t${JSON.stringify([this.app.wallet.publicKey])}`
        );
      ...
      if (mv[0] === "win") {
        ...
        await this.newRound();
        this.game.queue.push(
          `ROUNDOVER\t${JSON.stringify([this.app.wallet.publicKey])}\t${JSON.stringify([])}`
        );
