	...
	<img src="/img/hsk/hsk3.jpg" style="width:220px"/> \
      ...
      try { var trx = parseInt(document.getElementById("total_remaining").innerHTML); total_remaining = trx; } catch (err) { }
	...
	last_question_data = "";
        ...
        if (question_type == "generative_pinyin") { return check_pinyin_answer(choice); }
        if (question_type == "generative_english") { return check_english_answer(choice); }
	...
	if (choice == correct) {
	  ...
	  total_correct++;
	  show_points(10);
          if (endless_mode != 1) { total_remaining--; }
	} else {
	  answered_correctly = -1;
	  total_answered++;
          if (review_until_correct == 0 && endless_mode != 1) { total_remaining--; }
	  total_incorrect++;
	}

	if (correct == 1) { $('#option1').addClass('option_correct'); } else { $('#option1').addClass('option_incorrect'); }
	if (correct == 2) { $('#option2').addClass('option_correct'); } else { $('#option2').addClass('option_incorrect'); }
	if (correct == 3) { $('#option3').addClass('option_correct'); } else { $('#option3').addClass('option_incorrect'); }
	if (correct == 4) { $('#option4').addClass('option_correct'); } else { $('#option4').addClass('option_incorrect'); }
	...
	$('#question_hint').html(' ');
        ...
        if (endless_mode == 0) {
          if (total_remaining > 9)  { updateTotalRemaining(total_remaining); }
          if (total_remaining == 9) { updateTotalRemaining("nine"); }
          if (total_remaining == 8) { updateTotalRemaining("eight"); }
          if (total_remaining == 7) { updateTotalRemaining("seven"); }
          if (total_remaining == 6) { updateTotalRemaining("six"); }
          if (total_remaining == 5) { updateTotalRemaining("five"); }
          if (total_remaining == 4) { updateTotalRemaining("four"); }
          if (total_remaining == 3) { updateTotalRemaining("three"); }
          if (total_remaining == 2) { updateTotalRemaining("two"); }
          if (total_remaining == 1) { updateTotalRemaining("one"); $('#questions').html("question"); }
        }
	...
	if (source == "lesson" || source == "wordlist") {
	  ...
	  ajaxurl_set = 0;
	  if (source == "hsk-beginner") { ajaxurl = "/api/loadTestQuestion?type=hsk-beginner&nocache=" + new Date().getTime(); ajaxurl_set = 1; }
	  if (source == "hsk-intermediate") { ajaxurl = "/api/loadTestQuestion?type=hsk-intermediate&nocache=" + new Date().getTime(); ajaxurl_set = 1; }
	  if (source == "hsk-advanced") { ajaxurl = "/api/loadTestQuestion?type=hsk-advanced&nocache=" + new Date().getTime(); ajaxurl_set = 1; }
	  if (ajaxurl_set == 0) {
	...
	// or fetch a fresh one
	  ...
	  if (retry_load_state == 0) { return; } else { retry_load_state = 0; }
	  ...
	  wid                = jsonobj.Question.wid;
          ...
          source_audio_url   = "http://popupchinese.com/data" + jsonobj.Question.this_requested_audio_source;
          next_audio_url     = "http://popupchinese.com/data" + jsonobj.Question.next_requested_audio_source;
	  // disabling because of preloading -- just directly show
          //last_question_data = jsonobj.Question.last_question_data;

	  try { // fill_in_the_blanks has no english
	  ...
	  // catch errors
          if ((option1 == "" || option2 == "" || option3 == "" || question == "" || option4 == "") && mode == "multiple") {
            document.getElementById("lightbox_content").innerHTML = return_error_finished_html();
            return;
          }
	  ...
	  if (source_audio_url != "" && (source == "hsk-beginner" || source == "hsk-intermediate" || source == "hsk-advanced")) {
	    question = '<div style="float:left;width:50px;height:50px;margin-right:20px;"><img src="/img/buttons/play_button_medium.png" onclick="playTestAudio(\''+source_audio_url+'\',this);" /></div>'+question;
	  }
	  ...
	  if (last_question_data != "") {
	    tempLastQuestionData = last_question_data;
            document.getElementById("feedback").innerHTML = '<span onclick="addDefinition();return false;" class="editable">Are we wrong? Click to add '+last_question_data.split("_M_")[1]+' to system</span>';
	    last_question_data = "";
    	  } else {
	...
	if (last_question_data != "") {
	  $('#feedback').html('Testing new feature... please disregard for the moment!');
        }
	...
	if (question_type == "test") {
	  ...
	  if (option1 == "") { $('#option1').hide(); }
	  if (option2 == "") { $('#option2').hide(); }
	  if (option3 == "") { $('#option3').hide(); }
	  if (option4 == "") { $('#option4').hide(); }
	} else {
	  format_as_horizontal_question();
	}


        if (question_type == "multiple_choice_english" || question_type == "multiple_choice_pinyin" || question_type == "fill_in_the_blank" || question_type == "test") {

	  if (option1 != "") { $('#option1').show(); } 
	  if (option2 != "") { $('#option2').show(); } 
	  if (option3 != "") { $('#option3').show(); } 
	  if (option4 != "") { $('#option4').show(); } 
	  $('#instructions').hide();
	  $('#instructions').html("");
	  ...
	  $('#option1').bind('click', function() {check_answer_review(1);});
	  $('#option2').bind('click', function() {check_answer_review(2);});
	  $('#option3').bind('click', function() {check_answer_review(3);});
	  $('#option4').bind('click', function() {check_answer_review(4);});

	  $('#question_text').unbind('click');
	  $('#question_text').bind('click', function() {
	    try { playWordAudio(source_audio_url,this); } catch (err) {}
	  });
	  if (review_include_pinyin == 1) { $('#question_hint').show(); } else { $('#question_hint').hide(); }

	}


        if (question_type == "generative_english" || question_type == "generative_pinyin") {
	  ...
	  $('#instructions').show();
          if (question_type == "generative_english") { $('#instructions').html("type the <span class=\"bold red\">english</span> definition:"); } 
          if (question_type == "generative_pinyin") { $('#instructions').html("type the <span class=\"bold red\">pinyin</span> definition:"); } 
	  var inserthtml = '<form id="user_answer_form" action="" method="POST" onsubmit="check_answer_review();return false;"><input id="user_answer" type="text" onsubmit="check_answer_review();return false;" style="background-color:#F7F7F7;border:0px solid black;border-bottom:1px dashed #333;font-size:1.4em;width:330px;" /></form>';
	...
	var answer_for_prompt = english.split(/[,;]/)[0];
	  ...
	  last_question_data = question + "_M_" + user_answer;
          total_answered++;
          if (review_until_correct == 0) { total_remaining--; }
          total_incorrect++;
        }
	  ...
	  $('#instructions').html("you entered: <span class=\"green\">"+user_answer+"</span>");
        } else { 
	  incorrect_delay = generative_review_delay_on_incorrect;
          ...
          $('#instructions').css('background-color','#FFF');
	  $('#instructions').html("you entered: <span class=\"red\">"+user_answer+"</span>");
        }
        document.getElementById("user_answer").value = answer_for_prompt;
        if (answer.length > 10) { $('#user_answer').css('font-size','1em'); }
        if (answer.length > 15) { $('#user_answer').css('font-size','0.9em'); }
        if (answer.length > 19) { $('#user_answer').css('font-size','0.8em'); }
	...
	tmpx = tmpx.toLowerCase();
	  ...
	  tmpyy = tmpyy.replace(/[^a-zA-Z1-9]/g, "");
  	  tmpyy = tmpyy.toLowerCase();
          ...
          if (tmpx.indexOf("1") == -1) { tmpyy = tmpyy.replace(/1/g,""); }
          if (tmpx.indexOf("2") == -1) { tmpyy = tmpyy.replace(/2/g,""); }
          if (tmpx.indexOf("3") == -1) { tmpyy = tmpyy.replace(/3/g,""); }
          if (tmpx.indexOf("4") == -1) { tmpyy = tmpyy.replace(/4/g,""); }
	...
	advance_to_next_question();
	  ...
	  requested_wid = return_requested_wid();
	    ...
	    setTimeout(function() {
              document.getElementById("lightbox_content").innerHTML = return_test_finished_html();
	    },correct_delay);
            return;
          } else {
	    setTimeout(function() {
	...
	document.getElementById("lightbox_content").innerHTML = html2insert;
        ...
        $('.option').bind('mouseover', function() { $(this).addClass("option_hover"); });
        $('.option').bind('mouseout', function()  { $(this).removeClass("option_hover"); });
	...
	if (total_remaining == 0 && review_until_correct == 1) {
	  ...
	  return '\
	   <div id="lightbox_header" class="lightbox_header" style="margin-top:25px;font-size:2em"> \
  	     <span class="red">'+question+'</span> is missing data.... \
           </div> \
	     ...
	     <div style="font-size:1.2em"> \
	       <span style="font-size:0.8em;padding:15px;">english:</span> <br/><input type="text" id="missing_english" value="'+english+'" style="text-align:center;background-color:#F7F7F7;border:0px solid black;border-bottom:1px dashed #333;font-size:1.3em;width:600px;" /> \
      ...
      function updateMissing() {
        ...
        $.post("/api/updateMissingWordlistData",{
          field1: question,
          field3: tmp3,
          field4: tmp4
        },function(txt){
	...
	if (total_incorrect > 0) {
	  html = '\
	   <div id="lightbox_header" class="lightbox_header"> \
  	     You answered <span class="red">'+total_correct+'/'+total_answered+'</span> questions correctly.... \
           </div> \
...
function addDefinition() {
  $.post("/api/updateDictionaryWithUserSuggestion",{
    last_question_data: tempLastQuestionData
  }, function(txt) {
    document.getElementById("feedback").innerHTML = "Thank You... dictionary updated!";
  });
...
$('html').keydown(function(event){
  if (enableNumberShortcuts == 1) {
    if (question_type == "generative_pinyin" || question_type == "generative_english") {
      return;
    }
    var keycode = (event.keyCode ? event.keyCode : event.which);
    if (keycode == 49) { check_answer_review(1); }
    if (keycode == 50) { check_answer_review(2); }
    if (keycode == 51) { check_answer_review(3); }
    if (keycode == 52) { check_answer_review(4); }
  }
});
