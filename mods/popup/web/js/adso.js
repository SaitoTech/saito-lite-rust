...
try {
  if (content_id == undefined) { content_id = -1; };
} catch (err) {
 var content_id = -1;
}
...
function save_tooltip() {
  ...
  for (m = 3; m < entries.length; m+=2) {
    ...
    if (m == 3) { row_1 = entries2[0]; }
    if (m == 5) { row_2 = entries2[0]; }
    if (m == 7) { row_3 = entries2[0]; }
    if (m == 9) { row_4 = entries2[0]; }
    if (m == 11) { row_5 = entries2[0]; }
    if (m == 13) { row_6 = entries2[0]; }
  ...
  replacement_html = "<span id=\"adso_"+row_6+"\" onclick=\"onWordClick()\" onmousedown=\"adso_mousedown('"+row_6+"')\" onmouseup=\"adso_mouseup('"+row_6+"')\" onmouseover=\"tip(event,'"+row_4+"','"+row_3+"','"+row_1+"','"+row_2+"','"+row_5+"','"+row_6+"')\" onmouseout=\"htip()\">"+inset2+"</span>";
  var output = "";
  ...
  for (var i = 0; i < array_of_entries.length; i++) {
    ...
    if (field1 == "" && field2 == "" && field3 == "" && field4 == "" && field5 == "" && array_of_entries[i] != "") {
	...
	if (update_server_with_edits == 1) {
  	 ...
  	 output += pre + "<span id=\"adso_"+field6+"\" onclick=\"onWordClick()\" onmousedown=\"adso_mousedown('"+field6+"')\" onmouseup=\"adso_mouseup('"+field6+"')\" onmouseover=\"tip(event,'"+p(q(field1))+"','"+p(q(field2))+"','"+p(q(field3))+"','"+p(q(field4))+"','"+p(q(field5))+"','"+field6+"')\" onmouseout=\"htip()\">"+field3.replace(/\\\'/g,"'")+"</span>";
      }
    }
...
function update_tooltip(e){
  ...
  if (is_tooltip_locked == 1 || is_resegmenting == 1) { return; }

  try {
    rawx = current_x = (ff)?e.pageX : event.clientX+ie_backward_compatibility().scrollLeft;
    rawy = current_y = (ff)?e.pageY : event.clientY+ie_backward_compatibility().scrollTop;
  } catch (err) { 
...
function ie_backward_compatibility(){ return (document.compatMode && document.compatMode!="BackCompat")? document.documentElement : document.body }
...
function tip(event, event1, event2, event3, event4, event5, divdef){
  ...
  if (is_tooltip_locked == 1 || is_resegmenting == 1) { return; }
  try {
    if (event1 == "") {event1 = "unknown"; } 
    if (event2 == "") {event2 = "unknown"; } // english
    if (event3 == "") {event3 = "unknown"; } // english
    if (event4 == "") {event4 = "unknown"; } // english
    if (event5 == "") {event5 = "unknown"; } // part of speech
    if (content_id == -1) { // non-editing pages
      if (event1 == "unknown") {event1 = ""; } 
      if (event3 == "unknown") {event3 = ""; } // english
      if (event2 == "unknown") {event2 = ""; } // english
      if (event4 == "unknown") {event4 = ""; } // english
      if (event5 == "unknown") {event5 = ""; } // part of speech
      if (event5.search("color:") != -1) { event5 = ""; }
      if (event5.search("http:") != -1) { event5 = ""; }
  ...
  if (event1 == event2 && event2 == event3 && event3 == event4) { return; }

  event1 = rp(event1.replace(/`/g,'\''));
  event2 = rp(event2.replace(/`/g,'\''));
  event3 = rp(event3.replace(/`/g,'\''));
  event4 = rp(event4.replace(/`/g,'\''));
  try {  event5 = rp(event5.replace(/`/g,'\'')); } catch (err) {} // backwards compatible with older markup script

  try {
    tooltip.innerHTML='<div style="overflow:hidden"><div id="row_1" class=" row_1 " onclick="edit_row_num(1)" align="center">'+event3+'</div>'+'<div id="row_2" class="row_2" onclick="edit_row_num(2)" align="center">'+event4+'</div>'+'<div id="row_3" class="row_3" onclick="edit_row_num(3)" align="center">'+event2+'</div>'+'<div id="row_4" class="row_4" onclick="edit_row_num(4)" align="center">'+event1+'</div><div id="row_5" class="row_5" onclick="edit_row_num(5)" align="center">'+event5+'</div><div id="divdef" class="row_6">'+divdef+'</div></div>';
    show_tooltip();
...
function htip(){
  if (is_tooltip_locked == 0) { hide_tooltip(); } 
}
...
function onWordClick() {
   ...
   if (is_tooltip_locked == 1) { return; }
   is_tooltip_locked = 1;
   tooltip.innerHTML = '<img src="/img/icons/close.gif" onmouseup="try {save_tooltip();}catch (err) {is_tooltip_locked=0;hide_tooltip();}" style="float:right;margin-bottom:0px;margin: 0px 0px 0px"/>'+tooltip.innerHTML;
...
function adso_clicked(evt) {
   ...
   try {
     ...
     if (ff) { highlighted_selection = window.getSelection(); }
     if (ie) { highlighted_selection = document.selection.createRange().text; }
   } catch (err) { 
  ...
  if (highlighted_selection != "") {
        var html_insert =  '<center><div style="width=:15px;height:15px;float:right;">';
        html_insert += '<img src="/img/icons/close.gif" onmouseup="is_tooltip_locked = 0; hide_tooltip();" style="margin-bottom:0px;margin: 0px 0px 0px"/>';
        html_insert += '</div>';
        html_insert +=  '<form name="contribute" onsubmit="adso_resegment_submission(); hide_tooltip(); return false;">';
        html_insert += '<div class="adso_add_source">'+highlighted_selection+'</div>';
        html_insert += '<input type="hidden" name="source" value="'+highlighted_selection+'"/>';
        html_insert += '<input type="hidden" name="firstdef" value="'+firstdefinition+'"/>';
        html_insert += '<input type="hidden" name="lastdef" value="'+lastdefinition+'"/>';
        html_insert += '<input type="text" id="add_translation" class="adso_add_translation" name="translation" />';
        html_insert += '<br /><input type="submit" value="edit" onsubmit="adso_resegment_submission(); is_tooltip_locked = 0; hide_tooltip(); return false;" /> ';
...
function adso_resegment_ajax(source, translation,divdef1,divdef2) {
  ...
  for (z = parseInt(divdef2); stoploop < 0 && z >= parseInt(divdef1); z--) {
    ...
    try {
      if (document.getElementById("adso_"+z).innerHTML != "undefined") { 
        alltext = document.getElementById("adso_"+z).innerHTML + alltext; 
      } 
    } catch (err) {}

    if (alltext.search(source) != -1) { stoploop = z; }
  ...
  var temp = adso_resegment_engine(source,translation,object_under_editing.innerHTML,divdef1,divdef2);


  // Attempting to Resegment Non-Markup Text Returns Nothing
  if (temp == "") { is_tooltip_locked = 0; return false; }
  ...
  if (pre_field_lookup_id != "") {
    ...
    $.post("/updates/annotator",{
      ...
      replace_string  = '\''+new_field1+'\',\''+new_field2+'\',\''+new_field3+'\',\''+new_field4+'\',\''+new_field5+'\'';
  ...
  if (match_field_lookup_id != "") {
    ...
    $.post("/updates/annotator",{
      ...
      try { // popup may not be open/exist
      if (mnew_field6 == document.getElementById("row_6").innerHTML) {
        row_1 = mnew_field3; document.getElementById("row_1").innerHTML = mnew_field3;
        row_2 = mnew_field4; document.getElementById("row_2").innerHTML = mnew_field4;
        row_3 = mnew_field2; document.getElementById("row_3").innerHTML = mnew_field2;
      }
      } catch (err) {}
      ...
      replace_string  = '\''+mnew_field2+'\',\''+mnew_field3+'\',\''+mnew_field4+'\',\''+mnew_field5+'\'';
      re = new RegExp(original_string, "g"); 
      temp = temp.replace(re,replace_string);
      object_under_updating.innerHTML = temp;
  ...
  if (post_field_lookup_id != "") {
    ...
    $.post("/updates/annotator",{
      ...
      replace_string  = '\''+bnew_field1+'\',\''+bnew_field2+'\',\''+bnew_field3+'\',\''+bnew_field4+'\',\''+bnew_field5+'\'';
      re = new RegExp(original_string, "g"); 
      temp = temp.replace(re,replace_string);
      object_under_updating.innerHTML = temp;
...
function annotate_page(z) {
  if (z >= translation.length) { alert("Error: that page does not exist."); }
  else {
    paragraph=z;
...
function extract_field1(text) { return rp(extract_field(text,1)); }
function extract_field2(text) { return rp(extract_field(text,2)); }
function extract_field3(text) { return rp(extract_field(text,3)); }
function extract_field4(text) { return rp(extract_field(text,4)); }
function extract_field5(text) { return rp(extract_field(text,5)); }
function extract_field6(text) { return rp(extract_field(text,6)); }
...
function adso_redefine_word(source, pinyin, translation, text,divdef) {
  ...
  for (var i = 0; i < array_of_entries.length; i++) {
    ...
    if (field3 == source) {
      if (field6 == divdef) {
        alternate_segmentation += '<span id="adso_'+divdef+'" onmousedown="adso_mousedown(\''+divdef+'\');" onmouseup="adso_mouseup(\''+divdef+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(pinyin)+'\',\''+p(translation)+'\',\''+p(field3)+'\',\''+p(field4)+'\',\''+p(field5)+'\',\''+divdef+'\')" onmouseout="htip()">'+this_inset+'</span>';
      } else {
        alternate_segmentation += '<span id="adso_'+field6+'" onmousedown="adso_mousedown(\''+divdef+'\');" onmouseup="adso_mouseup(\''+divdef+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(field1)+'\',\''+p(field2)+'\',\''+p(field3)+'\',\''+p(field4)+'\',\''+p(field5)+'\',\''+field6+'\')" onmouseout="htip()">'+this_inset+'</span>';
      }
    } else {
        alternate_segmentation += '<span id="adso_'+field6+'" onmousedown="adso_mousedown(\''+divdef+'\');" onmouseup="adso_mouseup(\''+divdef+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(field1)+'\',\''+p(field2)+'\',\''+p(field3)+'\',\''+p(field4)+'\',\''+p(field5)+'\',\''+field6+'\')" onmouseout="htip()">'+this_inset+'</span>';
    }
...
function adso_resegment_engine(source, translation, text,divdef_first, divdef_last) {
  ...
  for (var i = 0, continue_loop = 1; i < array_of_entries.length && array_of_entries[i] != ""; i++) {
    ...
    if (field1 == "" && field2 == "" && field3 == "" && field4 == "" && field5 == "" && field6 == "") {
      // strip out our tags
      field3 = array_of_entries[i].replace(/(<([^>]+)>)/ig,""); 
      field1 = "";
      field2 = "";
      field4 = "";
      field5 = "";
    ...
    if (field3.indexOf(first_char) >= 0) {
      ...
      if (field3.indexOf(first_char) >= 0) {
        ...
        potential_pre_field3 = field3.substr(0, field3.indexOf(first_char));
        potential_post_field3 = temp2.substr(source.length);


        if (field3.length > (potential_pre_field3.length+source.length)) {
          continue_next = 0;
	  if (field3.indexOf(source) >= 0) {

	    /* Avoid Matching the First Character Only - Confirm our Full Match */
	    potential_match_position = field3.indexOf(first_char);
	    for (zk = 0, match_success = 0; zk < field3.length && match_success == 0; zk++) {	
		potential_match = field3.substr(potential_match_position,source.length);
		if (potential_match != source) {
			potential_match_position = field3.indexOf(first_char,potential_match_position+1);
		} else {
			match_success = 1;
			potential_pre_field3 = field3.substr(0,potential_match_position);
			potential_post_field3 = field3.substr(potential_match_position+source.length);
		}
	    }
      ...
      if (continue_next == 1) {
        while (source.length > temp.length && j < array_of_entries.length) {
          j++;
  	  temp += extract_pre(array_of_entries[j]) + extract_field3(array_of_entries[j]);
        }
        ...
        if (matched == source) {
          ...
          if (matched.length == temp.length) {
            ...
            post_field2 = "";
            temp67 = 0;
            for (zzp = 0; zzp < source.length;zzp++) { if (source[zzp] == "'") { temp67++; }; }
    ...
    if (match == 1) {
      ...
      alternate_segmentation = pre;
      if (pre_field3 != "") {
        alternate_segmentation += '<span id="adso_'+(location_adjustment+pre_position)+'" onmousedown="adso_mousedown(\''+(pre_position+location_adjustment)+'\');" onmouseup="adso_mouseup(\''+(location_adjustment+pre_position)+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(pre_field1)+'\',\''+p(pre_field2)+'\',\''+p(pre_field3)+'\',\''+p(pre_field4)+'\',\''+p(pre_field5)+'\',\''+(pre_position+location_adjustment)+'\')" onmouseout="htip()">'+pre_field3_inset+'</span>';
	pre_field_lookup = pre_field3;
	pre_field_lookup_id = pre_position+location_adjustment;
      }
      if (match_field3 != "") {
        alternate_segmentation += '<span id="adso_'+(location_adjustment+match_position)+'" onmousedown="adso_mousedown(\''+(location_adjustment+match_position)+'\');" onmouseup="adso_mouseup(\''+(location_adjustment+match_position)+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(match_field1)+'\',\''+p(match_field2)+'\',\''+p(match_field3)+'\',\''+p(match_field4)+'\',\''+p(match_field5)+'\',\''+(location_adjustment+match_position)+'\')" onmouseout="htip()">'+match_field3_inset+'</span>';
	match_field_lookup = match_field3;
	match_field_lookup_id = match_position+location_adjustment;
      }
      if (post_field3 != "") {
        alternate_segmentation += '<span id="adso_'+(location_adjustment+post_position)+'" onmousedown="adso_mousedown(\''+(location_adjustment+post_position)+'\');" onmouseup="adso_mouseup(\''+(location_adjustment+post_position)+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(post_field1)+'\',\''+p(post_field2)+'\',\''+p(post_field3)+'\',\''+p(post_field4)+'\',\''+p(post_field5)+'\',\''+(location_adjustment+post_position)+'\')" onmouseout="htip()">'+post_field3_inset+'</span>';
	...
	if (array_of_entries[i].indexOf("span ") > 0) {
          alternate_segmentation += '<span id="adso_'+field6+'" onmousedown="adso_mousedown(\''+field6+'\');" onmouseup="adso_mouseup(\''+field6+'\');" onclick="onWordClick()" onmouseover="tip(event,\''+p(field1)+'\',\''+p(field2)+'\',\''+p(field3)+'\',\''+p(field4)+'\',\''+p(field5)+'\',\''+p(field6)+'\')" onmouseout="htip()">'+myinset+'</span>';
	} else {
...
function rp(t) { try { return t.replace(/_APOS_/g,"&apos;"); } catch (err) { return ""; } }
...
function edit_row_num(num) {
  if (row_under_editing != 0) { update_row_edit("row_"+row_under_editing); }
  var numrow = "row_"+num;
  row_under_editing = num;
  var thisrow = document.getElementById(numrow);
  var newtextvalue = thisrow.innerHTML;
  var thisrowform = '<form name="edit_form" action="" method="POST" onsubmit="update_row_edit(\''+numrow+'\');return false;"><input type="text" name="row_edit" class="'+numrow+'" value="'+newtextvalue+'" /></form>';
  if (thisrow.innerHTML.indexOf("update_row_edit") > 0)  {} else {
...
function edit_comment(comment_id) {
  if (editing_comment == 1) { return; }
  editing_comment = 1;
  existing_obj = $("#comment_text_"+comment_id);
  existing_value = document.getElementById("comment_text_"+comment_id).innerHTML;
  var ourReplacementHTML = '<div id="comment_text_'+comment_id+'" class="comment_text"><textarea id="adso_new_editing_textarea" rows="10" cols="60">'+convertforediting(existing_value)+'</textarea><div><input type="button" value="SAVE" class="saveButton" /><input type="hidden" value="CANCEL" class="cancelButton" /></div></div>';
  $("#comment_text_"+comment_id).after(ourReplacementHTML).remove();
  $('.saveButton').click(function(){

    var txx = $(this).parent().siblings(0).val();
    editing_comment = 0;
    document.getElementById("comment_text_"+comment_id).innerHTML = convertfromediting(txx);
    $.post("/updates/editComment",{
...
function enable_display_mode(toggle_which_field) {
  ...
  if (document.getElementById("checkbox_field1").checked == true) { view_field1 = 1; }
  if (document.getElementById("checkbox_field2").checked == true) { view_field2 = 1; }
  if (document.getElementById("checkbox_field3").checked == true) { view_field3 = 1; }
  if (document.getElementById("checkbox_field4").checked == true) { view_field4 = 1; }
  if (document.getElementById("checkbox_field5").checked == true) { view_field5 = 1; }
  ...
  $.post("/account/updateVocabPreferences",{
...
function toggle_english() {
  ...
  if (document.getElementById("display_english").checked == true) { edmode = "on"; }

    // ping server to save user Preference
  $.post("/account/updateEnglishPreferences",{
...
function toggle_pinyin() {
  ...
  if (document.getElementById("display_pinyin").checked == true) { pdmode = "on"; }

  // ping server to save user Preference
  $.post("/account/updatePinyinPreferences",{
...
function contains_tonal_pinyin(p) {
	...
	if (p.indexOf("ǖ") != -1) { return 1; };
	if (p.indexOf("ǘ") != -1) { return 1; };
	if (p.indexOf("ǚ") != -1) { return 1; };
	if (p.indexOf("ǜ") != -1) { return 1; };
	if (p.indexOf("ü") != -1) { return 1; };
	if (p.indexOf("ā") != -1) { return 1; };
	if (p.indexOf("á") != -1) { return 1; };
	if (p.indexOf("ǎ") != -1) { return 1; };
	if (p.indexOf("à") != -1) { return 1; };
	if (p.indexOf("ē") != -1) { return 1; };
	if (p.indexOf("é") != -1) { return 1; };
	if (p.indexOf("ě") != -1) { return 1; };
	if (p.indexOf("è") != -1) { return 1; };
	if (p.indexOf("ī") != -1) { return 1; };
	if (p.indexOf("í") != -1) { return 1; };
	if (p.indexOf("ǐ") != -1) { return 1; };
	if (p.indexOf("ì") != -1) { return 1; };
	if (p.indexOf("ō") != -1) { return 1; };
	if (p.indexOf("ó") != -1) { return 1; };
	if (p.indexOf("ǒ") != -1) { return 1; };
	if (p.indexOf("ò") != -1) { return 1; };
	if (p.indexOf("ū") != -1) { return 1; };
	if (p.indexOf("ú") != -1) { return 1; };
	if (p.indexOf("ǔ") != -1) { return 1; };
	if (p.indexOf("ù") != -1) { return 1; };
