...
class VideoChatManager {
  ...
  constructor(app, mod) {
    ...
    this.app.connection.on(
      ...
      (app, mod, call_type = "Video", room_code) => {
  ...
  render() {
    this.app.browser.addElementToDom(
      ChatManagerLargeTemplate(this.call_type, this.room_code),
  ...
  attachEvents(app, mod) {
    document.querySelector(".disconnect_btn").addEventListener("click", async (e) => {
    ...
    if (add_users) {
      add_users.addEventListener("click", async (e) => {
        this.updateRoomLink();
        await this.chatInvitationOverlay.render();
      });
    }
    ...
    if (document.querySelector(".effects-control")) {
      document.querySelector(".effects-control").addEventListener("click", async (e) => {
        await this.effectsMenu.render();
      });
    }
  ...
  async hide() {
    ...
    this.isActive = false;
    await stun_mod.renderInto(this.mod.renderedInto);
  }
