        ...
        return {
            init: (app, game_mod) => {
                ...
                for (let i = 0; i < game_mod.game.players.length; i++) {
                    if (game_mod.game.players[i] != app.wallet.getPublicKey()) {
      ...
      try {
        pc.onicecandidate = (ice) => {
          if (!ice || !ice.candidate || !ice.candidate.candidate) {
            ...
            stunx_mod.sendMediaAnswerTransaction(stunx_mod.app.wallet.getPublicKey(), offer_creator, reply);
            return;
          }
          ;
      ...
      try {
        pc.onicecandidate = (ice) => {
          if (!ice || !ice.candidate || !ice.candidate.candidate) {
            ...
            stunx_mod.sendStunAnswerTransaction(stunx_mod.app.wallet.getPublicKey(), offer_creator, reply);
            return;
          }
          ;
          reply.ice_candidates.push(ice.candidate);
    ...
    try {
      ...
      if (peerConnectionOffers.length > 0) {
        ...
        let interval = setInterval(() => {
          let offer;
          offer = offers[index];
          this.sendMediaOfferTransaction(this.app.wallet.getPublicKey(), offer)
  ...
  async createStunConnectionWithPeers(public_keys) {
    ...
    try {
      ...
      if (peerConnectionOffers.length > 0) {
        ...
        this.sendStunOfferTransaction(this.app.wallet.getPublicKey(), offers);
      }
    } catch (error) {
      console.log('an error occurred with peer connection creation', error);
    }
