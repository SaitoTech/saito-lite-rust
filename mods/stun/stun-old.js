...
class Stun extends ModTemplate {
      ...
      let m = [
        {
          ...
          callback: async function (app, id) {
            let pub_key = await app.wallet.publicKey;
  ...
  async receiveCommandToPeerTransaction(app, tx, conf, blk) {
    ...
    let my_pubkey = app.wallet.publicKey;
  }
  ...
  //             pc.onicecandidate = (ice) => {
  //                 if (!ice || !ice.candidate || !ice.candidate.candidate) {
  //                     stunx_mod.sendMediaChannelAnswerTransaction(stunx_mod.app.wallet.publicKey, offer_creator, reply);
  //                     return;
  //                 };
  ...
  //                     remoteStream.addTrack(track);
  //                 });
  //                 let my_pubkey = this.app.wallet.publicKey;
        ...
        pc.onicecandidate = (ice) => {
          if (!ice || !ice.candidate || !ice.candidate.candidate) {
            ...
            stunx_mod.sendDataChannelAnswerTransaction(
              stunx_mod.app.wallet.publicKey,
              offer_creator,
              reply
            );
            return;
  ...
  //             // send notification
  //             this.sendMediaChannelNotificationTransaction(this.app.wallet.publicKey, public_keys[i], room_code)
  ...
  //                 let offer;
  //                 offer = offers[index];
  //                 this.sendMediaChannelOfferTransaction(this.app.wallet.publicKey, offer)
  ...
  //             this.sendDataChannelOfferTransaction(this.app.wallet.publicKey, offers);
  //         }
  //     } catch (error) {
  //         console.log('an error occurred with peer connection creation', error);
  //     }
  ...
  receiveRoomCodeTransaction(app, tx, conf, blk) {
    ...
    if (tx.msg.data.creator === app.wallet.publicKey) {
      return;
    }
